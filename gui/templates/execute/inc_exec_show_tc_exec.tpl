...
*}	
    ...
    *}
    ...
    {include file="inc_show_hide_mgmt.tpl"
      ...
      <br />
      <div class="exec_testsuite_details">
  ...
  {$exec_build_title="$build_title $title_sep $my_build_name"}
      ...
      {if $gui->issueTrackerIntegrationOn}
        <a target="_blank" href="{$gui->createIssueURL}">
      ...
      {if $abs_last_exec.status != '' and $abs_last_exec.status != $tlCfg->results.status_code.not_run}
       ...
       an array and execute them from Ext.onReady(). See execSetResults.tpl *}
        ...
        <div id="latest_exec_any_build_notes">
        </div>
        <hr>
        {/if}
        {* ///////////////////////////// *}
  			...
  			<thead class="thead-dark">
				<tr>
					...
					{if $gui->history_on == 0 || $cfg->exec_cfg->show_history_all_builds}
				  		<th>{$labels.build}</th>
					{/if}
					{if $gui->has_platforms && ($gui->history_on == 0 || $cfg->exec_cfg->show_history_all_platforms)}
						{$my_colspan=$my_colspan+1}
    				...
    				<th title="{$labels.testcaseversion}" width="5%">{$labels.version}</th>
					...
					{if $can_delete_exec}
         				...
         				{$my_colspan=$my_colspan+1}
        			{/if}
       				<th width="12%">{$labels.run_mode}</th>
       				<th>&nbsp;</th>
			...
			{foreach item=tc_old_exec from=$gui->other_execs.$tcversion_id}
				...
				<tr>
          				...
          				{* Check also that Build is Open *}
  			  			{if $can_edit_exec_notes && $tc_old_exec.build_is_open}
      		      			<img src="{$tlImages.note_edit}" style="vertical-align:middle" 
      		           		title="{$labels.edit_execution}" onclick="javascript: openExecEditWindow({$tc_old_exec.execution_id},{$tc_old_exec.id},{$gui->tplan_id},{$gui->tproject_id});">
  		      			{else}
    				...
    				<td>
      				...
      				<td class="{$tlCfg->results.code_status.$tc_status_code}" title="(ID:{$tc_old_exec.execution_id})">
      				    {localize_tc_status s=$tc_old_exec.status}
      				</td>
      				
      				{* IMPORTANT: Here we use tcversion_number because we want to display version number used when this execution was recorded. *}
    
              		<td>{$tc_old_exec.execution_duration}</td>
      				<td>{$tc_old_exec.tcversion_number}</td>
              	    ...
              	    $tc_old_exec.build_is_open && $can_manage_attachments)}
    					...
    					<td align="center">
    						<a href="javascript:openFileUploadWindow({$tc_old_exec.execution_id},'executions')">
    		    				<img src="{$tlImages.upload}" title="{$labels.alt_attachment_mgmt}" alt="{$labels.alt_attachment_mgmt}" style="border:none" />
    	    				</a>
                  		</td>
    			  	{else}
        			  	{if $attachment_model->show_upload_column && $can_manage_attachments}
           		  	...
           		  	<td align="center">
           		  		{if $tc_old_exec.build_is_open}
           		    		<a href="javascript:open_bug_add_window({$gui->tproject_id}, {$gui->tplan_id},{$tc_old_exec.id},{$tc_old_exec.execution_id},0,'link')">
           		   				<img src="{$tlImages.bug_link_tl_to_bts}" title="{$labels.bug_link_tl_to_bts}" style="border:none" />
       		   				</a>
           		    		&nbsp;&nbsp;
                  			{if $gui->tlCanCreateIssue}
          			    		...
          			    		<img src="{$tlImages.bug_create_into_bts}" title="{$labels.bug_create_into_bts}" style="border:none" />
      			    		</a>
                  			{/if}
           		  		{else}
           		    		<a><img src="{$tlImages.bug_link_tl_to_bts_disabled}" title="{$labels.bug_link_tl_to_bts}" style="border:none" /></a>
           		    		&nbsp;&nbsp;
                  			{if $gui->tlCanCreateIssue}
           		  	  			<a><img src="{$tlImages.bug_create_into_bts_disabled}" title="{$labels.bug_create_into_bts}" style="border:none" /></a>
                  			{/if}
                		{/if} 
           		  	</td>
    				{/if}
        			...
        			{if $can_delete_exec && $tc_old_exec.build_is_open}
        	         	...
        	         	<a href="javascript:confirm_and_submit(msg,'execSetResults','exec_to_delete', {$tc_old_exec.execution_id},'do_delete',1);">
          			    	<img src="{$tlImages.delete}" title="{$labels.img_title_delete_execution}" style="border:none" />
      			    	</a>
    				</td>
          			{else}
          				{if $can_delete_exec}
    				...
    				<td class="icon_cell" align="center">
           		  	{if $tc_old_exec.execution_run_type == $smarty.const.TESTCASE_EXECUTION_TYPE_MANUAL}
          		    	<img src="{$tlImages.testcase_execution_type_manual}" title="{$labels.execution_type_manual}" style="border:none" />
           		  	{else}
          		    	<img src="{$tlImages.testcase_execution_type_automatic}" title="{$labels.execution_type_auto}" style="border:none" />
           		  	{/if}
              		</td>
              		{* CORTADO *}
              		{if $tlCfg->exec_cfg->steps_exec }
            		<td class="icon_cell" align="center">
                  		<img src="{$tlImages.steps}" title="{$labels.access_test_steps_exec}" onclick="javascript:openPrintPreview('exec',{$tc_old_exec.execution_id}, null,null,'{$printExecutionAction}');"/>
                	</td>
              		{/if}
      			</tr>
				{if $tc_old_exec.execution_notes neq ""}
  				<script>
    				{* Initialize panel if notes exists. There might be multiple note panels visible at the same time, so we need to collect those init functions in an array and execute them from Ext.onReady(). See execSetResults.tpl *}
           			var panel_init = function(){
                        var p = new Ext.Panel({
                        title:'{$labels.exec_notes}',
                        collapsible:true,
				...
				<tr>
  			 		<td colspan="{$my_colspan}" id="exec_notes_container_{$tc_old_exec.execution_id}"></td>
   				</tr>
 			  	{/if}
  				{* Custom field values  *}
  				{if isset($gui->other_exec_cfields[$execID]) }
    			<tr>
