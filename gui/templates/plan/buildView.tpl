              	...
              	<table id="item_view" class="table table-striped table-bordered sortable">
              		<thead class="thead-dark">
                  		<tr>
                  			...
                  			<th class="{$noSortableColumnClass}" width="10%">{$labels.release_date}</th>
                  			<th class="{$noSortableColumnClass}" width="5%">{$labels.th_active}</th>
                  			<th class="{$noSortableColumnClass}" width="5%">{$labels.th_open}</th>
                  			<th class="{$noSortableColumnClass}" width="5%">{$labels.th_delete}</th>
                  		</tr>
              		</thead>
              		<tbody>
                    	...
                    	<tr>
              				<td><span class="api_info" style='display:none'>{$tlCfg->api->id_format|replace:"%s":$build.id}</span>
              				    <a href="{$editAction}{$build.id}" title="{$labels.alt_edit_build}">{$build.name|escape}
              					     {if $gsmarty_gui->show_icon_edit}
              					         <img style="border:none" alt="{$labels.alt_edit_build}" title="{$labels.alt_edit_build}" src="{$tlImages.edit}"/>
              					     {/if}    
              					  </a>   
              				</td>
  							...
  							<td class="clickable_icon">
        				       <img style="border:none;cursor: pointer;"  title="{$labels.alt_delete_build}" alt="{$labels.alt_delete_build}" 
         					            onclick="delete_confirmation({$build.id},'{$build.name|escape:'javascript'|escape}', '{$del_msgbox_title}','{$warning_msg}');"
          				            	src="{$tlImages.delete}"/>
  							</td>
  						</tr>
  						{/foreach}
