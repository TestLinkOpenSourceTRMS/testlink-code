...
<body>
	...
	<form method="post" id="newest_tcversions.tpl">
		<table class="table">
			<tr>
                <th width="15%">{$labels.testproject}{$smarty.const.TITLE_SEP}</td>
                <td>{$gui->tproject_name|escape}</td>
			</tr>
			<tr>
				<th width="15%">{$labels.test_plan}</td>
				<td>
					<select name="tplan_id" id="tplan_id" onchange="this.form.submit()">{html_options options=$gui->tplans selected=$gui->tplan_id}</select>
				</td>
			</tr>
		</table>
	</form>
