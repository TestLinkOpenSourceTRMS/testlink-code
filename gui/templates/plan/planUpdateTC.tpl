...
<body class="testlink">
	...
	{if $gui->hasItems}
	<form name="updateTcForm" id="updateTcForm" method="post" onSubmit="javascript:return validateForm(this);">
      	...
      	{foreach from=$gui->items item=ts}
            ...
            {assign var="div_id" value="div_$ts_id"}
      		<div id="{$div_id}"  style="margin:0px 0px 0px {$ts.level}0px;">
      	    	<h3 class="testlink">
        		{if $item_number ==1}
      	    		<img src="{$tlImages.toggle_all}" border="0" alt="{$labels.check_uncheck_all}" title="{$labels.check_uncheck_all}" 
    							...
    							<tr class="testlink">
    								<td width="20">
    									...
    									{if $tcase.canUpdateVersion }
    										<input type='checkbox' name='{$update_cb}[{$tcase.id}]' id='{$update_cb}{$tcase.id}' value='{$tcase.linked_version_id}' /> 
            				    		{/if}
          			      			</td>
    								<td>
                						<img class="clickable" src="{$tlImages.history_small}" onclick="javascript:openExecHistoryWindow({$tcase.id});" title="{$labels.execution_history}" />
                						<img class="clickable" src="{$tlImages.edit}" onclick="javascript:openTCaseWindow({$tcase.id});" title="{$labels.design}" />
                      				...
                      				<td>
                      	  				{if $tcase.updateTarget != ''}	
                          					<select name="new_tcversion_for_tcid[{$tcase.id}]">{html_options options=$tcase.updateTarget disabled=disabled}</select>
               			  				{/if}
                      				</td>     
      		...
      		<tbody>
    			{foreach from=$gui->testcases item=tc}
         			{assign var="item_number" value=$item_number+1}
          			<tr class="testlink">
          				<td width="20">
      		 				<input type='checkbox' name='{$update_cb}[{$tc.tc_id}]' id='{$update_cb}{$tc.tc_id}' value='{$tc.tcversion_id}' checked="checked" /> 
    		  			...
    		  			<td>
                            ...
                            <img class="clickable" src="{$smarty.const.TL_THEME_IMG_DIR}/edit_icon.png"
                               	...
                               	title="{$labels.design}" />
    						{$tc.path}{$gui->testCasePrefix|escape}{$tc.tc_external_id|escape}:{$tc.name|escape} 
    					</td>  
