...
<form id='tc_exec_assignment' name='tc_exec_assignment' method='post'>
  ...
  <p>&nbsp;<p>&nbsp;<p>
	...
	{if $gui->has_tc}
	<div class="page-content" id="tc_exec_assignment_cb">
					...
					<div id="{$div_id}" style="margin-left:0px; border:1;">
						...
						{if $ts.write_buttons eq 'yes'}
          					{if $ts.testcase_qty gt 0}
								...
								<table id="the-table-{$table_counter}" class="table table-striped table-bordered sortable">
		      						<thead class="thead-dark">
  										<tr>
          									...
          									<th>
          										{$labels.th_test_case}&nbsp;{$gsmarty_gui->role_separator_open}
          										{$labels.version}{$gsmarty_gui->role_separator_close}
          									</th>
          									{if $gui->platforms != ''}
														...
														{/if} 
          	    										<tr>
            												...
            												{/if} {* do it JUST ON first iteration *}
                  											...
                  											{if $smarty.foreach.testerSet.iteration == 1}
                        										<td>
                                                      		  		<select class="chosen-select" multiple="multiple" data-placeholder="{$labels.chosen_blank_option}"
                                                                        	name="tester_for_tcid[{$tcase.id}][{$platform_id}][]" id="tester_for_tcid_{$tcase.id}_{$platform_id}"
