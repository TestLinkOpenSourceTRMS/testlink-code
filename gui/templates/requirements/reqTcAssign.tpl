    ...
    <form id="reqList" method="post">
    	...
    	<div id="div_assigned_req">
 	    	{* used as memory for the check/uncheck all checkbox javascript logic *}
       		<input type="hidden" name="memory_assigned_req" id="memory_assigned_req"  value="0" />
    				...
    				<tr>
    					<td>
          					{if $cbDisabled == 1}
            					&nbsp;
          					{else}
            					<input type="checkbox" id="assigned_req{$gui->assignedReq[row].link_id}" value="{$gui->assignedReq[row].link_id}"
                    					name="link_id[{$gui->assignedReq[row].link_id}]" />
          					{/if}
        				</td>
        				<td><span class="bold">{$gui->assignedReq[row].req_doc_id|escape}</span></td>
    					<td><span class="bold"><a href="lib/requirements/reqView.php?requirement_id={$gui->assignedReq[row].id}">{$gui->assignedReq[row].title|escape}</a></span></td>
...
{if $gui->unassignedReq ne "" && $reqLinkingEnabled == 1}
	<div class="page-content">
		...
		<form id="reqList2" method="post">
			...
			<div id="div_free_req">
 	     		{* used as memory for the check/uncheck all checkbox javascript logic *}
   				<input type="hidden" name="memory_free_req" id="memory_free_req"  value="0" />
				<input type="hidden" name="idSRS" value="{$gui->selectedReqSpec}" />
  				<table class="table table-striped table-bordered sortable">
 		 			<thead class="thead-dark">
  						<tr>
  							<th width="5px">
  		    					<img src="{$tlImages.toggle_all}" onclick='cs_all_checkbox_in_div("div_free_req", "free_req","memory_free_req");'
  		             					title="{$labels.check_uncheck_all_checkboxes}" />
  							</th>
					...
					<tbody>
  					{section name=row2 loop=$gui->unassignedReq}
  						...
  						<tr>
  							<td><input type="checkbox" id="free_req{$freeReq.id}" value="{$freeReq.id}" name="req_id[{$freeReq.id}]" /></td>
  							<td><span class="bold">{$freeReq.req_doc_id|escape}</span></td>
  							<td><span class="bold"><a href="lib/requirements/reqView.php?requirement_id={$freeReq.id}&req_version_id={$freeReq.version_id}">{$freeReq.title|escape} [{$labels.version_short}{$freeReq.version}] </a></span></td>
