    ...
    <div class="page-content">
		...
		<form id="SRS_switch" name="SRS_switch" method="post">
        	...
        	<p><span class="labelHolder">{$labels.req_spec}</span>
      		<select name="idSRS" onchange="form.submit()">{html_options options=$gui->req_specs selected=$gui->selectedReqSpec}</select></p>
      	</form>
             		...
             		<input type="hidden" name="memory_assigned_req" id="memory_assigned_req"  value="0" />
          			...
          			<table class="table table-striped table-bordered sortable">
          				<thead class="thead-dark">
          					<tr>
            					<th class="{$noSortableColumnClass}" width="5px">
      					...
      					<tbody>
            				{$reqSet = $gui->requirements}
          					{section name=row loop=$gui->requirements}
          						...
          						<tr>
          							<td><input type="checkbox" id="assigned_req{$reqID}" name="req_id[{$reqID}]" /></td>
          					...
          					{sectionelse}
          						<tr>
          							<td></td>
          							<td><span class="bold">{$labels.req_msg_norequirement}</span></td>
      							</tr>
          					{/section}
      					</tbody>
          			</table>
