...
<h4>{$qex_labels.testplan_usage}</h4>
    ...
    {foreach from=$args_linked_versions item=link2tplan_platform}
		{foreach from=$link2tplan_platform item=link2platform key=tplan_id}
			...
			<tr>
				...
				<td>
					{$version_info.tplan_name|escape}
					<a href="{$execFeatureAction}" target="_parent" ><img class="clickable" src="{$tlImages.execute}" title="{$qex_labels.goto_execute}" /></a>
      			</td>
          		{if $gui->platforms != null}
            	<td>
