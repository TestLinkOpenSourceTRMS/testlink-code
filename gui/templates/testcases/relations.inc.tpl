...
<form method="post" action="{$basehref}lib/testcases/tcEdit.php">
	...
	<input type="hidden" name="relation_source_tcversion_id" id="relation_source_tcversion_id" value="{$gui->tcversion_id}" />
	...
	<h4>{$rel_labels.relations} {if $args_relations.num_relations > 0} ({$args_relations.num_relations}) {/if}</h4>
	<table class="table table-striped table-bordered" id="relations">
        {* Display Existent Relations *}
        {if $args_relations.num_relations > 0}
        <thead class="thead-dark">
            <tr>
                <th>{$rel_labels.relation_id} / {$rel_labels.relation_type}</th>
                <th colspan="1">{$rel_labels.test_case}</th>
                <th>{$rel_labels.relation_set_by}</th>
                <th>&nbsp;</th>
            </tr>
        </thead>
        <tbody>
		{foreach item=rx from=$args_relations.relations}
        	{$canDel = $args_edit_enabled && $args_frozen_version == 'no' && $rx.related_tcase.is_open && $rx.link_status == $smarty.const.LINK_TC_RELATION_OPEN}
        	{if $canDel == 0}
				{$cannotDelMsg = '...'}      
        	...
        	<tr>
				...
				<td>
            		<a href="javascript:openTCaseWindow({$rx.related_tcase.testcase_id},{$rx.related_tcase.id})">
            			{$rx.related_tcase.fullExternalID|escape}:{$rx.related_tcase.name|escape} [{$rel_labels.version} {$rx.related_tcase.version}]
        			</a>
    			</td>
          		<td>
          			<span title="{$rel_labels.title_created} {$rx.creation_ts} {$rel_labels.by} {$rx.author|escape}">
             			{$rx.author|escape}
         			</span>
     			</td>
		     	{if $canDel}
          			<td align="center">
            			<a href="javascript:relation_delete_confirmation({$args_relations.item.testcase_id}, {$rx.id}, delete_rel_msgbox_title, delete_rel_msgbox_msg, pF_delete_relation);">
           				<img src="{$tlImages.delete}" title="{$rel_labels.img_title_delete_relation}"  style="border:none" /></a>
          			</td>
	  			{else}
      		  		<td align="center">
      					<img style="border:none;" alt="{$cannotDelMsg}" title="{$cannotDelMsg}"	src="{$tlImages.vorsicht}" />
      		  		</td>
				{/if}
        	</tr>
		{/foreach}
		{if $args_edit_enabled}
			{$canWork = $args_is_latest_tcv == 1 || $tlCfg->testcase_cfg->addTCVRelationsOnlyOnLatestTCVersion == 0}
      		{if $canWork}
