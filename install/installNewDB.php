...
if ($upgrade) {
    ...
    $the_version_table = $my_ado->MetaTables('TABLES', false,
        $db_table_prefix . 'db_version');
        ...
        switch ($schema_version) {
            ...
            default:
                if (strlen($schema_version) == 0) {
                    ...
                    echo "<p class='notok'>This script doesn't recognize your schema version: " .
                        $schema_version . "</p>";
                }
...
$msg = create_user_for_db($dbDriverName, $db_name, $db_server, $db_admin_name,
    $db_admin_pass, $tl_db_login, $tl_db_passwd);
...
switch ($db_type) {
    ...
    case 'postgres':
        @$conn_result = $db->connect(NO_DSN, $db_server, $tl_db_login,
            $tl_db_passwd, $db_name);
        break;
}
...
if ($sqlParser->install_failed) {
    ...
    echo "<p />" .
        ...
        "The last error to occur was <i>" .
        $sqlParser->sql_errors[count($sqlParser->sql_errors) - 1]["error"] .
        '</i> during the execution of SQL statement <span class="mono">' .
        strip_tags(
            $sqlParser->sql_errors[count($sqlParser->sql_errors) - 1]["sql"]) .
        "</span>";
...
if (strcasecmp('ko', $yy['status']) == 0) {
    ...
    echo "<p />" .
        ...
        '<textarea style="width:400px; height:160px;">' . $yy['cfg_string'] .
        "</textarea>";
...
$successfull_message = '</b><p /><br><div><span class="headers">' .
    "{$inst_type_verbose} was successful!" . '</span><br>' .
...
function manual_operations($dbType)
{
    ...
    echo '</span><br>';
    echo '<h1>' . dirname(__FILE__) . '/sql/' . $dbType .
...
function drop_tables(&$dbHandler, $dbTablePrefix, $dbType)
{
    ...
    if (! empty($tablesOnDB) && isset($tablesOnDB[0])) {
...
function drop_views(&$dbHandler, $dbItemPrefix, $dbType)
{
    ...
    if (! empty($itemsOnDB) && isset($itemsOnDB[0])) {
