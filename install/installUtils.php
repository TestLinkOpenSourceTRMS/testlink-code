...
function getTableList($db)
{
    ...
    return $my_ado->MetaTables('TABLES', false, 'db_version');
}
    ...
    switch ($db_type) {
        ...
        case 'postgres':
            $result = $db->exec_query(
                'SELECT DISTINCT usename AS user FROM pg_user');
            break;
 ...
 *
 */
function create_user_for_db($db_type, $db_name, $db_server, $db_admin_name,
    $db_admin_pass, $login, $passwd)
{
    ...
    if ($try_create_user == 1 && ! empty($user_list)) {
        ...
        if (! $user_exists) {
            ...
            switch ($db_type) {
                ...
                case 'mssql':
                    $op = _mssql_make_user_with_grants($db, $the_host, $db_name,
                        $login, $passwd);
                    _mssql_set_passwd($db, $login, $passwd);
                    break;

                case 'postgres':
                    $op = _postgres_make_user_with_grants($db, $the_host,
                        $db_name, $login, $passwd);
                    break;
                ...
                default:
                    ...
                    // and assign rights
                    $op = _mysql_make_user($db, $the_host, $db_name, $login,
                        $passwd);
                    break;
            }
        } else {
            ...
            switch ($db_type) {
                case 'mysql':
                case 'mysqli':
                    $op = _mysql_assign_grants($db, $the_host, $db_name, $login,
                        $passwd);
                    break;

                case 'postgres':
                    $op = _postgres_assign_grants($db, $the_host, $db_name,
                        $login, $passwd);
                    break;

                case 'mssql':
                    $op = _mssql_assign_grants($db, $the_host, $db_name, $login,
                        $passwd);
                    break;
            }
        }
...
function _mysql_make_user($dbhandler, $db_host, $db_name, $login, $passwd)
{
    ...
    if (! @$dbhandler->exec_query($stmt)) {
        ...
        // Assign Grants!!
        $op = _mysql_assign_grants($dbhandler, $db_host, $db_name, $login,
            $passwd);
    }
 ...
 *
 */
function _postgres_make_user_with_grants(&$db, $db_host, $db_name, $login,
    $passwd)
{
    $op->status_ok = true;
    $op->msg = '';
    ...
    if ($int_op->status_ok) {
        $op->msg = $int_op->msg;
        $int_op = _postgres_assign_grants($db, $db_host, $db_name, $login,
            $passwd);
...
function _postgres_make_user(&$db, $db_host, $db_name, $login, $passwd)
{
    ...
    $sql = 'CREATE USER "' . $db->prepare_string($login) . '"' .
...
function _postgres_assign_grants(&$db, $db_host, $db_name, $login, $passwd)
{
    ...
    if ($op->status_ok) {
        $sql = 'ALTER SCHEMA public OWNER TO ' . '"' .
            $db->prepare_string($login) . '"';
        if (! @$db->exec_query($sql)) {
            $op->status_ok = false;
            $op->msg = "ko - " . $db->error_msg();
