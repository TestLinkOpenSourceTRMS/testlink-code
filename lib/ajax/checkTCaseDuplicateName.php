...
if ($_SESSION['currentUser']->hasRight($db, $rights2check[0],
    $args['testproject_id']) ||
    $_SESSION['currentUser']->hasRight($db, $rights2check[1],
        $args['testproject_id'])) {
    ...
    $check = $tree_manager->nodeNameExists($args['name'],
        $node_types_descr_id['testcase'], $args['testcase_id'],
        $args['testsuite_id']);
