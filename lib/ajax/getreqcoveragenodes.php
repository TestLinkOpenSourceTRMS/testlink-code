...
$show_children = isset($_REQUEST['show_children']) ? intval(
    $_REQUEST['show_children']) : 1;
$operation = isset($_REQUEST['operation']) ? $_REQUEST['operation'] : 'manage';

// for debug - file_put_contents('d:\request.txt', serialize($_REQUEST));
$nodes = display_children($db, $root_node, $node, $filter_node, $show_children,
    $operation);
    ...
    $show_children = ON, $operation = 'manage')
{
    ...
    $sql = " SELECT NHA.*, NT.description AS node_type, RSPEC.doc_id " .
        ...
        " ON NHA.node_type_id=NT.id " . " AND NT.description NOT IN " .
        ...
        " ON RSPEC.id = NHA.id " . " WHERE NHA.parent_id = " . intval($parent);
        ...
        foreach ($nodeSet as $key => $row) {
            ...
            switch ($row['node_type']) {
                ...
                case 'requirement_spec':
                    ...
                    $path['text'] = htmlspecialchars($row['doc_id'] . ":") .
                        $path['text'];
                    $path['forbidden_parent'] = $forbidden_parent[$row['node_type']];
                    if (! is_null($req_list)) {
