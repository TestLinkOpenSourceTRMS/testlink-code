...
class tlRestApi
{
    ...
    public function __construct()
    {
        ...
        $tl = array(
            'API_MISSING_REQUIRED_PROP' => null,
        ...
        $this->app->get('/testprojects', array(
            $this,
            'authenticate'
        ), array(
            $this,
        ...
        $this->app->get('/testprojects/:id', array(
            $this,
            'authenticate'
        ), array(
            $this,
        ...
        $this->app->get('/testplans/:id/builds', array(
            $this,
            'authenticate'
        ), array(
            $this,
        ...
        $this->app->post('/testprojects', array(
            $this,
            'authenticate'
        ), array(
            $this,
        ...
        $this->app->post('/executions', array(
            $this,
            'authenticate'
        ), array(
            $this,
        ...
        $this->app->post('/testplans/:id', array(
            $this,
            'authenticate'
        ), array(
            $this,
        ...
        $this->app->post('/testsuites', array(
            $this,
            'authenticate'
        ), array(
            $this,
        ...
        $this->cfg['exec']['codeStatus'] = array_flip(
            $this->cfg['exec']['statusCode']);
        ...
        // DEFAULTS
        $this->cfg['tcase']['defaults']['executionType'] = $this->cfg['tcase']['executionType']['manual'];

        $this->cfg['tcase']['defaults']['importance'] = config_get(
            'testcase_importance_default');
    ...
    public function whoAmI()
    {
        echo json_encode(
            array(
                'name' => __CLASS__ . ' : You have called Get Route /whoAmI'
            ));
    }
    ...
    public function superman()
    {
        echo json_encode(
            array(
                'name' => __CLASS__ .
                ' : You have called the Get Route /superman'
            ));
    }
     ...
     *
     * @param
     *            mixed idCard if provided identifies test project
    ...
    public function getProjects($idCard = null, $opt = null)
    {
        ...
        if (is_null($idCard)) {
            ...
            $op['item'] = $this->tprojectMgr->get_accessible_for_user(
            ...
            $zx = $this->tprojectMgr->get_accessible_for_user($this->userID,
                $opOptions);
            ...
            if ($tcaseIDSet !== []) {
                ...
                foreach ($tcaseIDSet as $tcaseID) {
                    ...
                    $item['customfields'] = $this->tcaseMgr->get_linked_cfields_at_design(
                        $tcaseID, $item['tcversion_id'], null, null,
                        $tproject['id']);
                    $op['items'][] = $item;
                }
            }
    ...
    public function createTestCaseExecution()
    {
        ...
        try {
            ...
            if (! property_exists($ex, 'platformID')) {
                $ex->platformID = 0;
            }

            if (! property_exists($ex, 'executionType')) {
                $ex->executionType = $this->cfg['tcase']['executionType']['auto'];
            }
    ...
    private function checkExecutionEnvironment($ex)
    {
        ...
        $getOpt = array(
            'output' => 'testPlanFields',
            'active' => 1,
            'testPlanFields' => 'id,testproject_id,is_public'
        );
        $status_ok = ! is_null(
            $testPlan = $this->tplanMgr->get_by_id($ex->testPlanID, $getOpt));
        ...
        if ($status_ok) {
            ...
            $getOpt = array(
                'fields' => 'id,active,is_open',
                'buildID' => $ex->buildID,
                'orderBy' => null
            );
            $status_ok = ! is_null(
                $build = $this->tplanMgr->get_builds($ex->testPlanID, null, null,
                    $getOpt));

            if ($status_ok) {
                // now check is execution can be done againts this build
                $msg = 'Build is not active and/or closed => execution can not be done';
                $status_ok = $build[$ex->buildID]['active'] &&
                    $build[$ex->buildID]['is_open'];
            }
        }
        ...
        if ($status_ok && property_exists($ex, 'platformID')) {
            ...
            $platformSet = $this->tplanMgr->getPlatforms($ex->testPlanID,
                $getOpt);
        ...
        if ($status_ok) {
            ...
            if ($status_ok = ($tcaseID > 0)) {
                ...
                $testCaseTestProject = $this->tcaseMgr->getTestProjectFromTestCase(
                    $tcaseID, 0);
            ...
            if ($status_ok) {
                ...
                $links = $this->tcaseMgr->get_linked_versions($tcaseID,
                    $getFilters, $getOpt);
        ...
        if ($status_ok) {
            ...
            $status_ok = ($ex->statusCode !=
                $this->cfg['exec']['statusCode']['not_run']);

            if ($status_ok) {
                $msg = 'Requested execution status is not configured on TestLink';
                $status_ok = isset(
                    $this->cfg['exec']['codeStatus'][$ex->statusCode]);
            }
        }
        ...
        if ($status_ok) {
            ...
            if (isset(
                ...
                $ret->testCaseVersionNumber = $links[$ret->testCaseVersionID][$ex->testPlanID][$platform]['version'];
                $status_ok = true;
            }
        }
    ...
    public function createTestSuite()
    {
        ...
        try {
            ...
            $op['id'] = $this->tsuiteMgr->createFromObject($item,
                array(
                    'doChecks' => true
                ));
        } catch (Exception $e) {
            $this->app->status(500);
            $op['message'] = __METHOD__ . ' >> ' . msgFromException($e);
        }
        echo json_encode($op);
    }
    ...
    public function createTestCase()
    {
        ...
        try {
            ...
            if (is_null($item)) {
                throw new Exception(
                    "Fatal Error " . __METHOD__ .
                    " json_decode(requesBody) is NULL", 1);
            }
     ...
     *            string [open]
     * @param
     *            string [releasedate]: format YYYY-MM-DD;
     * @param
     *            int [copytestersfrombuild]
     *
     *            step 1) is a number ?
    ...
    public function createBuild()
    {
        ...
        if ($statusOK) {
            ...
            if (is_numeric($item->testplan)) {
                ...
                if (null == $tplan) {
                    $statusOK = false;
                    $op['details'][] = sprintf(
                        $this->l10n['API_TESTPLAN_ID_DOES_NOT_EXIST'],
                        $item->testplan);
                    $this->app->status(404);
                }
            } else {
                ...
                if (null == $tplan) {
                    $statusOK = false;
                    $op['details'][] = sprintf(
                        $this->l10n['API_TESTPLAN_APIKEY_DOES_NOT_EXIST'],
                        $item->testplan);
                    $this->app->status(404);
                }
            }
        ...
        if ($statusOK) {
            ...
            if (! $this->userHasRight($rightToCheck, true, $context)) {
                $statusOK = false;
                $msg = lang_get('API_INSUFFICIENT_RIGHTS');
                $op['message'] = sprintf($msg, $rightToCheck, $this->user->login,
        ...
        if ($statusOK) {
            $build->id = $this->tplanMgr->get_build_id_by_name(
                $context['tplan_id'], $build->name);

            if ($build->id > 0) {
                $statusOK = false;
                $op['message'] = sprintf(
                    $this->l10n['API_BUILDNAME_ALREADY_EXISTS'], $build->name,
                    $build->id);
                $this->app->status(409);
            }
        ...
        if ($statusOK) {
            ...
            $buildProp = array(
                ...
                'copy_testers_from_build' => 'copytestersfrombuild'
            );
    ...
    private function getUserIDByAttr($user)
    {
        ...
        if (property_exists($user, 'login')) {
            $run = true;
            $sql .= " WHERE login='" .
                $this->db->prepare_string(trim($user->login)) . "'";
        }

        if (! $run && property_exists($user, 'id')) {
            $run = true;
    ...
    private function buildTestCaseObj(&$obj)
    {
        ...
        if (property_exists($obj, 'author') &&
            property_exists($obj->author, 'login') ||
            property_exists($obj->author, 'id')) {
        ...
        if ($accessKey['testProject']['prefix']) {
            $gFilters = array(
                'prefix' => array(
                    'op' => '=',
                    'value' => trim($obj->testProject->prefix)
                )
            );
            $msg .= "prefix={$obj->testProject->prefix} ";
        }

        $info = $this->tprojectMgr->get_accessible_for_user($this->userID, $gOpt,
            $gFilters);
        ...
        $tcfg = $this->cfg['tcase'];
        $ck2d = array(
            'executionType' => $tcfg['executionType']['manual'],
            'importance' => $tcfg['defaults']['importance'],
            'status' => $tcfg['status']['draft']
        );

        foreach ($ck2d as $prop => $defa) {
            $tcase->$prop = property_exists($obj, $prop) ? $tcfg[$prop][$obj->$prop->name] : $defa;
        }
    ...
    private function checkRelatives($ctx)
    {
        ...
        if ($testProjectID !=
            $this->tsuiteMgr->getTestProjectFromTestSuite($testSuiteID,
                $testSuiteID)) {
        ...
        $checkPublicPrivateAttr = false, $context = null)
    {
        ...
        if (is_null($context)) {
            ...
            $tproject_id = intval(
                isset($context['tproject_id']) ? $context['tproject_id'] : 0);
    ...
    public function createKeyword()
    {
        ...
        try {
            ...
            if (is_null($item)) {
                throw new Exception(
                    "Fatal Error " . __METHOD__ .
                    " json_decode(requestBody) is NULL", 1);
            }
            ...
            if (null == $pid) {
                ...
                $pid = $pid['id'];
                $ou = $this->tprojectMgr->addKeyword($pid, $item->keyword,
                    $item->notes);
                $op = array(
                    'status' => 'ok',
                    'message' => 'ok',
     ...
     *
     * @param
     *            mixed idCard identifies test plan via apikey
     *
     */
    ...
    public function updateBuild($id)
    {
        ...
        if ($statusOK) {
            ...
            if (null == $build) {
                $statusOK = false;
                $op['message'] = sprintf($this->l10n['API_INVALID_BUILDID'], $id);
                $this->app->status(404);
            }
        }
        ...
        if ($statusOK && property_exists($item, 'name') &&
            $this->tplanMgr->check_build_name_existence($tplan['id'],
                $item->name, $id)) {
            $statusOK = false;
            $op['message'] = sprintf(
                $this->l10n['API_BUILDNAME_ALREADY_EXISTS'], $item->name, $id);
            $this->app->status(409);
        }
        ...
        if ($statusOK) {
            ...
            $ox = $this->buildMgr->update($build['id'], $buildObj->name,
                $buildObj->notes, $attr);
            ...
            if ($statusOK) {
                ...
                foreach ($plat2link as $accessObj) {
                    ...
                    if (property_exists($accessObj, 'name')) {
                        ...
                        if (! $checkOK) {
                            $op['message'][] = "Platform with name:" . $needle .
                                " does not exist";
                        }
                    }
                    ...
                    if (property_exists($accessObj, 'id')) {
                        ...
                        $statusOK = $statusOK && $checkOK;
                        if (! $checkOK) {
                            $op['message'][] = "Platform with id:" . $needle .
                                " does not exist";
                        }
                    }
                }
    ...
    /**
     */
    private function byeHTTP500($msg = null)
    {
        $op = array();
        if (null == $msg) {
    ...
    /**
     */
    private function msgFromException($e)
    {
