...
class tlRestApi
{
    ...
    if(is_null($idCard)) {
      ...
      if( ($safeID = intval($idCard)) > 0) {
        ...
        foreach( $zx as $itemID => $value ) {
          if( strcmp($value['name'],$idCard) == 0 || strcmp($value['prefix'],$idCard) == 0 ) {
            $targetID = $itemID;
            break;
          }
        }
  ...
  public function createBuild() {
    ...
    if( $statusOK ) {
      ...
      if( is_numeric($item->testplan) ) {
        ...
        if( null == $tplan ) {
          $statusOK = false;
          $op['details'][] = sprintf($this->l10n['API_TESTPLAN_ID_DOES_NOT_EXIST'],$item->testplan);
          $this->app->status(404);
        }
      } else {
  ...
  public function updateBuild($id) {
    ...
    if( $statusOK && property_exists($item,'name') && $this->tplanMgr->check_build_name_existence(
