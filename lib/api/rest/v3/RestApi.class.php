...
class RestApi
{
    ...
    public function __construct()
    {
        ...
        $this->cfg['tcase']['executionType']['automatic'] = $this->cfg['tcase']['executionType']['auto'];
     ...
     *
     * @param
     *            array idCard if provided identifies test project
     ...
     *
     */
    public function updateBuild(Request $request, Response $response, $args)
    {
        $op = array(
            'status' => 'ko',
            'message' => 'ko',
    ...
    public function createTestPlan(Request $request, Response $response)
    {
        ...
        try {
            ...
            if (property_exists($item, 'testProjectPrefix')) {
                $pi = $this->tprojectMgr->get_by_prefix(
                    trim($item->testProjectPrefix));
     ...
     * 'is_public'
     */
    public function updateTestPlan(Request $request, Response $response, $args)
    {
        $op = $this->getStdIDKO();
        $id = intval($args['id']);
        try {
     ...
     */
    public function addPlatformsToTestPlan(Request $request, Response $response,
        $args)
    {
        $op = $this->getStdIDKO();
            ...
            if ($statusOK) {
                ...
                foreach ($plat2link as $accessObj) {
                    ...
                    if (property_exists($accessObj, 'name')) {
                        ...
                        if (! $checkOK) {
                            $op['message'][] = " WARNING! - Platform with name:" .
