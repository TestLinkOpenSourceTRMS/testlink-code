...
$unitTestDescription = "XMLRPC -Test - Call with valid parameters: testPlanID,testCaseID,buildID";
...
function executeTestCase($server_url, $context, $exec, $debug = false)
{
    ...
    if (property_exists($context, 'testcaseid') &&
        ! is_null($context->testcaseid)) {
        $data["testcaseid"] = $context->testcaseid;
    } elseif (property_exists($context, 'testcaseexternalid') &&
        ! is_null($context->testcaseexternalid)) {
    ...
    } elseif (property_exists($context, 'buildname') &&
        ! is_null($context->buildname)) {
        $data["buildname"] = $context->buildname;
    }

    if (property_exists($context, 'platformname') &&
