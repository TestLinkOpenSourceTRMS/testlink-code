...
class SampleXMLRPCServer extends TestlinkXMLRPCServer
{
    ...
    private function _getTestSuiteByName($args)
    {
        ...
        if ($num == 0) {
            $msg = $msg_prefix .
                sprintf(
                    "Name %s does not belong to a test suite present on system!",
                    $testSuiteName);
            $this->errors[] = new IXR_Error(8004, $msg);
        } else {
            // Check project id
            foreach ($result as $row) {
                $projectid = $this->tsuiteMgr->getTestProjectFromTestSuite(
                    $row['id']);
            ...
            if (! $status_ok) {
                ...
                $msg = sprintf(TESTSUITE_DONOTBELONGTO_TESTPROJECT_STR,
                    $result[0]['id'], $tprojectInfo['name'],
                    $args[self::$testProjectIDParamName]);
                $this->errors[] = new IXR_Error(
                    TESTSUITE_DONOTBELONGTO_TESTPROJECT, $msg_prefix . $msg);
            }
        }
