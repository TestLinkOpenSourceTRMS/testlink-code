 ...
 *
 * @param array $hash
 * @return array only with related to custom fields, where
...
function initArgs()
{
    ...
    $args->cfield_id = isset($_REQUEST['cfield_id']) ? intval(
        $_REQUEST['cfield_id']) : 0;
...
function edit(&$argsObj, &$cfieldMgr)
{
    ...
    if ($cfinfo) {
        ...
        $op->operation_descr = lang_get('title_cfield_edit') . TITLE_SEP_TYPE3 .
            $op->cf['name'];
        $op->linked_tprojects = $cfieldMgr->get_linked_testprojects(
            $argsObj->cfield_id);
        $op->cf_is_linked = ! is_null($op->linked_tprojects) &&
            count($op->linked_tprojects) > 0;
    }
    return $op;
}
...
function doCreate(&$hash_request, &$cfieldMgr, &$argsObj)
{
    ...
    if (is_null($dupcf)) {
        ...
        if (! $ret['status_ok']) {
            ...
            $op->template = null;
            logAuditEvent(TLS("audit_cfield_created", $op->cf['name']), "CREATE",
...
function doUpdate(&$hash_request, &$argsObj, &$cfieldMgr)
{
    ...
    if ($is_unique) {
        ...
        if ($ret) {
            $op->template = null;
            logAuditEvent(TLS("audit_cfield_saved", $op->cf['name']), "SAVE",
                $op->cf['id'], "custom_fields");
        }
    } else {
        $op->user_feedback = lang_get("cf_name_exists");
    }
...
function doDelete(&$argsObj, &$cfieldMgr)
{
    ...
    if ($cf) {
        ...
        if ($cfieldMgr->delete($argsObj->cfield_id)) {
            logAuditEvent(TLS("audit_cfield_deleted", $cf['name']), "DELETE",
                $argsObj->cfield_id, "custom_fields");
        }
    }
    return $op;
}
