...
function initArgs()
{
    ...
    $args->goback_url = $_SESSION['basehref'] . 'lib/cfields/cfieldsView.php';
...
function doImport(&$dbHandler)
{
    ...
    if (($source != 'none') && ($source != '')) {
        ...
        if (move_uploaded_file($source, $dest)) {
            $file_check['status_ok'] = (($xml = @simplexml_load_file_wrapper(
                $dest)) !== false);
        }
            ...
            foreach ($xml as $cf) {
                if (is_null($cfield_mgr->get_by_name($cf->name))) {
                    ...
                    $import_msg['ok'][] = sprintf(
                        lang_get('custom_field_imported'), $cf->name);
                } else {
                    $import_msg['ko'][] = sprintf(
                        lang_get('custom_field_already_exists'), $cf->name);
                }
            }
        } else {
