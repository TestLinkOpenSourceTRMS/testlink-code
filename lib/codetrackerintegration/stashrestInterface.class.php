...
class stashrestInterface extends codeTrackerInterface
{
    ...
    function connect()
    {
        try {
            ...
            if (! is_null($this->stashCfg['proxy']) &&
                is_null($this->stashCfg['proxy']->host)) {
    ...
    /**
     */
    public function getRepoContent($projectKey, $repoName, $path = '',
        $branch = '', $commit_id = '', $type = false)
    {
        try {
            return $this->APIClient->getRepoContent($projectKey, $repoName,
                $path, $branch, $commit_id, $type);
        } catch (Exception $e) {
            tLog(__METHOD__ . "  " . $e->getMessage(), 'ERROR');
        }
    }
    ...
    /**
     */
    public function getRepoContentForHTMLSelect($projectKey, $repoName,
        $path = '', $branch = '', $type = false)
    {
        $ret = null;
        $contentList = $this->getRepoContent($projectKey, $repoName, $path,
            $branch, '', $type);
    ...
    {
        try {
            return $this->APIClient->getCommits($projectKey, $repoName,
                $branchName);
        } catch (Exception $e) {
            tLog(__METHOD__ . "  " . $e->getMessage(), 'ERROR');
        }
    ...
    public function getCommitsForHTMLSelect($projectKey, $repoName, $branchName)
    {
        ...
        if (property_exists($commitList, 'values')) {
            ...
            foreach ($commitList->values as $elem) {
                $ret[$elem->id] = $elem->displayId . ' (' .
                    @strftime($localesDateFormat,
    ...
    private function objectAttrToKeyName($attrSet)
    {
        ...
        if (! is_null($attrSet)) {
            ...
            for ($idx = 0; $idx < $ic; $idx ++) {
                $ret[$attrSet[$idx]->key] = $attrSet[$idx]->name . " (" .
                    $attrSet[$idx]->key . ")";
            }
        }
        return $ret;
