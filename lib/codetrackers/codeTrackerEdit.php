 ...
 *
 * @filesource	codeTrackerEdit.php
...
function initializeGui(&$dbHandler, &$argsObj, &$commandMgr)
{
    ...
    if ($argsObj->id > 0) {
        ...
        // Now get good info
        $gui->testProjectSet = $commandMgr->codeTrackerMgr->getLinks(
            $argsObj->id);
    }
    return $gui;
}
...
{
    return $user->hasRight($db, 'codetracker_management');
}
?>
