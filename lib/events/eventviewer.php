...
switch ($args->doAction) {
    case 'clear':
        ...
        if (is_null($args->logLevel)) {
            logAuditEvent(
                TLS("audit_all_events_deleted", $args->currentUser->login),
                "DELETE", null, "events");
        } else {
            ...
            logAuditEvent(
                TLS("audit_events_with_level_deleted", $args->currentUser->login,
                    $logLevelVerbose), "DELETE", null, "events");
        }
...
$gui->events = $g_tlLogger->getEventsFor($args->logLevel,
    $args->object_id ? $args->object_id : null,
    $args->object_type ? $args->object_type : null, null, 500,
    $filters->startTime, $filters->endTime, $filters->users);
...
function buildExtTable($gui, $show_icon, $charset)
{
    ...
    if (! empty($gui->events)) {
        ...
        foreach ($gui->events as $event) {
            ...
            if (isset($event->userID) && $event->userID &&
                isset($gui->users[$event->userID])) {
                $rowData[] = $gui->users[$event->userID];
            } else {
                $rowData[] = lang_get("not_aplicable");
            }
            $description = htmlentities($event->description, ENT_QUOTES,
                $charset);
            $rowData[] = "<!--" . $description . "-->" .
                "<a onClick=\"showEventDetails({$event->dbID});\" style=\"cursor: hand; cursor: pointer;\">" .
                "<img title=\"" . lang_get("show_eventdetails") .
                "\" src=\"{$show_icon}\" /> </a>" . $description;
