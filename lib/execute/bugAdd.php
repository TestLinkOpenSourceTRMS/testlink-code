...
if($info['issue_tracker_enabled'])
{
    ...
    if( method_exists($its,'addIssue') ) 
    {
      ...
      $dummy = $tcaseMgr->tree_manager->get_node_hierarchy_info($args->tcversion_id);
      $item = $tcaseMgr->get_by_id($dummy['parent_id'],$args->tcversion_id);
      $item = $item[0];
      unset($tcaseMgr);
      
      $rs = $its->addIssue($item['name'],'generated by execution id:' . $args->exec_id);    
      if($rs['id'] > 0)
      {
        $gui->automaticMode = true;
  			if (write_execution_bug($db,$args->exec_id, $rs['id']))
  			{
  				$msg = lang_get("bug_added");
  				logAuditEvent(TLS("audit_executionbug_added",$rs['id']),"CREATE",$args->exec_id,"executions");
  			}
      }
    }
	}
