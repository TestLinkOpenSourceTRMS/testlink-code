            ...
            case 'link':
                ...
                if ($its->checkBugIDSyntax($args->bug_id)) {
                    ...
                    if ($its->checkBugIDExistence($bugID)) {
                        if (write_execution_bug($db, $args->exec_id, $bugID,
                            $args->tcstep_id)) {
                            $gui->msg = lang_get("bug_added");
                            logAuditEvent(
                                TLS("audit_executionbug_added", $args->bug_id),
                                "CREATE", $args->exec_id, "executions");
                            ...
                            if ($gui->issueTrackerCfg->tlCanAddIssueNote) {
                                ...
                                if ($args->addLinkToTL ||
                                    $args->addLinkToTLPrintView || $hasNotes) {
                                    $opt = new stdClass();
...
function initEnv(&$dbHandler)
{
    ...
    if ($args->exec_id) {
        ...
        $args->exec_id = intval(
            isset($_SESSION['bugAdd_execID']) ? $_SESSION['bugAdd_execID'] : 0);
    }
    ...
    $gui->addLinkToTLPrintViewChecked = $cfg->exec_mode->addLinkToTLPrintViewChecked;
...
function getIssueTracker(&$dbHandler, $argsObj, &$guiObj)
{
    ...
    if ($info['issue_tracker_enabled']) {
        ...
        if (! is_null($issueTrackerCfg)) {
            ...
            $guiObj->issueTrackerCfg->tlCanCreateIssue = method_exists($its,
                'addIssue');
            $guiObj->issueTrackerCfg->tlCanAddIssueNote = method_exists($its,
                'addNote');
        }
    }
    return array(
...
function getDirectLinkToExec(&$dbHandler, $execID)
{
    ...
    $sql = " SELECT EX.id,EX.build_id,EX.testplan_id," .
        ...
        " AND TPTCV.platform_id=EX.platform_id " . " WHERE EX.id=" .
        intval($execID);

    $rs = $dbHandler->get_recordset($sql);
    $rs = $rs[0];
    return trim($_SESSION['basehref'], '/') . "/ltx.php?item=exec&feature_id=" .
        $rs['feature_id'] . "&build_id=" . $rs['build_id'];
}
...
{
    return $user->hasRightOnProj($db, "testplan_execute");
}
