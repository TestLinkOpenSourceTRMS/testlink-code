...
function doUpdate(&$db, &$args, &$tcaseMgr, &$request)
{
    ...
    $cfield_mgr->execution_values_to_db($request, $args->tcversion_id,
        $args->exec_id, $args->tplan_id);
}
...
function initializeGui(&$argsObj, &$tcaseMgr)
{
    ...
    $guiObj->edit_enabled = $argsObj->user->hasRight($db, "exec_edit_notes") ==
        'yes' ? 1 : 0;
    $guiObj->cfields_exec = $tcaseMgr->html_table_of_custom_field_inputs(
        $argsObj->tcversion_id, null, 'execution', '_cf', $argsObj->exec_id,
...
function checkRights(&$db, &$user)
{
    return $user->hasRight($db, "testplan_execute") &&
        $user->hasRight($db, "exec_edit_notes");
}
