...
function initArgs(&$dbHandler)
{
    ...
    if (is_null($args->platform_id) || ($args->platform_id <= 0)) {
        ...
        if (! is_null($itemSet)) {
            ...
            $args->platform_id = isset($_SESSION[$key]) ? intval(
                $_SESSION[$key]) : null;
            if (is_null($args->platform_id) || ($args->platform_id <= 0)) {
...
function initializeRights(&$dbHandler, &$userObj, $tproject_id, $tplan_id)
{
    ...
    $grants->execute = $userObj->hasRight($dbHandler, "testplan_execute",
    ...
    $grants->delete_execution = $userObj->hasRight($dbHandler, "exec_delete",
        $tproject_id, $tplan_id);
    ...
    $grants->edit_exec_notes = $grants->execute &&
        $userObj->hasRight($dbHandler, "exec_edit_notes", $tproject_id,
            $tplan_id);

    $grants->edit_testcase = $userObj->hasRight($dbHandler, "mgt_modify_tc",
        $tproject_id, $tplan_id);
