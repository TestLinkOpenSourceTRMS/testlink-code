...
$tcversion_id = null;
$submitResult = null;
list ($args, $its, $cts) = initArgs($db, $cfg);
...
if (! is_null($linked_tcversions)) {
    ...
    $gui->plugins = array();
    $gui->plugins['EVENT_TESTRUN_DISPLAY'] = event_signal(
        'EVENT_TESTRUN_DISPLAY', $ctx);
    ...
    if ($args->doSave || $args->doNavigate || $args->saveStepsPartialExec) {
        ...
        if ($args->save_results || $args->do_bulk_save) {
            ...
            list ($execSet, $gui->addIssueOp, $gui->uploadOp) = write_execution(
            ...
            if ($args->copyAttFromLEXEC && $cfg->exec_cfg->exec_mode->new_exec &&
                $args->level == 'testcase' && $latestExecIDInContext > 0) {
                // need to copy :
                ...
                $to = (array) $db->fetchRowsIntoMap(
                    $sql . $execSet[$tcversion_id], 'step_number');
            ...
            if ($args->assignTask) {
                $fid = $tplan_mgr->getFeatureID($args->tplan_id,
                    $args->platform_id, $args->version_id);
            ...
            if ($args->level == 'testcase') {
                ...
                logAuditEvent(
                    TLS("audit_exec_saved", $tc_info[0], $build_info['name'],
                        $tp_info['name']), "CREATE", $execSet[$tcversion_id],
                    "execution");
            }
        }
        ...
        if ($args->save_and_next || $args->doMoveNext || $args->doMovePrevious) {
            ...
            if ($cfg->exec_cfg->exec_mode->save_and_move == 'unlimited') {
                if ($args->caller == 'tcAssignedToMe') {
                    $optz = array(
                        'order_by' => 'ORDER BY TPTCV.node_order'
                    );
            ...
            $opt4sibling = array(
                'move' => $args->moveTowards
            );
            switch ($args->caller) {
                case 'tcAssignedToMe':
                    $doSingleStep = true;
                    $opt4sibling['assigned_to'] = array(
            ...
            switch ($cfg->exec_cfg->exec_mode->save_and_move) {
                case 'unlimited':
                    ...
                    $opx = array(
                        'tcase_id' => $args->testcases_to_show[$nextInChain]
                    );
                    $nextItem = $tplan_mgr->get_linked_tcvid($args->tplan_id,
                        $args->platform_id, $opx);
                    $nextItem = current($nextItem);
                    break;

                case 'limited':
                    $nextItem = $tplan_mgr->getTestCaseNextSibling(
                        $args->tplan_id, $tcversion_id, $args->platform_id,
                        $opt4sibling);
                    if (! $doSingleStep) {
                        while (! is_null($nextItem) &&
                            ! in_array($nextItem['tcase_id'],
                                $args->testcases_to_show)) {
                            $nextItem = $tplan_mgr->getTestCaseNextSibling(
    ...
    if (! $args->reload_caller) {
        if ($args->doDelete) {
            ...
            if ($dummy) {
                ...
                $build_info = $tplan_mgr->get_build_by_id($args->tplan_id,
                    $args->build_id);
                logAuditEvent(
                    TLS("audit_exec_deleted", $tc_info[0], $build_info['name'],
                        $tp_info['name']), "DELETE", $args->exec_to_delete,
                    "execution");
            }
        }
        ...
        if ($args->level == 'testcase') {
            ...
            if ($args->reqEnabled) {
                $gui->req_details = $req_mgr->getActiveForTCVersion(
                    $tcversion_id);
            }
...
/**
 */
function initArgs(&$dbHandler, $cfgObj)
{
    $args = new stdClass();
    ...
    if (($args->treeFormToken = isset($_REQUEST['form_token']) ? $_REQUEST['form_token'] : 0) >
        0) {
    ...
    if (is_null($args->refreshTree)) {
        $args->refreshTree = isset($_REQUEST['refresh_tree']) ? intval(
            $_REQUEST['refresh_tree']) : 0;
    }
    ...
    $args->tproject_id = intval(
        isset($_REQUEST['tproject_id']) ? $_REQUEST['tproject_id'] : $_SESSION['testprojectID']);
 ...
 *
 */
function manageHistoryOn($hash_REQUEST, $hash_SESSION, $exec_cfg, $btn_on_name,
    $btn_off_name, $hidden_on_name)
{
 ...
 * [tsuite_name] => ts1)
 */
function getTestsuiteNameDetails(&$db, $tcase_id)
{
    $tables = array();
 ...
 *
 */
function smartyAssignTestsuiteInfo(&$smarty, &$tree_mgr, $tcase_id, $tproject_id,
    $cfgObj)
{
    if ((intval($tcase_id)) <= 0) {
        // hmm, no good
        return;
    }
    ...
    $tsuite_info = getTestsuiteNameDetails($tree_mgr->db, $tcase_id);
    ...
    foreach ($fpath as $key => $value) {
        ...
        foreach ($value['name'] as $jdx => $elem) {
            $str .= "<a href=\"javascript:openTestSuiteWindow(" .
                $value['node_id'][$jdx] . ")\"> ";
    ...
    if (! is_null($tsuite_info)) {
        ...
        foreach ($tsuite_info as $key => $elem) {
            ...
            if (! isset($cached_cf[$tsuite_id])) {
                $cached_cf[$tsuite_id] = $tsuite_mgr->html_table_of_custom_field_values(
                    $tsuite_id, 'design', null, $tproject_id);
            }
...
function doRemoteExecution(&$dbHandler, $context)
{
    ...
    foreach ($target['tc_versions'] as $version_id => $tcase_id) {
        ...
        $serverCfg = $cfield_mgr->getXMLRPCServerParams($version_id,
            $target['feature_id'][$version_id]);
        $execResult[$version_id] = executeTestCase($tcaseInfo, $serverCfg,
            $context['context']); // RPC call
...
function initializeExecMode(&$db, $exec_cfg, $userObj, $tproject_id, $tplan_id)
{
    ...
    $use_exec_cfg = isset($simple_tester_roles[$effective_role->dbID]) ||
    ...
    $platform_id, $build_id)
{
    foreach ($exec_info as $version_id => $value) {
        ...
        $exec_info[$version_id]['assigned_user'] = implode(',',
            (array) $exec_info[$version_id]['assigned_user']);
        $exec_info[$version_id]['assigned_user_id'] = implode(',',
            (array) $exec_info[$version_id]['assigned_user_id']);
    }
    return $exec_info;
}
 ...
 *
 */
function createExecNotesWebEditor(&$tcversions, $basehref, $editorCfg, $execCfg,
    $initValue = null)
{
    ...
    if ($execCfg->exec_mode->new_exec == 'latest') {
        ...
        $itemTemplateValue = getItemTemplateContents('execution_template',
            'notes', null);
    }
...
function getCfg()
{
    ...
    $cfg->kwHeadsUpTSuiteOnExec = trim(
        config_get('keywords')->headsUpTSuiteOnExec);
    ...
    &$issueTracker, &$codeTracker)
{
    ...
    list ($gui->execStatusValues, $gui->execStepStatusValues) = initExecValuesMenus(
    ...
    if (! $argsObj->followTheWhiteRabbit && ! $argsObj->statusSingle ||
    ...
    if ($argsObj->filter_assigned_to) {
        $userSet = tlUser::getByIds($dbHandler,
            array_values($argsObj->filter_assigned_to));
        if ($userSet) {
    ...
    $gui->grants = initializeRights($dbHandler, $argsObj->user,
    ...
    $gui->history_on = manageHistoryOn($_REQUEST, $_SESSION, $cfgObj->exec_cfg,
    ...
    $gui->platform_div_title = lang_get('platform') . ' ' .
        $gui->platform_info['name'];
    ...
    if ($issueTrackerExists) {
        if ($issueTracker->isConnected()) {
            ...
            if ($gui->issueTrackerCfg->crudIssueViaAPI) {
                // get metadata
                $gui->issueTrackerMetaData = getIssueTrackerMetaData(
                    $issueTracker);
                $gui->tlCanCreateIssue = method_exists($issueTracker, 'addIssue') &&
                    $issueTracker->canCreateViaAPI();
 ...
 * processTestCase
 */
function processTestCase($tcase, &$guiObj, &$argsObj, &$cfgObj, $tcv, &$treeMgr,
    &$tcaseMgr, &$docRepository)
{
    ...
    if ($guiObj->grants->execute) {
        if ($eid > 0) {
            ...
            $cf_map = $tcaseMgr->get_linked_cfields_at_execution($tcversion_id,
                null, null, $eid, $argsObj->tplan_id, $argsObj->tproject_id);
        }

        $guiObj->execution_time_cfields[$tcase_id] = $tcaseMgr->html_table_of_custom_field_inputs(
            $tcase_id, null, 'execution', "_{$tcase_id}", null, null,
            $argsObj->tproject_id, null, $cf_map);
    }

    $guiObj->tcAttachments[$tcase_id] = getAttachmentInfos($docRepository,
        $tcversion_id, 'tcversions', 1);

    foreach ($locationFilters as $locationKey => $filterValue) {
        $finalFilters = $cf_filters + $filterValue;
        $guiObj->design_time_cfields[$tcase_id][$locationKey] = $tcaseMgr->html_table_of_custom_field_values(
            $tcase_id, 'design', $finalFilters, null, null,
            $argsObj->tproject_id, null, $tcversion_id);

        $guiObj->testplan_design_time_cfields[$tcase_id] = $tcaseMgr->html_table_of_custom_field_values(
            $tcversion_id, 'testplan_design', $cf_filters, null, null,
            $argsObj->tproject_id, null, $link_id);
    }

    $tc_info = $treeMgr->get_node_hierarchy_info($tcase_id);
    $guiObj->tSuiteAttachments[$tc_info['parent_id']] = getAttachmentInfos(
        $docRepository, $tc_info['parent_id'], 'nodes_hierarchy', true, 1);
    // Direct Link
    $lk = current($tcv);
    $guiObj->direct_link = trim($_SESSION['basehref'], '/') .
        "/ltx.php?item=exec&feature_id=" . $lk[0]['feature_id'] . "&build_id=" .
        $argsObj->build_id;
    ...
    list ($guiObj->bug_summary, $guiObj->issueSummaryForStep) = genIssueSummary(
 ...
 *
 */
function getLatestExec(&$dbHandler, $tcase_id, $tcversion_id, $guiObj, $argsObj,
    &$tcaseMgr)
{
    $options = array(
        'getNoExecutions' => 1,
    ...
    if (! is_null($last_exec)) {
        ...
        $last_exec = setCanExecute($last_exec, $guiObj->exec_mode, $can_execute,
            $argsObj->user_id);
    }
...
function getBackupSteps(&$tcaseMgr, $guiObj, $testPlanId, $platformId, $buildId)
{
    ...
    return $tcaseMgr->getBackupSteps($stepsIds, $testPlanId, $platformId,
        $buildId);
}
 ...
 * rev:
 */
function getOtherExecutions(&$dbHandler, $tcase_id, $tcversion_id, $guiObj,
    $argsObj, &$cfgObj, &$tcaseMgr)
{
    $other_execs = null;
    if ($guiObj->history_on) {
        ...
        $options = array(
            'exec_id_order' => $cfgObj->exec_cfg->history_order
        );
        $other_execs = $tcaseMgr->getExecutionSet($tcase_id, $tcversion_id,
            $execContext, $options);
    } else {
        // Warning!!!:
 ...
 *
 */
function processTestSuite(&$dbHandler, &$guiObj, &$argsObj, $testSet, &$treeMgr,
    &$tcaseMgr, &$docRepository)
{
    $locationFilters = $tcaseMgr->buildCFLocationMap();
    $cf_filters = array(
    ...
    if ($testCaseQty > 0) {
        ...
        $suffix = '_' . $index;
        $execution_time_cfields = $tcaseMgr->html_table_of_custom_field_inputs(
            $dummy_id, $argsObj->tproject_id, 'execution', $suffix, null, null,
            $argsObj->tproject_id);
        ...
        foreach ($testSet->tcase_id as $testcase_id) {
            ...
            foreach ($path_f as $path_elem) {
                if ($path_elem['parent_id'] == $argsObj->id) {
                    ...
                    $guiObj->tcAttachments[$testcase_id] = getAttachmentInfos(
                        $docRepository, $testcase_id, 'nodes_hierarchy', true, 1);

                    foreach ($locationFilters as $locationKey => $filterValue) {
                        $finalFilters = $cf_filters + $filterValue;
                        $guiObj->design_time_cfields[$testcase_id][$locationKey] = $tcaseMgr->html_table_of_custom_field_values(
                            $testcase_id, 'design', $finalFilters, null, null,
                            $argsObj->tproject_id, null,
                            $testSet->tcversion_id[$gdx]);

                        $guiObj->testplan_design_time_cfields[$testcase_id] = $tcaseMgr->html_table_of_custom_field_values(
                            $testcase_id, 'testplan_design', $cf_filters, null,
                            null, $argsObj->tproject_id);
                    }

                    if ($guiObj->grants->execute) {
                        $guiObj->execution_time_cfields[$testcase_id] = $tcaseMgr->html_table_of_custom_field_inputs(
                            $testcase_id, null, 'execution', "_" . $testcase_id,
                ...
                if ($path_elem['node_table'] == 'testsuites' &&
                    ! isset($guiObj->tSuiteAttachments[$path_elem['id']])) {
                    $guiObj->tSuiteAttachments[$path_elem['id']] = getAttachmentInfos(
                        $docRepository, $path_elem['id'], 'nodes_hierarchy',
                        true, 1);
                }
            }
            $gdx ++;
...
function buildExecContext(&$argsObj, $tcasePrefix, &$tplanMgr, &$tcaseMgr)
{
    ...
    foreach ($ret['target']['tc_versions'] as $tcv_id => $tc_id) {
        $ret['target']['feature_id'][$tcv_id] = $tplanMgr->getFeatureID(
            $ret['context']['tplan_id'], $ret['context']['platform_id'], $tcv_id);

        $dummy = $tcaseMgr->get_basic_info($tc_id, array(
            'id' => $tcv_id
        ));
...
function launchRemoteExec(&$dbHandler, &$argsObj, $tcasePrefix, &$tplanMgr,
    &$tcaseMgr)
{
    // IMPORTANT NOTICE
    ...
    $feedback = doRemoteExecution($dbHandler, $execContext);
    ...
    $identity = null)
{
    ...
    if (null == $argsObj->tsuite_id) {
        ...
        $idCard['version_id'] = $tplanMgr->getVersionLinked($argsObj->tplan_id,
            $idCard['id']);
    }
    ...
    if (! is_null($idCard)) {
        ...
        if (! $historyOn || is_null($ltcv)) {
            $opt = null;
            $ltcv = $tcaseMgr->getLatestExecSingleContext($idCard, $execContext,
                $opt);
        }
    } else {
        // -----------------------------------------------------------
        ...
        if (is_null($argsObj->filter_status) ||
            in_array($cfgObj->tc_status['not_run'],
                (array) $argsObj->filter_status)) {
        ...
        if (! is_null(
            $sql2do = $tplanMgr->getLinkedForExecTree($argsObj->tplan_id,
                $filters, $options))) {
            if (is_array($sql2do)) {
                if (isset($filters['keyword_filter_type']) &&
                    ($filters['keyword_filter_type'] == 'And')) {
...
function initWebEditors(&$guiObj, $cfgObj, $baseHREF)
{
    if ($guiObj->can_use_bulk_op) {
        ...
        $of->Value = getItemTemplateContents('execution_template',
            $of->InstanceName, null);

        // Magic numbers that can be determined by trial and error
        $cols = intval(
            isset($editorCfg['cols']) ? $cfgObj->editorCfg['cols'] : 60);
        $rows = intval(
            isset($editorCfg['rows']) ? $cfgObj->editorCfg['rows'] : 10);
        $guiObj->bulk_exec_notes_editor = $of->CreateHTML($rows, $cols);
        unset($of);
    } else {
        $guiObj->exec_notes_editors = createExecNotesWebEditor(
            $guiObj->map_last_exec, $baseHREF, $cfgObj->editorCfg,
...
function manageCookies(&$argsObj, $cfgObj)
{
    ...
    $key4cookies = array(
        'tpn_view_status' => 'testplan_notes',
...
function genIssueSummary(&$tcaseMgr, $signature, $context)
{
    ...
    if (null != $steps) {
        ...
        foreach ($steps as $elem) {
            $nu['tcstep'][$elem['id']] = str_replace('%%STEPNUMBER%%',
                $elem['step_number'], $tstx);
        }
    }
...
function helperLabels($haystack)
{
    ...
    return str_replace($searchFor, $replaceWith, $haystack);
}
