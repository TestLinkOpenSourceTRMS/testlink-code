...
class assignment_mgr extends tlObjectWithDB
{
     ...
     *            contains the filename that has to be used as the key of the returned hash.
     * @return array
     */
     ...
     *
     * @param string $key_field
     *            contains the name column that has to be used as the key of the returned hash.
     * @return array
     */
     ...
     * @param int $build_id
     *            The ID of the build for which the user assignments shall be deleted.
     * @param boolean $delete_all_types
     *            If true, all assignments regardless of type will be deleted,
     ...
     */
    public function deleteBySignature($items)
    {
        ...
        foreach ($feature_map as $feature_id => $elem) {
            ...
            foreach ($uSet as $user_id) {
                ...
                $check .= " SELECT id FROM {$this->tables['user_assignments']} " .
                    ...
                    $safe['type'] . " AND user_id = " . $safe['user_id'];

                $rs = $this->db->get_recordset($check);
                if ((is_null($rs) || count($rs) == 0) && ($safe['user_id'] > 0)) {
     ...
     *
     * @param int $build_id
     *            ID of the build to check
     * @param int $count_all_types
     *            if true, all assignments will be counted, otherwise
     *            only tester assignments
     * @param int $user_id
     *            if given, user ID for which the assignments per build shall be counted
     ...
     * @param int $user_id
     *            if set and != 0, counts only the assignments for the given user
     *
     * @internal revisions
     */
    public function get_not_run_tc_count_per_build($build_id, $all_types = false,
        $user_id = 0)
    {
        $count = 0;
     ...
     *            ID of the build to copy the assignments from
     * @param int $target_build_id
     *            ID of the target build to which the assignments will be copied
     * @param int $assigner_id
     *            will be set as assigner ID of the new assignments if != 0,
        ...
        $assigner_id = 0, $opt = null)
    {
        ...
        if (! $my['opt']['keep_old_assignments']) {
        ...
        $sql = " INSERT INTO {$ua} " .
            ...
            " FROM {$ua} " . " WHERE build_id = " . intval($source_build_id) .
            $type_sql;

        if (! is_null($my['opt']['feature_set'])) {
            $sql .= " AND feature_id IN (" .
                implode(',', $my['opt']['feature_set']) . ")";
        }
     ...
     * @author Francisco Mancardi
     * @param mixed $buildID
     *            can be single value or array of build ID.
     * @return array
     */
    ...
    private function getNotRunAssignmentsCountByBuild($buildID)
    {
        ...
        $sql = "/* {$debugMsg} */ " . " SELECT count(0) as qty, UA.build_id " .
            ...
            "     AND E.tcversion_id = TPTCV.tcversion_id " .
            "     AND E.platform_id = TPTCV.platform_id " .
            "     AND E.build_id = UA.build_id " . " WHERE UA.build_id IN ( " .
     ...
     * @return array
     */
    public function getUsersByFeatureBuild($featureSet, $buildID,
        $assignmentType)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    ...
    public function emailLinkToExecPlanning($context, $targetUsers = null)
    {
        ...
        $sql = "/* {$debugMsg} */ " . " SELECT UA.user_id, U.email " .
            ...
            " WHERE B.testplan_id = " . $tplan_id . " AND B.id = " . $build_id .
        ...
        if (! is_null($rs) && ! empty($rs)) {
            $bye = false;
            $sql = " SELECT NHTPRJ.name AS tproject, " . " NHTPL.name AS tplan " .
                ...
                " WHERE NT.description = 'testproject' " . " AND NHTPL.id = " .
                $tplan_id;
            $names = $this->db->get_recordset($sql);
            $names = $names[0];
            $body_flines = lang_get('testproject') . ': ' . $names['tproject'] .
                '<br />' . lang_get('testplan') . ': ' . $names['tplan'] .
                '<br /><br />';
        }
        ...
        $ln = $_SESSION['basehref'] . 'ltx.php?item=xta2m&tplan_id=' . $tplan_id .
            '&user_id=';
