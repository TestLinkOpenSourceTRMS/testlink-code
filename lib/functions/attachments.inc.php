 ...
 *
 * @param object $attachmentRepository
 *            [ref] the attachment Repository
 * @param int $fkid
 *            the id of the object (attachments.fk_id);
 * @param string $fkTableName
 *            the name of the table $fkid refers to (attachments.fk_table)
 * @param bool $storeListInSession
 *            if true, the attachment list will be stored within the session
 * @param int $counter
 *            if $counter > 0 the attachments are appended to existing attachments within the session
 *
 * @return array infos about the attachment on success, NULL else
 */
function getAttachmentInfos(&$attachmentRepository, $fkid, $fkTableName,
    $storeListInSession = true, $counter = 0)
{
    $attachmentInfos = $attachmentRepository->getAttachmentInfosFor($fkid,
        $fkTableName);
 ...
 * Get infos about the attachments of a given object
 *
 * @param tlObjectWithAttachments $object
 *            The object whose attachment should be fetched
 * @param int $fkid
 *            the id of the object (attachments.fk_id);
 * @param bool $storeListInSession
 *            if true, the attachment list will be stored within the session
 ...
 */
function getAttachmentInfosFrom(&$object, $fkid, $storeListInSession = true,
    $counter = 0)
{
 ...
 *
 * @param array $attachmentInfos
 *            infos about attachment
 * @param int $counter
 *            counter for the attachments in the session
 */
function storeAttachmentsInSession($attachmentInfos, $counter = 0)
{
    ...
    if ($counter == 0) {
        ...
        $_SESSION['s_lastAttachmentInfos'] = array_merge(
            $_SESSION['s_lastAttachmentInfos'], $attachmentInfos);
    }
}
 ...
 * @param integer $id
 *            the database identifier of the attachment
 * @param
 *            $attachmentInfo
    ...
    if ($fInfo && $id) {
        ...
        if ($fSize && $fTmpName != "") {
            ...
            if ($uploadOp->statusOK) {
                logAuditEvent(
                    TLS("audit_attachment_created", $title, $fInfo['name']),
...
function deleteAttachment(&$dbHandler, $fileID, $checkOnSession = true)
{
    ...
    if ($info) {
        ...
        if ($doIt && $repo->deleteAttachment($fileID, $info)) {
            logAuditEvent(TLS("audit_attachment_deleted", $info['title']),
                "DELETE", $fileID, "attachments");
        }
    }
    return $info;
}
