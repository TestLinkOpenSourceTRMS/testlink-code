...
class dBug
{
    ...
    public function makeTableHeader($type, $header, $colspan = 2)
    {
        ...
        echo "<table cellspacing=2 cellpadding=3 class=\"dBug_" . $type .
            "\">
				<tr>
					<td class=\"dBug_" . $type . "Header\" colspan=" . $colspan .
    ...
    public function varIsDBResource($var, $db = "mysql")
    {
        ...
        for ($i = 0; $i < $numfields; $i ++) {
            ...
            echo "<td class=\"dBug_resourceKey\" title=\"" . $field_header .
                "\">" . $field_name . "</td>";
        }
        echo "</tr>";
        for ($i = 0; $i < $numrows; $i ++) {
            $row = call_user_func($db . "_fetch_array", $var,
                constant(strtoupper($db) . "_ASSOC"));
            echo "<tr>\n";
