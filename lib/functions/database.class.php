...
class database
{
     ...
     * @param string $p_key
     *            key type to check for (eg: PRI, MUL, ...etc)
     *
     * @return boolean
     */
     ...
     *            (optional) number of rows
     *
     * @return array whose keys are the values of column1 and the values are:
     *
     ...
     * @param string $sql
     *            the query to be executed
     * @param string $column_main_key
     *            the name of the column
     * @param string $column_sec_key
     *            the name of the column
     * @param boolean $cumulative
     * @param integer $limit
     *            (optional) number of rows
     ...
     */
    public function fetchRowsIntoMapAddRC($sql, $column, $limit = - 1)
    {
        $items = null;
        $result = $this->exec_query($sql, $limit);
        if ($result) {
