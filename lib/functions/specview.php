 ...
 * @param integer $tobj_id
 *            can be a testproject id, or testplan id.
 * @param integer $id
 *            node id, that we are using as root for the view we want to build
 * @param string $name
 * @param array $linked_items
 *            map where key=testcase_id
...
function gen_spec_view(&$db, $specViewType, $tobj_id, $id, $name, &$linked_items, $map_node_tccount, $filters = null, $options = null, $tproject_id = null)
{
    ...
    if (! empty($out) && ! is_null($out[0]) && count($a_tcid)) {
        $optGBI = array(
 ...
 *            stdClass object with information about filters
 * @param testplan $tplanMgr:
 *            test plan manager object
 * @param testcase $tcaseMgr:
 *            test case manager object
 * @param array $options:
 *            default null (at today 20110820 seems not be used).
 * @param boolean $isTestSuite:
 ...
 *            test case manager
 * @param array $filters:
 *            keys keywordsFilter, testcaseFilter,assignedToFilter,
 ...
 *
 * @param integer $masterContainerId
 *            can be a Test Project Id, or a Test Plan id.
 *            is used only if keyword id filter has been specified
 *            to get all keyword defined on masterContainer.
 *
 * @param integer $nodeId
 *            node that will be root of the view we want to build.
 *
 * @param string $specViewType:
 ...
 *
 * @param string $type:
 *            request selected for create tree
 *
...
function getTestSpecFromNode(&$dbHandler, &$tcaseMgr, &$linkedItems, $masterContainerId, $nodeId, $specViewType, $filters, $type = 'spec_order')
{
    ...
    if ($useFilter['tcase_id']) {
        $testCaseSet = is_array($filters['tcase_id']) ? $filters['tcase_id'] : array(
            $filters['tcase_id']
        );
    }
    ...
    if ($applyFilters) {
        ...
        if (! empty($itemSet) && ($useFilter['execution_type'] || $useFilter['importance'] || $useFilter['cfields'] || $useFilter['status'])) {
            ...
            $getFilters = $useFilter['cfields'] ? array(
                'cfields' => $filters['cfields']
            ) : null;
            $s2h = config_get('tplanDesign')->hideTestCaseWithStatusIn;
            if (! is_null($s2h)) {
                $getFilters['status'] = array(
                    'not_in' => array_keys($s2h)
                );
            }
 ...
 *            reference to set to analyse and clean.
 * @param tree $treeMgr
 *            reference to object
 * @param array $pruneUnlinkedTcversions
 *            useful when working on test plans
...
function buildSkeleton($id, $name, $config, &$test_spec, &$platforms)
{
    ...
    foreach ($test_spec as $current) {
        ...
        if ($node_types[$current['node_type_id']] == "testcase") {
            ...
            $out[$parent_idx]['testcases'][$tc_id] = array(
                'id' => $tc_id,
                'name' => $current['name']
            );
            ...
            if ($is_uncovered_view_type) {
                ...
                $outRef['executed'] = null;

                // useful for tc_exec_assignment.php
                $outRef['platforms'] = $platforms;
                $outRef['feature_id'] = null;
                $outRef['linked_by'] = null;
...
function genSpecViewFlat(&$db, $specViewType, $tobj_id, $id, $name, &$linked_items, $map_node_tccount, $filters = null, $options = null, $tproject_id = null)
{
    ...
    if (! empty($out) && ! is_null($out[$firtsElemIDX]) && count($a_tcid)) {
        $optGBI = array(
            'output' => 'full_without_users',
            'order_by' => " ORDER BY NHTC.node_order, NHTC.name, TCV.version DESC "
        );
