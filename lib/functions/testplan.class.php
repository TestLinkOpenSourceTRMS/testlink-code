 ...
 *							full external id => get_linked_tcversions()
 *
 *  20111227 - franciscom - get_linked_tcversions() find some issue when trying to do exec assignment 
...
class testplan extends tlObjectWithAttachments
{
	...
	*/
	...
	function get_ln_tcversions($id,$filters=null,$options=null)
	{
		...
		list($my,$join,$sql_filter,$left_join,$extra_fields) = $this->init_get_ln_tcversions($filters,$options);
		
		list($join['keywords'],$sql_filter['keywords']) = $this->helper_keywords_sql($my['filters']['keyword_id'],
																					 array('output' => 'array'));
		 
		$domain = array('active' => 1, 'inactive' => 0 , 'any' => null);
		$build_active_value = $domain[$my['options']['build_active_status']];
		...
		$sql = "/* $debugMsg */ " .
			   ...
			   $extra_fields['executions'] . $extra_fields['user_assignment'] . " (urgency * importance) AS priority "; 
		...
		$sql .=	" FROM {$this->tables['nodes_hierarchy']} NH_TCV " .
			   	...
			   	" JOIN  {$this->tables['tcversions']} TCV ON NH_TCV.id = TCV.id {$sql_filter['exec_type']} " .
			   	" {$join['keywords']} " .	
			   	" LEFT OUTER JOIN {$this->tables['platforms']} PLAT ON PLAT.id = T.platform_id " .
			   	" {$join['executions']} {$join['user_assignment']} " ;
	
		$sql .= " WHERE T.testplan_id = {$id} {$sql_filter['keywords']} {$sql_filter['tcase']} " .
		...
		{ 
			...
			case 'map':   
			default:
				// 20110821 - PROBABLY NONSENSE because retreive lots of records
				// to generate one
				// echo $sql;
				$recordset = $this->db->fetchRowsIntoMap($sql,'tc_id',0, -2);
		...
		//echo '<br>' . '<b>after Accessing DB' . ' - Xdebug Timing:' . xdebug_time_index() .'</b><br>'; 
	 ...
	 * 
 	 * 
 	 */
	function helper_sql_filters(&$sqlfx,$filters,$options)
	{
		if( !is_null($filters['platform_id']) )
		{
			$sqlfx['platforms'] = " AND T.platform_id = {$filters['platform_id']} ";
	    }

		if( !is_null($filters['exec_type']) )
		{
			$sqlfx['exec_type'] = " AND TCV.execution_type IN (" . implode(",",(array)$filters['exec_type']) . ") ";     
		}

		if (!is_null($filters['tcase_id']) )
		{
			$sqlfx['tcase'] = " AND NH_TCV.parent_id ";
			if( is_array($filters['tcase_id']) )
			{
				$sqlfx['tcase'] .= " IN (" . implode(',',$filters['tcase_id']) . ")";          	
			}
			else if ($my['filters']['tcase_id'] > 0 )
			{
				$sqlfx['tcase'] .= " = {$filters['tcase_id']} ";
			}
		}

		if (!is_null($filters['tsuites_id']))
		{
			$sqlfx['tsuites'] = " AND NH_TCASE.parent_id IN (" . implode(',',(array)$filters['tsuites_id']) . ")";
		}
		...
		if( $targetBuild > 0 )
		{
			...
			{  
				...
				list($jx,$fx) = $this->helper_assigned_to_sql($filters['assigned_to'],$options);
				$sqlfx['user_assignment'] = $fx;
			}
	...
	function helper_joins(&$join,&$sqlfx,$filters,$options,$tplanID)
	{
		...
		{ 		
			...
			// 20111227 -
			// Hmmm. here there is something wrong, why ALWAYS I've to look for MAX EXEC ?
			//
	...
	function init_get_ln_tcversions($filtersCfg,$optionsCfg)
	{
        ...
        $ic['options'] = array('output' => 'map', 'details' => 'simple',
        					   'include_unassigned' => false,'last_execution' => false,
                               'steps_info' => false,'build_active_status' => 'any');
		...
		$extra_fields['executions'] = " E.id AS exec_id, E.tcversion_number, E.tcversion_id AS executed, " .
									  " E.testplan_id AS exec_on_tplan, E.execution_type AS execution_run_type, " .
									  " E.testplan_id AS exec_on_tplan, E.execution_ts, E.tester_id, " .
...
// &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&	
	...
	There are special situations like: 
		...
		Arguments have been grouped trying to explain, how are used to answer
		different user questions:
		
		** Main Filtering (static view) **
		Means we act mainly on testplan_tcversions, tcversions and nodes_hierarchy,
		we are not trying to get any info about executions.
		
        [tcase_id]: main filter on testplan_tcversions, that have effects on
        			info retrieved from all other related tables.

		[platform_id]: main filter on testplan_tcversions, that have effects on
        			   info retrieved from all other related tables.
					   Execution is RECORDED at Test Plan + Platform + Build Level	

		[tsuites_id]: nodes_hierachy table 	

        [keyword_id]: after getting tcversions from testplan_tcversions
        			  filter is applied analizing test case spec.

		[exec_type]: tcversions table

		[urgencyImportance] : testplan_tcversions (urgency)
							  tcversions (importance)


        [assigned_to]/[assigned_on_build]: work as a team. 
        								   Till addition of tester execution task AT BUILD
        								   [assigned_to] was only argument to use to 
        ...
        [build_id]: HAS NO EFFECT on testplan_tcversions.
        			Used to filter EXECUTIONS NOT user assignments.
					used when we want more info that just test case versions that
					makes a test plan. (Attention: do not forget platforms)
       			  
        [exec_status]: need to understand if we are talking about LAST EXECUTION STATUS
        			   or status of one of multiple executions done.
	
					
					
		Still to be documented
		[cf_hash]: is not Clear on WHAT AREA CF are searched for.
  	...
  	args :
         ...
         [filters]: map with following keys
         	...
         	[assigned_on_build]: default 0 / NULL 
         						 numeric 
         ...
         [options]: map with following keys
			...
			[exclude_info]: array() with following values
							exec_info,priority
							useful to remove joins and fields 
	...
	@internal revisions
	@since 1.9.4
	20111229 - franciscom - TICKET 4847 - output recordeset provide full_external_id in addition
							to just numeric piece of external id
	...
	*/
	public function get_linked_tcversions($id,$filters=null,$options=null)
	{
		...
		// THIS YET.
		//
		// if( !is_null($sql2do = $this->getLinkedForTree($id,$filters,$options)) )
		// {
		// 	// 
		...
		// 	$setTestCaseStatus = $this->db->$kmethod($sql2run,'tcase_id');
		// 	
		// 	new dBug($setTestCaseStatus);
		// }
        // 
		// Now we need to go ahead to get apply other filters that can not be applied
        // 
		
		// new dBug($MYK);
		// die('dying on:' . __LINE__);
		

		// list($myNew,$build_active_statusNew) = $this->init_get_linked_tcversionsNEW($id,$filters,$options);
		// list($sqlNew,$joinNew,$order_byNew) = $this->helperColumns($myNew['filters'],$myNew['options']);

		$safe_id = intval($id);
		// list($myNew,$build_active_statusNew,
		//	 $sqlNew,$joinNew,$order_byNew) = $this->init_get_linked_tcversionsNEW($safe_id,$filters,$options);

		// echo '<b>NEW</b><br>';
		//new dBug($myNew);
		//new dBug($build_active_statusNew);
		//new dBug($sqlNew);
		//new dBug($joinNew);
		...
		//			!in_array('exec_info',$myNew['options']['exclude_info']) )
		//{
		//	$sqlLEBBP = $this->helperLastExecution($id,$myNew['filters'],$myNew['options']);
        //
		//	// new we need to understand if we are going to use a Union or not.
		//	// This dependens of execution status filter
		//	$union['not_run'] = "/* {$debugMsg} sqlUnion - NOT RUN X */" . 
		//						$sqlNew .
        ...
        //
		//	// echo '<br><b>UNION:' . $union['not_run'] . '</b><br>';
		//	// die('UNI');
        //
		//	// $useUnion = is_null($myNew['filters']['exec_status']));
		//	$useNotRunUnion = false;
		//	if(!$useUnion)			
		//	{
		//		// need to understand what pieces need to use
		//		$statusQty = count($myNew['filters']['exec_status']);
		//		
		//		$getNotRun = in_array($this->notRunStatusCode,$myNew['filters']['exec_status']);
		//		if(!$getNotRun)
		//		{
		//			$union['not_run'] = '';
		//		}
		...
		if( !is_null($build_active_status) )
		{
			...
			else
			{
				...
				// and do not use $build_active_status
				//
		...
		// Important Notice:
		// when $my['options']['only_executed'] == false
		// options['last_execution'] has NO SENSE
		...
		{
			foreach($my['options']['exclude_info'] as $victim)
			{
				switch($victim)
				{
					case 'exec_info':
	    ...
	    if( is_null($sql) )
	    {
			...
			if( $exec_fields != '' )
			{
				...
				{
					$sql .=	 ", COALESCE(E.status,'" . $this->notRunStatusCode . "') AS exec_status ";
				}
				else
				{
			...
			$sql .=" FROM {$this->tables['nodes_hierarchy']} NH_TCV " .
				   ...
				   $join_for_parent .
				   " JOIN {$this->tables['testplan_tcversions']} T ON NH_TCV.id = T.tcversion_id " .
				   " JOIN  {$this->tables['tcversions']} TCV ON NH_TCV.id = TCV.id " .
				   " {$my['join']['executions']} " .  
				   " {$my['join']['keywords']} " .	
				   " {$my['join']['builds']} " .
				   " LEFT OUTER JOIN {$this->tables['platforms']} PLAT ON PLAT.id = T.platform_id ";
			...
			{  
				list($jx,$fx) = $this->helper_assigned_to_sql($my['filters']['assigned_to'],$my['options']);
				$sql .= $fx;
			}
	...
	function init_get_linked_tcversionsNEW($tplanID,$filtersCfg,$optionsCfg)
	{
		...
		if (!is_null($ic['filters'][$dk]) )
		{
			...
			{
				$ic['where'][$dk] = " AND NH_TCV.parent_id = {$ic['filters'][$dk]} ";
			}
	    	$ic['where']['where'] .= $ic['where'][$dk];
		}
		...
		if( !is_null($ic['filters']['keyword_id']) )
		{
			...
			$ic['where']['where'] .= $ic['join']['keywords'];
		}
		// ------------------------------------------------------------------------------------------
		...
		// -------------------------------------------------------------
		list($sqlNew,$joinNew,$order_byNew) = $this->helperColumns($tplanID,$ic['filters'],$ic['options']);

		return array($ic,$build_active_status,$sqlNew,$joinNew,$order_byNew);
	}
	...
	function init_get_linked_tcversions($tplanID,$filtersCfg,$optionsCfg)
	{
		...
		if( !is_null($ic['filters']['platform_id']) )
		{
			$ic['where']['platforms'] = " AND T.platform_id = {$ic['filters']['platform_id']} ";
	    	$ic['where']['where'] .= $ic['where']['platforms'];
	    }
		...
		if( $ic['options']['last_execution'] || !is_null($ic['filters']['exec_status']) )
		{
			...
			// new dBug($ic['options']['last_execution']);
		    ...
		    // new dBug($ic['join']['executions']);	               			 
		}
		else if (!$ic['options']['last_execution'])
		{
			// with this we try to cover situation when user wants COMPLETE EXECUTION HISTORY
			$ic['join']['executions'] .= " JOIN {$this->tables['executions']} E ON ";
			$ic['join']['executions'] .= " (E.testplan_id = T.testplan_id AND " .
			    		               	 "  E.tcversion_id = T.tcversion_id AND " .
			            		       	 "  E.platform_id = T.platform_id " .
	 ...
	 * 
 	 * 
 	 */
	function helper_last_exec_sql($cfgFilters,$cfgOpt,$activeStatus,$tplanID)
	{

		
		 
		// DO NOT REMOVE THESE COMMENTS - can be useful to avoid repeat same errors
...
*/
	...
	*/
	...
	function getSkeleton($id,$tprojectID,$filters=null,$options=null)
	{
		...
		// echo 'READY TO CALL:' . $method2call;
	 ...
	 *
	 */
	function getNotRunAllBuildsForPlatform($id,$platformID,$buildQty=0) 
	{
		$debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
		...
		$sql = 	"/* $debugMsg */ " .
				// " SELECT count(0) AS COUNTER ,TPTCV.tcversion_id, E.status " .
				" SELECT count(0) AS COUNTER ,NHTCV.parent_id AS tcase_id, E.status " .
				" FROM {$this->tables['testplan_tcversions']} TPTCV " .
				" JOIN {$this->tables['builds']} B ON B.testplan_id = TPTCV.testplan_id " .
				" AND B.active = 1 " .
				...
				" HAVING COUNTER = " . intval($buildQty) ; 
		$recordset = $this->db->fetchRowsIntoMap($sql,'tcase_id');
		return $recordset;
	}
	...
	/**
	 * getHitsStatusSetFull($id,$platformID,$statusSet,$buildQty=0) 
	 *
	 * returns recordset with:
	...
	/**
	 * getHitsStatusSetPartial($id,$platformID,$statusSet) 
	 *
	 * returns recordset with:
	 * test cases that has at least ONE of requested status 
	...
	function getLinkedForTree($id,$filters=null,$options=null)
	{
		...
		// $fullEIDClause = $this->helperConcatTCasePrefix($safe['tplan_id']) . ' AS external_id,';
