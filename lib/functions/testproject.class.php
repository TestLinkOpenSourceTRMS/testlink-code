...
class testproject extends tlObjectWithAttachments
{
    ...
    const TESTCASE_PREFIX_MAXLEN = 16;

    // must be changed if field dimension changes
     ...
     *
     */
    public function update($id, $name, $color, $notes, $options, $active = null, $tcasePrefix = null, $is_public = null)
    {
        $status_ok = 1;
        ...
        $sql = " UPDATE {$this->object_table} SET color='" . $this->db->prepare_string($color) . "', " . " options='" . $serOptions . "', " . " notes='" . $this->db->prepare_string($notes) . "' {$add_upd} " . " WHERE id=" . $safeID;
        $result = $this->db->exec_query($sql);

        if ($result) {
            // update related node
            $sql = "UPDATE {$this->tables['nodes_hierarchy']} SET name='" . $this->db->prepare_string($name) . "' WHERE id= {$safeID}";
            $result = $this->db->exec_query($sql);
        }
     ...
     *            Project ID; zero causes unset data
     */
    public function setSessionProject($projectId)
    {
        ...
        if ($tproject_info) {
            ...
            $_SESSION['testprojectOptions']->requirementsEnabled = isset($tproject_info['opt']->requirementsEnabled) ? $tproject_info['opt']->requirementsEnabled : 0;
            $_SESSION['testprojectOptions']->testPriorityEnabled = isset($tproject_info['opt']->testPriorityEnabled) ? $tproject_info['opt']->testPriorityEnabled : 0;
            $_SESSION['testprojectOptions']->automationEnabled = isset($tproject_info['opt']->automationEnabled) ? $tproject_info['opt']->automationEnabled : 0;
            $_SESSION['testprojectOptions']->inventoryEnabled = isset($tproject_info['opt']->inventoryEnabled) ? $tproject_info['opt']->inventoryEnabled : 0;

            tLog("Test Project was activated: [" . $tproject_info['id'] . "]" . $tproject_info['name'], 'INFO');
        } else {
            if (isset($_SESSION['testprojectID'])) {
                tLog("Test Project deactivated: [" . $_SESSION['testprojectID'] . "] " . $_SESSION['testprojectName'], 'INFO');
            }
     ...
     *
     * @param array $recorset
     *            produced by getTestProject()
     */
     ...
     *            (optional) additional SQL condition(s)
    ...
    protected function getTestProject($condition = null, $opt = null)
    {
        ...
        switch ($my['options']['output']) {
            case 'existsByID':
                $doParse = false;
                $sql = "/* $debugMsg */ SELECT testprojects.id " . " FROM {$this->object_table} testprojects " . " WHERE 1=1 ";
                break;

            case 'existsByName':
                $doParse = false;
                $sql = "/* $debugMsg */ SELECT testprojects.id " . " FROM {$this->object_table} testprojects, " . " {$this->tables['nodes_hierarchy']} nodes_hierarchy" . " WHERE testprojects.id = nodes_hierarchy.id " . " AND nodes_hierarchy.node_type_id = " . $this->tree_manager->node_descr_id['testproject'];
                break;
            ...
            case 'full':
            default:
                $sql = "/* $debugMsg */ SELECT {$tprojCols}, nodes_hierarchy.name " . " FROM {$this->object_table} testprojects, " . " {$this->tables['nodes_hierarchy']} nodes_hierarchy" . " WHERE testprojects.id = nodes_hierarchy.id " . " AND nodes_hierarchy.node_type_id = " . $this->tree_manager->node_descr_id['testproject'];
                break;
        }
     ...
     * @param string $prefix
     * @param string $addClause
     *            optional additional SQL 'AND filter' clause
     *
     * @return array map with test project info; null if query fails
     */
    ...
    public function get_all($filters = null, $options = null)
    {
        ...
        if ($my['options']['output'] == 'count') {
            $sql = "/* $debugMsg */ SELECT COUNT(testprojects.id) AS qty " . " FROM {$this->object_table} testprojects";
        ...
        //
        $sql = "/* $debugMsg */ SELECT testprojects.*, nodes_hierarchy.name " . " FROM {$this->object_table} testprojects, " . " {$this->tables['nodes_hierarchy']} nodes_hierarchy " . " WHERE testprojects.id = nodes_hierarchy.id ";
    ...
    public function get_accessible_for_user($user_id, $opt = null, $filters = null)
    {
        ...
        if ($my['opt']['add_issuetracker']) {
            $itsql = " LEFT OUTER JOIN {$this->tables['testproject_issuetracker']} AS TIT " . " ON TIT.testproject_id  = TPROJ.id " . " LEFT OUTER JOIN {$this->tables['issuetrackers']} AS ITMD " . " ON ITMD.id = TIT.issuetracker_id ";
        ...
        if ($my['opt']['add_codetracker']) {
            $ctsql = " LEFT OUTER JOIN {$this->tables['testproject_codetracker']} AS TCT " . " ON TCT.testproject_id  = TPROJ.id " . " LEFT OUTER JOIN {$this->tables['codetrackers']} AS CTMD " . " ON CTMD.id = TCT.codetracker_id ";
        ...
        if ($my['opt']['add_reqmgrsystem']) {
            $rmssql = " LEFT OUTER JOIN {$this->tables['testproject_reqmgrsystem']} AS TRMS " . " ON TRMS.testproject_id  = TPROJ.id " . " LEFT OUTER JOIN {$this->tables['reqmgrsystems']} AS RMSMD " . " ON RMSMD.id = TRMS.reqmgrsystem_id ";
            ...
            switch ($my['opt']['output']) {
                case 'map_name_with_inactive_mark':
                    foreach ($arrTemp as $id => $row) {
                        ...
                        $items[$id] = $noteActive . (($my['opt']['field_set'] == 'prefix') ? ($row['prefix'] . ':') : '') . $row['name'];
                    }
                    break;
    ...
    {
        $ret = array();
        $test_spec = $this->get_subtree($id, array(
     ...
     * @param string $name
     *            the name to check
     * @return array with keys: status_ok, msg
     */
    ...
    private function checkTestCasePrefixExistence($prefix, $id = 0)
    {
        ...
        $sql = " SELECT id FROM {$this->object_table} " . " WHERE prefix='" . $this->db->prepare_string($prefix) . "'" . " AND id <> {$id}";
    ...
    public function generateTestCaseNumber($id)
    {
        ...
        if ($gotLock || $retry == 0) {
            ...
            $ret = null;
            $sql = "/* $debugMsg */ UPDATE {$this->object_table} " . " SET tc_counter=tc_counter+1 WHERE id = {$safeID}";
    ...
    protected function setTestCaseCounter($id, $value, $force = false)
    {
        ...
        $sql = " /* $debugMsg */ UPDATE {$this->object_table} " . ' SET tc_counter=' . $safeValue . ' WHERE id =' . intval($id);
     ...
     *            the test project id
     * @param int $keywordID
     *            [default = null] the optional keyword id
     *
        ...
        if ($result >= tl::OK && $this->auditCfg->logEnabled) {
            ...
            switch ($my['opt']['context']) {
                case 'getTestProjectName':
                    $dummy = $this->get_by_id($my['opt']['tproject_id'], array(
                        'output' => 'name'
                    ));
    ...
    {
        $query = " SELECT id FROM {$this->tables['keywords']}  " . " WHERE testproject_id = {$testproject_id}" . " ORDER BY keyword ASC";
    ...
    {
        $sql = " SELECT id,keyword FROM {$this->tables['keywords']}  " . " WHERE testproject_id = {$tproject_id}" . " ORDER BY keyword ASC";
    ...
    public function hasKeywords($id)
    {
        ...
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ SELECT COUNT(0) AS qty FROM {$this->tables['keywords']}  " . " WHERE testproject_id = " . intval($id);
        $rs = $this->db->get_recordset($sql);
     ...
     *
     * @return string the generated XML Code
     */
    ...
    public function importKeywordsFromCSV($testproject_id, $fileName, $delim = ';')
    {
        ...
        if ($handle) {
            while ($data = fgetcsv($handle, TL_IMPORT_ROW_MAX, $delim)) {
                ...
                if ($kw->readFromCSV(implode($delim, $data)) >= tl::OK && $kw->writeToDB($this->db) >= tl::OK) {
     ...
     * @return array List of titles according to IDs
     *
     * @internal revisions
     *
     */
    public function getOptionReqSpec($tproject_id, $get_not_empty = self::GET_EMPTY_REQSPEC)
    {
        ...
        $sql = " SELECT SRS.id,NH.name AS title " . " FROM {$this->tables['req_specs']} SRS, " . " {$this->tables['nodes_hierarchy']} NH " . $additional_table . " WHERE testproject_id={$tproject_id} " . " AND SRS.id=NH.id " . $additional_join . " ORDER BY title";
     ...
     * @param integer $testproject_id
     * @param string $id
     *            optional id of the requirement specification
     *
     * @return mixed null if no srs exits, or no srs exists for id
    ...
    public function getReqSpec($testproject_id, $id = null, $fields = null, $access_key = null)
    {
        ...
        $sql = " /* $debugMsg */ " . " SELECT {$fields} FROM {$this->tables['req_specs_revisions']} RSPECREV, " . " {$this->tables['req_specs']} RSPEC, {$this->tables['nodes_hierarchy']} NH, " . " {$this->tables['requirements']} REQ " . " WHERE RSPECREV.parent_id=RSPEC.id " . " AND NH.id=RSPEC.id AND REQ.srs_id = RSPEC.id " . " AND RSPEC.testproject_id={$testproject_id} ";
    ...
    private function createReqSpec($testproject_id, $title, $scope, $countReq, $user_id, $type = 'n')
    {
        ...
        if ($chk['status_ok']) {
            $sql = "INSERT INTO {$this->tables['req_specs']} " . " (testproject_id, title, scope, type, total_req, author_id, creation_ts)
              VALUES (" . $testproject_id . ",'" . $this->db->prepare_string($title) . "','" . $this->db->prepare_string($scope) . "','" . $this->db->prepare_string($type) . "','" . $this->db->prepare_string($countReq) . "'," . $this->db->prepare_string($user_id) . ", " . $this->db->db_now() . ")";
    ...
    public function deleteUserRoles($tproject_id, $users = null, $opt = null)
    {
        ...
        $my['opt'] = array_merge($my['opt'], (array) $opt);
        $query = " DELETE FROM {$this->tables['user_testproject_roles']} " . " WHERE testproject_id = " . intval($tproject_id);
    ...
    {
        $query = "SELECT user_id,role_id FROM {$this->tables['user_testproject_roles']} " . "WHERE testproject_id = {$tproject_id}";
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $query = "/* $debugMsg */ INSERT INTO {$this->tables['user_testproject_roles']} " . " (user_id,testproject_id,role_id) VALUES ({$userID},{$tproject_id},{$roleID})";
    ...
    public function delete($id)
    {
        ...
        $a_sql[] = array(
            "/* $debugMsg */ UPDATE {$this->tables['users']}  " . " SET default_testproject_id = NULL " . " WHERE default_testproject_id = {$id}",
        ...
        foreach ($xSQL as $target) {
            $sql = "/* $debugMsg */ DELETE FROM " . $this->tables[$target] . " WHERE testproject_id = " . intval($id);
            $this->db->exec_query($sql);
        }
        ...
        if (empty($error)) {
            ...
            $this->tree_manager->delete_subtree_objects($id, $id, '', array(
                'testcase' => 'exclude_tcversion_nodes'
            ));
            $sql = "/* $debugMsg */ " . " DELETE FROM {$this->tables['nodes_hierarchy']} " . " WHERE id = {$id} AND node_type_id=" . $this->tree_manager->node_descr_id['testproject'];
    ...
    public function get_all_testcases_id($idList, &$tcIDs, $options = null)
    {
        ...
        $sql = "/* $debugMsg */  SELECT id,node_type_id from {$this->tables['nodes_hierarchy']} " . " WHERE parent_id IN ({$idList})";
        ...
        if ($result) {
            ...
            while ($row = $this->db->fetch_array($result)) {
                if ($row['node_type_id'] == $tcNodeTypeID) {
                    if ($use_array) {
                        $sql = " SELECT DISTINCT NH.parent_id, TCV.tc_external_id " . " FROM {$this->tables['nodes_hierarchy']} NH " . " JOIN  {$this->tables['tcversions']} TCV ON TCV.id = NH.id " . " WHERE NH.parent_id = {$row['id']} ";
    ...
    private function DEPRECATED_get_keywords_tcases($testproject_id, $keyword_id = 0, $keyword_filter_type = 'Or')
    {
        ...
        if (is_array($keyword_id)) {
            ...
            if ($keyword_filter_type == 'And') {
                $subquery = "AND testcase_id IN (" . " SELECT FOXDOG.testcase_id FROM
                          ( SELECT COUNT(testcase_id) AS HITS,testcase_id
                            ...
                            {$keyword_filter}
                            GROUP BY testcase_id ) AS FOXDOG " . " WHERE FOXDOG.HITS=" . count($keyword_id) . ")";
    ...
    public function getKeywordsLatestTCV($tproject_id, $keyword_id = 0, $kwFilterType = 'Or')
    {
        ...
        if (is_array($keyword_id)) {
            ...
            if ($kwFilterType == 'And') {
                $ltcvJoin = " ";
                $sqlCount = " /* SQL COUNT */ " . " SELECT COUNT(TK.tcversion_id) AS HITS,TK.tcversion_id
                ...
                $subquery = " AND tcversion_id IN (" . " SELECT FOXDOG.tcversion_id FROM
                          ( $sqlCount ) AS FOXDOG " . " WHERE FOXDOG.HITS=" . count($keyword_id) . ")";
                $kwFilter = '';
            }
        } elseif ($keyword_id > 0) {
    ...
    private function XXXgetPlatformsLatestTCV($tproject_id, $platform_id = 0, $filterType = 'Or')
    {
        ...
        if (is_array($platform_id)) {
            ...
            if ($filterType == 'And') {
                $ltcvJoin = " ";
                $sqlCount = " /* SQL COUNT */ " . " SELECT COUNT(TK.tcversion_id) AS HITS,TPL.tcversion_id
                ...
                $subquery = " AND tcversion_id IN (" . " SELECT FOXDOG.tcversion_id FROM
                          ( $sqlCount ) AS FOXDOG " . " WHERE FOXDOG.HITS=" . count($platform_id) . ")";
                $platFilter = '';
            }
    ...
    public function get_all_testplans($id, $filters = null, $options = null)
    {
        ...
        $sql = " SELECT {$my['options']['fields2get']} " . " FROM {$this->tables['nodes_hierarchy']} NH,{$this->tables['testplans']} TPLAN";

        $where = " WHERE NH.id=TPLAN.id AND (testproject_id = " . $this->db->prepare_int($id) . " ";
        if (! is_null($filters)) {
            $key2check = array(
    ...
    {
        $sql = " SELECT NH.id, NH.name, testproject_id " . " FROM {$this->tables['nodes_hierarchy']} NH, {$this->tables['testplans']} testplans " . " WHERE NH.id=testplans.id " . " AND testproject_id = {$tproject_id} ";
    ...
    {
        $fl = $this->tree_manager->get_children($tproject_id, array(
            'testcase',
            'exclude_me',
            'testplan' => 'exclude_me',
            'requirement_spec' => 'exclude_me'
    ...
    private function getTCasesLinkedToAnyTPlan($id)
    {
        ...
        $sql = " SELECT DISTINCT NHTCV.parent_id AS testcase_id " . " FROM {$this->tables['nodes_hierarchy']} NHTCV " . " JOIN {$this->tables['testplan_tcversions']} TPTCV " . " ON NHTCV.id = TPTCV.tcversion_id ";

        // get testplan id for target test�project, to get test case versions linked to testplan.
        $sql .= " JOIN {$this->tables['nodes_hierarchy']} NHTPLAN " . " ON TPTCV.testplan_id = NHTPLAN.id  " . " WHERE NHTPLAN.node_type_id = {$tplanNodeType} AND NHTPLAN.parent_id = " . intval($id);
    ...
    public function get_linked_custom_fields($id, $node_type = null, $access_key = 'id')
    {
        ...
        $sql = "SELECT CF.*,CFTP.display_order " . " FROM {$this->tables['custom_fields']} CF, {$this->tables['cfield_testprojects']} CFTP " . $additional_table . " WHERE CF.id=CFTP.field_id " . " AND   CFTP.testproject_id={$id} " . $additional_join . " ORDER BY CFTP.display_order";
    ...
    public function copy_as($id, $new_id, $user_id, $new_name = null, $options = null)
    {
        ...
        if (! is_null($new_name)) {
            $sql = "/* $debugMsg */ UPDATE {$this->tables['nodes_hierarchy']} " . "SET name='" . $this->db->prepare_string(trim($new_name)) . "' " . "WHERE id={$new_id}";
            $this->db->exec_query($sql);
        }
        ...
        if ($my['options']['copy_requirements']) {
            list ($oldNewMappings['requirements'], $onReqSet) = $this->copy_requirements($id, $new_id, $user_id);

            // need to copy relations between requirements
            $rel = null;
            foreach ($oldNewMappings['requirements'] as $erek) {
                foreach ($erek['req'] as $okey => $nkey) {
                    $sql = "/* $debugMsg */ SELECT id, source_id, destination_id," . " relation_type, author_id, creation_ts " . " FROM {$this->tables['req_relations']} " . " WHERE source_id=$okey OR destination_id=$okey ";
                ...
                foreach ($rel as $okey => $ir) {
                    if (! is_null($ir)) {
                        foreach ($ir as $rval) {
                            ...
                            $sql = "/* $debugMsg */
                     INSERT INTO {$this->tables['req_relations']} " . " (source_id, destination_id, relation_type, author_id, creation_ts) " . " values (" . $onReqSet[$rval['source_id']] . "," . $onReqSet[$rval['destination_id']] . "," . $rval['relation_type'] . "," . $rval['author_id'] . "," . "$totti)";
                            $this->db->exec_query($sql);
                        }
                    }
                }
        ...
        $sql = "/* $debugMsg */ UPDATE {$this->object_table} " . " SET tc_counter = {$rs_source['tc_counter']} " . " WHERE id = {$new_id}";
        $this->db->exec_query($sql);
    }
     ...
     *
     * @param int $source_id
     *            original Test Project identificator
     * @param int $target_id
     *            new Test Project identificator
     */
    private function copy_user_roles($source_id, $target_id)
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;

        $sql = "/* $debugMsg */ SELECT * FROM {$this->tables['user_testproject_roles']} " . "WHERE testproject_id={$source_id} ";
        $rs = $this->db->get_recordset($sql);

        if (! is_null($rs)) {
            foreach ($rs as $elem) {
                $sql = "/* $debugMsg */ INSERT INTO {$this->tables['user_testproject_roles']}  " . "(testproject_id,user_id,role_id) " . "VALUES({$target_id}," . $elem['user_id'] . "," . $elem['role_id'] . ")";
                $this->db->exec_query($sql);
            }
        }
    }
    ...
    private function copy_keywords($source_id, $target_id)
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $old_new = null;
        $sql = "/* $debugMsg */ SELECT * FROM {$this->tables['keywords']} " . " WHERE testproject_id = {$source_id}";
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ " . " SELECT field_id FROM {$this->tables['cfield_testprojects']} " . " WHERE testproject_id = {$source_id}";
        ...
        if (! is_null($tplanSet)) {
            ...
            foreach ($keySet as $itemID) {
                ...
                if ($new_id > 0) {
                    ...
                    $tplanMgr->copy_as($itemID, $new_id, null, $target_id, $user_id, array(
                        'copy_assigned_to' => 1
                    ), $mappings);
                }
            }
    ...
    private function getTestSpec($id, $filters = null, $options = null)
    {
        ...
        if (! is_null($my['filters']['testcase_name']) || ! is_null($my['filters']['testcase_id']) || ! is_null($my['filters']['execution_type']) || ! is_null($my['filters']['exclude_branches']) || ! is_null($my['filters']['importance']) || $my['options']['remove_empty_branches']) {
    ...
    private function _get_subtree_rec($node_id, &$pnode, $filters = null, $options = null)
    {
        ...
        if (! $my) {
            ...
            $staticSql = " SELECT DISTINCT {$tfields} " . " FROM {$this->tables['nodes_hierarchy']} NH ";
            ...
            foreach ($inClause as $tgf => $dummy) {
                if ($tcversionFilter[$tgf]) {
                    $inClause[$tgf] = " TCV.$tgf IN (" . implode(',', $my['filters'][$tgf]) . ')';
                }
            }
        }

        $sql = $staticSql . " WHERE NH.parent_id = " . intval($node_id) . " AND (" . "      NH.node_type_id = {$this->tree_manager->node_descr_id['testsuite']} " . "      OR (NH.node_type_id = {$this->tree_manager->node_descr_id['testcase']} ";
        ...
        if (! is_null($tclist)) {
            ...
            $glvn = " /* Get LATEST ACTIVE tcversion NUMBER */ " . " SELECT MAX(TCVX.version) AS version, NHTCX.parent_id AS tc_id " . " FROM {$this->tables['tcversions']} TCVX " . " JOIN {$this->tables['nodes_hierarchy']} NHTCX " . " ON NHTCX.id = TCVX.id AND TCVX.active = 1 " . " WHERE NHTCX.parent_id IN (" . implode(',', $tclist) . ")" . " GROUP BY NHTCX.parent_id";

            // 2018, again where is the active check?
            $ssx = " /* Get LATEST ACTIVE tcversion MAIN ATTRIBUTES */ " . " SELECT TCV.id AS tcversion_id, TCV.tc_external_id AS external_id, SQ.tc_id " . " FROM {$this->tables['nodes_hierarchy']} NHTCV " . " JOIN ( $glvn ) SQ " . " ON NHTCV.parent_id = SQ.tc_id " . " JOIN {$this->tables['tcversions']} TCV " . " ON NHTCV.id = TCV.id ";
            ...
            if ($tcversionFilter['enabled'] || $tcaseFilter['is_active']) {
                $addAnd = false;
                if ($tcversionFilter['importance'] || $tcversionFilter['execution_type'] || $tcversionFilter['status']) {
                    $where .= " AND ";
                }
        ...
        if ($getWithOutKeywords && $hasTCases) {
            ...
            switch ($keyword_filter_type) {
                ...
                case 'And':
                    ...
                    // Test case id.
                    $sqlCount = " /* SQL COUNT */ " . " SELECT COUNT(TK.tcversion_id) AS HITS,
                ...
                case 'Or':
                default:
                    $sql = " /* Filter Type = OR */ " . " SELECT TK.testcase_id " . " FROM {$this->tables['testcase_keywords']} TK" . " JOIN {$this->views['latest_tcase_version_id']} LTVC " . " ON LTVC.tcversion_id = TK.tcversion_id " . " JOIN {$this->tables['keywords']} KW " . " ON KW.id = TK.keyword_id " . " WHERE {$kwFilter} " . " AND KW.testproject_id=" . $tproject_id;
                    break;
            }
        }
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ " . "SELECT issue_tracker_enabled FROM {$this->object_table} " . "WHERE id =" . intval($id);
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ " . " UPDATE {$this->object_table} " . " SET issue_tracker_enabled = " . (intval($value) > 0 ? 1 : 0) . " WHERE id =" . intval($id);
        $this->db->exec_query($sql);
    }
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ " . "SELECT code_tracker_enabled FROM {$this->object_table} " . "WHERE id =" . intval($id);
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ " . " UPDATE {$this->object_table} " . " SET code_tracker_enabled = " . (intval($value) > 0 ? 1 : 0) . " WHERE id =" . intval($id);
        $this->db->exec_query($sql);
    }
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ " . " SELECT COUNT(0) AS qty FROM {$this->object_table} ";
        $ret = $this->db->get_recordset($sql);
        return $ret[0]['qty'];
    }
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ " . " SELECT is_public FROM {$this->object_table} " . " WHERE id =" . intval($id);
    ...
    public function getTestCasesCreatedByUser($id, $user_id, $options = null)
    {
        ...
        if ($itemQty > 0) {
            ...
            if (! is_null($rs)) {
                $k2g = array_keys($rs);
                $path_info = $this->tree_manager->get_full_path_verbose($k2g, array(
    ...
    private function isReqMgrIntegrationEnabled($id)
    {
        ...
        $sql = "/* $debugMsg */ " . "SELECT {$targetField} FROM {$this->object_table} " . "WHERE id =" . intval($id);
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ " . " UPDATE {$this->object_table} " . " SET {$field} = " . (intval($value) > 0 ? 1 : 0) . " WHERE id =" . intval($id);
        $this->db->exec_query($sql);
    }
    ...
    {
        // I've to use testsuiteID because this is how is name on containerEdit.php
        $url = "lib/testcases/containerEdit.php?containerType=testproject&doAction=deleteFile&tprojectID=" . intval($id) . "&file_id=";
        return $url;
    }
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ SELECT testprojects.options " . " FROM {$this->object_table} testprojects " . " WHERE testprojects.id = " . intval($id);
        $rs = $this->db->get_recordset($sql);
    ...
    private function setOptions($id, $optObj)
    {
        ...
        if ($nike) {
            $sql = "/* $debugMsg */ UPDATE {$this->object_table} " . " SET options = '" . $this->db->prepare_string(serialize($itemOpt)) . "'" . " WHERE id = " . $safeID;
    ...
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $sql = "/* $debugMsg */ SELECT COUNT(0) AS qty" . " FROM {$this->tables['nodes_hierarchy']} NH_TPLAN " . " JOIN {$this->tables['testplans']} TPLAN ON NH_TPLAN.id = TPLAN.id " . " WHERE NH_TPLAN.parent_id = " . $this->db->prepare_int($id) . " AND TPLAN.active = 1";
        ...
        if ($getWithOutPlatforms && $hasTCases) {
            ...
            switch ($filter_type) {
                ...
                case 'And':
                    // MAX(TK.testcase_id) needed to be able to extract
                    // Test case id.
                    $sqlCount = " /* SQL COUNT */ " . " SELECT COUNT(TPL.tcversion_id) AS HITS,
                ...
                case 'Or':
                default:
                    $sql = " /* Filter Type = OR */ " . " SELECT TK.testcase_id " . " FROM {$this->tables['testcase_platforms']} TPL" . " JOIN {$this->views['latest_tcase_version_id']} LTVC " . " ON LTVC.tcversion_id = TPL.tcversion_id " . " JOIN {$this->tables['platforms']} PL " . " ON PL.id = TK.platform_id " . " WHERE {$filter} " . " AND PL.testproject_id=" . $tproject_id;
                    break;
            }
        }
