...
class testproject extends tlObjectWithAttachments
{
    ...
    public function create($item, $opt = null)
    {
        ...
        $sql = " INSERT INTO {$this->object_table} (id,color," .
            " options,notes,active,is_public,prefix,api_key) " . " VALUES (" .
            $id . ", '" . $this->db->prepare_string($item->color) . "','" .
            $serOptions . "','" . $this->db->prepare_string($item->notes) . "'," .
        ...
        $tcasePrefix = null, $is_public = null)
    {
        ...
        $sql = " UPDATE {$this->object_table} SET color='" .
            $this->db->prepare_string($color) . "', " . " options='" .
            $serOptions . "', " . " notes='" . $this->db->prepare_string($notes) .
            "' {$add_upd} " . " WHERE id=" . $safeID;
        $result = $this->db->exec_query($sql);
    ...
    public function setSessionProject($projectId)
    {
        ...
        if ($tproject_info) {
            ...
            $_SESSION['testprojectOptions']->requirementsEnabled = isset(
                $tproject_info['opt']->requirementsEnabled) ? $tproject_info['opt']->requirementsEnabled : 0;
            $_SESSION['testprojectOptions']->testPriorityEnabled = isset(
                $tproject_info['opt']->testPriorityEnabled) ? $tproject_info['opt']->testPriorityEnabled : 0;
            $_SESSION['testprojectOptions']->automationEnabled = isset(
                $tproject_info['opt']->automationEnabled) ? $tproject_info['opt']->automationEnabled : 0;
            $_SESSION['testprojectOptions']->inventoryEnabled = isset(
                $tproject_info['opt']->inventoryEnabled) ? $tproject_info['opt']->inventoryEnabled : 0;

            tLog(
                "Test Project was activated: [" . $tproject_info['id'] . "]" .
                $tproject_info['name'], 'INFO');
        } else {
            if (isset($_SESSION['testprojectID'])) {
                tLog(
                    "Test Project deactivated: [" . $_SESSION['testprojectID'] .
     ...
     *
     */
    public function get_accessible_for_user($user_id, $opt = null,
        $filters = null)
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $my = array();
        ...
        $sql = " /* $debugMsg */ SELECT {$cols} {$itf} {$ctf} {$rmsf} " .
            ...
            " ON TPROJ.id = UTR.testproject_id " . " AND UTR.user_id =" .
            $safe_user_id . $itsql . $ctsql . $rmssql . " WHERE 1=1 ";

        // Private test project feature
        if ($globalRoleID != TL_ROLES_ADMIN) {
            if ($globalRoleID != TL_ROLES_NO_RIGHTS) {
                $sql .= " AND ";
                $sql_public = " ( TPROJ.is_public = 1 AND (UTR.role_id IS NULL OR UTR.role_id != " .
                    TL_ROLES_NO_RIGHTS . ") )";
                $sql_private = " ( TPROJ.is_public = 0 AND UTR.role_id != " .
                    TL_ROLES_NO_RIGHTS . ") ";
                $sql .= " ( {$sql_public}  OR {$sql_private} ) ";
            } else {
                // User needs specific role
                $sql .= " AND (UTR.role_id IS NOT NULL AND UTR.role_id != " .
                    TL_ROLES_NO_RIGHTS . ")";
            }
        }

        $userObj = tlUser::getByID($this->db, $safe_user_id,
            ...
            switch ($my['opt']['output']) {
                case 'map_name_with_inactive_mark':
                    foreach ($arrTemp as $id => $row) {
                        ...
                        $items[$id] = $noteActive .
                            (($my['opt']['field_set'] == 'prefix') ? ($row['prefix'] .
                            ':') : '') . $row['name'];
                    }
                    break;
    ...
    public function get_subtree($id, $filters = null, $opt = null)
    {
        ...
        return $this->tree_manager->get_subtree(intval($id), $my['filters'],
            $my['options']);
    }
     ...
     *
     */
    public function show(&$smarty, $guiObj, $template_dir, $id, $sqlResult = '',
        $action = 'update', $modded_item_id = 0)
    {
        $gui = $guiObj;
        ...
        $gui->fileUploadURL = $_SESSION['basehref'] .
            $this->getFileUploadRelativeURL($safeID);
        $gui->delAttachmentURL = $_SESSION['basehref'] .
            $this->getDeleteAttachmentRelativeURL($safeID);
    ...
    private function checkTestCasePrefixExistence($prefix, $id = 0)
    {
        ...
        $rs = $this->db->get_recordset($sql);
        if (! is_null($rs)) {
            $check_op['msg'] = sprintf(lang_get('error_tcase_prefix_exists'),
                $prefix);
            $check_op['status_ok'] = 0;
        }
    ...
    {
        $sql = "UPDATE {$this->tables['testprojects']} SET active=" . $status .
            " WHERE id=" . $id;
        $result = $this->db->exec_query($sql);
    ...
    public function deleteKeyword($id, $opt = null)
    {
        ...
        if ($my['opt']['checkBeforeDelete']) {
            ...
            if ($this->cfg->keywords->onDeleteCheckExecutedTCVersions) {
                $linkedAndNotExec = $this->checkKeywordIsLinkedAndNotExecuted(
                    $id);
                $doIt = $doIt && $linkedAndNotExec;
            }

            if ($this->cfg->keywords->onDeleteCheckFrozenTCVersions) {
                $linkedToFrozen = $this->checkKeywordIsLinkedToFrozenVersions(
                    $id);
        ...
        if ($result >= self::OK && $this->auditCfg->logEnabled) {
            ...
            logAuditEvent(
                TLS("audit_keyword_deleted", $keyword, $my['opt']['context']),
                "DELETE", $id, "keywords");
        }
        return $result;
    }
    ...
    protected function getKeywordIDsFor($testproject_id)
    {
        $query = " SELECT id FROM {$this->tables['keywords']}  " .
            " WHERE testproject_id = {$testproject_id}" . " ORDER BY keyword ASC";
    ...
    public function getKeywordSet($tproject_id)
    {
        $sql = " SELECT id,keyword FROM {$this->tables['keywords']}  " .
            " WHERE testproject_id = {$tproject_id}" . " ORDER BY keyword ASC";
    ...
    public function importKeywordsFromCSV($testproject_id, $fileName,
        $delim = ';')
    {
    ...
    {
        $simpleXMLObj = @$this->simplexmlLoadFileHelper($fileName);
    ...
    public function getUsedKeywordsMap($id)
    {
        ...
        return $this->db->fetchColumnsIntoMap($sql, 'id', 'keyword');
    }
     ...
     *
     */
    public function getOptionReqSpec($tproject_id,
        $get_not_empty = self::GET_EMPTY_REQSPEC)
    {
        $additional_table = '';
        $additional_join = '';
        if ($get_not_empty) {
        ...
        $sql = " SELECT SRS.id,NH.name AS title " .
            ...
            " {$this->tables['nodes_hierarchy']} NH " . $additional_table .
            " WHERE testproject_id={$tproject_id} " . " AND SRS.id=NH.id " .
            $additional_join . " ORDER BY title";
        ...
        foreach ($array2map as $elem) {
            ...
            switch ($mode) {
                case 'dotted':
                    ...
                    $hmap[$current['id']] = str_repeat($dot, $pding) . $dm .
                        $current['name'];
                    break;
     ...
     *
     */
    public function getReqSpec($testproject_id, $id = null, $fields = null,
        $access_key = null)
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        ...
        return is_null($access_key) ? $this->db->get_recordset($sql) : $this->db->fetchRowsIntoMap(
            $sql, $access_key);
    }
     ...
     */
    private function createReqSpec($testproject_id, $title, $scope, $countReq,
        $user_id, $type = 'n')
    {
        $ignore_case = 1;
        $result = array();
        ...
        if ($chk['status_ok']) {
            $sql = "INSERT INTO {$this->tables['req_specs']} " .
              ...
              VALUES (" . $testproject_id . ",'" .
                $this->db->prepare_string($title) . "','" .
                $this->db->prepare_string($scope) . "','" .
                $this->db->prepare_string($type) . "','" .
                $this->db->prepare_string($countReq) . "'," .
                $this->db->prepare_string($user_id) . ", " . $this->db->db_now() .
                ")";
    ...
    public function get_srs_by_title($testproject_id, $title, $ignore_case = 0)
    {
        ...
        if ($ignore_case) {
            $sql .= " WHERE UPPER(title)='" .
                strtoupper($this->db->prepare_string($title)) . "'";
        } else {
    ...
    public function deleteUserRoles($tproject_id, $users = null, $opt = null)
    {
        ...
        if ($this->db->exec_query($query) && $my['opt']['auditlog']) {
            ...
            if ($testProject) {
                if (is_null($users)) {
                    logAuditEvent(
                        TLS("audit_all_user_roles_removed_testproject",
                            $testProject['name']), "ASSIGN", $tproject_id,
                        "testprojects");
                } else {
                    // TBD
                }
    ...
    private function getUserRoleIDs($tproject_id)
    {
        ...
        return $this->db->fetchRowsIntoMap($query, 'user_id');
    }
    ...
    public function addUserRole($userID, $tproject_id, $roleID)
    {
        ...
        if ($this->db->exec_query($query)) {
            ...
            if ($user && $testProject && $role) {
                logAuditEvent(
                    TLS("audit_users_roles_added_testproject",
                        $user->getDisplayName(), $testProject['name'],
                        $role->name), "ASSIGN", $tproject_id, "testprojects");
            }
            unset($user);
            unset($role);
            unset($testProject);
     ...
     */
    private function DEPRECATED_get_keywords_tcases($testproject_id,
        $keyword_id = 0, $keyword_filter_type = 'Or')
    {
        $keyword_filter = '';
        $subquery = '';
    ...
    /**
     */
    public function getKeywordsLatestTCV($tproject_id, $keyword_id = 0,
        $kwFilterType = 'Or')
    {
        $kwFilter = '';
        $subquery = '';
     ...
     */
    private function XXXgetPlatformsLatestTCV($tproject_id, $platform_id = 0,
        $filterType = 'Or')
    {
        $platFilter = '';
        $subquery = '';
        ...
        return $this->db->fetchMapRowsIntoMap($sql, 'testcase_id', 'platform_id');
    }
    ...
    public function get_all_testplans($id, $filters = null, $options = null)
    {
        ...
        $forHMLSelect = false;
        if (! is_null($my['options']['outputType']) &&
            $my['options']['outputType'] == 'forHMLSelect') {
            $forHMLSelect = true;
     ...
     *
     */
    public function check_tplan_name_existence($tproject_id, $tplan_name,
        $case_sensitive = 0)
    {
     ...
     *
     */
    public function get_first_level_test_suites($tproject_id, $mode = 'simple',
        $opt = null)
    {
    ...
    private function getTCasesLinkedToAnyTPlan($id)
    {
        ...
        $sql .= " JOIN {$this->tables['nodes_hierarchy']} NHTPLAN " .
            ...
            " WHERE NHTPLAN.node_type_id = {$tplanNodeType} AND NHTPLAN.parent_id = " .
            intval($id);
     ...
     *
     */
    public function get_linked_custom_fields($id, $node_type = null,
        $access_key = 'id')
    {
        $additional_table = "";
        $additional_join = "";
        ...
        $sql = "SELECT CF.*,CFTP.display_order " .
            ...
            $additional_table . " WHERE CF.id=CFTP.field_id " .
            " AND   CFTP.testproject_id={$id} " . $additional_join .
            " ORDER BY CFTP.display_order";
        return $this->db->fetchRowsIntoMap($sql, $access_key);
    }
     ...
     *
     */
    public function copy_as($id, $new_id, $user_id, $new_name = null,
        $options = null)
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        ...
        $this->copyCfieldsAssignments($id, $new_id);

        // Keywords
        $oldNewMappings['keywords'] = $this->copyKeywords($id, $new_id);

        // Platforms
        $oldNewMappings['platforms'] = $this->copyPlatforms($id, $new_id);
        ...
        $this->copyTestplans($id, $new_id, $user_id, $oldNewMappings);

        $this->copyUserRoles($id, $new_id);
     ...
     *            new Test Project identificator
     */
    private function copyPlatforms($source_id, $target_id)
    {
        ...
        if (! is_null($platformSet)) {
            ...
            foreach ($platformSet as $platform) {
                ...
                $item->enable_on_execution = intval(
     ...
     *            new Test Project identificator
     */
    private function copyKeywords($source_id, $target_id)
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $old_new = null;
        ...
        if (! is_null($itemSet)) {
            foreach ($itemSet as $item) {
                $op = $this->addKeyword($target_id, $item['keyword'],
                    $item['notes']);
    ...
    /**
     */
    private function copyTestplans($source_id, $target_id, $user_id, $mappings)
    {
        static $tplanMgr;
        ...
        if (! is_null($tplanSet)) {
            ...
            foreach ($keySet as $itemID) {
                $new_id = $tplanMgr->create($tplanSet[$itemID]['name'],
                    $tplanSet[$itemID]['notes'], $target_id,
                    $tplanSet[$itemID]['active'],
                    $tplanSet[$itemID]['is_public']);
     ...
     * @internal revisions
     */
    private function getSubtreeRec($node_id, &$pnode, $filters = null,
        $options = null)
    {
        static $qnum;
        static $my;
        ...
        if (! $my) {
            ...
            foreach ($actOnVersion as $target) {
                $tcversionFilter['enabled'] = $tcversionFilter['enabled'] ||
        ...
        if ($tcaseFilter['enabled']) {
            foreach ($tcaseFilter as $key => $apply) {
                if ($apply) {
                    switch ($key) {
                        case 'name':
                            $safe4DB = $this->db->prepare_string(
                                $my['filters']['testcase_name']);
        ...
        $ks = array_keys($rs);
        foreach ($ks as $ikey) {
            if ($rs[$ikey]['node_type_id'] ==
                $this->tree_manager->node_descr_id['testcase']) {
        ...
        if (! is_null($tclist)) {
            ...
            if ($tcversionFilter['enabled'] || $tcaseFilter['is_active']) {
                ...
                if ($tcversionFilter['importance'] ||
                    $tcversionFilter['execution_type'] ||
        ...
        foreach ($rs as $row) {
            if (! isset($exclude_branches[$row['id']])) {
                ...
                if (! isset(
                    ...
                    $this->getSubtreeRec($row['id'], $node, $my['filters'],
                        $my['options']);
                }
     ...
     * -1 => WITHOUT KEYWORDS
     */
    protected function getTCLatestVersionFilteredByKeywords($tproject_id,
        $keyword_id = 0, $keyword_filter_type = 'Or')
    {
    ...
    public function getTestCasesCreatedByUser($id, $user_id, $options = null)
    {
        ...
        if ($itemQty > 0) {
            $sql = " /* $debugMsg */ SELECT TPROJ.id AS tproject_id, TCV.id AS tcversion_id," .
                ...
                " WHERE TPROJ.id = {$safe['tproject_id']} " . " AND NHTC.id IN (" .
                implode(',', $target) . ")";
            ...
            $rs = $this->db->fetchRowsIntoMap($sql, 'tcase_id',
                database::CUMULATIVE);
            if (! is_null($rs)) {
                $k2g = array_keys($rs);
    ...
    public static function getAPIKey(&$dbh, $id)
    {
        ...
        $sql = "SELECT api_key FROM {$sch['testprojects']} WHERE id=" .
            intval($id);
    ...
    /**
     */
    private function checkKeywordIsLinkedAndNotExecuted($keyword_id,
        $tproject_id = null)
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $whereAdd = '';
    ...
    /**
     */
    private function checkKeywordIsLinkedToFrozenVersions($keyword_id,
        $tproject_id = null)
    {
        $debugMsg = $this->debugMsg . __FUNCTION__;
        $whereAdd = '';
    ...
    public function getKeywordsExecStatus($keywordSet, $tproject_id = null)
    {
        ...
        return $this->db->fetchRowsIntoMap($sql, 'keyword_id');
    }
     ...
     *
     */
    protected function getTCLatestVersionFilteredByPlatforms($tproject_id,
        $platform_id = 0)
    {
            ...
            switch ($filter_type) {
                ...
                default:
                    $sql = " /* Filter Type = OR */ " . " SELECT TK.testcase_id " .
                        ...
                        " ON PL.id = TK.platform_id " . " WHERE {$filter} " .
