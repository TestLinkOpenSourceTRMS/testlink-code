...
abstract class tlFilterControl extends tlObjectWithDB
{
     ...
     * @var bool
     */
    public $draw_export_testplan_button = false;
     ...
     * It is TRUE, when user has sent data with filter/settings forms,
    ...
    protected function read_config()
    {
        ...
        $this->option_strings['somebody'] = $go . lang_get('filter_somebody') .
            $gc;
     ...
     * Then it modifies configuration,
    ...
    protected function init_args()
    {
        ...
        $this->args->testproject_id = intval(
        ...
        if (isset($_REQUEST['loadExecDashboard'])) {
            $this->args->loadExecDashboard = intval(
                $_REQUEST['loadExecDashboard']);
        }
    }
    ...
    protected function init_filter_custom_fields($application_areas = null)
    {
        ...
        $btn_label = $collapsed ? lang_get('btn_show_cf') : lang_get(
            'btn_hide_cf');
        ...
        if (! is_null($cfields)) {
            ...
            foreach ($cfields as $cf) {
                ...
                $type = $cf['type'];
                $verbose_type = trim(
                    $this->cfield_mgr->custom_field_types[$type]);
                ...
                if ($this->args->reset_filters) {
                    ...
                    if ($verbose_type == 'datetime' &&
                        ...
                        $_REQUEST[$cf_input_name . '_hour'] != '' &&
                        isset($_REQUEST[$cf_input_name . '_minute']) &&
                        $_REQUEST[$cf_input_name . '_minute'] != '' &&
                        isset($_REQUEST[$cf_input_name . '_second']) &&
                        $_REQUEST[$cf_input_name . '_second'] != '') {
                        ...
                        $value = mktime($hour, $minute, $second,
                    ...
                    if ($verbose_type == 'date' &&
                        ...
                        $date = $_REQUEST[$cf_input_name . '_input'];
                        $date_array = split_localized_date($date, $date_format);
                        $value = mktime(0, 0, 0, $date_array['month'],
     ...
     */
    abstract protected function init_filters();
     ...
     */
    abstract protected function get_active_filters();
     ...
     *
     * @param object $gui
     *            Reference to GUI object (information will be written to it)
     * @return object $tree_menu Tree object for display of JavaScript tree menu.
     */
    abstract public function build_tree_menu(&$gui);

    abstract protected function getCustomFields();
}
