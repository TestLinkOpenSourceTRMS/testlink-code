...
class tlInventory extends tlObjectWithDB
{
     ...
     * @param integer $dbHandler
     *            the database connection handler
     */
    public function __construct($testProjectID, &$dbHandler = null)
    {
     ...
     *
     * @param mixed $ids
     *            integer or array of integer - ID of inventory items
     */
    ...
    protected function writeToDB(&$db)
    {
        ...
        $data_serialized = $db->prepare_string(
            serialize($this->inventoryContent));
        if (is_null($this->inventoryId) || ($this->inventoryId == 0)) {
            $sql = "/* {$debugMsg} */
			        INSERT INTO {$this->tables['inventory']}
			        (name,testproject_id,content,ipaddress,
			         owner_id,creation_ts) " . " VALUES ('" . $name . "'," .
                $this->testProjectID . ",'" . $data_serialized . "','" . $ip .
            ...
            if ($result) {
                ...
                logAuditEvent(
                    TLS("audit_inventory_created", $this->name,
                        $auditData['tproject_name']), "CREATE", $this->name,
                    "inventory");
                $this->userFeedback = langGetFormated(
                    'inventory_create_success', $this->name);
            } else {
                $this->userFeedback = langGetFormated('inventory_create_fails',
                    $this->name);
                tLog(
                    'Internal error: An inventory device "' . $this->name .
            ...
            $sql = "/* {$debugMsg} */UPDATE {$this->tables['inventory']} " .
                ...
                ", testproject_id={$this->testProjectID}, owner_id=" .
                $this->ownerId . " WHERE id={$this->inventoryId}";
            $result = $this->db->exec_query($sql);
            if ($result) {
                tLog('A device "' . $this->name . '" was not updated.', 'INFO');
                $this->userFeedback = langGetFormated(
                    'inventory_update_success', $this->name);
            } else {
                $this->setUserFeedback(
                    langGetFormated('inventory_update_fails', $this->name));
                tLog(
                    'Internal error: An inventory device "' . $this->name .
    ...
    public function deleteInventory($itemID)
    {
        ...
        if (! is_null($recordset)) {
            ...
            if ($result == tl::OK) {
                logAuditEvent(
                    TLS("audit_inventory_deleted", $this->name,
                        $auditData['tproject_name']), "DELETE", $this->name,
                    "inventory");
                $this->userFeedback = langGetFormated(
                    'inventory_delete_success', $this->name);
            } else {
                $this->userFeedback = langGetFormated('inventory_delete_fails',
                    $this->name);
                tLog(
                    'Internal error: The device "' . $this->name .
                    '" was not deleted.', 'ERROR');
            }
        } else {
            $this->userFeedback = lang_get('inventory_no_device') . ' ID=' .
                $this->inventoryId;
            tLog(
                'Internal error: The device "' . $this->name .
                '" was not deleted.', 'ERROR');
    ...
    protected function checkInventoryData()
    {
        ...
        if ($result == tl::OK) {
            $sql = "/* {$debugMsg} */
			        SELECT id FROM {$this->tables['inventory']} " . " WHERE name='" .
                $name . "' AND testproject_id={$this->testProjectID}";
            ...
            if ($this->db->fetchFirstRow($sql)) {
                ...
                $this->userFeedback = langGetFormated('inventory_name_exists',
                    $this->name);
            }
        }
        ...
        if ($result == tl::OK && ! empty($ipAddress)) {
            $sql = "/* {$debugMsg} */
			        SELECT id FROM {$this->tables['inventory']} " . " WHERE ipaddress='" .
                $ipAddress . "' AND testproject_id={$this->testProjectID}";
            ...
            if ($this->db->fetchFirstRow($sql)) {
                ...
                $this->userFeedback = langGetFormated('inventory_ip_exists',
                    $ipAddress);
            }
        }
