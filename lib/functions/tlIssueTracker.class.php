...
class tlIssueTracker extends tlObject
{
     ...
     *
     */
    public function getImplementationForType($issueTrackerType)
    {
    ...
    public function update($it)
    {
        ...
        if ($ret['status_ok']) {
            $sql = "UPDATE {$this->tables['issuetrackers']}  " . " SET name = '" .
        ...
        if (! is_null($rs)) {
            ...
            if ($my['options']['output'] == 'add_link_count') {
                ...
                $sql .= " FROM {$this->tables['issuetrackers']} ITD " .
                    ...
                    " ON issuetracker_id = ITD.id " . " GROUP BY ITD.id ";
     ...
     *
     */
    public function checkConnection($its)
    {
        $xx = $this->getByID($its);
