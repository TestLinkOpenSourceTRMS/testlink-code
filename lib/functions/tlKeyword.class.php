 ...
 * @package TestLink
 */
class tlKeyword extends tlDBObject implements iSerialization,
    iSerializationToXML, iSerializationToCSV, iDBBulkReadSerialization
{
     ...
     *
     * @param integer $dbID
     *            the database identifier of the keywords
     */
     ...
     *            the id of the testproject the keywords belongs to
     * @param string $name
     *            the name of the keyword
     * @param string $notes
     *            the notes for the keywords
     */
    public function initialize($dbID, $testprojectID, $name, $notes)
    {
        $this->dbID = $dbID;
        $this->name = $name;
        $this->notes = $notes;
    ...
    {
        $this->initialize($row['id'], $row['testproject_id'], $row['keyword'],
            $row['notes']);
    }
     ...
     *
     * @param
     *            resource &$db [ref] database connection
     *
    ...
    {
        $sql = "DELETE FROM {$this->tables['testcase_keywords']} WHERE keyword_id = " .
            $this->dbID;
        $result = $db->exec_query($sql);
        if ($result) {
            $sql = "DELETE FROM {$this->tables['object_keywords']}  WHERE keyword_id = " .
                $this->dbID;
            $result = $db->exec_query($sql);
        }
        if ($result) {
            $sql = "DELETE FROM {$this->tables['keywords']} WHERE id = " .
                $this->dbID;
            $result = $db->exec_query($sql);
        }
     ...
     * @param integer $id
     *            the databse identifier of the keyword
     * @param integer $detailLevel
     *            an optional detaillevel, any combination of TLOBJ_O_GET_DETAIL Flags
     *
        ...
        $detailLevel = self::TLOBJ_O_GET_DETAIL_FULL)
    {
        return tlDBObject::createObjectFromDB($db, $id, __CLASS__,
            tlKeyword::TLOBJ_O_SEARCH_BY_ID, $detailLevel);
    }
     ...
     *            [ref] the database connection
     * @param array $ids
     *            the database identifiers of the keywords
     * @param integer $detailLevel
     *            an optional detaillevel, any combination of TLOBJ_O_GET_DETAIL Flags
     *
    ...
    public static function getByIDs(&$db, $ids,
        $detailLevel = self::TLOBJ_O_GET_DETAIL_FULL)
    {
        return tlDBObject::createObjectsFromDB($db, $ids, __CLASS__, false,
            $detailLevel);
    }
     ...
     *
     * @param string $name
     *            the name of the keyword which should be checked
     *
     * @return integer returns tl::OK if the check was sucesssful, else errorcode
     */
    public static function checkKeywordName($name)
    {
        $result = tl::OK;
        if ($name != "") {
     ...
     *            the testprojectID
     * @param integer $kwID
     *            an additional keyword id which is excluded in the search
