...
class tlPlatform extends tlObjectWithDB
{
     ...
     *            to
     *            work on. If null (default) the project in session
     *            is used
     ...
     * @return tl::OK on success, otherwise E_DBERROR
     */
    public function update($id, $name, $notes, $enable_on_design = null,
        $enable_on_execution = null, $is_open = 1)
    {
        $this->throwIfEmptyName($name);
        $sql = " UPDATE {$this->tables['platforms']} " . " SET name = '" .
            $this->db->prepare_string($name) . "' " . ", notes =  '" .
            $this->db->prepare_string($notes) . "' ";

        /* Optional */
        if (! is_null($enable_on_design)) {
            $sql .= ", enable_on_design =  " .
                ((($enable_on_design > 0) || $enable_on_design) ? 1 : 0);
        }
        if (! is_null($enable_on_execution)) {
            $sql .= ", enable_on_execution =  " .
                ((($enable_on_execution > 0) || $enable_on_execution) ? 1 : 0);
        }
    ...
    public function testProjectCount($opt = null)
    {
        ...
        //
        $sql = $debugMsg .
            " SELECT COALESCE(COUNT(PLAT.id),0) AS platform_qty, TPROJ.id AS tproject_id " .
    ...
    public function belongsToTestProject($id, $tproject_id = null)
    {
        ...
        $sql = " SELECT id FROM {$this->tables['platforms']} " . " WHERE id = " .
            intval($id) . " AND testproject_id=" . $pid;
    ...
    /**
     */
    private function initViewGUI(&$userObj, &$argsObj)
    {
        $gaga = new stdClass();
    ...
    /**
     */
    public function disableExec($id)
    {
        $sql = "UPDATE {$this->tables['platforms']}
    ...
    /**
     */
    public function openForExec($id)
    {
        $sql = "UPDATE {$this->tables['platforms']}
            SET is_open = 1
            WHERE id = $id";
    ...
    /**
     */
    public function closeForExec($id)
    {
        $sql = "UPDATE {$this->tables['platforms']}
            SET is_open = 0
            WHERE id = $id";
