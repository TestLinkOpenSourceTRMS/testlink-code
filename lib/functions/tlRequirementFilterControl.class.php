...
class tlRequirementFilterControl extends tlFilterControl
{
    ...
    public function build_tree_menu(&$gui)
    {
        ...
        $drag_and_drop->BackEndUrl = $gui->basehref .
        ...
        if ($this->do_filtering) {
            ...
            $loader = $gui->basehref .
                'lib/ajax/getrequirementnodes.php?mode=reqspec&' .
        ...
        $gui->ajaxTree->cookiePrefix = 'req_specification_tproject_id_' .
            $root_node->id . "_";
    }
    ...
    public function init_filter_coverage()
    {
        ...
        if ($this->configuration->req_cfg->expected_coverage_management) {
            ...
            if (! $selection || ! is_numeric($selection) ||
                $this->args->reset_filters) {
                $selection = null;
            } else {
                $this->do_filtering = true;
            }
    ...
    public function init_filter_tc_id()
    {
        ...
        $tc_prefix = $this->testproject_mgr->getTestCasePrefix(
            $this->args->testproject_id);
        $tc_prefix .= $tc_cfg->glue_character;

        if (! $selection || $selection == $tc_prefix ||
            $this->args->reset_filters) {
            $selection = null;
        } else {
            $this->do_filtering = true;
