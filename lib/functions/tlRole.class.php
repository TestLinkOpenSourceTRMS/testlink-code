...
class tlRole extends tlDBObject
{
    ...
    public static function doesRoleExist(&$db, $name, $id)
    {
        $role = new tlRole();
        $role->name = $name;
        if ($role->readFromDB($db, self::ROLE_O_SEARCH_BYNAME) >= tl::OK &&
            $role->dbID != $id) {
            return $role->dbID;
        }
        return null;
    ...
    public function getUsersWithGlobalRole(&$db, $opt = null)
    {
        ...
        return self::createObjectsFromDB($db, $idSet, "tlUser", true,
            self::TLOBJ_O_GET_DETAIL_MINIMUM);
    }
    ...
    public function getAllUsersWithRole(&$db)
    {
        ...
        $affectedUsers = (array) $global_users + (array) $tplan_users +
            (array) $tproject_users;
        $affectedUsers = array_unique($affectedUsers);
        return self::createObjectsFromDB($db, $affectedUsers, "tlUser", true,
            self::TLOBJ_O_GET_DETAIL_MINIMUM);
    }
     ...
     * @param
     *            resource &$db reference to database handler
     * @return int returns tl::OK on success, tl::ERROR else
     */
        ...
        $orderBy = null, $detailLevel = self::TLOBJ_O_GET_DETAIL_FULL)
    {
        ...
        $roles = tlDBObject::createObjectsFromDBbySQL($db, $sql, 'id', __CLASS__,
            true, $detailLevel);
