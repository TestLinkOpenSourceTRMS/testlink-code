...
class tlTestCaseFilterControl extends tlFilterControl
{
    ...
    /**
     * The mode used.
     * Depending on the feature for which this class will be instantiated.
     ...
     * @param string $mode
     *            can be edit_mode/execution_mode/plan_mode/plan_add_mode, depending on usage
     */
    ...
    protected function init_args()
    {
        ...
        if ($this->args->advanced_filter_mode) {
            ...
            switch ($this->mode) {
                case 'plan_add_mode':
                case 'edit_mode':
                    $this->all_filters['filter_workflow_status'] = array(
                        "POST",
                        tlInputParameter::ARRAY_INT
                    );

                    $this->all_filters['filter_importance'] = array(
                        "POST",
                        tlInputParameter::ARRAY_INT
                    );
                    break;
        ...
        $this->args->{'filter_assigned_user_include_unassigned'} = isset(
            $_REQUEST['filter_assigned_user_include_unassigned']) ? 1 : 0;
        ...
        $this->args->feature = isset($_REQUEST['feature']) ? trim(
            $_REQUEST['feature']) : null;
        $doLog = false;
        switch ($this->mode) {
            case 'plan_mode':
        ...
        if ($doLog) {
            tLog(
                ...
                ' - Wrong or missing GET argument: feature', 'ERROR');
            exit();
        }
    }
    ...
    protected function init_settings()
    {
        ...
        if ($this->mode == 'plan_mode' &&
            ...
            $this->args->feature != 'test_urgency')) {
            ...
            if ($this->configuration->show_filters == self::ENABLED &&
                ...
                switch ($name) {
                    case 'filter_custom_fields':
                        $params = $this->mode == 'execution_mode' ? array(
                            'design' => true,
     ...
     *
     * @author Andreas Simon
     * @param int $token_validity_duration
     *            data older than given timespan will be deleted
     */
    public function delete_old_session_data($token_validity_duration = 0)
    {
        ...
        if (! is_numeric($token_validity_duration) ||
            $token_validity_duration <= 0) {
    ...
    public function delete_own_session_data()
    {
        if (isset($_SESSION[$this->mode]) &&
            isset($_SESSION[$this->mode][$this->form_token])) {
        ...
        if (! $string) {
            ...
            if ($this->active_filters['filter_priority'] > 0) {
                $string .= '&filter_priority=' .
                    $this->active_filters['filter_priority'];
            }

            $keyword_list = null;
            if (is_array($this->active_filters['filter_keywords'])) {
                $keyword_list = implode(',',
                    $this->active_filters['filter_keywords']);
            ...
            if ($this->active_filters['filter_assigned_user']) {
                $string .= '&filter_assigned_user=' .
                    json_encode($this->active_filters['filter_assigned_user']) .
                    '&filter_assigned_user_include_unassigned=' .
            ...
            if ($this->active_filters['filter_result_result']) {
                $string .= '&filter_result_result=' .
                    json_encode($this->active_filters['filter_result_result']) .
                    '&filter_result_method=' .
     ...
     * @author Andreas Simon
     * @param object $gui
     *            Reference to GUI object (data will be written to it)
     */
    public function build_tree_menu(&$gui)
    {
        ...
        switch ($this->mode) {
            case 'plan_mode':
                ...
                list ($tree_menu, $testcases_to_show) = testPlanTree($this->db,
                ...
                $cookie_prefix = $this->args->feature . "_tplan_id_" .
                    $filters->setting_testplan . "_";
                break;
            ...
            case 'edit_mode':
                ...
                $cookie_prefix = $this->args->feature . "_tproject_id_" .
                    $this->args->testproject_id . "_";
                ...
                if ($this->do_filtering) {
                    ...
                    $forrest = generateTestSpecTree($this->db,
                        $this->args->testproject_id,
                        $this->args->testproject_name, $gui->menuUrl, $filters,
                        $options);
                    ...
                    $loader = $this->args->basehref .
                        ...
                        "root_node={$this->args->testproject_id}&" . "tcprefix=" .
                        urlencode(
                            $tc_prefix .
                            $this->configuration->tc_cfg->glue_character);

                    $tcase_qty = $this->testproject_mgr->count_testcases(
                        $this->args->testproject_id);
            ...
            default:
                ...
                list ($tree_menu, $testcases_to_show) = execTree($this->db,
                    $gui->menuUrl,
                    array(
                ...
                $cookie_prefix = 'test_exec_build_id_' . $filters->setting_build .
                    '_';
                if (isset($filters->setting_platform)) {
                    $cookie_prefix .= 'platform_id_' . $filters->setting_platform .
                        '_';
                }
                break;
        }
    ...
    public function init_setting_build()
    {
        ...
        $this->settings[$key]['items'] = $this->testplan_mgr->get_builds_for_html_options(
            $tplan_id, $active, $open);
        ...
        $newest_build_id = $this->testplan_mgr->get_max_build_id($tplan_id,
            $active, $open);
        ...
        $this->settings[$key]['selected'] = (in_array($this->args->$key,
    ...
    public function init_setting_testplan()
    {
        ...
        $testplans = $this->user->getAccessibleTestPlans($this->db,
            $this->args->testproject_id);
    ...
    public function init_setting_platform()
    {
        ...
        $platformSet = $this->platform_mgr->getLinkedToTestplanAsMap(
            $testplan_id);
        ...
        switch ($this->mode) {
            ...
            default:
                throw new Exception(
                    __METHOD__ . "Mode:" . $this->mode . 'Do not know what to do',
                    1);
                break;
        }
        ...
        if ($this->args->$key <= 0) {
            $this->settings[$key]['selected'] = key(
                $this->settings[$key]['items']);
        }
    ...
    public function init_filter_toplevel_testsuite()
    {
        ...
        $first_level_suites = $this->testproject_mgr->get_first_level_test_suites(
            $this->args->testproject_id, 'smarty_html_options');
    ...
    public function init_filter_keywords()
    {
        ...
        switch ($this->mode) {
            ...
            case 'plan_add_mode':
                ...
                $keywords = $this->testproject_mgr->getUsedKeywordsMap(
                    $this->args->testproject_id);
                break;
            ...
            default:
                ...
                $tplan_id = $this->settings['setting_testplan']['selected'];
                $keywords = $this->testplan_mgr->get_keywords_map($tplan_id,
                    ' ORDER BY keyword ');
                break;
        }
        ...
        if ($atLeastOneKW) {
            ...
            $this->filters[$key]['size'] = min(
                count($this->filters[$key]['items']),
    ...
    public function init_filter_importance()
    {
        ...
        $tp_info = $this->testproject_mgr->get_by_id(
            $this->args->testproject_id);
    ...
    public function init_filter_assigned_user()
    {
        ...
        $tproject_info = $this->testproject_mgr->get_by_id(
            $this->args->testproject_id);
        ...
        if ($this->mode == 'execution_mode') {
            $role = $this->user->getEffectiveRole($this->db,
                $this->args->testproject_id, $tplan_id);

            $simple_tester_roles = array_flip(
                $this->configuration->exec_cfg->simple_tester_roles);
            ...
            $simple = false;
            if (isset($simple_tester_roles[$role->dbID]) ||
                ($right_to_execute && ! $right_to_manage)) {
            ...
            if (! $selection &&
                $this->configuration->exec_cfg->user_filter_default ==
                'logged_user') {
                $selection = (array) $this->user->dbID;
            }
        }
    ...
    public function init_filter_result()
    {
        ...
        foreach ($this->configuration->filter_methods['status_code'] as $statusname => $statusshortcut) {
            ...
            $this->filters[$key][$method_key]['items'][$code] = lang_get(
                $this->configuration->filter_methods['status_label'][$statusname]);
        }

        // init menu for build selection
        $this->filters[$key][$build_key]['items'] = $this->testplan_mgr->get_builds_for_html_options(
            $tplan_id, testplan::GET_ACTIVE_BUILD);

        // if "any" is selected, nullify the active filters
        if ((is_array($result_selection) &&
            in_array($any_result_key, $result_selection)) ||
            $result_selection == $any_result_key) {
    ...
    protected function getCustomFields($application_area = null)
    {
        ...
        $cfields = array(
            'design' => null,
            'execution' => null,
            'testplan_design' => null
        );
        if ($scope['design']) {
            $cfields['design'] = $this->cfield_mgr->get_linked_cfields_at_design(
                $this->args->testproject_id, 1, null, 'testcase');
        }

        if ($scope['testplan_design']) {
            $cfields['testplan_design'] = $this->cfield_mgr->get_linked_cfields_at_testplan_design(
                $this->args->testproject_id, 1, 'testcase');
        }
    ...
    protected function init_setting_testsgroupby()
    {
        ...
        $this->settings[$key]['items']['mode_test_suite'] = lang_get(
            'mode_test_suite');
        $this->settings[$key]['items']['mode_req_coverage'] = lang_get(
            'mode_req_coverage');
    }
    ...
    protected function init_setting_exec_tree_counters_logic()
    {
        ...
        $_SESSION[$key] = $this->args->{$key} = $this->settings[$key]['selected'] = $algo;
    }

    /**
     */
    private function setFiltersDefinition()
    {
        $this->all_filters = array(
            ...
            'filter_workflow_status' => array(
                "POST",
                tlInputParameter::INT_N
            ),
            'filter_importance' => array(
                "POST",
                tlInputParameter::INT_N
            ),
