...
class tlTestPlanMetrics extends testplan
{
    ...
    public function __construct(&$db)
    {
        ...
        $this->statusCode = array_flip(
            array_keys($this->resultsCfg['status_label_for_exec_ui']));
     ...
     *
     * @param int $milestoneTargetDate
     *            - (optional) milestone deadline
     * @param int $milestoneStartDate
     *            - (optional) milestone start date
     * @return array with three priority counters
     */
    public function getPrioritizedResults($tplanID, $milestoneTargetDate = null,
    ...
    /**
     */
    public function getMilestonesMetrics($tplanID, $milestoneSet = null)
    {
        $results = array();
        ...
        $pc = array(
            LOW => 'result_low_percentage',
            MEDIUM => 'result_medium_percentage',
        ...
        foreach ($milestones as $item) {
            ...
            $item['results'] = $this->getPrioritizedResults($tplanID,
                $item['target_date'], $item['start_date']);
            $item['tc_completed'] = 0;
            ...
            foreach ($pc as $key => $item_key) {
                ...
                if ($item[$target_key] == 0) {
                    ...
                    $item[$item_key] = ($priorityCounters[$key] > 0) ? $this->get_percentage(
                        $priorityCounters[$key], $item['results'][$key]) : 0;
                }
                $item['tc_completed'] += $item['results'][$key];
            }

            // amount of all executed tc with any priority before target_date / all test cases
            $item['percentage_completed'] = $this->get_percentage(
                $item['tc_total'], $item['tc_completed']);
     ...
     */
    private function get_percentage($total, $parameter)
    {
     ...
     *
     */
    public function getExecCountersByBuildExecStatus($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        $safe_id = intval($id);
        list ($my, $builds, $sqlStm) = $this->helperGetExecCounters($safe_id,
            $filters, $opt);
        ...
        $sqlUnionAB = "/* {$debugMsg} sqlUnionAB - executions */" .
            ...
            " AND E.build_id = UA.build_id " . " WHERE TPTCV.testplan_id=" .
            $safe_id . " AND UA.build_id IN ({$builds->inClause}) ";
        ...
        $sqlUnionBB = "/* {$debugMsg} sqlUnionBB - NOT RUN */" .
            ...
            " AND LEBBP.build_id = UA.build_id " . " AND LEBBP.testplan_id = " .
            $safe_id . " LEFT OUTER JOIN {$this->tables['executions']} E " .
        ...
        if ($my['opt']['groupByPlatform']) {
            ...
            $exec['with_tester'] = (array) $this->db->fetchRowsIntoMap3l($sql,
                $kol);
        ...
        if ($my['opt']['groupByPlatform']) {
            ...
            $exec['total'] = (array) $this->db->fetchRowsIntoMap($sql,
                'build_id');
        }
    ...
    /**
     */
    public function getOverallBuildStatusForRender($id,
        $totalKey = 'total_assigned')
    {
        $renderObj = null;
            ...
            foreach ($buildList as $buildID) {
                ...
                foreach ($code_verbose as $statusCode => $statusVerbose) {
                    ...
                    if ($renderObj->info[$buildID][$totalKey] > 0) {
                        $rf[$statusVerbose]['percentage'] = number_format(
                            100 *
                ...
                $renderObj->info[$buildID]['percentage_completed'] = number_format(
                    100 * ($totalRun / $renderObj->info[$buildID][$totalKey]), 1);
            }

            foreach ($code_verbose as $human) {
                $l10n = isset($labels[$human]) ? lang_get($labels[$human]) : lang_get(
                    $human);
     ...
     *
     */
    private function getExecCountersByKeywordExecStatus($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        $safe_id = intval($id);
        ...
        $sqlUnionAK = "/* {$debugMsg} sqlUnionAK - executions */" .
            ...
            " ON  KW.id = TCK.keyword_id " . " WHERE TPTCV.testplan_id=" .
            $safe_id . $builds->whereAddExec;
        ...
        $sqlUnionBK = "/* {$debugMsg} sqlUnionBK - NOT RUN */" .
            ...
            " ON  E.tcversion_id = TPTCV.tcversion_id " .
            " AND E.testplan_id = TPTCV.testplan_id " .
            " AND E.platform_id = TPTCV.platform_id " . $builds->joinAdd .
            " /* Get ONLY Test case versions that has AT LEAST one Keyword assigned */ " .
            " JOIN {$this->tables['nodes_hierarchy']} NHTCV " .
            " ON NHTCV.id = TPTCV.tcversion_id " .
            " JOIN {$this->tables['testcase_keywords']} TCK " .
            ...
            " WHERE TPTCV.testplan_id=" . $safe_id . $builds->whereAddNotRun .
        ...
        if ($my['opt']['groupByPlatform']) {
            ...
            $exec[$exec['key4total']] = (array) $this->db->fetchMapRowsIntoMap(
                $sql, 'platform_id', 'keyword_id');
        } else {
            $exec[$exec['key4total']] = (array) $this->db->fetchRowsIntoMap(
                $sql, 'keyword_id');
        }
     ...
     *        20120429 - franciscom -
     */
    public function getExecCountersByPlatformExecStatus($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        $safe_id = intval($id);
     ...
     */
    private function getExecCountersByPriorityExecStatus($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        $safe_id = intval($id);
        ...
        $sqlUnionA = "/* {$debugMsg} sqlUnionA - executions */" .
            ...
            " COALESCE(E.status,'{$this->notRunStatusCode}') AS status " .
            " FROM {$this->tables['testplan_tcversions']} TPTCV " .
            $sqlStm['getAssignedFeatures'] . " /* Get importance  */ " .
            ...
            $builds->joinAdd . " /* FILTER BUILD Set on target test plan */ " .
        ...
        if ($my['opt']['groupByPlatform']) {
            $this->helperStatusDomainMatrix($exec, 'platform_id',
                'priority_level');
        } else {
    ...
    /**
     */
    public function getExecCountersByExecStatus($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
     ...
     */
    private function getExecCountersByBuildUAExecStatus($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        $safe_id = intval($id);
        ...
        $sqlUnionBU = "/* {$debugMsg} */" .
            ...
            " AND E.testplan_id = TPTCV.testplan_id " .
            " AND E.platform_id = TPTCV.platform_id " .
            " AND E.tcversion_id = TPTCV.tcversion_id " . " AND E.id = LEBBP.id " . // TICKET 5192
            " WHERE TPTCV.testplan_id=" . $safe_id .
            " AND UA.build_id IN ({$builds->inClause}) ";
                ...
                foreach ($itemSet as $itemID) {
                    ...
                    foreach ($this->statusCode as $code) {
                        ...
                        if (! isset($totals[$topLevelItemID][$itemID])) {
                            $totals[$topLevelItemID][$itemID] = array(
                                'build_id' => $topLevelItemID,
                                'user_id' => $itemID,
                                'qty' => 0,
                                'total_time' => 0
     ...
     * @since 1.9.6
     */
    public function getStatusTotalsByBuildUAForRender($id, $opt = null)
    {
        $my = array(
            'opt' => array(
        ...
        $renderObj = null;
        $code_verbose = $this->getStatusForReports();
        $metrics = $this->getExecCountersByBuildUAExecStatus($id, null,
            $my['opt']);
            ...
            foreach ($topItemSet as $topItemID) {
                ...
                foreach ($itemSet as $itemID) {
                    ...
                    foreach ($code_verbose as $statusCode => $statusVerbose) {
                        ...
                        $pc = ($elem[$statusCode]['exec_qty'] /
                            $out[$topItemID][$itemID]['total']) * 100;
                        $out[$topItemID][$itemID][$statusVerbose]['percentage'] = number_format(
                            $pc, 1);
                    ...
                    $progress = ($progress / $out[$topItemID][$itemID]['total']) *
                        100;
                    $out[$topItemID][$itemID]['progress'] = number_format(
                        $progress, 1);
                    $out[$topItemID][$itemID]['total_time'] = number_format(
                        $metrics['total'][$topItemID][$itemID]['total_time'], 2,
                        '.', '');
                }
            }
        }
     ...
     *
     */
    private function getStatusTotalsByItemForRender($id, $itemType,
        $filters = null, $opt = null)
    {
        ...
        switch ($itemType) {
            ...
            case 'tsuite':
                $metrics = $this->getExecCountersByTestSuiteExecStatus($id,
                    $filters, $opt);
                $setKey = 'tsuites';
                $returnArray = true;
                $byPlatform = isset($opt['groupByPlatform']) &&
        ...
        if (! is_null($metrics) && ! is_null($metrics[$setKey]) > 0) {
            ...
            if (! $byPlatform) {
                ...
                foreach ($itemList as $itemID) {
                    if (isset($metrics['with_tester'][$itemID])) {
                        ...
                        if ($doPerc) {
                            $renderObj->info[$itemID]['percentage_completed'] = number_format(
                                100 *
                                ($totalRun /
                                $renderObj->info[$itemID]['total_tc']), 1);
                        }
                ...
                foreach ($platList as $platID) {
                    ...
                    foreach ($itemList as $itemID) {
                        if (isset($mex[$platID])) {
                            ...
                            $doPerc = ($renderObj->info[$platID][$itemID]['total_tc'] >
                                0);
                            foreach ($code_verbose as $statusCode => $statusVerbose) {
                                $rf[$statusVerbose] = array(
                                    'qty' => 0,
                                ...
                                if ($doPerc) {
                                    $rf[$statusVerbose]['percentage'] = number_format(
                                        100 *
                                        ($rf[$statusVerbose]['qty'] /
                                        $renderObj->info[$platID][$itemID]['total_tc']),
                                        1);
                                }
                                $totalRun += ($statusVerbose == 'not_run' ? 0 : $rf[$statusVerbose]['qty']);
                            }
                            if ($doPerc) {
                                $renderObj->info[$platID][$itemID]['percentage_completed'] = number_format(
                                    100 *
                                    ($totalRun /
                                    $renderObj->info[$platID][$itemID]['total_tc']),
                                    1);
            ...
            foreach ($code_verbose as $status_verbose) {
                $l10n = isset($labels[$status_verbose]) ? lang_get(
                    $labels[$status_verbose]) : lang_get($status_verbose);
     ...
     *        20120429 - franciscom -
     */
    private function getStatusTotalsByTestSuiteForRender($id, $filters = null,
        $opt = null)
    {
        list ($renderObj, $staircase) = $this->getStatusTotalsByItemForRender(
            $id, 'tsuite', $filters, $opt);
        unset($staircase);
        return $renderObj;
    }
     ...
     * @since 1.9.4
     */
    public function getStatusTotalsByTopLevelTestSuiteForRender($id,
        $filters = null, $opt = null)
    {
        list ($rx, $staircase) = $this->getStatusTotalsByItemForRender($id,
            'tsuite', $filters, $opt);
        ...
        foreach ($this->statusCode as $verbose => $code) {
            $template['details'][$verbose] = array(
                'qty' => 0,
                'percentage' => 0
            );
        }
        ...
        if ($wp) {
                ...
                foreach ($plat2loop as $platId) {
                    ...
                    if ($initName) {
                        $dummy = $this->tree_manager->get_node_hierarchy_info(
                            $topSuiteID);
                        $renderObj->info[$platId][$topSuiteID]['name'] = $dummy['name'];
                        unset($dummy);
                    }
                    ...
                    if (null !== $r2d2) {
                        foreach ($r2d2['details'] as $code => &$elem) {
                            $renderObj->info[$platId][$topSuiteID]['details'][$code]['qty'] += $elem['qty'];
                            $renderObj->info[$platId][$topSuiteID]['total_tc'] += $elem['qty'];
            ...
            foreach ($renderObj->info as $platID => &$tsuiteMetrics) {
                foreach ($tsuiteMetrics as $tsuite_id => &$elem) {
                    if ($execQty[$platID][$tsuite_id] > 0) {
                        $elem['percentage_completed'] = number_format(
                            100 *
                            ($execQty[$platID][$tsuite_id] / $elem['total_tc']),
                            1);
                    }
                    if ($elem['total_tc'] > 0) {
            ...
            foreach ($renderObj->info as $tsuite_id => &$elem) {
                if ($execQty[$tsuite_id] > 0) {
                    $elem['percentage_completed'] = number_format(
                        100 * ($execQty[$tsuite_id] / $elem['total_tc']), 1);
                }
    ...
    /**
     */
    private function getExecCountersByTestSuiteExecStatus($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        $safe_id = intval($id);
        list ($my, $builds, $sqlStm) = $this->helperGetExecCounters($id,
            $filters, $opt);
        ...
        $sqlUnionAT = "/* {$debugMsg} sqlUnionAT - executions */" .
            ...
            " ON  NHTC.id = NHTCV.parent_id " . " WHERE TPTCV.testplan_id=" .
            $safe_id . $builds->whereAddExec;
        ...
        $dx = $this->tree_manager->get_full_path_verbose($keySet,
            array(
                'output_format' => 'stairway2heaven'
            ));
        for ($idx = 0; $idx < $loop2do; $idx ++) {
    ...
    /**
     */
    public function getExecStatusMatrix($id, $filters = null, $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;

        $my = array();
        $my['opt'] = array(
            'getExecutionNotes' => false,
            'getTester' => false,
        ...
        list ($my, $builds, $sqlStm, $union) = $this->helperBuildSQLTestSuiteExecCounters(
            $id, $filters, $my['opt']);
        ...
        foreach ($item2loop as $item_id) {
            $stairway2heaven = $this->tree_manager->get_path($item_id, null,
                'name');
            ...
            foreach ($tcase2loop as $tcase_id) {
                ...
                foreach ($platform2loop as $platform_id) {
                    $latestExec[$platform_id][$tcase_id] = array(
                        'id' => - 1,
     ...
     *
     */
    private function helperGetExecCounters($id, $filters, $opt)
    {
        $sql = array();
        $my = array();
        $my['opt'] = array(
            'getOnlyAssigned' => false,
            'tprojectID' => 0,
            'getUserAssignment' => false,
        ...
        if (is_null($bi->idSet)) {
            throw new Exception(
                __METHOD__ . " - Can not work with empty build set");
        }
        // =========================================================
        ...
        if ($my['opt']['getUserAssignment']) {
            $sql['getUserAssignment']['not_run'] = " LEFT JOIN {$this->tables['user_assignments']} UA " .
                " ON UA.feature_id = TPTCV.id " . " AND UA.build_id = BU.id " .
                " AND UA.type = {$this->execTaskCode} ";

            $sql['getUserAssignment']['exec'] = " LEFT JOIN {$this->tables['user_assignments']} UA " .
                " ON UA.feature_id = TPTCV.id " .
        ...
        $sql['LE'] = " SELECT EE.tcversion_id,EE.testplan_id,MAX(EE.id) AS id " .
            " FROM {$this->tables['executions']} EE " . " WHERE EE.testplan_id=" .
            intval($id) . " AND EE.build_id IN ({$bi->inClause}) " .
    ...
    /**
     */
    private function helperCompleteStatusDomain(&$out, $key)
    {
        $totalByItemID = array();
    ...
    /**
     */
    private function helperBuildSQLExecCounters($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        try {
        ...
        $union['exec'] = $dummy['exec'] . " WHERE TPTCV.testplan_id=" . $safe_id .
            $builds->whereAddExec;

        $union['execActive'] = $dummy['exec'] .
            " /* Used to filter ON ACTIVE TCVersion */ " .
            " JOIN {$this->tables['tcversions']} TCV " .
            " ON  TCV.id = TPTCV.tcversion_id " . " WHERE TPTCV.testplan_id=" .
            $safe_id . $builds->whereAddExec . " AND TCV.active = 1 ";
        ...
        $union['not_runActive'] = $dummy['not_run'] .
            ...
            " JOIN {$this->tables['tcversions']} TCV " .
            " ON  TCV.id = TPTCV.tcversion_id " . " WHERE TPTCV.testplan_id=" .
            $safe_id . $builds->whereAddNotRun .
            " /* Get REALLY NOT RUN => BOTH LE.id AND E.id NULL  */ " .
            " AND E.id IS NULL AND LEBP.id IS NULL" . " AND TCV.active = 1 ";
     ...
     *
     */
    private function helperBuildSQLTestSuiteExecCounters($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        $union['exec'] = "/* {$debugMsg} sqlUnion Test suites - executions */" .
            ...
            " JOIN {$this->tables['tcversions']} TCV " .
            // " ON TCV.id = TPTCV.tcversion_id " .
            " ON  TCV.id = E.tcversion_id " . " WHERE TPTCV.testplan_id=" .
            $safe_id . $builds->whereAddExec;
        ...
        $union['not_run'] = "/* {$debugMsg} sqlUnion Test suites - not run */" .
            ...
            " COALESCE(E.status,'{$this->notRunStatusCode}') AS status, " .
            $moreExecFields . " TCV.execution_type AS exec_type," .
            " (TPTCV.urgency * TCV.importance) AS urg_imp " .
            " FROM {$this->tables['testplan_tcversions']} TPTCV " .
            $sqlStm['getAssignedFeatures'] .
            ...
            " AND LEBBP.platform_id = TPTCV.platform_id " .
            " AND LEBBP.tcversion_id = TPTCV.tcversion_id " .
            " AND LEBBP.build_id = BU.id " . " AND LEBBP.testplan_id = " .
            $safe_id . " /* Get execution status WRITTEN on DB */ " .
            " LEFT OUTER JOIN {$this->tables['executions']} E " .
            ...
            " JOIN {$this->tables['tcversions']} TCV " .
            " ON  TCV.id = TPTCV.tcversion_id " . " WHERE TPTCV.testplan_id=" .
            $safe_id . " AND BU.id IN ({$builds->inClause}) " .
     ...
     *
     */
    public function getExecutionsByStatus($id, $status, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        list ($my, $builds, $sqlStm) = $this->helperGetExecCounters($id,
        ...
        $sql = "/* {$debugMsg} executions with status WRITTEN on DB => not run is not present */" .
            ...
            " /* Get execution status WRITTEN on DB */ " .
            " JOIN {$this->tables['executions']} E " . " ON  E.id = LEBBP.id " .
            " AND E.build_id = LEBBP.build_id " . $builds->joinAdd .
            " /* Get Test Case info from Test Case Version */ " .
            " JOIN {$this->tables['nodes_hierarchy']} NHTCV " .
            " ON  NHTCV.id = TPTCV.tcversion_id " .
            " /* Get Test Suite info from Test Case  */ " .
            ...
            " JOIN {$this->tables['tcversions']} TCV " .
            " ON  TCV.id = TPTCV.tcversion_id " . " WHERE TPTCV.testplan_id=" .
            $safe_id . " AND E.status='{$status}' " . $builds->whereAddExec;
        ...
        switch ($my['opt']['output']) {
            ...
            case 'mapByExecID':
                $dummy = (array) $this->db->fetchRowsIntoMap($sql,
                    'executions_id');
                break;
     ...
     *
     */
    public function getNotRunWithTesterAssigned($id, $filters = null,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        $sql = "/* {$debugMsg} Not Run */" .
            ...
            " AND E.platform_id = TPTCV.platform_id " .
            " AND E.tcversion_id = TPTCV.tcversion_id " .
            " AND E.build_id = B.id " . " WHERE TPTCV.testplan_id=" . $safe_id .
            " AND E.id IS NULL " . " AND B.id IN ({$builds->inClause}) ";
     ...
     */
    public function getNotRunWOTesterAssigned($id, $buildSet = null,
        $filters = null, $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        list ($my, $builds,) = $this->helperGetExecCounters($id, $filters, $opt);
        ...
        $sqlc = "/* {$debugMsg} */ " .
            ...
            " ON  A_UA.feature_id = A_TPTCV.id " . " AND A_UA.build_id = A_B.id " .
            " AND A_UA.type = {$this->execTaskCode} " .
            " WHERE A_TPTCV.testplan_id = " . $safe_id .
            " AND A_E.status IS NULL " . " AND A_UA.user_id IS NULL ";
        ...
        $sql = "/* {$debugMsg} Not Run */" .
            ...
            " /* Get Test Case Version attributes */ " .
            " JOIN {$this->tables['tcversions']} TCV " .
            " ON  TCV.id = TPTCV.tcversion_id " . " JOIN ({$sqlc}) AS NR " .
     ...
     * @since 1.9.4
     */
    private function queryMetrics($id, $filters = null, $options = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        $my = array();
        list ($my, $sqlLEX) = $this->initQueryMetrics($safe['tplan_id'],
            $filters, $options);
     ...
     * @since 1.9.4
     */
    private function initQueryMetrics($tplanID, $filtersCfg, $optionsCfg)
    {
        $ic = array();
    ...
    /**
     */
    public function getExecStatusMatrixFlat($id, $filters = null, $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        if (! is_null($rs)) {
            ...
            for ($adx = 0; $adx < $loop2do; $adx ++) {
                if (! isset($cache['tsuite'][$rs[$adx]['tsuite_id']])) {
                    $stairway2heaven = $this->tree_manager->get_path(
                        $rs[$adx]['tsuite_id'], null, 'name');
                    $cache['tsuite'][$rs[$adx]['tsuite_id']] = implode("/",
                        $stairway2heaven);
                }
                ...
                if ($doSet) {
                    $ltx[$rs[$adx]['platform_id']][$rs[$adx]['tcase_id']] = array(
                        'id' => $rs[$adx]['executions_id'],
                        'build_id' => $rs[$adx]['build_id'],
    ...
    /**
     */
    private function helperStatusDomainMatrix(&$out, $rowKey, $colKey)
    {
        $totalByMatrix = array();
            ...
            foreach ($rowSet as $rowID) {
                ...
                foreach ($colSet as $colID) {
                    ...
                    foreach ($this->statusCode as $code) {
                        ...
                        $totalByMatrix[$rowID][$colID]['qty'] += $elem[$rowID][$colID][$code]['exec_qty'];
                    }
                }
            }
        }
    ...
    /**
     */
    public function getBuildByPlatStatusForRender($id,
        $totalKey = 'total_assigned')
    {
        ...
        if (! is_null($metrics)) {
            ...
            foreach ($mwt as $platID => $buildMetrics) {
                foreach ($buildMetrics as $buildID => $met) {
                    ...
                    $yo['build_name'] = $metrics['active_builds'][$buildID]['name'];
                    $yo[$totalKey] = $metrics['total'][$platID][$buildID]['qty'];
                    $yo['details'] = array();
                    ...
                    foreach ($codeSet as $cCode => $code4h) {
                        ...
                        if ($yo[$totalKey] > 0) {
                            $rf[$code4h]['percentage'] = number_format(
                                100 * ($rf[$code4h]['qty'] / $yo[$totalKey]), 1);
                        }
                    }
                }
            }
            ...
            foreach ($platList as $platID) {
                ...
                foreach ($itemList as $itemID) {
                    if (isset($renObj->info[$platID])) {
                        ...
                        if ($doPerc) {
                            $renObj->info[$platID][$itemID]['percentage_completed'] = number_format(
                                100 *
                                ($totalRun / $renObj->info[$platID][$itemID][$tk]),
                                1);
                        }
                    }
                }
    ...
    /**
     */
    public function getNeverRunByPlatform($tplanID, $platformSet = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        // we will need a second step to populate this info.
        //
        $sql = "/* {$debugMsg} Not Run */" . "SELECT COUNT(0) AS qty, " .
        ...
        if (null != $platformSet) {
            $sql .= " AND TPTCV.platform_id IN (" . implode(",", $platformSet) .
                ")";
        }
        $sql .= " GROUP BY tsuite_id, tcase_id, " .
            " TPTCV.platform_id,NHTC.name,platform_name, " . " full_external_id ";

        $buildSet = $this->get_builds($safeID, testplan::ACTIVE_BUILDS,
            testplan::OPEN_BUILDS);

        $sql .= " HAVING COUNT(0) > " . count($buildSet) - 1;
    ...
    /**
     */
    public function getLatestExecOnSinglePlatformMatrix($id, $platform_id,
        $filters = null, $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        $my['opt'] = array_merge($my['opt'], (array) $opt);
        list ($my, $builds, $sqlStm, $union) = $this->buildSQLTSLatestExecOneSinglePlatform(
            $id, $platform_id, $filters, $my['opt']);
    ...
    /**
     */
    private function getNeverRunOnTestPlanWithoutPlatforms($tplanID)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        $safeID = intval($tplanID);
    ...
    /**
     */
    public function getNeverRunOnSinglePlatform($tplanID, $platformID,
        $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        $safeID = intval($tplanID);
     ...
     * We also need to use E.tcversion_id and NOT TPTCV.tcversion_id.
     */
    private function buildSQLTSLatestExecOneSinglePlatform($id, $platform_id,
        $filters = null, $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
       ...
       SELECT NHTC.parent_id AS tsuite_id,NHTC.id AS tcase_id,
            ...
            " /* Get execution status WRITTEN on DB */
                JOIN {$this->tables['executions']} E
                ON  E.id = LEBP.id " . " /* getUserAssignment */ " .
            $sqlStm['getUserAssignment']['exec'] .
            " /* Get Test Case info from Test Case Version */ " .
            " JOIN {$this->tables['nodes_hierarchy']} NHTCV " .
            " ON  NHTCV.id = TPTCV.tcversion_id " .
            ...
            " WHERE TPTCV.testplan_id={$safe_id} " . " AND TPTCV.platform_id=" .
            intval($platform_id);
     ...
     */
    public function getStatusTotalsTSuiteDepth2ForRender($id, $filters = null,
        $opt = null)
    {
        list ($rx, $staircase) = $this->getStatusTotalsByItemForRender($id,
            'tsuite', $filters, $opt);
            ...
            foreach ($key2loop as $tsuite_id) {
                ...
                foreach ($plat2loop as $platId) {
                    ...
                    if ($initName) {
                        ...
                        $renderObj->info[$platId][$topSuiteID]['name'] = $tsNameCache[$topSuiteID];
                    }
                    ...
                    if ($l2id > 0 && $initNameL2) {
                        if (! isset($tsNameCache[$l2id])) {
                            ...
                            $tsNameCache[$l2id] = $this->tree_manager->getNameL2(
                                $l2id);
                        }
                        $renderObj->infoL2[$platId][$l2id]['name'] = $tsNameCache[$l2id];
                    ...
                    if (null !== $r2d2) {
                        foreach ($r2d2['details'] as $code => &$elem) {
                            ...
                            if ($l2id > 0) {
                                $renderObj->infoL2[$platId][$l2id]['details'][$code]['qty'] += $elem['qty'];
                                $renderObj->infoL2[$platId][$l2id]['total_tc'] += $elem['qty'];
                            }
            ...
            foreach ($renderObj->infoL2 as $platID => &$tsuiteMetrics) {
                foreach ($tsuiteMetrics as $tsuite_id => &$elem) {
                    if ($execQtyL2[$platID][$tsuite_id] > 0) {
                        $elem['percentage_completed'] = number_format(
                            100 *
                            ($execQtyL2[$platID][$tsuite_id] / $elem['total_tc']),
                            1);
                    }
                    if ($elem['total_tc'] > 0) {
                        foreach ($elem['details'] as &$yumyum) {
     ...
     */
    public function getExecTimelineStats($id, $filters = null, $opt = null)
    {
        $flt = array(
            'yyyy_mm_dd' => null
        );
        ...
        if ($options['workforce']) {
            ...
            switch ($options['timeline']) {
                case 'day_hour':
                    $rswf = $this->db->fetchMapRowsIntoMap($sqlwf, 'yyyy_mm_dd',
                        'hh');
                    break;
