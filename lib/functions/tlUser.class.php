...
class tlUser extends tlDBObject
{
     ...
     *
     * @param string $method2check
     *            must be one of the keys of configuration $tlCfg->authentication['domain']
     *
     ...
     *
     * @param integer $numAlpha
     *            number alphanumeric characters in generated password
     * @param integer $numNonAlpha
     *            number special characters in generated password
     *
     ...
     * @param mixed $options
     *            (optional) tlUser/tlObject options
     *
     ...
     * @param
     *            resource &$db reference to database handler
     * @param integer $testProjectID
     *            Identifier of the testproject to read the roles for,
        ...
        if (null != $allRoles && count($allRoles)) {
            ...
            foreach ($allRoles as $tprojectID => $roleID) {
                if (! isset($roleCache[$roleID])) {
                    $tprojectRole = tlRole::createObjectFromDB($db, $roleID,
                        "tlRole", true);
     ...
     * @param
     *            resource &$db reference to database handler
     * @param integer $testPlanID
     *            Identifier of the testplan to read the roles for, if null all roles are read
     *
     * @return integer returns tl::OK
     */
    public function readTestPlanRoles(&$db, $testPlanID = null)
    {
        ...
        if (null != $allRoles && count($allRoles)) {
            ...
            foreach ($allRoles as $tplanID => $roleID) {
                if (! isset($roleCache[$roleID])) {
                    $tplanRole = tlRole::createObjectFromDB($db, $roleID,
                        "tlRole", true);
    ...
    public function writeToDB(&$db)
    {
        ...
        if ($result >= tl::OK) {
            ...
            if ($this->dbID) {
                $gsql = " /* debugMsg */ SELECT password FROM {$this->object_table} WHERE id = " .
                    $this->dbID;
                ...
                $sql = "/* debugMsg */ UPDATE {$this->tables['users']} " .
                    ...
                    "'" . ", email = '" .
                    $db->prepare_string($this->emailAddress) . "'" .
                    ", locale = " . "'" . $db->prepare_string($this->locale) .
                    "'" . ", password = " . "'" .
                    $db->prepare_string($this->password) . "'" . ", role_id = " .
                    $db->prepare_int($this->globalRoleID) . ", active = " .
                    $db->prepare_string($this->isActive) . ", auth_method = " .
                ...
                $sql = "/* debugMsg */ INSERT INTO {$this->tables['users']} " .
                    ...
                    " VALUES ('" . $db->prepare_string($this->login) . "','" .
                    $db->prepare_string($this->password) . "','" .
                    $db->prepare_string($t_cookie_string) . "','" .
                    $db->prepare_string($this->firstName) . "','" .
                    $db->prepare_string($this->lastName) . "','" .
                    $db->prepare_string($this->emailAddress) . "'," .
                    $db->prepare_int($this->globalRoleID) . ",'" .
    ...
    {
        $sql = "DELETE FROM {$this->tables['user_testproject_roles']} WHERE user_id = " .
            intval($this->dbID);
     ...
     *
     * @param string $pwd
     *            the password to compate with the password actually set
    ...
    public function getEffectiveRole(&$db, $tproject_id, $tplan_id)
    {
        ...
        } elseif (! is_null($tprojects_role) &&
            isset($tprojects_role[$tproject_id])) {
     ...
     * @param integer $db
     *            DB Identifier
     * @param string $rightNick
     *            key corresponding with description in rights table
     * @param integer $testprojectID
     ...
     */
    public function getNamesForProjectRight(&$db, $rightNick,
        $testprojectID = null)
    {
     ...
     * @internal revisions
     */
    public function hasRight(&$db, $roleQuestion, $tprojectID = null,
        $tplanID = null, $getAccess = false)
    {
        ...
        if (isset($userTestProjectRoles[$testprojectID])) {
            ...
            if ($doMoreAnalysis) {
                ...
                $testProjectRights = array_diff($testProjectRights,
                    array_keys($g_propRights_global));
                propagateRights($globalRights, $g_propRights_global,
                    $testProjectRights);
     ...
     *
     */
    public function getAccessibleTestPlans(&$db, $testprojectID,
        $testplanID = null, $options = null)
    {
        ...
        if (isset($this->tprojectRoles[$testprojectID]->dbID)) {
            $userProjectRoleIsNoRights = ($this->tprojectRoles[$testprojectID]->dbID ==
                TL_ROLES_NO_RIGHTS);
        }
        ...
        $orderBy = null, $detailLevel = self::TLOBJ_O_GET_DETAIL_FULL)
    {
        ...
        return tlDBObject::createObjectsFromDBbySQL($db, $sql, 'id', __CLASS__,
            true, $detailLevel);
    }
    ...
    public function writePasswordToDB(&$db)
    {
        if ($this->dbID) {
            ...
            $gsql = " SELECT password FROM {$this->object_table} WHERE id = " .
                intval($this->dbID);
            $rs = $db->get_recordset($gsql);
            if (strcmp($rs[0]['password'], $this->password) != 0) {
                // Password HAS CHANGED
                $t_cookie_string = $this->auth_generate_unique_cookie_string(
                    $db);
            }

            $sql = "UPDATE {$this->tables['users']} " . " SET password = " . "'" .
                $db->prepare_string($this->password) . "'";
    ...
    public static function hasRoleOnTestProject(&$dbHandler, $id, $tprojectID)
    {
        ...
        $sql = " SELECT user_id FROM {$tables['user_testproject_roles']} " .
            ' WHERE testproject_id=' . intval($tprojectID) . ' AND user_id=' .
            intval($id);
    ...
    public static function hasRoleOnTestPlan(&$dbHandler, $id, $tplanID)
    {
        ...
        $sql = " SELECT user_id FROM {$tables['user_testplan_roles']} " .
            ' WHERE testplan_id=' . intval($tplanID) . ' AND user_id=' .
            intval($id);
        $rs = $dbHandler->fetchRowsIntoMap($sql, "user_id");
        return ! is_null($rs);
    }
     ...
     *
     */
    public function checkGUISecurityClearance(&$dbHandler, $context,
        $rightsToCheck, $checkMode)
    {
        $doExit = false;
        $action = 'any';
        $myContext = array(
        ...
        if ($doExit = (is_null($myContext) || $myContext['tproject_id'] == 0)) {
            logAuditEvent(
                TLS("audit_security_no_environment", $myContext['script']),
                $action, $this->dbID, "users");
        }

        if (! $doExit) {
            foreach ($rightsToCheck as $verboseRight) {
                $status = $this->hasRight($dbHandler, $verboseRight,
                    $myContext['tproject_id'], $myContext['tplan_id']);

                if (($doExit = ! $status) && ($checkMode == 'and')) {
                    $action = 'any';
                    logAuditEvent(
                        TLS("audit_security_user_right_missing", $this->login,
                            $myContext['script'], $action), $action, $this->dbID,
                        "users");
                    break;
                }
            }
    ...
    public static function setExpirationDate(&$dbHandler, $userID, $isoDate)
    {
        ...
        $sql = " UPDATE {$sch['users']} {$setClause} " . " WHERE id = " .
            intval($userID);
