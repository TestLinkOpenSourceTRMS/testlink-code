...
class tree extends tlObject
{
    ...
    public function get_available_node_types()
    {
        ...
        if (! $nodeTypes) {
            ...
            $nodeTypes = $this->db->fetchColumnsIntoMap($sql, 'description',
                'id');
        }
        return $nodeTypes;
    }
     ...
     */
    public function new_root_node($name = '')
    {
     ...
     *
     */
    public function new_node($parent_id, $node_type_id, $name = '',
        $node_order = 0, $node_id = 0)
    {
        ...
        if (is_array($node_id)) {
            ...
            if (! is_null($opt['nodeType'])) {
                $sql .= " AND node_type_id=" .
                    intval($this->node_descr_id[$opt['nodeType']]);
            }
     ...
     * args : node_id: root of subtree
     *
     * returns: array with nodes_i
     */
    private function _get_subtree_list($node_id, &$node_list,
        $node_type_id = null)
    {
     ...
     */
    public function delete_subtree($node_id)
    {
     ...
     * returns: array
     */
    private function _get_path($node_id, &$node_list, $to_node_id = null,
        $format = 'full')
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        while ($row = $this->db->fetch_array($result)) {
            ...
            if ($row['parent_id'] != '' && $row['id'] != $to_node_id) {
                ...
                $this->_get_path($row['parent_id'], $node_list, $to_node_id,
                    $format);
            }
        }
    }
     ...
     * node_table
     */
    public function get_children($id, $exclude_node_types = null, $opt = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        while ($row = $this->db->fetch_array($result)) {
            if (! isset(
                $exclude_node_types[$this->node_types[$row['node_type_id']]])) {
     ...
     * returns: -
     */
    public function change_order_bulk($nodes)
    {
     ...
     *
     */
    public function change_child_order($parent_id, $node_id, $top_bottom,
        $exclude_node_types = null)
    {
        ...
        $sql = " SELECT NH.id, NH.node_order, NH.name " .
            ...
            $node_type_filter . " ORDER BY NH.node_order,NH.id";
     ...
     * returns: order
     */
    public function getBottomOrder($parentID, $opt = null)
    {
        $my['opt'] = array_merge([
            'node_type' => null
        ], (array) $opt);
        ...
        $sql = "SELECT MAX(node_order) AS max_order" .
            " FROM {$this->object_table} " . " WHERE parent_id={$parentID} ";

        if (! is_null($my['opt']['node_type'])) {
            $sql .= " AND node_type_id = " .
                $this->node_descr_id[$my['opt']['node_type']];
        }
    ...
    public function get_subtree($node_id, $filters = null, $options = null)
    {
        ...
        if (! is_null($my['filters']['exclude_node_types'])) {
            ...
            $my['filters']['additionalWhereClause'] .= " AND node_type_id NOT IN (" .
                implode(",", $exclude) . ")";
        }

        $method2call = $my['options']['recursive'] ? 'getSubtreeRec' : '_get_subtree';
        $this->$method2call($node_id, $the_subtree, $my['filters'],
            $my['options']);
        return $the_subtree;
    }
    ...
    /**
     */
    private function _get_subtree($node_id, &$node_list, $filters = null,
        $options = null)
    {
        static $my;
        if (! $my) {
        ...
        switch ($my['options']['order_cfg']['type']) {
            ...
            case 'rspec':
                $sql = " SELECT OBT.id,name,parent_id,node_type_id,node_order," .
                    " RSPEC.doc_id " . " FROM {$this->object_table} AS OBT " .
                    " JOIN {$this->tables['req_specs']} AS RSPEC " .
                    " ON RSPEC.id = OBT.id " . " WHERE parent_id = {$node_id} " .
                    " {$my['filters']['additionalWhereClause']}" .
        ...
        while ($row = $this->db->fetch_array($result)) {
            if (! isset($my['filters']['exclude_branches'][$row['id']])) {
                ...
                if (! isset(
                    $my['filters']['exclude_children_of'][$this->node_types[$row['node_type_id']]]) &&
                    ! isset($my['filters']['exclude_branches'][$row['id']])) {
                    $this->_get_subtree($row['id'], $node_list, $filters,
                        $options);
                }
            }
        }
    ...
    /**
     */
    public function getSubtreeRec($node_id, &$pnode, $filters = null,
        $options = null)
    {
        static $tcNodeTypeID;
        static $qnum;
        ...
        if (! $tcNodeTypeID) {
            ...
            if (isset($my['options']['order_cfg']['platform_id']) &&
                ($safe_pid = intval($my['options']['order_cfg']['platform_id'])) >
                0) {
            ...
            if (! is_null($my['options']['remove_empty_nodes_of_type']) &&
                ...
                $my['options']['remove_empty_nodes_of_type'] = $this->node_descr_id[$my['options']['remove_empty_nodes_of_type']];
            }
        ...
        while ($row = $this->db->fetch_array($result)) {
            if (! isset($exclude_branches[$row['id']])) {
                ...
                // in a null result set.
                if (! isset(
                    $exclude_children_of[$this->node_types[$row['node_type_id']]]) &&
     ...
     * @internal revisions
     */
    public function get_full_path_verbose(&$items, $options = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method:' . __FUNCTION__ . ' :: ';
        $goto_root = null;
        $path_to = null;
        ...
        if (count((array) $items) > 200) {
            ...
            foreach ((array) $items as $item_id) {
                $stairway2heaven[$item_id] = $this->get_path($item_id,
                    $goto_root, $path_format);
                $path_to[$item_id]['name'] = $stairway2heaven[$item_id];
                $all_nodes = array_merge($all_nodes,
                    (array) $path_to[$item_id]['name']);
            }
        }
     ...
     * @param
     *            int node_type_id: node types to check.
     * @param
     *            int id: optional. exclude this node id from result set
     ...
     *
     * @param
     *            int parent_id: optional. Mandatory if id is null
     *            Used to get children nodes to check for
     *            name existence.
     *
     * @return array ret: ret['status']=1 if name exists
     *         0 if name does not exist
     *         ret['msg']= localized message
     *
     */
    public function nodeNameExists($name, $node_type_id, $id = null,
        $parent_id = null)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method:' . __FUNCTION__ . ' :: ';
        $ret['status'] = 0;
        ...
        if (is_null($id) && is_null($parent_id)) {
            $msg = $debugMsg .
                'Error on call $id and $parent_id can not be both null';
            throw new Exception($msg);
        }
        ...
        $sql = "/* {$debugMsg} */ " .
            ...
            " WHERE NHA.node_type_id  = {$node_type_id} " . " AND NHA.name = '" .
            $this->db->prepare_string($name) . "'" . " AND NHA.parent_id = " .
            $this->db->prepare_int($parentNodeID) . " {$additionalFilters} ";
     ...
     * getTreeRoot()
     */
    public function getTreeRoot($node_id)
    {
        $path = (array) $this->get_path($node_id);
        $path_len = count($path);
        return ($path_len > 0) ? $path[0]['parent_id'] : $node_id;
    }
     ...
     */
    public function delete_subtree_objects($root_id, $node_id,
        $additionalWhereClause = '', $exclude_children_of = null,
        $exclude_branches = null)
    {
        static $debugMsg;
        if (is_null($debugMsg)) {
        ...
        foreach ($array2map as $elem) {
            ...
            switch ($mode) {
                case 'dotted':
                    ...
                    if ($addField) {
                        $hmap[$current['id']] .= sprintf($field2add['format'],
                            $current[$field2add['field']]);
                    }
    ...
    /**
     */
    public function getNodeByAttributes($attr)
    {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        foreach ($attr as $key => $value) {
            switch ($key) {
                case 'type':
                    $where .= " AND NT.description = '" .
                        $this->db->prepare_string($value) . "'";
                    break;

                case 'id':
                    $where .= " AND NH_MAIN.is = " .
                        $this->db->prepare_int($value);
                    break;

                case 'name':
                    $where .= " AND NH_MAIN.name = '" .
                        $this->db->prepare_string($value) . "'";
                    break;

                case 'parent_id':
                    $where .= " AND NH_MAIN.parent_id = " .
                        $this->db->prepare_int($value);
    ...
    /**
     */
    private function getNodeType($id)
    {
