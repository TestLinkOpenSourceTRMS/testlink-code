    ...
    $filters = null, $options = null)
{
    ...
    if ($my['options']['viewType'] == 'testSpecTree') {
        ...
        if ($filters['filter_keywords'] != null &&
            ...
            $usedKeywordsByKeyID = $tproject_mgr->getUsedKeywordsMap(
                $tproject_id);
    ...
    // $exclude_branches = isset($filters['filter_toplevel_testsuite']) && is_array($filters['filter_toplevel_testsuite']) ? $filters['filter_toplevel_testsuite'] : null;
    ...
    if ($test_spec) {
        ...
        if (! is_null($my['filters']['filter_keywords'])) {
            ...
            // if
            $attr_map['keywords'] = $tproject_mgr->getKeywordsLatestTCV(
                $tproject_id, $my['filters']['filter_keywords'],
        ...
        if (! is_null($my['filters']['filter_platforms'])) {
            $attr_map['platforms'] = $tproject_mgr->getPlatformsLatestTCV(
                $tproject_id, $my['filters']['filter_platforms']);
        ...
        $pnOptions = array(
            'hideTestCases' => $my['options']['hideTestCases'],
        ...
        $testcase_counters = prepareNode($db, $test_spec, $map_node_tccount,
            $attr_map, $tplan_tcs, $pnFilters, $pnOptions);

        foreach ($testcase_counters as $key => $value) {
            $test_spec[$key] = $testcase_counters[$key];
        }

        $tc2show = renderTreeNode(1, $test_spec, $hash_id_descr, $linkto,
    ...
    if (isset($test_spec['childNodes'])) {
        $menustring = str_ireplace('childNodes', 'children',
            json_encode($test_spec['childNodes']));
    }
    ...
    &$tplan_tcases = null, $filters = null, $options = null)
{
    ...
    if (! $tables) {
        ...
        $enabledFiltersOn['importance'] = isset(
            $my['filters']['filter_priority']);
        $enabledFiltersOn['testcase_id'] = isset($my['filters']['filter_tc_id']);

        $enabledFiltersOn['testcase_name'] = isset(
            $my['filters']['filter_testcase_name']);
        $enabledFiltersOn['executionType'] = isset(
            $my['filters']['filter_execution_type']);
        $enabledFiltersOn['custom_fields'] = isset(
            $my['filters']['filter_custom_fields']);

        $enabledFiltersOn['keywords'] = (null != $attr_map &&
            isset($attr_map['keywords']) && null != $attr_map['keywords'] &&
            count($attr_map['keywords']) > 0);

        $enabledFiltersOn['platforms'] = (null != $attr_map &&
            isset($attr_map['platforms']) && null != $attr_map['platforms'] &&
            count($attr_map['platforms']) > 0);

        $filterOnTCVersionAttribute = $enabledFiltersOn['executionType'] ||
            $enabledFiltersOn['importance'];
    ...
    $node_type = isset($node['node_type_id']) ? $nodesCodeType[$node['node_type_id']] : null;
            ...
            if ($my['options']['viewType'] == 'executionTree') {
                ...
                if (! ($delete_node = is_null($tpNode))) {
                    ...
                    if (! $delete_node && ! is_null($users2filter)) {
                        $somebody_wanted_but_nobody_there = isset(
                            $users2filter[TL_USER_SOMEBODY]) &&
                            ! is_numeric($tpNode['user_id']);

                        $unassigned_wanted_but_someone_assigned = isset(
                            $users2filter[TL_USER_NOBODY]) &&
                        ...
                        $delete_node = $unassigned_wanted_but_someone_assigned ||
                            $wrong_user || $somebody_wanted_but_nobody_there;
                    }
                }
 ...
 * @internal revisions
 */
function renderTreeNode($level, &$node, $hash_id_descr, $linkto, $testCasePrefix,
    $opt)
{
    static $f2call;
    ...
    $node['testlink_node_name'] = htmlspecialchars($node['testlink_node_name'],
        ENT_QUOTES);
        ...
        case 'testsuite':
            ...
            if (isset($opt['nodeHelpText'][$node['testlink_node_type']])) {
                $node['text'] = '<span title="' .
                    $opt['nodeHelpText'][$node['testlink_node_type']] . '">' .
                    $node['text'] . '</span>';
            }
            break;
    ...
    if (isset($node['childNodes']) && $node['childNodes']) {
        ...
        for ($idx = 0; $idx < $nChildren; $idx ++) {
            ...
            $testCasesIDList .= renderTreeNode($level + 1,
                $node['childNodes'][$idx], $hash_id_descr, $linkto,
                $testCasePrefix, $opt);
        }
    }
 ...
 * @param integer $level
 * @param
 *            array &$node reference to recursive map
 * @param
 *            array &$tcases_node reference to map that contains info about testcase exec status
 *            when node is of testcase type.
 * @param array $hash_id_descr
 * @param string $linkto
 * @param string $testCasePrefix
 * @param array $opt
    ...
    $linkto, $testCasePrefix, $opt)
{
    ...
    if (! $resultsCfg) {
        ...
        foreach ($resultsCfg['status_label'] as $key => $value) {
            ...
            $cssClasses[$status_descr_code[$key]] = $doColouringOn['testcase'] ? ('class="light_' .
                $key . '"') : '';
        }
    ...
    switch ($node_type) {
        ...
        case 'testsuite':
            ...
            if ($opt['useCounters']) {
                $node['text'] .= create_counters_info($node,
                    $doColouringOn['counters']);
            }

            if (isset($opt['nodeHelpText'][$node_type])) {
                $node['text'] = '<span title="' .
                    $opt['nodeHelpText'][$node_type] . '">' . $node['text'] .
                    '</span>';
            }
        ...
        case 'testcase':
            ...
            $node['text'] = "<span ";
            if (isset($tcase_node[$node['id']]) && $opt['showTestCaseExecStatus']) {
                $status_code = $tcase_node[$node['id']]['exec_status'];
                $node['text'] .= "{$cssClasses[$status_code]} " . '  title="' .
                    $l18n[$status_code] . '" alt="' . $l18n[$status_code] . '">';
            }

            if ($opt['showTestCaseID']) {
                // optimizable
                $node['text'] .= "<b>" .
                    htmlspecialchars($testCasePrefix . $node['external_id']) .
                    "</b>:";
            }
            $node['text'] .= "{$name}</span>";
            break;
 ...
 *
 * @param
 *            resource &$db reference to DB handler object
 * @param
 *            array &$tcase_tree reference to test case set/tree to filter
 * @param
 *            array &$cf_hash reference to selected custom field information
 * @param int $node_types
 *            IDs of node types
 *
    ...
    foreach ($tcase_tree as $key => $node) {
        ...
        if ($node['node_type_id'] == $node_types['testsuite']) {
            ...
            if (isset($node['childNodes']) && is_array($node['childNodes'])) {
                ...
                $tcase_tree[$key]['childNodes'] = filter_by_cf_values($db,
                    $tcase_tree[$key]['childNodes'], $cf_hash, $node_types);
        ...
        } elseif ($node['node_type_id'] == $node_types['testcase']) {
 ...
 *
 * @param
 *            object &$tplan_mgr reference to test plan manager object
 * @param
 *            array &$tcase_set reference to test case set to filter
 * @param integer $tplan_id
 *            ID of test plan
 * @param array $filters
 *            filters to apply to test case set
 * @return array new tcase_set
 */
function filterStatusSetAtLeastOneOfActiveBuilds(&$tplan_mgr, &$tcase_set,
    $tplan_id, $filters)
{
    $safe_platform = intval($filters->setting_platform);
    $buildSet = array_keys(
        $tplan_mgr->get_builds($tplan_id, testplan::ACTIVE_BUILDS));
    if (! is_null($buildSet)) {
        if ($safe_platform > 0) {
            tLog(
                basename(__FILE__) . __FUNCTION__ .
                ':: $tplan_mgr->getHitsSameStatusPartialOnPlatform', 'DEBUG');
            $hits = $tplan_mgr->getHitsSameStatusPartialOnPlatform($tplan_id,
                $safe_platform, (array) $filters->filter_result_result);
        } else {
            tLog(
                basename(__FILE__) . __FUNCTION__ .
                ':: $tplan_mgr->getHitsSameStatusPartialALOP', 'DEBUG');
            $hits = $tplan_mgr->getHitsSameStatusPartialALOP($tplan_id,
 ...
 * @return array new tcase_set
 */
function filterStatusSetAllActiveBuilds(&$tplan_mgr, &$tcase_set, $tplan_id,
    $filters)
{
    $buildSet = array_keys(
        $tplan_mgr->get_builds($tplan_id, testplan::ACTIVE_BUILDS));
    if (! is_null($buildSet)) {
        ...
        if ($safe_platform > 0) {
            tLog(
                basename(__FILE__) . __FUNCTION__ .
                ':: $tplan_mgr->getHitsSameStatusFullOnPlatform', 'DEBUG');
            $hits = $tplan_mgr->getHitsSameStatusFullOnPlatform($tplan_id,
                $safe_platform, (array) $filters->filter_result_result,
                $buildSet);
        } else {
            tLog(
                basename(__FILE__) . __FUNCTION__ .
                ':: $tplan_mgr->getHitsSameStatusFullALOP', 'DEBUG');
 ...
 * @return array new tcase_set
 */
function filter_by_status_for_build(&$tplan_mgr, &$tcase_set, $tplan_id,
    $filters)
{
    $safe_platform = intval($filters->setting_platform);
    $safe_build = intval($filters->filter_result_build);
    if ($safe_platform > 0) {
        tLog(__FUNCTION__ . ':: $tplan_mgr->getHitsStatusSetOnBuildPlatform',
            'DEBUG');
        $hits = $tplan_mgr->getHitsStatusSetOnBuildPlatform($tplan_id,
            $safe_platform, $safe_build, (array) $filters->filter_result_result);
    } else {
        tLog(__FUNCTION__ . ':: $tplan_mgr->getHitsStatusSetOnBuildALOP',
            'DEBUG');
 ...
 *
 * @param
 *            object &$db reference to database handler
 * @param
 *            object &$tplan_mgr reference to test plan manager object
 * @param
 *            array &$tcase_set reference to test case set to filter
 ...
 * @return array new tcase_set
 */
function filter_by_status_for_latest_execution(&$tplan_mgr, &$tcase_set,
    $tplan_id, $filters)
{
    ...
    if ($safe_platform > 0) {
        $hits = $tplan_mgr->getHitsStatusSetOnLatestExecOnPlatform($safe_tplan,
            $safe_platform, (array) $filters->filter_result_result);
    } else {
        $hits = $tplan_mgr->getHitsStatusSetOnLatestExecALOP($safe_tplan,
            (array) $filters->filter_result_result);
    }
 ...
 * @return array new tcase_set
 */
function filter_not_run_for_any_build(&$tplan_mgr, &$tcase_set, $tplan_id,
    $filters)
{
    $safe_platform = intval($filters->setting_platform);
    if ($safe_platform > 0) {
        $hits = $tplan_mgr->getHitsNotRunPartialOnPlatform($tplan_id,
            intval($filters->setting_platform));
    } else {
 ...
 * @author Andreas Simon
 * @param Database $db
 *            reference to database handler object
 * @param testproject $testproject_mgr
 *            reference to testproject manager object
 * @param int $testproject_id
 *            ID of the project for which the tree shall be generated
 * @param string $testproject_name
 *            Name of the test project
 * @param array $filters
 *            Filter settings which shall be applied to the tree, possible values are:
 *            'filter_doc_id',
 *            'filter_title',
 *            'filter_status',
 *            'filter_type',
 ...
 * @param array $options
 *            Further options which shall be applied on generating the tree
 * @return stdClass $treeMenu object with which ExtJS can generate the graphical tree
 */
function generate_reqspec_tree(&$db, &$testproject_mgr, $testproject_id,
    $testproject_name, $filters = null, $options = null)
{
    ...
    $req_spec = $tree_manager->get_subtree($testproject_id, $my['filters'],
        $my['options']);
    ...
    $level = 1;
    $req_spec = prepare_reqspec_treenode($db, $level, $req_spec, $filtered_map,
        $map_id_nodetype, $map_nodetype_id, $my['filters'], $my['options']);
 ...
 * @return stdClass $treeMenu object with which ExtJS can generate the graphical tree
 */
function generateTestReqCoverageTree(&$db, $tproject_id, $tproject_name,
    $filters = null, $options = null)
{
    $tproject_mgr = new testproject($db);
    $tree_manager = &$tproject_mgr->tree_manager;
    ...
    $req_spec = $tree_manager->get_subtree($tproject_id, $my['filters'],
        $my['options']);
    ...
    $level = 1;
    $req_spec = prepare_reqspeccoverage_treenode($db, $level, $req_spec,
        $filtered_map, $map_id_nodetype, $map_nodetype_id, $my['filters'],
        $my['options']);
 ...
 *            reference to testproject manager object
 * @param array $filters
 *            Filter settings which shall be applied to the tree
 * @param array $options
 *            Further options which shall be applied on generating the tree
...
function get_filtered_req_map(&$db, $testproject_id, &$testproject_mgr, $filters)
{
    ...
    if (isset($filters['filter_tc_id'])) {
        ...
        $tc_ext_id = $db->prepare_int(
            str_replace($tc_prefix, '', $filters['filter_tc_id']));
 ...
 *            reference to database handler object
 * @param int $level
 *            gets increased by one for each sublevel in recursion
 * @param array $node
 *            the tree structure to traverse
 * @param array $filtered_map
 *            a map of filtered requirements, req that are not in this map will be deleted
 * @param array $map_id_nodetype
 *            array with node type IDs as keys, node type descriptions as values
 * @param array $map_nodetype_id
 *            array with node type descriptions as keys, node type IDs as values
 * @param array $filters
 * @param array $options
 * @return array tree structure after filtering out unneeded nodes
 */
function prepare_reqspec_treenode(&$db, $level, &$node, &$filtered_map,
    &$map_id_nodetype, &$map_nodetype_id, &$filters, &$options)
{
    $child_req_count = 0;
    if (isset($node['childNodes']) && is_array($node['childNodes'])) {
    ...
    switch ($node_type) {
        ...
        case 'requirement':
            ...
            $delete_node = (is_null($filtered_map) ||
                ! array_key_exists($node['id'], $filtered_map));
            break;
    }
    ...
    if ($delete_node) {
        ...
        $node = render_reqspec_treenode($db, $node, $filtered_map,
            $map_id_nodetype);
    }
 ...
 * @return array tree structure after filtering out unneeded nodes
 */
function prepare_reqspeccoverage_treenode(&$db, $level, &$node, &$filtered_map,
    &$map_id_nodetype, &$map_nodetype_id, &$filters, &$options)
{
    $child_req_count = 0;
    ...
    if ($delete_node) {
        ...
        $node = render_reqspeccoverage_treenode($db, $node, $filtered_map,
            $map_id_nodetype);
    }
...
function render_reqspec_treenode(&$db, &$node, &$filtered_map, &$map_id_nodetype)
{
    ...
    if (! $js_functions) {
        $js_functions = array(
            'testproject' => 'TPROJECT_REQ_SPEC_MGMT',
            ...
            foreach ($node['childNodes'] as $child) {
                if (is_array($child)) {
                    ...
                    if (isset($filtered_map[$child_id])) {
                        $doc_id = htmlspecialchars(
                            $filtered_map[$child_id]['req_spec_doc_id']);
                    }
 ...
 * @return array tree object with all needed data for ExtJS tree
 */
function render_reqspeccoverage_treenode(&$db, &$node, &$filtered_map,
    &$map_id_nodetype)
{
    static $js_functions;
    static $forbidden_parents;
...
function update_status_for_colors(&$dbHandler, &$items, $context, $statusCfg)
{
    ...
    $sql = " SELECT E.status, NH_TCV.parent_id AS tcase_id " .
        ...
        " JOIN " . " ( SELECT MAX(E2.id) AS last_exec_id " .
        "   FROM {$tables['executions']} E2 " .
        "   WHERE testplan_id = {$tplanID} " . "   AND tcversion_id IN (" .
        implode(',', $keySet) . ") " .
    ...
    if (! is_null($rs)) {
        foreach ($key2scan as $tcase_id) {
            $rr = isset($rs[$tcase_id]['status']) &&
                ! is_null($rs[$tcase_id]['status']) ? $rs[$tcase_id]['status'] : $statusCfg['not_run'];
 ...
 */
function generateTestSpecTreeNew(&$db, $tproject_id, $tproject_name, $linkto,
    $filters = null, $options = null)
{
    $chronos[] = microtime(true);
    ...
    if ($test_spec) {
        ...
        $pnFilters = array(
            'keywords' => $my['filters']['filter_keywords'],
        ...
        $testcase_counters = prepareTestSpecNode($db, $tproject_mgr,
            $tproject_id, $test_spec, $map_node_tccount, $pnFilters, $pnOptions);
...
{
    $flt = array();
    $flt['exclude_branches'] = isset($fObj['filter_toplevel_testsuite']) &&
        is_array($fObj['filter_toplevel_testsuite']) ? $fObj['filter_toplevel_testsuite'] : null;
    ...
    if (isset($fObj['filter_execution_type']) &&
        ! is_null($fObj['filter_execution_type'])) {
        $flt['execution_type'] = intval($fObj['filter_execution_type']);
    }

    if (isset($fObj['filter_importance']) &&
        ! is_null($fObj['filter_importance'])) {
    ...
    if (isset($fObj['filter_workflow_status']) &&
        ! is_null($fObj['filter_workflow_status'])) {
    ...
    $piece = 'platforms';
    $full = 'filter_' . $piece;
    if (isset($fObj[$full]) && ! is_null($fObj[$full])) {
        $xx = (array) $fObj[$full];
        if ($xx[0] > 0) {
            $flt[$piece] = $xx;
        }
...
/**
 */
function prepareTestSpecNode(&$db, &$tprojectMgr, $tprojectID, &$node,
    &$map_node_tccount, $filters = null, $options = null)
{
    ...
    if (! $tables) {
        ...
        $decoding_info = array(
            'node_id_descr' => array_flip(
                $tprojectMgr->tree_manager->get_available_node_types())
        );
        $my = array();
        $my['options'] = array(
            'hideTestCases' => 0
        ...
        if ($doFilterOn['keywords']) {
            $tcFilterByKeywords = $tprojectMgr->getTCLatestVersionFilteredByKeywords(
                $tprojectID, $my['filters']['keywords'],
        ...
        if ($doFilterOn['platforms']) {
            $tcFilterByPlatforms = $tprojectMgr->getTCLatestVersionFilteredByPlatforms(
        ...
        $filtersApplied = $doFilterOn['keywords'] ||
            ...
            $my['options']['ignoreActiveTestCases'] || $doFilterOn['platforms'];
    }
    ...
    if ($node_type == 'testcase') {
        ...
        } elseif ($my['options']['ignoreActiveTestCases']) {
            $sql = " SELECT COUNT(TCV.id) AS count_active_versions " .
                " FROM {$tables['tcversions']} TCV, {$tables['nodes_hierarchy']} NH " .
                " WHERE NH.parent_id=" . $node['id'] .
                " AND NH.id = TCV.id AND TCV.active=1";
    ...
    if (! is_null($node) && isset($node['childNodes']) &&
        is_array($node['childNodes'])) {
        ...
        for ($idx = 0; $idx < $childNodesQty; $idx ++) {
            ...
            $counters_map = prepareTestSpecNode($db, $tprojectMgr, $tprojectID,
                $current, $map_node_tccount);
        ...
        // node must be destroyed if empty had we have using filtering conditions
        if ($filtersApplied && ! $tcase_counters['testcase_count'] &&
