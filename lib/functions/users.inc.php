 ...
 *
 * @param
 *            resource &$db reference to DB identifier
 * @param string $user
 * @param integer $id
 ...
 *
 */
function setUserSession(&$db, $user, $id, $roleID, $email, $locale = null)
{
    tLog(
        'setUserSession: $user=' . $user . ' $id=' . $id . ' $roleID=' . $roleID .
    ...
    if (isset($_COOKIE[$tproject_cookie]) &&
        isset($arrProducts[$_COOKIE[$tproject_cookie]]) &&
 ...
 * rev :
 */
function getUsersForHtmlOptions(&$db, $whereClause = null,
    $additional_users = null, $active_filter = null, $users = null, $opt = null)
{
    if (! $users) {
        ...
        $users = tlUser::getAll($db, $sqlWhere, "id", null,
 ...
 *            default 'send_password_by_mail'
 *
 * @return array status: integer result status code
    ...
    $passwordSendMethod = 'send_password_by_mail')
{
            ...
            if ($retval['status'] >= tl::OK) {
                ...
                if ($passwordSendMethod == 'send_password_by_mail') {
                    ...
                    $mail_op = @email_send(config_get('from_email'),
                        $user->emailAddress, lang_get('mail_passwd_subject'),
                        $msgBody);
                }

                if ($mail_op->status_ok ||
                    ($passwordSendMethod == 'display_on_screen')) {
    ...
    $retval['msg'] = ($retval['msg'] != "") ? $retval['msg'] : getUserErrorMessage(
        $retval['status']);
    return $retval;
}
...
function getAllUsersRoles(&$db, $order_by = null)
{
    ...
    $users = tlDBObject::createObjectsFromDBbySQL($db, $sql, "id", "tlUser",
        false, tlUser::TLOBJ_O_GET_DETAIL_MINIMUM);
 ...
 *            test project id
 * @param array $users
 * @param array $additional_testers
 *            TBD
 ...
 *
 */
function getGrantsForUserMgmt(&$dbHandler, &$userObj, $tprojectID = null,
    $tplanID = null)
{
    ...
    if ($grants->user_mgmt == 'yes') {
        ...
        $grants->tplan_user_role_assignment = $userObj->hasRight($dbHandler,
            "testplan_user_role_assignment", $tprojectID, $tplanID);

        $answers->user_role_assignment = $userObj->hasRight($dbHandler,
            "user_role_assignment", null, - 1);
        $answers->testproject_user_role_assignment = $userObj->hasRight(
            $dbHandler, "testproject_user_role_assignment", $tprojectID, - 1);
        if ($answers->user_role_assignment == "yes" ||
            $answers->testproject_user_role_assignment == "yes") {
            $grants->tproject_user_role_assignment = "yes";
        }
    }
