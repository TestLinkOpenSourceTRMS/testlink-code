...
if (in_array($showFeature,
    ...
    if (isset($_SESSION['testplanID']) || ! is_null($args->tplan_id)) {
        ...
        if (! is_null($args->tplan_id)) {
            ...
            $dummy = $tplanMgr->tree_manager->get_node_hierarchy_info(
                $tplanIDCard->id);
...
function validateBuildAvailability(&$db, &$tplanMgr, $context, $attrFilter)
{
    ...
    if (! $tplanMgr->getNumberOfBuilds($tpID, $attrFilter['active'],
        $attrFilter['open'])) {
        $msx = [];
        ...
        $message = "<p>" . $context->featureTitle . "<p>" .
            sprintf(lang_get('no_good_build'), $mzx) . "<b> " .
