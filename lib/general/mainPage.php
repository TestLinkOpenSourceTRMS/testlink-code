...
$testprojectID = isset($_SESSION['testprojectID']) ? intval(
...
if ($testplanID) {
    ...
    if (null != $rd) {
        $gui->testplanRole = $tlCfg->gui->role_separator_open . $rd .
...
if ($currentUser->hasRight($db, "testproject_user_role_assignment",
    $testprojectID, - 1) == "yes" ||
...
{
    $target_dir = '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR .
        'docs';
    $documents = null;
...
function getGrants($dbHandler, $user, $tproject_id, $forceToNo = false)
{
    ...
    $grants['project_edit'] = $user->hasRight($dbHandler,
        $right2check['project_edit'], $tproject_id);
