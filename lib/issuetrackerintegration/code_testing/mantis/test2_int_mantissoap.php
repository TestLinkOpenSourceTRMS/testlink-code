...
define('BUG_TRACK_SOAP_HREF',
    BUG_TRACK_HREF . "bugs/api/soap/mantisconnect.php?wsdl");
...
if ($op) {
    ...
    foreach ($issue2check as $elem) {
        $issue = $elem['issue'];
        $msg = $elem['exists'] ? "<br>Ask info about EXISTENT ISSUE:{$issue}<br>" : "<br>Ask info about <b>INEXISTENT ISSUE:{$issue}</b><br>";
