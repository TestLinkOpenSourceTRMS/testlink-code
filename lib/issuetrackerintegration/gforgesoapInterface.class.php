...
class gforgesoapInterface extends issueTrackerInterface
{
    ...
    {
        try {
            $issue = $this->APIClient->getTrackerItemFull($this->authToken,
                $issueID);
            new dBug($issue);
            ...
            $extraFields = $this->APIClient->getTrackerExtraFieldArray(
                $this->authToken, $issue->tracker_id, $target);
            ...
            $zz = $this->APIClient->getTrackerExtraField($this->authToken,
                $issue->tracker_id, $statusObj->tracker_extra_field_id);
            ...
            $yy = $this->APIClient->getTrackerExtraFieldDatas($this->authToken,
                8305, 55108);
            echo __LINE__;
            new dBug($yy);
    ...
    public function connect()
    {
        ...
        if ($this->connected = $op['connected']) {
            ...
            try {
                ...
                $this->authToken = $this->APIClient->login(
                    (string) $this->cfg->username, (string) $this->cfg->password);
    ...
    /**
     */
    private function buildSummaryHTMLString($issue)
    {
