...
abstract class issueTrackerInterface
{
    ...
    public function buildViewBugLink($issueID, $opt = null)
    {
        ...
        $link = "<a href='" . $this->buildViewBugURL($issueID) .
            "' target='_blank'>";
        ...
        if ($useIconv) {
            $link .= iconv((string) $this->cfg->dbcharset, $this->tlCharSet,
                $issue->IDHTMLString);
        } else {
        ...
        if ($my['opt']['addSummary'] && ! is_null($issue->summaryHTMLString)) {
            $link .= " : ";
            if ($useIconv) {
                $link .= iconv((string) $this->cfg->dbcharset, $this->tlCharSet,
        ...
        if ($my['opt']['addReporter'] && property_exists($issue, 'reportedBy')) {
            ...
            if ('' != $who) {
                ...
                if ($useIconv) {
                    $link .= iconv((string) $this->cfg->dbcharset,
                        $this->tlCharSet, $who);
                } else {
                    $link .= $who;
                }
     ...
     *
     * @param
     *            issueID according to BTS can be number or string
     *
     * @return
     */
    ...
    public function setResolvedStatusCfg()
    {
        ...
        $this->resolvedStatus->byName = array_flip(
            $this->resolvedStatus->byCode);
    }
     ...
     *
     * @param
     *            issueID (can be number of string according to specific BTS)
     *
