...
class jirarestInterface extends issueTrackerInterface
{
    ...
    public function connect()
    {
        try {
            ...
            if (! is_null($this->jiraCfg['proxy']) &&
                is_null($this->jiraCfg['proxy']->host)) {
    ...
    public function getIssue($issueID)
    {
        ...
        try {
            ...
            if (! is_null($issue) && is_object($issue) &&
                ! property_exists($issue, 'errorMessages')) {
    ...
    public function addIssue($summary, $description, $opt = null)
    {
        try {
            ...
            if (! is_null($this->issueAttr)) {
                $issue['fields'] = array_merge($issue['fields'],
                    $this->issueAttr);
            }
    ...
    public function getVersions()
    {
        $items = null;
        try {
            $items = $this->APIClient->getVersions(
                (string) $this->cfg->projectkey);
        } catch (Exception $e) {
            tLog(__METHOD__ . "  " . $e->getMessage(), 'ERROR');
        }
        return $items;
    ...
    {
        try {
            return $this->APIClient->getComponents(
                (string) $this->cfg->projectkey);
        } catch (Exception $e) {
            tLog(__METHOD__ . "  " . $e->getMessage(), 'ERROR');
        }
    ...
    {
        try {
            return $this->APIClient->getCreateIssueFields(
                (string) $this->cfg->projectkey);
        } catch (Exception $e) {
            tLog(__METHOD__ . "  " . $e->getMessage(), 'ERROR');
        }
