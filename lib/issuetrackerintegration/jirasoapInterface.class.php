...
class jirasoapInterface extends issueTrackerInterface
{
    ...
    public function addIssue($summary, $description)
    {
        try {
            $issue = array(
                'project' => (string) $this->cfg->projectkey,
    ...
    /**
     */
    private function getRelaxedAttribute($name, $elem)
    {
        if (is_object($elem)) {
    ...
    /**
     */
    private function getCustomFieldsAttribute($name, $objCFSet)
    {
        ...
        foreach ($elem as $item) {
            ...
            $this->issueAttr[$name][] = array(
                'customfieldId' => trim((string) $item->customfieldId),
     ...
     * </affectsVersions>
     */
    private function getAffectsVersionsAttribute($name, $objItemSet)
    {
