...
class kaitenrestInterface extends issueTrackerInterface
{
    ...
    public function connect()
    {
        ...
        try {
            ...
            $tlContext = [
                ...
                'cfg' => [
                    'setcardowneremail' => $this->cfg->setcardowneremail
                ]
            ];
    ...
    public function getIssue($issueID)
    {
        ...
        try {
            ...
            if (! is_null($jsonObj) && is_object($jsonObj)) {
                $conditionData = isset($this->conditionMap[$jsonObj->condition]) ? ' / ' .
                    $this->conditionMap[$jsonObj->condition] : '';
                $issue = new stdClass();
    ...
    /**
     */
    private function parseAddInfo($info)
    {
        $result = [
            'descr' => $info,
            'links' => []
    ...
    public function addIssue($summary, $moreInfo, $opt = null)
    {
        ...
        try {
            ...
            $ret = [
                'status_ok' => true,
                'id' => (string) $op->id,
                'msg' => sprintf(lang_get('kaiten_bug_created'), $summary,
                    (string) $op->board_id)
            ];
        } catch (Exception $e) {
    ...
    public function addNote($issueID, $noteText, $opt = null)
    {
        ...
        return [
            'status_ok' => true,
            'id' => (string) $op->iid,
            'msg' => sprintf(lang_get('kaiten_bug_comment'), $op->body,
