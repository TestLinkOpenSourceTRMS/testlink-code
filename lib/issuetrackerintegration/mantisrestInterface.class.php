...
class mantisrestInterface extends issueTrackerInterface
{
    ...
    public function addLink($issueID, $link)
    {
        try {
            ...
            $msg = "Create Mantis Link Via REST OK => TICKET:" . $issueID .
            ...
            $msg = "Create Mantis Link Via REST FAILURE => TICKET -> " . $issueID .
                ' >> link: ' . json_encode($link);
            tLog($msg, 'WARNING');
    ...
    {
        try {
            $this->APIClient->addExecLink($issueID, $link);
            $ret = [
                'status_ok' => true,
                'msg' => 'ok'
            ];
        } catch (Exception $e) {
            $msg = "Create Mantis Exec Link Via REST FAILURE => " .
                $e->getMessage();
            tLog($msg, 'WARNING');
            $ret = [
                'status_ok' => false,
