...
class mantissoapInterface extends issueTrackerInterface
{
    ...
    public function connect()
    {
        ...
        if ($this->connected = $op['connected']) {
            ...
            try {
                $op['client']->mc_enum_status($this->cfg->username,
                    $this->cfg->password);
            } catch (SoapFault $f) {
                $this->connected = false;
                tLog(
                    "SOAP Fault: (code: {$f->faultcode}, string: {$f->faultstring})",
                    "ERROR");
            }
        }
    ...
    public function getClient($opt = null)
    {
        ...
        try {
            ...
            $res['client'] = new SoapClient((string) $this->cfg->uriwsdl,
                $this->soapOpt);
    ...
    public function checkBugIDExistence($id)
    {
        ...
        try {
            $status_ok = $client->mc_issue_exists($safe->username,
                $safe->password, $safe->id) ? 1 : 0;
        } catch (SoapFault $f) {
    ...
    public function getIssue($id)
    {
        ...
        try {
            ...
            if ($client->mc_issue_exists($safe->username, $safe->password,
                $safe->id)) {
                $issue = $client->mc_issue_get($safe->username, $safe->password,
                    $safe->id);
                if (! is_null($issue) && is_object($issue)) {
                    ...
                    $issue->statusColor = isset(
                        $this->status_color[$issue->statusVerbose]) ? $this->status_color[$issue->statusVerbose] : 'white';
    ...
    /**
     */
    private function buildStatusHTMLString($statusVerbose)
    {
        $str = '';
    ...
    public function addIssue($summary, $description, $opt = null)
    {
        ...
        $mpid = $client->mc_project_get_id_from_name($safe->username,
            $safe->password, $safe->project);
        if ($mpid > 0) {
            ...
            $issue['category'] = (is_null($categoryName) ||
                ! isset($codeName[$categoryName])) ? current($nameCode) : $categoryName;
            ...
            $ret['id'] = $client->mc_issue_add($safe->username, $safe->password,
                $issue);
            $ret['id'] = sprintf('%07u', $ret['id']);

            $ret['status_ok'] = true;
            $ret['msg'] = sprintf(lang_get('mantis_bug_created'), $ret['id'],
                $safeSummary, $safe->project);
        } else {
            $ret['msg'] = sprintf(lang_get('bts_project_does_not_exist'),
    ...
    public function addNote($issueID, $noteText, $opt = null)
    {
        ...
        if ($client->mc_issue_exists($safe->username, $safe->password,
            ...
            try {
                $ret['note_id'] = $client->mc_issue_note_add($safe->username,
                    $safe->password, $safe->issueID, $issueNoteData);
            } catch (SoapFault $f) {
                // "User id missing";
                ...
                switch ($faultMsg) {
                    case "User id missing":
                        $ret['msg'] = "Cannot create note, using TestLink logged user: " .
