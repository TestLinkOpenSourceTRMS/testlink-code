...
require_once TL_ABS_PATH .
    '/third_party/redmine-php-api/lib/redmine-rest-api.php';
...
class redminerestInterface extends issueTrackerInterface
{
    ...
    public function addIssue($summary, $description, $opt = null)
    {
        ...
        try {
            // needs json or xml
            $issueXmlObj = new SimpleXMLElement(
            ...
            $issueXmlObj->addChild('subject',
                substr(htmlspecialchars($summary), 0, 255));
            ...
            if (property_exists($this->cfg, 'trackerid')) {
                $issueXmlObj->addChild('tracker_id',
                    (string) $this->cfg->trackerid);
            }

            // try to be generic
            if (property_exists($this->cfg, 'parent_issue_id')) {
                $issueXmlObj->addChild('parent_issue_id',
                    (string) $this->cfg->parent_issue_id);
            }
            ...
            if (! is_null($this->issueOtherAttr)) {
                foreach ($this->issueOtherAttr as $ka => $kv) {
                    ...
                    $issueXmlObj->addChild(
                        (isset($this->translate[$ka]) ? $this->translate[$ka] : $ka),
                        (string) $kv);
                }
            }
