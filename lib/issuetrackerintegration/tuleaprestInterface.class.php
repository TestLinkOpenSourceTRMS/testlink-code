...
class tuleaprestInterface extends issueTrackerInterface
{
        ...
        try {
            ...
            if ($tracker) {
                ...
                if (! $status) {
                    throw new Exception(
                        'The field ' . $statusID .
                        ' cannot be found in the tracker "' . $tracker->label .
                        '" (' . $tracker->id . ').');
                }
                ...
                if (count($ret['status']) !=
                    (count($status->values) - count($statusValuesID))) {
                    throw new Exception('Some labels was not found.');
                }
            } else {
                throw new Exception(
                    'The tracker ' . $this->trackerID . ' was not found.');
            }
        } catch (Exception $e) {
     ...
     *            Tracker field containing the status semantic
     * @param array $valuesID
     *            List of opened values ID
     * @author Aurelien TISNE <aurelien.tisne@csgroup.eu>
     */
    ...
    public static function getCfgTemplate()
    {
        return "<!-- Template " . __CLASS__ . " -->\n" . "<issuetracker>\n" .
            ...
            "<uriview>https://" . $_SERVER['SERVER_NAME'] .
            "/plugins/tracker/?aid=</uriview>\n" . "<uricreate>https://" .
            $_SERVER['SERVER_NAME'] .
            "/plugins/tracker/?func=new-artifact&tracker=TULEAP TRACKER ID</uricreate>\n" .
    ...
    {
        try {
            $op = $this->APIClient->createIssue((int) $this->trackerID, $summary,
                $description);
