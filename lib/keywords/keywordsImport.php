        ...
        if (move_uploaded_file($args->source, $args->dest)) {
            ...
            if ($pfn) {
                ...
                if ($result != tl::OK) {
                    ...
                    header(
                        "Location: keywordsView.php?tproject_id={$gui->tproject_id}");
                    exit();
                }
            }
            @unlink($args->dest);
...
function initArgs(&$dbHandler)
{
    ...
    $args->dest = TL_TEMP_PATH . session_id() . "-importkeywords." .
...
function initializeGui(&$argsObj)
{
    ...
    $gui->fileSizeLimitMsg = sprintf(lang_get('max_file_size_is'),
        $fslimit / 1024 . ' KB ');
