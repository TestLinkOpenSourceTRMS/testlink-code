...
$args = initArgs($build_mgr);
$gui = initGui($db, $args, $tplan_mgr);
...
switch ($args->doAction) {
    case 'copy':
        ...
        $assignment_mgr->copy_assignments($args->source_build_id,
            $args->build_id, $args->user_id);
...
function initArgs(&$buildMgr)
{
    ...
    if ($args->build_id <= 0) {
        throw new Exception(
            "Error Processing Request - Target build is not set", 1);
    }
...
/**
 */
function initGui(&$dbHandler, &$argsObj, &$tplanMgr)
{
    $gui = new stdClass();
