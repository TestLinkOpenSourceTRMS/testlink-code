...
$args = initArgs($_REQUEST, $_SESSION, $date_format_cfg, $tplan_mgr);
...
$of->Value = getItemTemplateContents('build_template', $of->InstanceName,
    $args->notes);
...
$dummy = null;
$gui->release_date = (isset($op->status_ok) && $op->status_ok &&
    $args->release_date != "") ? localize_dateOrTimeStamp(null, $dummy,
    'date_format', $args->release_date) : $args->release_date_original;
$gui->closed_on_date = $args->closed_on_date;
 ...
 *
 * @param array $request_hash
 *            hash the $_REQUEST
 * @param array $session_hash
 *            hash the the $_SESSION
 * @param string $date_format
 * @param testplan $tplanMgr
 * @return stdClass object with html values tranformed and other generated variables.
 * @internal revisions
 */
function initArgs($request_hash, $session_hash, $date_format, &$tplanMgr)
{
    $args = new stdClass();
    ...
    if (isset($request_hash['release_date']) &&
        $request_hash['release_date'] != '') {
        $date_array = split_localized_date($request_hash['release_date'],
            $date_format);
        if ($date_array != null) {
            // set date in iso format
            $args->release_date = $date_array['year'] . "-" .
                $date_array['month'] . "-" . $date_array['day'];
        }
    }

    $args->release_date_original = isset($request_hash['release_date']) &&
        $request_hash['release_date'] != '' ? $request_hash['release_date'] : null;
    ...
    if (isset($request_hash['tplan_id']) && intval($request_hash['tplan_id']) > 0) {
        ...
        $args->tplan_id = isset($session_hash['testplanID']) ? intval(
            $session_hash['testplanID']) : 0;
    ...
    $args->exec_status_filter = isset($request_hash['exec_status_filter']) ? $request_hash['exec_status_filter'] : null;
...
function initializeGui(&$argsObj, &$buildMgr)
{
    ...
    $guiObj->main_descr = lang_get('title_build_2') .
        config_get('gui_title_separator_2') . lang_get('test_plan') .
        config_get('gui_title_separator_1') . $argsObj->tplan_name;

    $guiObj->cfields = $buildMgr->html_custom_field_inputs($argsObj->build_id,
        $argsObj->testprojectID, 'design', '', $_REQUEST);

    $dummy = config_get('results');
    foreach ($dummy['status_label_for_exec_ui'] as $kv => $vl) {
        $guiObj->exec_status_filter['items'][$dummy['status_code'][$kv]] = lang_get(
            $vl);
    }
    $guiObj->exec_status_filter['selected'] = $argsObj->exec_status_filter;
...
/**
 * This action can be used to edit a build
 *
 * @param stdClass $argsObj
 * @param build_mgr $buildMgr
 * @return stdClass
 */
function edit(&$argsObj, &$buildMgr)
{
    ...
    if ($binfo['closed_on_date'] == '') {
        $argsObj->closed_on_date = mktime(0, 0, 0, date("m"), date("d"),
            date("Y"));
    } else {
        $datePieces = explode("-", $binfo['closed_on_date']);
        $argsObj->closed_on_date = mktime(0, 0, 0, $datePieces[1],
...
/**
 * This action creates a new build
 *
 * prepares environment to manage user interaction on a create operations
 *
 * @param stdClass $argsObj
 *            reference to input values received by page.
 * @return stdClass object with part of gui configuration
 */
...
/**
 * This action deletes an existing build
 *
...
function doDelete(&$dbHandler, &$argsObj, &$buildMgr, &$tplanMgr)
{
    ...
    if ($qty > 0 && ! $argsObj->user->hasRightOnProj($dbHandler, 'exec_delete')) {
        ...
        $op->user_feedback = sprintf(
            lang_get("cannot_delete_build_no_exec_delete"), $build['name']);
        return $op;
    }
    ...
    if (! $buildMgr->delete($argsObj->build_id)) {
        ...
        logAuditEvent(
            TLS("audit_build_deleted", $argsObj->testprojectName,
                $argsObj->tplan_name, $build['name']), "DELETE",
            $argsObj->build_id, "builds");
    }
    return $op;
}
 ...
 * @param ckeditorInterface $owebeditor
 * @param stdClass $guiObj
 */
function renderGui(&$smartyObj, &$argsObj, &$tplanMgr, &$buildMgr, $templateCfg,
    $owebeditor, &$guiObj)
{
    $doRender = false;
    switch ($argsObj->do_action) {
        ...
        case "close":
            ...
            $availableCF = (array) $buildMgr->get_linked_cfields_at_design(
                $guiObj->build, $guiObj->tproject_id);
            ...
            //
            $ppfx = $tplanMgr->tproject_mgr->getTestCasePrefix(
                $guiObj->tproject_id);
            $suffixSet = [
                '_' . $ppfx,
                ''
            ...
            foreach ($suffixSet as $suf) {
                ...
                $col2hideCF = $tplanMgr->cfield_mgr->get_linked_to_testproject(
                    $guiObj->tproject_id, null, $gopt);
                if ($col2hideCF != null) {
                    $col2hideCF = current($col2hideCF);
                    $col2hide = array_flip(
                        explode('|', $col2hideCF['possible_values']));
                    $col2hide[$gopt['name']] = '';
                    break;
                }
            }
            ...
            foreach ($guiObj->buildSet as $elemBuild) {
                ...
                if ($hasCF) {
                    ...
                    foreach ($cfields as $cfd) {
                        ...
                        $guiObj->buildSet[$idk][$cfd['label']] = [
                            'value' => $cfd['value'],
                            'data-order' => $cfd['value']
                        ];
                        if ($cfd['type'] == 'date') {
                            $guiObj->buildSet[$idk][$cfd['label']]['data-order'] = locateDateToISO(
                                $cfd['value'], $localeDateFormat);
                        }
                    }
                    $initCFCol = false;
                }
    ...
    if ($doRender) {
        ...
        // Attention this is affected by changes in templates
        $guiObj->enable_copy = ($argsObj->do_action == 'create' ||
            $argsObj->do_action == 'do_create') ? 1 : 0;
...
function doCreate(&$argsObj, &$buildMgr, &$tplanMgr, $dateFormat)
{
    ...
    if ($check->status_ok) {
        ...
        if ($buildID) {
            $cf_map = $buildMgr->get_linked_cfields_at_design($buildID,
                $argsObj->testprojectID);
                    ...
                    foreach ($platformSet as $platform_id => $pname) {
                        ...
                        foreach ($argsObj->exec_status_filter as $ec) {
                            switch ($execVerboseDomain[$ec]) {
                                ...
                                default:
                                    $tcaseSet = $tplanMgr->getHitsSingleStatusFull(
                                        $argsObj->tplan_id, $platform_id, $ec,
                                        $buildSet);
                                    break;
                            }
                            ...
                            if (! is_null($tcaseSet)) {
                                ...
                                $features = $tplanMgr->getLinkedFeatures(
                                    $argsObj->tplan_id, $glf['filters']);
                                $caOpt['feature_set'] = null;
                                foreach ($targetSet as $tcase_id) {
                                    $caOpt['feature_set'][] = $features[$tcase_id][$glf['filters']['platform_id']]['feature_id'];
                                }
                                $tplanMgr->assignment_mgr->copy_assignments(
                                    $argsObj->source_build_id, $buildID,
                                    $argsObj->userID, $caOpt);
                            }
                        }
                    }
                } else {
                    $tplanMgr->assignment_mgr->copy_assignments(
                        $argsObj->source_build_id, $buildID, $argsObj->userID);
                }
            }
            ...
            $op->status_ok = 1;
            logAuditEvent(
                TLS("audit_build_created", $argsObj->testprojectName,
                    $argsObj->tplan_name, $argsObj->build_name), "CREATE",
                $buildID, "builds");
        }
    }
...
function doUpdate(&$argsObj, &$buildMgr, &$tplanMgr, $dateFormat)
{
    ...
    if ($check->status_ok) {
        ...
        if ($buildMgr->update($argsObj->build_id, $argsObj->build_name,
            $argsObj->notes, $attr)) {
            $cf_map = $buildMgr->get_linked_cfields_at_design(
                $argsObj->build_id, $argsObj->testprojectID);
            $buildMgr->cfield_mgr->design_values_to_db($_REQUEST,
                $argsObj->build_id, $cf_map, null, 'build');
            ...
            $op->status_ok = 1;
            logAuditEvent(
                TLS("audit_build_saved", $argsObj->testprojectName,
                    $argsObj->tplan_name, $argsObj->build_name), "SAVE",
                $argsObj->build_id, "builds");
        }
    }
...
function crossChecks($argsObj, &$tplanMgr, $dateFormat)
{
    ...
    if ($tplanMgr->check_build_name_existence($argsObj->tplan_id,
        $argsObj->build_name, $buildID)) {
        $op->user_feedback = lang_get("warning_duplicate_build") .
            TITLE_SEP_TYPE3 . $argsObj->build_name;
        $op->status_ok = 0;
    }
...
function doCopyToTestPlans(&$argsObj, &$buildMgr, &$tplanMgr)
{
    ...
    if (! is_null($tplanset)) {
        foreach ($tplanset as $id => $info) {
            if (! $tplanMgr->check_build_name_existence($id,
                $argsObj->build_name)) {
 ...
 * @param testplan $testplan_mgr
 *            reference to testplan manager object
 * @param object $argsObj
 *            reference to user input object
...
function initSourceBuildSelector(&$testplan_mgr, &$argsObj)
{
    ...
    if (! is_null($htmlMenu['items'])) {
        ...
        foreach ($htmlMenu['items'] as $key => $name) {
            $count = $testplan_mgr->assignment_mgr->get_count_of_assignments_for_build_id(
                $key);
