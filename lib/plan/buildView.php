...
function initEnv(&$dbHandler)
{
    ...
    $gui->tplan_id = isset($_REQUEST['tplan_id']) ? intval(
        $_REQUEST['tplan_id']) : 0;
    if ($gui->tplan_id == 0) {
    ...
    $info = $tplan_mgr->tree_manager->get_node_hierarchy_info($gui->tplan_id,
        null, array(
            'nodeType' => 'testplan'
        ));
    ...
    if (! is_null($gui->buildSet)) {
        $availableCF = (array) $build_mgr->get_linked_cfields_at_design(
            current($gui->buildSet), $gui->tproject_id);
    }
    ...
    foreach ($gui->buildSet as $elemBuild) {
        ...
        if ($hasCF) {
            ...
            foreach ($cfields as $cfd) {
                ...
                $gui->buildSet[$idk][$cfd['label']] = [
                    'value' => $cfd['value'],
                    'data-order' => $cfd['value']
                ];
                if ($cfd['type'] == 'date') {
                    $gui->buildSet[$idk][$cfd['label']]['data-order'] = locateDateToISO(
