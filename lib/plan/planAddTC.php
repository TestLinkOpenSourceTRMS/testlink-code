...
if ($do_display) {
    ...
    $cfields = (array) $tsuite_mgr->cfield_mgr->get_linked_cfields_at_testplan_design(
    ...
    if (isset($args->control_panel['filter_testcase_name'])) {
        $filters['tcase_name'] = $args->control_panel['filter_testcase_name'];
    }

    if (isset($args->control_panel['filter_platforms'])) {
        $filters['platforms'] = $args->control_panel['filter_platforms'];
    }

    $out = gen_spec_view($db, 'testPlanLinking', $args->tproject_id,
        $args->object_id, $tsuite_data['name'], $tplan_linked_tcversions, null,
        $filters, $opt);
    ...
    if ($args->item_level == 'reqcoverage') {
        ...
        $requirement_data = $req_mgr->get_by_id($args->object_id,
            requirement_mgr::LATEST_VERSION);
        $requirement_data_name = $requirement_data[0]['req_doc_id'] . ' : ' .
            $requirement_data[0]['title'];
        // get chekbox value : setting_get_parent_child_relation.
        if ($_SESSION['setting_get_parent_child_relation']) {
            // if checkbox is checked.
            $requirements_child = $req_spec_mgr->get_requirement_child_by_id(
                $args->object_id, requirement_mgr::LATEST_VERSION);
        } else {
        ...
        $requirements = $req_spec_mgr->get_requirements($args->object_id, 'all',
            null, $getOptions);
    }
    ...
    if ($args->item_level == 'reqcoverage') {
        $out = array();
        $out = gen_coverage_view($db, 'testPlanLinking', $args->tproject_id,
            $args->object_id, $requirement_data_name, $tplan_linked_tcversions,
            null, $filters, $opt);
        ...
        if (! is_null($requirements_child)) {
            ...
            foreach ($requirements_child as $req) {
                $requirement_data_name = $req['req_doc_id'] . ' : ' .
                    ...
                    $parentName;
                $tmp = gen_coverage_view($db, 'testPlanLinking',
                    $args->tproject_id, $req['destination_id'],
                    $requirement_data_name, $tplan_linked_tcversions, null,
                    $filters, $opt);
                // update parent name.
        ...
        foreach ($requirements as $req) {
            ...
            $tmp = gen_coverage_view($db, 'testPlanLinking', $args->tproject_id,
                $req['id'], $coverage_name, $tplan_linked_tcversions, null,
                $filters, $opt);
...
function initArgs(&$tproject_mgr)
{
    ...
    $args->tplan_id = isset($_REQUEST['tplan_id']) ? intval(
        $_REQUEST['tplan_id']) : intval($_SESSION['testplanID']);
    ...
    $args->testerID = isset($_REQUEST['testerID']) ? intval(
        $_REQUEST['testerID']) : 0;
    ...
    if ($args->treeFormToken > 0 &&
        ($args->item_level == 'testsuite' || $args->item_level == 'testcase')) {
    ...
    $booleankeys = array(
        'refreshTree' => 'setting_refresh_tree_on_action',
    ...
    if ($getFromSession && isset($pageCache['filter_toplevel_testsuite']) &&
        ...
        // get all
        $first_level_suites = $tproject_mgr->get_first_level_test_suites(
            $args->tproject_id, 'simple', array(
                'accessKey' => 'id'
            ));

        // remove unneeded
        $hit = array_diff_key($first_level_suites,
            $pageCache['filter_toplevel_testsuite']);
...
function initializeGui(&$dbHandler, $argsObj, &$tplanMgr, &$tcaseMgr)
{
    ...
    $gui->testCasePrefix = $tcaseMgr->tproject_mgr->getTestCasePrefix(
        $argsObj->tproject_id);
    $gui->testCasePrefix .= $tcase_cfg->glue_character;

    $gui->can_remove_executed_testcases = $argsObj->user->hasRight($dbHandler,
        "testplan_unlink_executed_testcases", $argsObj->tproject_id,
        $argsObj->tplan_id);
    ...
    $gui->keywordsFilterType->options = array(
        'OR' => 'Or',
        'AND' => 'And'
    );
    ...
    if (! is_null($gui->keyword_id) && $gui->keyword_id != 0) {
        $gui->keywords_filter_feedback = buildKeywordsFeedbackMsg($dbHandler,
            $argsObj, $gui);
    }
    ...
    if (! is_null($gui->platform_id) && $gui->platform_id != 0) {
        $gui->platforms_filter_feedback = buildPlatformsFeedbackMsg($dbHandler,
            $argsObj, $gui);
    }
    ...
    $gui->pageTitle = lang_get('test_plan') . $title_separator .
        $gui->testPlanName;
    $gui->refreshTree = $argsObj->refreshTree;
    $gui->canAssignExecTask = $argsObj->user->hasRight($dbHandler,
        "exec_assign_testcases", $argsObj->tproject_id, $argsObj->tplan_id);

    $tproject_mgr = new testproject($dbHandler);
    $tproject_info = $tproject_mgr->get_by_id($argsObj->tproject_id);

    $gui->testers = getTestersForHtmlOptions($dbHandler, $argsObj->tplan_id,
        $tproject_info);
    ...
    $gui->usePlatforms = $platform_mgr->platformsActiveForTestplan(
        $argsObj->tplan_id);
    if ($gui->usePlatforms) {
        ...
        $optLTT = null;
        $gui->bulk_platforms = $platform_mgr->getLinkedToTestplanAsMap(
            $argsObj->tplan_id, $optLTT);
    ...
    $gui->build = initBuildSelector($tplanMgr, $argsObj);
...
function doSaveCustomFields(&$argsObj, &$userInput, &$tplanMgr, &$tcaseMgr)
{
    ...
    foreach ($nodeid_array_cfnames as $link_id => $customFieldsNames) {
        ...
        $tcaseMgr->cfield_mgr->testplan_design_values_to_db($cfvalues, null,
            $link_id);
    }
}
...
function doSavePlatforms(&$argsObj, &$tplanMgr)
{
    foreach ($argsObj->feature2fix as $tcversion_platform) {
        ...
        if ($platform_id != 0) {
            $tplanMgr->changeLinkedTCVersionsPlatform($argsObj->tplan_id, 0,
                $platform_id, $tcversion_id);
        }
    }
}
 ...
 * @return void
 */
function sendMailToTesters(&$dbHandler, &$tcaseMgr, &$guiObj, &$argsObj,
    $features, $operation)
{
    $testers['new'] = null;
    ...
    $assigner = $userData[$argsObj->userID]->firstName . ' ' .
        $userData[$argsObj->userID]->lastName;

    $email['from_address'] = config_get('from_email');
    $body_first_lines = lang_get('testproject') . ': ' . $argsObj->tproject_name .
        '<br />' . lang_get('testplan') . ': ' . $guiObj->testPlanName .
        '<br /><br />';
        ...
        if (! is_null($tester_set)) {
            ...
            foreach ($tester_set as $user_id => $value) {
                ...
                $email_op = email_send($email['from_address'],
                    ...
                    $email['cc'], $email['attachment'], $email['exit_on_error'],
                    $email['htmlFormat']);
            }
        }
    }
        ...
        foreach ($keySet as $key) {
            ...
            if ($testSuite['linked_testcase_qty'] > 0) {
                ...
                foreach ($tcaseSet as $tcaseKey) {
                    if (isset(
                        $testSuite['testcases'][$tcaseKey]['feature_id'][0])) {
                        $breakLoop = true;
 ...
 */
function initBuildSelector(&$testplan_mgr, &$argsObj)
{
    ...
    $menu['items'] = (array) $testplan_mgr->get_builds_for_html_options(
        $argsObj->tplan_id, testplan::GET_ACTIVE_BUILD, testplan::GET_OPEN_BUILD);
...
function addToTestPlan(&$dbHandler, &$argsObj, &$guiObj, &$tplanMgr, &$tcaseMgr)
{
    ...
    $linked_features = $tplanMgr->link_tcversions($argsObj->tplan_id,
        $items_to_link, $argsObj->userID);
    ...
    if ($argsObj->testerID > 0) {
        ...
        if ($argsObj->send_mail) {
            foreach ($called as $ope => $ope_status) {
                if ($ope_status) {
                    sendMailToTesters($dbHandler, $tcaseMgr, $guiObj, $argsObj,
                        $features2['add'], $ope);
                }
            }
        }
...
function buildStatusFeedbackMsg()
{
    ...
    if (! is_null($hideStatusSet)) {
        ...
        $ret = sprintf(lang_get('hint_add_testcases_to_testplan_status'),
            implode(',', $msg));
    }
