...
$of->Value = getItemTemplateContents('testplan_template', $of->InstanceName,
    $args->notes);
...
if ($args->do_action == "do_create" || $args->do_action == "do_update") {
    ...
    $name_exists = $tproject_mgr->check_tplan_name_existence($args->tproject_id,
        $args->testplan_name);
...
$gui->cfields = $tplan_mgr->html_table_of_custom_field_inputs($args->tplan_id,
    $args->tproject_id, 'design', '', $_REQUEST);

switch ($args->do_action) {
    case 'fileUpload':
        $gui->uploadOp = fileUploadManagement($db, $args->tplan_id,
            $args->fileTitle, $tplan_mgr->getAttachmentTableName());
    ...
    case 'do_delete':
        ...
        if ($tplanInfo) {
            ...
            logAuditEvent(
                TLS("audit_testplan_deleted", $args->tproject_name,
                    $tplanInfo['name']), "DELETE", $args->tplan_id, "testplan");
        }

        // unset the session test plan if it is deleted
        if (isset($_SESSION['testplanID']) &&
            ($_SESSION['testplanID'] = $args->tplan_id)) {
    ...
    case 'do_update':
        ...
        if (! $name_exists || $name_id_rel_ok) {
            if (! $tplan_mgr->update($args->tplan_id, $args->testplan_name,
                $args->notes, $args->active, $args->is_public)) {
                $gui->user_feedback = lang_get('update_tp_failed1') .
                    $gui->testplan_name . lang_get('update_tp_failed2') . ": " .
                    $db->error_msg() . "<br />";
            } else {
                logAuditEvent(
                    TLS("audit_testplan_saved", $args->tproject_name,
                        $args->testplan_name), "SAVE", $args->tplan_id,
                    "testplans");
                $cf_map = $tplan_mgr->get_linked_cfields_at_design(
                    $args->tplan_id);
                $tplan_mgr->cfield_mgr->design_values_to_db($_REQUEST,
                    $args->tplan_id, $cf_map);
                ...
                if (! $args->is_public) {
                    $tprojectEffectiveRole = $args->user->getEffectiveRole($db,
                        $args->tproject_id, null);

                    // does user have an SPECIFIC role on TestPlan ?
                    // if answer is yes => do nothing
                    if (! tlUser::hasRoleOnTestPlan($db, $args->user_id,
                        $args->tplan_id)) {
                        $tplan_mgr->addUserRole($args->user_id, $args->tplan_id,
                            $tprojectEffectiveRole->dbID);
                    }
                }
            }
        } else {
    ...
    case 'do_create':
        ...
        if (! $name_exists) {
            $new_tplan_id = $tplan_mgr->create($args->testplan_name,
                $args->notes, $args->tproject_id, $args->active,
                $args->is_public);
            if ($new_tplan_id == 0) {
                $gui->user_feedback = $db->error_msg();
            } else {
                logAuditEvent(
                    TLS("audit_testplan_created", $args->tproject_name,
                        $args->testplan_name), "CREATED", $new_tplan_id,
                    "testplans");
                $cf_map = $tplan_mgr->get_linked_cfields_at_design(
                    $new_tplan_id, $args->tproject_id);
                $tplan_mgr->cfield_mgr->design_values_to_db($_REQUEST,
                    $new_tplan_id, $cf_map);
                ...
                if (! $args->is_public &&
                    ! tlUser::hasRoleOnTestPlan($db, $args->user_id,
                        $new_tplan_id)) {
                    $effectiveRole = $args->user->getEffectiveRole($db,
                        $args->tproject_id, null);
                    $tplan_mgr->addUserRole($args->user_id, $new_tplan_id,
                        $effectiveRole->dbID);
                }
            }
        } else {
...
switch ($args->do_action) {
    ...
    case 'setInactive':
        ...
        if (! is_null($gui->tplans)) {
            ...
            $gui->drawPlatformQtyColumn = $dummy[$args->tproject_id]['platform_qty'] >
                0;
            ...
            foreach ($suffixSet as $suf) {
                ...
                $col2hideCF = $tplan_mgr->cfield_mgr->get_linked_to_testproject(
                    $gui->tproject_id, null, $gopt);
            ...
            foreach ($tplanSet as $idk) {
                if ($hasCF !== 0) {
                    ...
                    foreach ($cfields as $cfd) {
                        ...
                        $gui->tplans[$idk][$cfd['label']] = [
                            'value' => $cfd['value'],
                            'data-order' => $cfd['value']
                        ];

                        if ($cfd['type'] == 'date') {
                            $gui->tplans[$idk][$cfd['label']]['data-order'] = locateDateToISO(
                ...
                $gui->tplans[$idk]['tcase_qty'] = isset($dummy[$idk]['qty']) ? intval(
                    $dummy[$idk]['qty']) : 0;
                $gui->tplans[$idk]['build_qty'] = isset(
                    $buildQty[$idk]['build_qty']) ? intval(
                    $buildQty[$idk]['build_qty']) : 0;
                if ($gui->drawPlatformQtyColumn) {
                    $plat = $tplan_mgr->getPlatforms($idk);
                    $gui->tplans[$idk]['platform_qty'] = is_null($plat) ? 0 : count(
                        $plat);
                }
                ...
                foreach ($rightSet as $target) {
                    ...
                    if ($gui->tplans[$idk]['has_role'] > 0) {
                        if (isset(
                            $args->user->tplanRoles[$gui->tplans[$idk]['has_role']])) {
                            ...
                            $roleObj = $args->user->getEffectiveRole($db,
                                $args->tproject_id, $idk);
                        }
                    ...
                    $gui->tplans[$idk]['rights'][$target] = $roleObj->hasRight(
                        $target);
                }
            }
        }
...
function initializeGui(&$dbHandler, &$argsObj, &$editorCfg)
{
    ...
    $guiObj->grants->testplan_create = $argsObj->user->hasRight($dbHandler,
        "mgt_testplan_create", $argsObj->tproject_id);
    $guiObj->grants->mgt_view_events = $argsObj->user->hasRight($dbHandler,
        "mgt_view_events");
    $guiObj->notes = '';

    $guiObj->attachments[$guiObj->tplan_id] = getAttachmentInfosFrom($tplan_mgr,
        $guiObj->tplan_id);
    $guiObj->attachmentTableName = $tplan_mgr->getAttachmentTableName();

    $guiObj->fileUploadURL = $_SESSION['basehref'] .
        $tplan_mgr->getFileUploadRelativeURL($guiObj->tplan_id);
    $guiObj->delAttachmentURL = $_SESSION['basehref'] .
        $tplan_mgr->getDeleteAttachmentRelativeURL($guiObj->tplan_id);
    ...
    $updateAttachments = false)
{
    ...
    if (count($dummy) > 0) {
        ...
        if ($updateAttachments) {
            $guiObj->attachments[$guiObj->tplan_id] = getAttachmentInfosFrom(
                $itemMgr, $guiObj->tplan_id);
        }
    }
}
