...
if ($args->doExport) {
    $tLogMsg = 'basename(__FILE__) : ' . basename(__FILE__) .
...
function initArgs()
{
    ...
    $default = 'linkedItems';
    $args->exportContent = isset($_REQUEST['exportContent']) ? substr(
        $_REQUEST['exportContent'], 0, strlen($default)) : $default;
    switch ($args->exportContent) {
        case 'tree':
        case '4results':
        case 'linkedItems':
...
function initializeGui(&$argsObj, &$tplanMgr)
{
    ...
    if ($argsObj->build_id > 0) {
        ...
        $add2name .= '_' .
            str_replace(' ', '_', $dummy[$argsObj->build_id]['name']);
    }
