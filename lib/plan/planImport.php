...
if (! $tprojectHasTC) {
    $gui->resultMap[] = array(
        '',
        sprintf(lang_get('tproject_has_zero_testcases'), $dummy['name'])
    );
}
 ...
 * @internal revisions
 */
function importTestPlanLinksFromXML(&$dbHandler, &$tplanMgr, $targetFile,
    $contextObj)
{
    // <testplan>
    // <name></name>
    ...
    $labels = init_labels(
        array(
            'link_without_required_platform' => null,
            'ok' => null,
            'link_without_platform_element' => null,
            'no_platforms_on_tproject' => null,
            'tcase_link_updated' => null,
            'link_with_platform_not_needed' => null,
            'tproject_has_zero_testcases' => null,
            'platform_not_on_tproject' => null,
            'platform_linked' => null,
            'platform_not_linked' => null,
            'tcase_doesnot_exist' => null,
            'tcversion_doesnot_exist' => null,
            'not_imported' => null,
            'link_to_tplan_feedback' => null,
            'link_to_platform' => null,
    ...
    if ($xml !== false) {
        ...
        $tcaseSet = array();
        $tprojectMgr->get_all_testcases_id($contextObj->tproject_id, $tcaseSet,
            array(
                'output' => 'external_id'
            ));
        ...
        if (property_exists($xml, 'platforms')) {
            ...
            if (is_null($platformUniverse)) {
                $status_ok = false;
                $msg[] = array(
                    $labels['no_platforms_on_tproject'],
                    $labels['not_imported']
                );
            } else {
                $platformUniverse = array_flip($platformUniverse);
                $op = processPlatforms($platformMgr, $tplanMgr,
                    $platformUniverse, $xml->platforms, $labels,
                    $contextObj->tplan_id);
        ...
        if ($status_ok && $xml->xpath('//executables')) {
            $platformSet = $tplanMgr->getPlatforms($contextObj->tplan_id,
                array(
                    'outputFormat' => 'mapAccessByName'
                ));
                ...
                if ($targetHasPlatforms) {
                    ...
                    if ($linkWithPlatform && isset($platformSet[$targetName])) {
                        ...
                        if (! $platformElementExists) {
                            $dummy_msg = sprintf(
                                $labels['link_without_platform_element'],
                                $idx + 1);
                        } elseif (! $linkWithPlatform) {
                            $dummy_msg = sprintf(
                                $labels['link_without_required_platform'],
                                $idx + 1);
                        } else {
                            $dummy_msg = sprintf($labels['platform_not_linked'],
                ...
                if ($status_ok) {
                    ...
                    if (isset($tcaseSet[$externalID])) {
                        ...
                        if (! empty($dummy)) {
                            ...
                            //
                            $lvFilters = array(
                                'tplan_id' => $contextObj->tplan_id
                            );
                            $linkedVersions = $tcaseMgr->get_linked_versions(
                                $dummy[0]['id'], $lvFilters);
                            $updateLink = false;
                            ...
                            if (! ($createLink = is_null($linkedVersions))) {
                                ...
                                if (! isset(
                                    $linkedVersions[$dummy[0]['tcversion_id']])) {
                                    ...
                                    if (! $createLink) {
                                        ...
                                        $dummy_msg = sprintf(
                                            $labels['cant_link_to_tplan_feedback'],
                                            $externalID, $version);

                                        $msg[] = array(
                                            $dummy_msg,
                                            sprintf(
                                                $labels['tcversion_status_forbidden'],
                                                lang_get($rogue))
                                        );
                                    }
                                } else {
                                    ...
                                    $createLink = false;
                                    $updateLink = false;
                                    $plat_keys = array_keys(
                                        $linkedVersions[$dummy[0]['tcversion_id']][$contextObj->tplan_id]);
                            ...
                            if ($createLink) {
                                $item2link['items'] = array(
                                    $dummy[0]['id'] => array(
                                        $platformID => $dummy[0]['tcversion_id']
                                    )
                                );
                                $item2link['tcversion'] = array(
                                    $dummy[0]['id'] => $dummy[0]['tcversion_id']
                                );
                                $tplanMgr->link_tcversions(
                                    $contextObj->tplan_id, $item2link,
                                    $contextObj->userID);
                                $dummy_msg = sprintf(
                                    $labels['link_to_tplan_feedback'],
                                    $externalID, $version);

                                if ($platformID > 0) {
                                    $dummy_msg .= sprintf(
                                        $labels['link_to_platform'], $targetName);
                                }
                                $msg[] = array(
                                    $dummy_msg,
                                    $labels['ok']
                            ...
                            if ($updateLink) {
                                $newOrder = array(
                                    $dummy[0]['tcversion_id'] => $execOrder
                                );
                                $tplanMgr->setExecutionOrder(
                                    $contextObj->tplan_id, $newOrder);

                                if ($doUpdateFeedBack) {
                                    $dummy_msg = sprintf(
                                        $labels['tcase_link_updated'],
                                        $tcasePrefix . $externalID . ' ' .
                                        $tcaseName, $version);
                                    $msg[] = array(
                                        $dummy_msg,
                                        $labels['ok']
                            ...
                            $msg[] = array(
                                sprintf($labels['tcversion_doesnot_exist'],
                                    $externalID, $version, $tprojectInfo['name'])
                            );
                        }
                    } else {
                        $msg[] = array(
                            sprintf($labels['tcase_doesnot_exist'], $externalID,
                                $tprojectInfo['name'])
                        );
                    }
                }
...
/**
 */
function processPlatforms(&$platMgr, &$tplanMgr, $universe, $xmlSubset, $lbl,
    $tplanID)
{
    $ret = array(
        'status_ok' => true,
    ...
    for ($idx = 0; $idx < $loops2do; $idx ++) {
        ...
        if (isset($universe[$targetName])) {
            ...
            $status_ok = false;
            $ret['msg'][] = array(
                sprintf($lbl['platform_not_on_tproject'], $targetName),
                $lbl['not_imported']
            );
        }
    }
    if ($status_ok) {
        ...
        $currentPlatformSet = $tplanMgr->getPlatforms($tplanID,
            array(
        ...
        foreach ($idSet as $platformID => $platformName) {
            if (! isset($currentPlatformSet[$platformID])) {
                ...
                $msg_ok[] = array(
                    sprintf($lbl['platform_linked'], $platformName),
                    $lbl['ok']
                );
            }
        }
