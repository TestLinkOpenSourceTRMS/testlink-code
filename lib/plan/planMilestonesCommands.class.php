...
// needed because milestone_mgr is inside
class planMilestonesCommands
{
    ...
    public function edit(&$argsObj)
    {
        ...
        $guiObj->milestone['target_date'] = localize_dateOrTimeStamp(null,
            $dummy, 'date_format', $guiObj->milestone['target_date']);

        // as start date is optional it can be "0000-00-00" (default timestamp)
        if ($guiObj->milestone['start_date'] != "0000-00-00") {
            $guiObj->milestone['start_date'] = localize_dateOrTimeStamp(null,
                $dummy, 'date_format', $guiObj->milestone['start_date']);
        } else {
            $guiObj->milestone['start_date'] = "";
        }

        $guiObj->main_descr = lang_get('testplan') . TITLE_SEP;
        $guiObj->action_descr = sprintf(lang_get('edit_milestone'),
            $guiObj->milestone['name']);
        $guiObj->template = $this->defaultTemplate;
        $guiObj->submit_button_label = $this->submit_button_label;
        return $guiObj;
    }
    ...
    public function doCreate(&$argsObj, $basehref)
    {
        ...
        $name_exists = $this->milestone_mgr->check_name_existence(
            $argsObj->tplan_id, $argsObj->name);
        if ($name_exists) {
            $guiObj->user_feedback = sprintf(
                lang_get('milestone_name_already_exists'), $argsObj->name);
            $op_ok = 0;
        }
        ...
        if ($op_ok !== 0) {
            ...
            $op_ok = is_valid_date($argsObj->target_date_original,
                $date_format_cfg) &&
                ($argsObj->start_date_original == '' ||
                is_valid_date($argsObj->start_date_original, $date_format_cfg));
            if (! $op_ok) {
                $guiObj->user_feedback = sprintf(
                    lang_get('warning_invalid_date'));
            }
        }
        ...
        if ($op_ok) {
            ...
            if ($argsObj->id > 0) {
                logAuditEvent(
                    TLS("audit_milestone_created", $argsObj->tplan_name,
                        $argsObj->name), "CREATE", $argsObj->id, "milestones");
                $guiObj->user_feedback = sprintf(lang_get('milestone_created'),
                    $argsObj->name);
    ...
    public function doUpdate(&$argsObj, $basehref)
    {
        ...
        // Check name do not exists
        $name_exists = $this->milestone_mgr->check_name_existence(
            $originalMilestone['testplan_id'], $argsObj->name, $argsObj->id);
        if ($name_exists) {
            $obj->user_feedback = sprintf(
                lang_get('milestone_name_already_exists'), $argsObj->name);
            $op_ok = 0;
        }
        ...
        if ($op_ok) {
            ...
            $timestamp['original_target'] = strtotime(
                $originalMilestone['target_date'] . " 23:59:59");
            $timestamp['now'] = strtotime("now");

            if (($timestamp['target'] != $timestamp['original_target']) &&
        ...
        if ($op_ok) {
            ...
            $op_ok = $this->milestone_mgr->update($argsObj->id, $argsObj->name,
        ...
        if ($op_ok) {
            ...
            logAuditEvent(
                TLS("audit_milestone_saved", $argsObj->tplan_name,
                    $argsObj->name), "SAVE", $argsObj->id, "milestones");
        } else {
    ...
    public function doDelete(&$argsObj, $basehref)
    {
        ...
        logAuditEvent(
            TLS("audit_milestone_deleted", $milestone['testplan_name'],
                $milestone['name']), "DELETE", $argsObj->id, "milestones");

        $obj = new stdClass();
        $obj->template = $basehref . $this->viewAction;
        $obj->user_feedback = sprintf(lang_get('milestone_deleted'),
            $milestone['name']);
        $obj->main_descr = null;
