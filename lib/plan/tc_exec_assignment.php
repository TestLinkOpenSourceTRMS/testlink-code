...
switch ($args->level) {
    ...
    case 'testsuite':
        ...
        $filters['assignedToFilter'] = property_exists($args,
            'filter_assigned_to') ? $args->filter_assigned_to : null;
...
function initArgs()
{
    ...
    $mode = 'plan_mode';
    $session_data = isset($_SESSION[$mode]) &&
        isset($_SESSION[$mode][$form_token]) ? $_SESSION[$mode][$form_token] : null;
    ...
    $args->build_id = intval(
        isset($session_data['setting_build']) ? $session_data['setting_build'] : 0);
    $args->platform_id = intval(
        isset($session_data['setting_platform']) ? $session_data['setting_platform'] : 0);
    ...
    $args->targetFeature = intval(
        isset($_REQUEST['targetFeature']) ? $_REQUEST['targetFeature'] : 0);
    $args->targetUser = intval(
...
function initializeGui(&$dbHandler, $argsObj, &$tplanMgr, &$tcaseMgr)
{
    ...
    $optLTT = null;
    $gui->platforms = $platform_mgr->getLinkedToTestplanAsMap(
    ...
    if ($argsObj->level != 'testproject') {
        ...
        $build_info = $tplanMgr->get_build_by_id($argsObj->tplan_id,
            $argsObj->build_id);
        ...
        $gui->users = getUsersForHtmlOptions($dbHandler, null, null, null,
            $gui->all_users);
        $gui->testers = getTestersForHtmlOptions($dbHandler, $argsObj->tplan_id,
            $tproject_info, $gui->all_users);
    }
    ...
    $features, $operation)
{
    ...
    $body_header = $lbl['testproject'] . ': ' . $argsObj->tproject_name .
        '<br />' . $lbl['testplan'] . ': ' . $guiObj->testPlanName . '<br />' .
        ...
        if (! is_null($tester_set)) {
            ...
            foreach ($tester_set as $user_id => $set2work) {
                ...
                foreach ($set2work as $pid => $value) {
                    if ($pid != 0) {
                        $email['body'] .= $lbl['platform'] . ': ' .
                            $platSet[$pid] . '<br />';
                    }

                    foreach ($value as $tcase_id) {
                        $email['body'] .= $flat_path[$tcase_id] . '<br />';
                        $wl = $tcaseMgr->buildDirectWebLink(
                            $_SESSION['basehref'], $tcase_id,
                            $argsObj->testproject_id);

                        $email['body'] .= '<a href="' . $wl . '">' .
                            'direct link to test case spec ' . '</a>' .
                            '<br /><br />';
                    }
                }
...
function doRemoveAll(&$dbH, &$argsObj, &$guiObj, $cfg, $oMgr)
{
    ...
    if ($argsObj->send_mail) {
        sendMailToTesters($dbH, $oMgr['tcase'], $guiObj, $argsObj, $f4mail,
            'del');
    }
}
