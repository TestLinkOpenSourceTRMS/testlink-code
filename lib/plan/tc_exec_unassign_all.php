...
$build_name = "";
if ($args->build_id) {
    $assignment_count = $assignment_mgr->get_count_of_assignments_for_build_id(
        $args->build_id);
...
if ($assignment_count) {
    ...
    if ($args->confirmed) {
        ...
        $gui->popup_message = sprintf(lang_get('unassign_all_tcs_warning_msg'),
            $build_name);
        $gui->message = sprintf(lang_get('number_of_assignments_per_build'),
            $assignment_count, $build_name);
    }
} else {
...
/**
 */
function initGui(&$dbHandler, &$argsObj)
{
    $gui = new stdClass();
