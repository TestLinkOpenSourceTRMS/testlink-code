...
if (isset($args->tplan_id)) {
    ...
    //
    $qtyByPlatform = $tplan_mgr->countLinkedTCVersionsByPlatform(
        $args->tplan_id);
    ...
    $gui->platform_count_js = initOptionPanels($tplan_mgr, $platform_mgr,
        $opt_cfg, $args);
    ...
    if ($args->doAction == 'doAssignPlatforms') {
        ...
        $platform_mgr->unlinkFromTestplan($args->platformsToRemove,
            $args->tplan_id);
        if ($fix_needed && count($args->platformsToAdd) == 1) {
            reset($args->platformsToAdd);
            $tplan_mgr->changeLinkedTCVersionsPlatform($args->tplan_id, 0,
                current($args->platformsToAdd));
        }
 ...
 */
function initOptionPanels(&$tplan_mgr, &$platform_mgr, &$opt_cfg, &$args)
{
    $opt_cfg->from->map = $platform_mgr->getAllAsMap(
        config_get('platforms')->allowedOnAssign);
