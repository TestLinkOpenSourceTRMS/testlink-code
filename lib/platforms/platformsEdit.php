...
$of->Value = getItemTemplateContents('platform_template', $of->InstanceName,
    $args->notes);
...
function do_update(&$args, &$gui, &$platform_mgr)
{
    ...
    $ret = new stdClass();
    $ret->template = 'platformsView.tpl';
    $ret->status = $platform_mgr->update($args->platform_id, $args->name,
        $args->notes, $args->enable_on_design, $args->enable_on_execution,
        $args->is_open);
...
function getErrorMessage($code, $platform_name)
{
    switch ($code) {
        ...
        case tlPlatform::E_NAMEALREADYEXISTS:
            $msg = sprintf(lang_get('platform_name_already_exists'),
                $platform_name);
            break;
...
/**
 */
function initGui(&$db, &$args, &$platMgr)
{
