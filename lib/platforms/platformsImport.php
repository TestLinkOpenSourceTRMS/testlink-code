 ...
 *
 * @param
 *            object dbHandler reference to db handler
 *
 */
function doImport(&$dbHandler, $testproject_id)
{
    ...
    if (($source != 'none') && ($source != '')) {
        ...
        if ($xml !== false) {
            ...
            $platformsOnSystem = $platform_mgr->getAllAsMap(
                [
                    'accessKey' => 'name',
                    'output' => 'rows',
            ...
            foreach ($xml as $platform) {
                if (property_exists($platform, 'name')) {
                    ...
                    if (isset($platformsOnSystem[$name])) {
                        $import_msg['ok'][] = sprintf(
                            lang_get('platform_updated'), $name);
                        ...
                        $import_msg['ok'][] = sprintf(
                            lang_get('platform_imported'), $name);
                        $item = new stdClass();
                        $item->name = $name;
                        $item->notes = (string) $platform->notes;
                        $item->enable_on_design = intval(
