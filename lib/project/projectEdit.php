...
$args = initArgs($tproject_mgr, $_REQUEST);
...
$smarty->assign('mgt_view_events',
    $_SESSION['currentUser']->hasRight($db, "mgt_view_events"));
...
switch ($args->doAction) {
    ...
    case 'enableRequirements':
    case 'disableRequirements':
        if ($addIssueTracker = $addCodeTracker = $addReqMgrSystem = is_null(
            $template)) {
        ...
        if ($gui->itemQty > 0) {
            $gui->pageTitle .= ' ' .
                sprintf(lang_get('available_test_projects'), $gui->itemQty);
        }
        ...
        if ($addIssueTracker) {
            ...
            for ($idx = 0; $idx < $tprojQty; $idx ++) {
                ...
                if ($gui->tprojects[$idx]['itname'] != '') {
                    ...
                    $gui->tprojects[$idx]['itstatusImg'] = ' <img title="' .
                        $labels[$ak . '_integration'] . '" ' . ' alt="' .
                        $labels[$ak . '_integration'] . '" ' . ' src="' .
                        $imgSet[$ak] . '"/>';
                }
            }
        }
        ...
        if ($addCodeTracker) {
            ...
            for ($idx = 0; $idx < $tprojQty; $idx ++) {
                ...
                if ($gui->tprojects[$idx]['ctname'] != '') {
                    ...
                    $gui->tprojects[$idx]['ctstatusImg'] = ' <img title="' .
                        $labels[$ak . '_integration'] . '" ' . ' alt="' .
                        $labels[$ak . '_integration'] . '" ' . ' src="' .
                        $imgSet[$ak] . '"/>';
                }
        ...
        if ($addReqMgrSystem) {
            ...
            for ($idx = 0; $idx < $tprojQty; $idx ++) {
                ...
                if ($gui->tprojects[$idx]['rmsname'] != '') {
                    ...
                    $gui->tprojects[$idx]['rmsstatusImg'] = ' <img title="' .
                        $labels[$ak . '_integration'] . '" ' . ' alt="' .
                        $labels[$ak . '_integration'] . '" ' . ' src="' .
                        $imgSet[$ak] . '"/>';
                }
    ...
    default:
        if ($args->doAction != "edit" && $args->doAction != "ErrorOnAction") {
            $of->Value = getItemTemplateContents('project_template',
 ...
 *            the $_REQUEST
 * @param
 *            hash session_hash the $_SESSION
 * @return stdClass object with html values tranformed and other
 *         generated variables.
 * @internal
 */
function initArgs($tprojectMgr, $request_hash)
{
    $args = new stdClass();
    ...
    $args->issue_tracker_id = isset($request_hash['issue_tracker_id']) ? intval(
        $request_hash['issue_tracker_id']) : 0;
    $args->code_tracker_id = isset($request_hash['code_tracker_id']) ? intval(
        $request_hash['code_tracker_id']) : 0;
    $args->reqmgrsystem_id = isset($request_hash['reqmgrsystem_id']) ? intval(
        $request_hash['reqmgrsystem_id']) : 0;
    ...
    if ($args->doAction != 'doUpdate' && $args->doAction != 'doCreate') {
        if ($args->tprojectID > 0) {
            ...
            $args->issue_tracker_enabled = intval(
                $the_data['issue_tracker_enabled']);
            ...
            $args->code_tracker_enabled = intval(
                $the_data['code_tracker_enabled']);
            ...
            $args->reqmgr_integration_enabled = intval(
                $the_data['reqmgr_integration_enabled']);
    ...
    if ($op->status_ok) {
        ...
        if ($new_id <= 0) {
            ...
            if (! $argsObj->is_public) {
                ...
                // it invisible for me!!!
                $tprojectMgr->addUserRole($argsObj->userID, $new_id,
...
function doUpdate($argsObj, &$tprojectMgr, $sessionTprojectID)
{
    ...
    if ($op->status_ok) {
        ...
        if ($tprojectMgr->update($argsObj->tprojectID,
            ...
            $options, $argsObj->active, $argsObj->tcasePrefix,
            $argsObj->is_public)) {
            $op->msg = '';
            $tprojectMgr->activate($argsObj->tprojectID, $argsObj->active);

            $tprojectMgr->setIssueTrackerEnabled($argsObj->tprojectID,
                $argsObj->issue_tracker_enabled);
            $itMgr = new tlIssueTracker($tprojectMgr->db);
            if ($argsObj->issue_tracker_id > 0) {
                ...
                if (! is_null($issueT)) {
                    $itMgr->unlink($issueT['issuetracker_id'],
                        $issueT['testproject_id']);
                }
            }

            $tprojectMgr->setCodeTrackerEnabled($argsObj->tprojectID,
                $argsObj->code_tracker_enabled);
            $ctMgr = new tlCodeTracker($tprojectMgr->db);
            if ($argsObj->code_tracker_id > 0) {
                ...
                if (! is_null($codeT)) {
                    $ctMgr->unlink($codeT['codetracker_id'],
                        $codeT['testproject_id']);
                }
            }

            $tprojectMgr->setReqMgrIntegrationEnabled($argsObj->tprojectID,
                $argsObj->reqmgr_integration_enabled);
            $mgr = new tlReqMgrSystem($tprojectMgr->db);
            ...
            $event = new stdClass();
            $event->message = TLS("audit_testproject_saved",
                $argsObj->tprojectName);
    ...
    if ($op->status_ok) {
        ...
        $op->ui->caption = sprintf(lang_get('caption_edit_tproject'),
            $op->oldName);
    }
...
function edit(&$argsObj, &$tprojectMgr)
{
    ...
    $ui->caption = sprintf(lang_get('caption_edit_tproject'),
        $argsObj->tprojectName);
    return $ui;
}
...
function crossChecks($argsObj, &$tprojectMgr)
{
    ...
    if ($check_op['status_ok']) {
        if ($tprojectMgr->get_by_name($argsObj->tprojectName,
            $updateAdditionalSQLFilter)) {
            $check_op['msg'][] = sprintf(
                lang_get('error_product_name_duplicate'), $argsObj->tprojectName);
            $check_op['status_ok'] = 0;
        }

        // Check prefix no matter what has happen with previous check
        $rs = $tprojectMgr->get_by_prefix($argsObj->tcasePrefix,
            $updateAdditionalSQLFilter);
        if (! is_null($rs)) {
...
function doDelete($argsObj, &$tprojectMgr)
{
    ...
    if ($ope_status['status_ok']) {
        $op->reloadType = 'reloadNavBar';
        $op->msg = sprintf(lang_get('test_project_deleted'),
            $argsObj->tprojectName);
    } else {
        $op->msg = lang_get('info_product_not_deleted_check_log') . ' ' .
            $ope_status['msg'];
    }
...
{
    $guiObj = $argsObj;
    $guiObj->canManage = $argsObj->user->hasRight($dbHandler,
        "mgt_modify_product");
    $guiObj->found = 'yes';
