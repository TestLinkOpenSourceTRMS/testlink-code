...
class reqCommands
{
    ...
    public function __construct(&$db)
    {
        ...
        $this->reqRelationTypeDescr = init_labels(
     ...
     */
    public function initGuiBean($argsObj = null)
    {
        $obj = new stdClass();
        $obj->pageTitle = '';
        $obj->bodyOnLoad = '';
    ...
    public function create(&$argsObj)
    {
        ...
        $obj->main_descr = lang_get('req_spec_short') . TITLE_SEP .
            $req_spec['title'];
        $obj->action_descr = lang_get('create_req');

        $obj->cfields = $this->reqMgr->html_table_of_custom_field_inputs(null,
        ...
        if (defined('TL_REQ_TYPE_USE_CASE') &&
            isset($obj->reqTypeDomain[TL_REQ_TYPE_USE_CASE])) {
    ...
    public function edit(&$argsObj, $overwriteArgs = true)
    {
        ...
        $obj->main_descr = lang_get('req_short') . TITLE_SEP .
            $obj->req['req_doc_id'] . " (" . lang_get('version') . ' ' .
            $obj->req['version'] . " " . lang_get('revision') . ' ' .
            $obj->req['revision'] . ")" . TITLE_SEP . TITLE_SEP .
            $obj->req['title'];

        $obj->action_descr = lang_get('edit_req');

        $obj->cfields = $this->reqMgr->html_table_of_custom_field_inputs(
            $argsObj->req_id, $argsObj->req_version_id, $argsObj->tproject_id);
    ...
    public function doCreate(&$argsObj, $request)
    {
        ...
        $obj->action_descr = lang_get('create_req');
        $obj->cfields = $this->reqMgr->html_table_of_custom_field_inputs(null,
            null, $argsObj->tproject_id, null, $request);
        ...
        $ret = $this->reqMgr->create($argsObj->req_spec_id, $argsObj->reqDocId,
            $argsObj->title, $argsObj->scope, $argsObj->user_id,
            $argsObj->reqStatus, $argsObj->reqType, $argsObj->expected_coverage,
            $order);

        $obj->user_feedback = $ret['msg'];
        if ($ret['status_ok']) {
            logAuditEvent(TLS("audit_requirement_created", $argsObj->reqDocId),
                "CREATE", $ret['id'], "requirements");
            $obj->user_feedback = sprintf(lang_get('req_created'),
                $argsObj->reqDocId, $argsObj->title);

            $cf_map = $this->reqMgr->get_linked_cfields(null, null,
                $argsObj->tproject_id);
    ...
    public function doUpdate(&$argsObj, $request)
    {
        ...
        $oldData = $this->reqMgr->get_by_id($argsObj->req_id,
            $argsObj->req_version_id);
        $oldCFields = $this->reqMgr->get_linked_cfields(null,
            $argsObj->req_version_id, $argsObj->tproject_id);

        $cf_map = $this->reqMgr->get_linked_cfields(null, null,
            $argsObj->tproject_id);
        $newCFields = $this->reqMgr->cfield_mgr->_build_cfield($request, $cf_map);

        $diff = $this->simpleCompare($oldData[0], $argsObj, $oldCFields,
            $newCFields);

        $obj = $this->edit($argsObj, ! self::OVERWRITESCOPE);
        $obj->user_feedback = '';
        $obj->template = null;
        $obj->suggest_revision = false;
        ...
        if ($diff['force'] && ! $argsObj->do_save) {
            ...
            $obj->cfields = $this->reqMgr->html_table_of_custom_field_inputs(
                null, null, $argsObj->tproject_id, null, $request);
        } elseif ($diff['nochange'] ||
            (($createRev = $diff['force'] && ! $obj->prompt_for_log) ||
            $argsObj->do_save)) {
            if ($argsObj->do_save == 1) {
                $createRev = ($argsObj->save_rev == 1);
            }

            $ret = $this->reqMgr->update($argsObj->req_id,
                $argsObj->req_version_id, trim($argsObj->reqDocId),
                $argsObj->title, $argsObj->scope, $argsObj->user_id,
                $argsObj->reqStatus, $argsObj->reqType,
                $argsObj->expected_coverage, null, null, 0, $createRev,
                $argsObj->log_message);
            ...
            if ($ret['status_ok']) {
                ...
                $this->reqMgr->values_to_db($request, $argsObj->req_version_id,
                    $cf_map);

                logAuditEvent(
                    TLS("audit_requirement_saved", $argsObj->reqDocId), "SAVE",
                    $argsObj->req_id, "requirements");
                ...
                $obj->cfields = $this->reqMgr->html_table_of_custom_field_values(
                    $argsObj->req_id, $argsObj->req_version_id,
                    $argsObj->tproject_id);
            }
        } elseif ($diff['suggest']) {
    ...
    /**
     */
    public function doDelete(&$argsObj)
    {
        $obj = $this->initGuiBean();
        $obj->display_path = false;
        ...
        logAuditEvent(TLS("audit_requirement_deleted", $req['req_doc_id']),
            "DELETE", $argsObj->req_id, "requirements");

        $obj->template = 'show_message.tpl';
        $obj->template_dir = '';
        $obj->user_feedback = sprintf(lang_get('req_deleted'),
    ...
    /**
     */
    public function doUnfreezeVersion(&$argsObj)
    {
        $obj = $this->initGuiBean();
        $node = $this->reqMgr->tree_mgr->get_node_hierarchy_info(
            $argsObj->req_version_id);
        $req_version = $this->reqMgr->get_by_id($node['parent_id'],
            $argsObj->req_version_id);
        $req_version = $req_version[0];

        $this->reqMgr->updateOpen($req_version['version_id'], true);
        logAuditEvent(
            TLS("audit_req_version_unfrozen", $req_version['version'],
                $req_version['req_doc_id'], $req_version['title']), "UNFREEZE",
            $argsObj->req_version_id, "req_version");

        $obj->template = 'show_message.tpl';
        $obj->template_dir = '';

        $obj->user_feedback = sprintf(lang_get('req_version_unfrozen'),
            $req_version['req_doc_id'], $req_version['title'],
            $req_version['version']);

        $obj->main_descr = lang_get('requirement') . TITLE_SEP .
            $req_version['title'];
    ...
    /**
     */
    private function reorder(&$argsObj)
    {
        $obj = $this->initGuiBean();
    ...
    /**
     */
    public function doReorder(&$argsObj)
    {
        $obj = $this->initGuiBean();
    ...
    /**
     */
    private function createTestCases(&$argsObj)
    {
        $guiObj = $this->initGuiBean();
        $guiObj->template = 'reqCreateTestCases.tpl';
        $req_spec = $this->reqSpecMgr->get_by_id($argsObj->req_spec_id);
        $guiObj->main_descr = lang_get('req_spec_short') . TITLE_SEP .
            $req_spec['title'];
        ...
        $guiObj->all_reqs = $this->reqSpecMgr->get_requirements(
            $argsObj->req_spec_id);

        foreach ($guiObj->all_reqs as $key => $req) {
            $count = count($this->reqMgr->get_coverage($req['id']));
            $guiObj->all_reqs[$key]['coverage_percent'] = round(
                100 / $guiObj->all_reqs[$key]['expected_coverage'] * $count, 2);
    ...
    /**
     */
    public function doCreateTestCases(&$argsObj)
    {
        $this->initGuiBean();
        $this->createTestCases($argsObj);
        $msg = $this->reqMgr->create_tc_from_requirement($argsObj->arrReqIds,
            $argsObj->req_spec_id, $argsObj->user_id, $argsObj->tproject_id,
    ...
    /**
     */
    public function doCopy(&$argsObj)
    {
        $this->initGuiBean();

        $this->reqSpecMgr->get_by_id($argsObj->containerID);
     ...
     * doCreateVersion
     */
    public function doCreateVersion(&$argsObj)
    {
    ...
    /**
     */
    public function doDeleteVersion(&$argsObj)
    {
        $obj = $this->initGuiBean();
        $node = $this->reqMgr->tree_mgr->get_node_hierarchy_info(
            $argsObj->req_version_id);
        ...
        logAuditEvent(
            TLS("audit_req_version_deleted", $req_version['version'],
                $req_version['req_doc_id'], $req_version['title']), "DELETE",
            $argsObj->req_version_id, "req_version");

        $obj->template = 'show_message.tpl';
        $obj->template_dir = '';

        $obj->user_feedback = sprintf(lang_get('req_version_deleted'),
            $req_version['req_doc_id'], $req_version['title'],
            $req_version['version']);
    ...
    public function doAddRelation($argsObj)
    {
        ...
        $other_req = $this->reqMgr->getByDocID(
            $argsObj->relation_destination_req_doc_id, $tproject_id);
        if (empty($other_req)) {
        ...
        if ($op['ok']) {
            ...
            if (! is_numeric($authorID) || ! is_numeric($source_id) ||
                ! is_numeric($destination_id)) {
                $op['ok'] = false;
        ...
        if ($op['ok']) {
            $exists = $this->reqMgr->check_if_relation_exists($source_id,
                $destination_id, $relTypeID);
            if ($exists) {
                $op['ok'] = false;
        ...
        if ($op['ok']) {
            $this->reqMgr->add_relation($source_id, $destination_id, $relTypeID,
                $authorID);
        }

        $obj = $this->initGuiBean();
        $op['msg'] = ($op['ok'] ? '<div class="info">' : '<div class="error">') .
            $op['msg'] . '</div>';
        $obj->template = "reqView.php?requirement_id={$own_id}&relation_add_result_msg=" .
            $op['msg'];
    ...
    public function doDeleteRelation($argsObj)
    {
        ...
        if (is_null($relation_id) || ! is_numeric($relation_id) ||
            ...
            $op['ok'] = false;
            $op['msg'] = '<div class="error">' . lang_get('error_deleting_rel') .
                '</div>';
        }
     ...
     *
     */
    public function doCreateRevision(&$argsObj)
    {
        $req = $this->reqMgr->get_by_id($argsObj->req_id,
            $argsObj->req_version_id);
        $req = $req[0];
        $ret = $this->reqMgr->create_new_revision($argsObj->req_version_id,
            $argsObj->user_id, $argsObj->tproject_id, $req,
            $argsObj->log_message);
    ...
    /**
     */
    public function doFreezeVersion(&$argsObj)
    {
        $obj = $this->initGuiBean();
        $node = $this->reqMgr->tree_mgr->get_node_hierarchy_info(
            $argsObj->req_version_id);
        ...
        // BUGID 3312
        logAuditEvent(
            TLS("audit_req_version_frozen", $req_version['version'],
                $req_version['req_doc_id'], $req_version['title']), "FREEZE",
            $argsObj->req_version_id, "req_version");

        $obj->template = 'show_message.tpl';
        $obj->template_dir = '';

        $obj->user_feedback = sprintf(lang_get('req_version_frozen'),
            $req_version['req_doc_id'], $req_version['title'],
            $req_version['version']);
    ...
    /**
     */
    public function addTestCase(&$argsObj)
    {
        $obj = $this->initGuiBean();
        $node = $this->reqMgr->tree_mgr->get_node_hierarchy_info(
            $argsObj->req_version_id);
        $dummy = $this->reqMgr->get_by_id($node['parent_id'],
            $argsObj->req_version_id);
        $req_version = $dummy[0];
        ...
        $status_ok = false;
        $msg = sprintf(lang_get('provide_full_external_tcase_id'),
            $argsObj->tcasePrefix, $tcaseCfg->glue_character);
        ...
        if ($isFullExternal) {
            ...
            if (! $status_ok) {
                $msg = sprintf(lang_get('seems_to_belong_to_other_tproject'),
                    $rawTestCasePrefix, $argsObj->tcasePrefix);
            }
        }
            ...
            if ($tcase_id > 0) {
                ...
                if ($tcaseCfg->reqLinkingDisabledAfterExec) {
                    if ($alienMgr->latestVersionHasBeenExecuted($tcase_id) == 0) {
                        ...
                        $msg = sprintf(
                            lang_get(
                                'cannot_link_latest_version_reason_has_been_exec'),
                            $argsObj->tcaseIdentity);
                    }
                }
                if ($doLink) {
                    $this->reqMgr->assign_to_tcase($argsObj->req_id, $tcase_id,
                        intval($argsObj->user_id));
                }
            } else {
                $status_ok = false;
                $msg = sprintf(lang_get('tcase_doesnot_exist'),
                    $argsObj->tcaseIdentity);
            }
        }
    ...
    /**
     */
    public function removeTestCase(&$argsObj)
    {
        // IMPORTANT NOTICE: audit info is managed on reqMgr method
        $obj = $this->initGuiBean();
        $bond = array(
    ...
    public function fileUpload(&$argsObj)
    {
        $argsObj->uploadOp = fileUploadManagement($this->db,
            $argsObj->req_version_id, $argsObj->fileTitle,
    ...
    /**
     */
    public function deleteFile(&$argsObj)
    {
    ...
    /**
     */
    public function stopMonitoring(&$argsObj)
    {
        $this->reqMgr->monitorOff($argsObj->req_id, $argsObj->user_id,
            $argsObj->tproject_id);
    ...
    /**
     */
    public function startMonitoring(&$argsObj)
    {
        $this->reqMgr->monitorOn($argsObj->req_id, $argsObj->user_id,
            $argsObj->tproject_id);
