...
if ($args->compare_selected_versions) {
    // Side By Side
    $sbs = getItemsToCompare($args->left_item_id, $args->right_item_id,
        $gui->items);
    ...
    foreach ($gui->diff as $key => $val) {
        if ($args->use_daisydiff) {
            ...
            $gui->diff[$key]["right"] = explode("\n",
                str_replace("</p>", "</p>\n", $sbs['right_item'][$key]));
            $gui->diff[$key]["diff"] = $differ->inline($gui->diff[$key]["left"],
                $gui->leftID, $gui->diff[$key]["right"], $gui->rightID,
                $args->context);
        ...
        $gui->diff[$key]["message"] = (sprintf($labels[$msg_key], $key,
            $additional));
    }
}
...
function getCFToCompare($sides, $tprojectID, &$reqMgr)
{
    ...
    foreach ($cfields as $item_side => $dummy) {
        ...
        $cfields[$item_side]['value'] = $reqMgr->get_linked_cfields(null,
            $target_id, $tprojectID);
    }
    return $cfields;
}
...
function getCFDiff($cfields, &$reqMgr)
{
    ...
    if (! is_null($cfieldsLeft)) {
        ...
        foreach ($key2loop as $cf_key) {
            ...
            if ($cfCfg->show_custom_fields_without_value ||
                (! $cfCfg->show_custom_fields_without_value &&
                ((! is_null($cfieldsRight) &&
...
function initArgs()
{
    ...
    $args->req_id = isset($_REQUEST['requirement_id']) ? intval(
        $_REQUEST['requirement_id']) : 0;

    $args->compare_selected_versions = isset(
        $_REQUEST['compare_selected_versions']);
    ...
    if (! isset($_REQUEST['context_show_all'])) {
        $args->context = (isset($_REQUEST['context']) &&
            is_numeric($_REQUEST['context'])) ? $_REQUEST['context'] : $diffEngineCfg->context;
    }
...
function initializeGui(&$dbHandler, &$argsObj, $lbl, &$reqMgr)
{
    ...
    if ($reqCfg->log_message_len > 0) {
        ...
        for ($idx = 0; $idx < $loop2do; $idx ++) {
            if (strlen($guiObj->items[$idx]['log_message']) >
                $reqCfg->log_message_len) {
                $guiObj->items[$idx]['log_message'] = substr(
                    $guiObj->items[$idx]['log_message'], 0,
                    $reqCfg->log_message_len) . '...';
            }
            $guiObj->items[$idx]['log_message'] = htmlspecialchars(
                $guiObj->items[$idx]['log_message']);
        }
    }
...
{
    $guiObj->leftID = sprintf($labels['version_revision'],
        $sbs['left_item']['version'], $sbs['left_item']['revision']);
    $guiObj->rightID = sprintf($labels['version_revision'],
    ...
    $guiObj->subtitle = sprintf($labels['diff_details_req'],
        ...
        $sbs['right_item']['version'], $sbs['right_item']['revision'],
        $sbs['right_item']['version'], $sbs['right_item']['revision'],
        $mini_me['req_doc_id'] . config_get('gui_title_separator_1') .
        $mini_me['name']);
}
