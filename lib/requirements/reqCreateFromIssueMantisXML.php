...
switch($args->doAction)
{
  case 'uploadFile':
    ...
    if(array_key_exists("syntaxError", $gui->userFeedback) && !empty($gui->userFeedback['syntaxError']))
    {
...
function doExecuteImport($fileName,&$argsObj,&$reqSpecMgr,&$reqMgr)
{
  ...
  {
    if (move_uploaded_file($source, $fileName) && strcasecmp($argsObj->importType,'XML') == 0 )
    {
