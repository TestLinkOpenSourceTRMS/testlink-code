...
function initArgs()
{
    ...
    $argsObj->tproject_id = isset($_SESSION['testprojectID']) ? intval(
...
function doExecuteImport($fileName, &$argsObj, &$reqSpecMgr, &$reqMgr)
{
    ...
    if (($source != 'none') && ($source != '')) {
        if (move_uploaded_file($source, $fileName) &&
            strcasecmp($argsObj->importType, 'XML') == 0) {
...
function doReqImportFromMantisXML(&$reqMgr, &$simpleXMLObj, $importContext)
{
    ...
    for ($kdx = 0; $kdx < $loop2do; $kdx ++) {
        $dummy = $reqMgr->createFromMap($inputItems[$kdx],
            $importContext->tproject_id, $importContext->req_spec_id,
            $importContext->user_id);
