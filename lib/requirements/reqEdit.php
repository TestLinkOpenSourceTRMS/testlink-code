...
$args = initArgs($db);
$gui = initializeGui($db, $args, $commandMgr);
...
function initArgs(&$dbHandler)
{
    ...
    if ($args->tproject_id <= 0) {
        throw new Exception(
            __FILE__ . '::' . __FUNCTION__ . " Test project ID can not be <= 0 ");
    }
    ...
    if (is_null($info)) {
        throw new Exception(
            __FILE__ . '::' . __FUNCTION__ . " Unable to get test project data ");
    }
    ...
    &$dbHandler)
{
    ...
    $actionOpe = array(
        ...
        'doDeleteVersion' => '',
        'doFreezeVersion' => 'doFreezeVersion',
        'doAddRelation' => 'doAddRelation',
        'doDeleteRelation' => 'doDeleteRelation',
    ...
    switch ($argsObj->doAction) {
        ...
        default:
            if ($opObj->suggest_revision || $opObj->prompt_for_log) {
                ...
                $owebEditor->Value = getItemTemplateContents(
                    'requirement_template', $owebEditor->InstanceName,
                    $argsObj->scope);
            }
            break;
    }
...
function initializeGui(&$dbHandler, &$argsObj, &$commandMgr)
{
    ...
    if ($argsObj->req_spec_id) {
        $gui->requirements_count = $req_spec_mgr->get_requirements_count(
            $gui->req_spec_id);
    ...
    if (isset($gui->req_spec)) {
        $gui->main_descr .= config_get('gui_title_separator_1') .
            $gui->req_spec['title'];
    }
    $gui->action_descr = null;

    $gui->grants = new stdClass();
    $gui->grants->req_mgmt = $argsObj->user->hasRightOnProj($dbHandler,
        "mgt_modify_req");
    $gui->grants->mgt_view_events = $argsObj->user->hasRightOnProj($dbHandler,
        "mgt_view_events");
