...
function doExport(&$argsObj, &$req_spec_mgr)
{
    ...
    switch ($argsObj->exportType) {
        case 'csv':
            $requirements_map = $req_spec_mgr->get_requirements(
                $argsObj->req_spec_id);
        ...
        case 'XML':
            ...
            switch ($argsObj->scope) {
                case 'tree':
                    $reqSpecSet = $req_spec_mgr->getFirstLevelInTestProject(
                        $argsObj->tproject_id);
            ...
            if (! is_null($reqSpecSet)) {
                foreach ($reqSpecSet as $reqSpecID) {
                    $content .= $req_spec_mgr->$pfn($reqSpecID,
                        $argsObj->tproject_id, $optionsForExport);
                }
            }
            $content .= "</" . $openTag . "\n";
            break;
