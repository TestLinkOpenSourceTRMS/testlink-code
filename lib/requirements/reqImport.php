...
function doExecuteImport($fileName, &$argsObj, &$reqSpecMgr, &$reqMgr)
{
    ...
    if (($source != 'none') && ($source != '')) {
        if (move_uploaded_file($source, $fileName) &&
            $argsObj->importType == 'XML') {
            $retval->file_check['status_ok'] = (($xml = simplexml_load_file_wrapper(
                $fileName)) !== false);
            if (! $retval->file_check['status_ok']) {
                $retval->file_check['msg'] = lang_get(
                    'import_failed_xml_load_failed');
            }
        }
    } else {
        $retval->file_check = array(
...
function initArgs()
{
    ...
    $args->req_spec_id = isset($request['req_spec_id']) ? intval(
        $request['req_spec_id']) : null;
...
function initializeGui(&$dbHandler, &$argsObj, $session, &$reqSpecMgr, &$reqMgr)
{
    ...
    switch ($gui->scope) {
        case 'tree':
            $gui->main_descr = sprintf(lang_get('tproject_import_req_spec'),
        ...
        case 'branch':
            ...
            $gui->main_descr = sprintf(lang_get('reqspec_import_req_spec'),
                $gui->req_spec['title']);
            $gui->importTypes = $reqSpecMgr->get_import_file_types();
            break;
        ...
        case 'items':
            $gui->req_spec = $reqSpecMgr->get_by_id($argsObj->req_spec_id);
            $gui->main_descr = sprintf(lang_get('reqspec_import_requirements'),
                $gui->req_spec['title']);
    ...
    $gui->importFileGui->fileSizeLimitMsg = sprintf(
        lang_get('max_file_size_is'), $gui->importFileGui->maxFileSize . ' KB ');
    ...
    $importContext, $importOptions)
{
    ...
    if ($isReqSpec) {
        foreach ($simpleXMLObj->req_spec as $xkm) {
            $dummy = $reqSpecMgr->createFromXML($xkm,
        ...
        for ($kdx = 0; $kdx < $loop2do; $kdx ++) {
            $dummy = $reqMgr->createFromXML($simpleXMLObj->requirement[$kdx],
                $importContext->tproject_id, $importContext->req_spec_id,
                $importContext->user_id, null, $importOptions);
            $items = array_merge($items, $dummy);
        }
