...
if(count($gui->reqIDs) > 0) 
{
  ...
  foreach($gui->reqIDs as $id) 
  {
    ...
    * 4. subscribed
    ...
    if($isReqSubscribed) {
		$result[] = "<!--subscribed--><form method=\"POST\" action=\"lib/requirements/reqManageSubs.php\"><input type=\"submit\" name=\"subscribe\" value=\"".lang_get("btn_unsubscribe")."\" /><input type=\"text\" name=\"reqId\" value=\"{$version["id"]}\" style=\"display:none\" /></form>";
	  }
	  else {
		$result[] = "<!--subscribed--><form method=\"POST\" action=\"lib/requirements/reqManageSubs.php\"><input type=\"submit\" name=\"subscribe\" value=\"".lang_get("btn_subscribe")."\" /><input type=\"text\" name=\"reqId\" value=\"{$version["id"]}\" style=\"display:none\" /></form>";
	  }
