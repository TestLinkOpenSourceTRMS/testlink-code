...
if (! empty($gui->reqIDSet)) {
    ...
    foreach ($gui->reqIDSet as $id) {
        ...
        $title = htmlentities($req['req_doc_id'], ENT_QUOTES, $cfg->charset) .
            $cfg->glue_char .
        ...
        if (! isset($pathCache[$req['srs_id']])) {
            ...
            $pathCache[$req['srs_id']] = htmlentities(implode("/", $path),
        ...
        $edit_link = '<a href="javascript:openLinkedReqVersionWindow(' . $id .
            ',' . $req['version_id'] . ')">' . '<img title="' .
            $lbl['mixed']['requirement'] . '" src="' . $imgSet['edit'] .
            '" /></a> ';
        ...
        $result[] = "<!--{$req['creation_ts']}-->" .
            localizeTimeStamp($req['creation_ts'], $cfg->datetime) .
            " ({$req['author']})";
...
function initializeGui(&$argsObj, &$tprojectMgr)
{
    ...
    $gui->tableSet = null;
    $gui->reqIDSet = $tprojectMgr->get_all_requirement_ids(
        $argsObj->tproject_id);
