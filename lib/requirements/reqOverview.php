...
if (count($gui->reqIDs) > 0) {
    ...
    //
    if ($cfg->req->expected_coverage_management || $gui->processCF) {
    ...
    foreach ($gui->reqIDs as $id) {
        ...
        if (! isset($pathCache[$req[0]['srs_id']])) {
            ...
            $pathCache[$req[0]['srs_id']] = htmlentities(implode("/", $path),
                ENT_QUOTES, $cfg->charset);
        }
        ...
        foreach ($req as $version) {
            ...
            $edit_link = '<a href="javascript:openLinkedReqVersionWindow(' . $id .
                ',' . $version['version_id'] . ')">' . '<img title="' .
                $labels['requirement'] . '" src="' . $imgSet['edit'] .
                '" /></a> ';
            ...
            //
            // use html comment to sort properly by this column (extjs)
            if (! is_null($version['modification_ts']) &&
                ($version['modification_ts'] != $cfg->neverModifiedTS)) {
                $result[] = "<!--{$version['modification_ts']}-->" .
                    localizeTimeStamp($version['modification_ts'],
                        $cfg->datetime) . " ({$version['modifier']})";
            } else {
                ...
                foreach ($cfield_ids as $cf_id) {
                    if (isset($linkedCFWithContent[$cf_id])) {
                        ...
                        if (($verbose_type == 'date' ||
                            $verbose_type == 'datetime') && is_numeric($value) &&
                            $value != 0) {
                            $value = @strftime(
                                $cfg->$verbose_type .
                                " ({$labels['week_short']} %W)", $value); # fix typo: missing 's' in labels
                        }
                        $result[] = $value;
                    } else {
                        $result[] = '';
