...
$gui->pageTitle = $gui->main_descr . " - " . lang_get('match_count') . ": " .
    $gui->row_qty;
...
function buildExtTable($gui, $charset)
{
    ...
    if (count($gui->resultSet) > 0) {
        ...
        $reqVerHref = '<a href="javascript:openLinkedReqVersionWindow(%s,%s)">' .
            $labels['version_revision_tag'] . ' </a>';
        // req_revision_id
        $reqRevHref = '<a href="javascript:openReqRevisionWindow(%s)">' .
            $labels['version_revision_tag'] . ' </a>';
        ...
        foreach ($key2loop as $req_id) {
            ...
            $rowData[] = htmlentities($gui->path_info[$rfx['id']], ENT_QUOTES,
                $charset);

            $edit_link = "<a href=\"javascript:openLinkedReqWindow(" . $rfx['id'] .
                ")\">" . "{$img}</a> ";
            ...
            foreach ($itemSet as $rx) {
                if ($rx['revision_id'] > 0) {
                    $dummy = sprintf($reqRevHref, $rx['revision_id'],
                        $rx['version'], $rx['revision']);
                } else {
                    $dummy = sprintf($reqVerHref, $req_id, $rx['version_id'],
                        $rx['version'], $rx['revision']);
                }
                $matches .= $dummy;
            }
...
function initArgs($dateFormat)
{
    ...
    foreach ($strnull as $keyvar) {
        ...
        $args->$keyvar = ! is_null($args->$keyvar) && strlen($args->$keyvar) > 0 ? trim(
            $args->$keyvar) : null;
    }
    ...
    foreach ($dk as $tdk => $hhmmss) {
        if (isset($args->$tdk) && trim($args->$tdk) != '') {
            ...
            if ($l10ndate != null && is_array($l10ndate)) {
                ...
                $args->$tdk = $l10ndate['year'] . "-" . $l10ndate['month'] . "-" .
                    $l10ndate['day'] . $hhmmss;
            }
        }
    }
...
function build_search_sql(&$dbHandler, &$argsObj, &$guiObj)
{
    ...
    return $stm['ver'] .
        " UNION ({$sql4Union}) ORDER BY id ASC, version DESC, revision DESC ";
}
