...
class reqSpecCommands
{
    ...
    public function __construct(&$db, $tproject_id)
    {
        $this->db = $db;
    ...
    public function setAuditContext($auditContext)
    {
        $this->auditContext = $auditContext;
    }

    public function getReqMgrSystem()
    {
     ...
     */
    public function initGuiBean($options = null)
    {
        $obj = new stdClass();
        $obj->pageTitle = '';
        $obj->bodyOnLoad = '';
        ...
        if (! is_null($options) && isset($options['getReqSpec'])) {
            ...
            $obj->req_spec = $this->reqSpecMgr->get_by_id($ref['id'],
                $ref['options']);
        }
    ...
    public function create(&$argsObj)
    {
        ...
        $guiObj->cfields = $this->reqSpecMgr->html_table_of_custom_field_inputs(
    ...
    public function edit(&$argsObj, $request, $overwriteArgs = true)
    {
        ...
        $guiObj->cfields = $this->reqSpecMgr->html_table_of_custom_field_inputs(
            $argsObj->req_spec_id, $argsObj->req_spec_revision_id,
            $argsObj->tproject_id);
    ...
    public function doCreate(&$argsObj, $request)
    {
        ...
        $ret = $this->reqSpecMgr->create($argsObj->tproject_id,
            $argsObj->parentID, $argsObj->doc_id, $argsObj->title,
            $argsObj->scope, $argsObj->countReq, $argsObj->user_id,
            $argsObj->reqSpecType, $order);
        ...
        if ($ret['status_ok']) {
            ...
            $this->reqSpecMgr->values_to_db($request, $ret['revision_id'],
                $cf_map);
            logAuditEvent(
                TLS("audit_req_spec_created", $this->auditContext->tproject,
                    $argsObj->title), "CREATE", $ret['id'], "req_specs");
        } else {
    ...
    public function doUpdate(&$argsObj, $request)
    {
        ...
        $chk = $this->reqSpecMgr->check_main_data($argsObj->title,
            $argsObj->doc_id, $argsObj->tproject_id, $argsObj->parentID,
            $argsObj->req_spec_id);
        ...
        if ($chk['status_ok']) {
            ...
            $guiObj->cfields = $this->reqSpecMgr->html_table_of_custom_field_inputs(
                null, null, $argsObj->tproject_id, null, null, $request);
        }
    ...
    public function doDelete(&$argsObj)
    {
        ...
        logAuditEvent(
            TLS("audit_req_spec_deleted", $this->auditContext->tproject,
                $req_spec['title']), "DELETE", $argsObj->req_spec_id,
            "req_specs");
        ...
        $guiObj->user_feedback = sprintf(lang_get('req_spec_deleted'),
            $req_spec['title']);
     ...
     *
     */
    public function reorder(&$argsObj)
    {
        $guiObj = $this->initGuiBean();
        ...
        $guiObj->all_req_spec = $this->reqSpecMgr->get_all_in_testproject(
            $argsObj->tproject_id, $order_by);
     ...
     *
     */
    public function createChild(&$argsObj)
    {
     ...
     *
     */
    public function copyRequirements(&$argsObj, $options = null)
    {
     ...
     *
     */
    public function copy(&$argsObj, $options = null)
    {
        $obj = $this->initGuiBean();
        $req_spec = $this->reqSpecMgr->get_by_id($argsObj->req_spec_id);
    ...
    public function doCopy(&$argsObj)
    {
        ...
        $op = $this->reqSpecMgr->copy_to($argsObj->req_spec_id,
            $argsObj->containerID, $argsObj->tproject_id, $argsObj->user_id);

        if ($op['status_ok']) {
            $new_req_spec = $this->reqSpecMgr->get_by_id($op['id']);
            $obj->array_of_msg[] = sprintf(lang_get('req_spec_copy_done'),
    ...
    public function doFreeze(&$argsObj, $request)
    {
        ...
        if (! is_null($childNodes)) {
            ...
            for ($idx = 0; $idx < $loop_qty; $idx ++) {
                ...
                if ($cNode['node_table'] == 'req_specs') {
                    ...
                    $req = $this->reqMgr->get_by_id($cNode['id'],
                        requirement_mgr::LATEST_VERSION);
        ...
        $obj->main_descr = lang_get('req_spec') . TITLE_SEP .
            $req_spec_info['title'];
    ...
    /**
     */
    private function processRevision(&$guiObj, &$argsObj, &$userInput)
    {
        // TICKET 4661
        ...
        $diff = $this->simpleCompare($itemOnDB, $argsObj, $oldCFields,
            $newCFields);
        ...
        if ($diff['force'] && ! $argsObj->do_save) {
            ...
            $guiObj->cfields = $this->reqSpecMgr->html_table_of_custom_field_inputs(
                null, null, $argsObj->tproject_id, null, null, $userInput);
        } elseif ($diff['nochange'] ||
            (($createRev = $diff['force'] && ! $guiObj->askForLog) ||
            $argsObj->do_save)) {
            ...
            if ($ret['status_ok']) {
                ...
                $this->reqSpecMgr->values_to_db($userInput, $ret['revision_id'],
                    $cf_map);
                ...
                $guiObj->cfields = $this->reqSpecMgr->html_table_of_custom_field_values(
                    $argsObj->req_spec_id, $argsObj->req_spec_revision_id,
                    $argsObj->tproject_id);
            }
        } elseif ($diff['suggest']) {
    ...
    /**
     */
    public function fileUpload(&$argsObj, $request)
    {
        $argsObj->uploadOp = fileUploadManagement($this->db,
            $argsObj->req_spec_id, $argsObj->fileTitle,
            $this->reqSpecMgr->getAttachmentTableName());
     ...
     *
     */
    public function bulkReqMon(&$argsObj, $options = null)
    {
        $obj = $this->initGuiBean();
        $req_spec = $this->reqSpecMgr->get_by_id($argsObj->req_spec_id);
    ...
    /**
     */
    public function doBulkReqMon(&$argsObj)
    {
        $obj = $this->initGuiBean();
        $obj->req = null;
        $obj->req_spec_id = $argsObj->req_spec_id;
        ...
        switch ($argsObj->op) {
            case 'toogleMon':
                ...
                foreach ($argsObj->itemSet as $req_id) {
                    ...
                    $this->reqMgr->$f2r($req_id, $argsObj->user_id,
                        $argsObj->tproject_id);
                }
                break;
        ...
        if (! is_null($m2r)) {
            foreach ($argsObj->itemSet as $req_id) {
                $this->reqMgr->$m2r($req_id, $argsObj->user_id,
                    $argsObj->tproject_id);
            }
        }
