...
$gui = initializeGui($db, $args, $req_cfg, $commandMgr);
...
function renderGui(&$argsObj, $guiObj, $opObj, $templateCfg, $editorCfg)
{
    ...
    switch ($argsObj->doAction) {
        ...
        default:
            if ($opObj->askForRevision || $opObj->askForLog ||
                ! $opObj->action_status_ok) {
                $owebEditor->Value = $argsObj->scope;
            } else {
                $owebEditor->Value = getItemTemplateContents(
                    'req_spec_template', $owebEditor->InstanceName,
                    $argsObj->scope);
            }
            break;
    }
    ...
    switch ($renderType) {
        case 'template':
            $smartyObj->assign('mgt_view_events',
                has_rights($db, "mgt_view_events"));
...
/**
 */
function initializeGui(&$dbHandler, &$argsObj, &$req_cfg, &$commandMgr)
{
