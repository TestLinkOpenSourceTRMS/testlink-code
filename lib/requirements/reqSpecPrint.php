...
$gui->page_title = sprintf(lang_get('print_requirement_specification'),
    $node['name']);
...
$text2print .= renderHTMLHeader($gui->page_title, $_SESSION['basehref'],
    SINGLE_REQSPEC) . '<body>';
$text2print .= renderReqSpecNodeForPrinting($db, $node, $options, null, 0);
...
if (! is_null($childrenReq) && $req_cfg->show_child_reqs_on_reqspec_print_view) {
    ...
    for ($rdx = 0; $rdx < $loop2do; $rdx ++) {
        $text2print .= renderReqForPrinting($db, $childrenReq[$rdx],
            $reqPrintOpts, 0, $args->tproject_id);
    }
}
...
function initArgs()
{
    ...
    $args = new stdClass();
    $args->reqspec_id = isset($_REQUEST['reqspec_id']) ? intval(
        $_REQUEST['reqspec_id']) : 0;
    $args->reqspec_revision_id = isset($_REQUEST['reqspec_revision_id']) ? intval(
        $_REQUEST['reqspec_revision_id']) : 0;
