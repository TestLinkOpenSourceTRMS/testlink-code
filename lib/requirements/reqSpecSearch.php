...
function buildExtTable($gui, $charset)
{
    ...
    if (! empty($gui->resultSet)) {
        ...
        foreach ($key2loop as $rspec_id) {
            ...
            $rfx = &$itemSet[0];
            $edit_link = "<a href=\"javascript:openLinkedReqSpecWindow(" .
                $rfx['id'] . ")\">" .
            ...
            $cm = '<a href="javascript:openReqSpecRevisionWindow(%s)" title="' .
                $labels['open_on_new_window'] . '" >' . $labels['revision_tag'] .
                ' </a>';
            $matches = '';
