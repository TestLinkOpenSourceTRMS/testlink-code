...
switch ($args->edit) {
    ...
    case 'testsuite':
        ...
        if ($bulkDone) {
            $gui->user_feedback = sprintf(lang_get('bulk_assigment_done'),
                $bulkCounter);
        }
        break;
...
function initArgs()
{
    ...
    } elseif (isset($_SESSION['currentSrsId']) &&
        intval($_SESSION['currentSrsId']) > 0) {
...
function processTestSuite(&$dbHandler, &$argsObj, &$guiObj)
{
    ...
    $tsuite_info = $tproject_mgr->tree_manager->get_node_hierarchy_info(
        $guiObj->tsuite_id);
    $guiObj->pageTitle = lang_get('test_suite') .
        config_get('gui_title_separator_1') . $tsuite_info['name'];

    $guiObj->req_specs = $tproject_mgr->genComboReqSpec($argsObj->tproject_id,
        'dotted', "&nbsp;");
    ...
    if (! is_null($guiObj->req_specs) && count($guiObj->req_specs)) {
        ...
        $guiObj->selectedReqSpecName = trim(
            $guiObj->req_specs[$guiObj->selectedReqSpec], '&nbsp;');
        ...
        $guiObj->reqCountFeedback = sprintf(lang_get('req_on_req_spec'),
            $guiObj->reqCountOnReqSpec, $guiObj->selectedReqSpecName);

        $tcase_set = getTargetTestCases($dbHandler, $argsObj);
        $guiObj->tcase_number = count($tcase_set);
        if ($guiObj->tcase_number > 0) {
            $guiObj->bulkassign_warning_msg = sprintf(
                lang_get('bulk_req_assign_msg'), $guiObj->tcase_number,
                $tsuite_info['name']);
        } else {
            $guiObj->bulkassign_warning_msg = lang_get(
                'bulk_req_assign_no_test_cases');
        }
    }
    return $guiObj;
}
...
function doBulkAssignment(&$dbHandler, &$argsObj, $targetTestCaseSet = null)
{
    ...
    if (! is_null($requirements) && $requirements !== []) {
        ...
        if (! is_null($tcase_set) && count($tcase_set)) {
            $assignmentCounter = $req_mgr->bulkAssignLatestREQVTCV(
                $requirements, $tcase_set, $argsObj->user->dbID);
        }
    }
    ...
    switch ($pfn) {
        case 'assign_to_tcase':
            foreach ($items as $idOneReq) {
                $res = $req_mgr->$pfn($idOneReq, $argsObj->id,
                    $argsObj->user->dbID);
                if (! $res) {
                    $msg .= $idOneReq . ', ';
                }
...
/**
 */
function arrayDiffById($arrAll, $arrPart)
{
...
function processTestCase(&$dbHandler, &$argsObj, &$guiObj)
{
    ...
    if ($srs_qty > 0) {
        ...
        if ($tcase) {
            ...
            $statusQuo = $tc_mgr->getVersionsStatusQuo($argsObj->id,
                $tcversion_id);
            ...
            if ($argsObj->idReqSpec) {
                ...
                $guiObj->allReq = $req_spec_mgr->get_requirements(
                    $argsObj->idReqSpec);

                $guiObj->unassignedReq = arrayDiffById($guiObj->allReq,
                    $guiObj->assignedReq);
            }
        }
    }
...
function initializeGui(&$dbH, $argsObj)
{
    ...
    $guiObj->req_tcase_link_management = $argsObj->user->hasRightOnProj($dbH,
