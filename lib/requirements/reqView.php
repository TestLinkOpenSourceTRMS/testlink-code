...
$args = initArgs($req_mgr);
$gui = initializeGui($db, $args, $tproject_mgr, $req_mgr);
...
/**
 */
function initializeGui(&$dbHandler, $argsObj, &$tproject_mgr, &$req_mgr)
{
    ...
    for ($cvx = 0; $cvx < $loop2do; $cvx ++) {
        ...
        if ($cvx == 0) {
            ...
            $gui->other_req_coverage[0][] = $req_mgr->getActiveForReqVersion(
                $bebe);
        }
    }
    ...
    if (count($gui->req_versions) > 1) {
        ...
        for ($qdx = 0; $qdx < $loop2do; $qdx ++) {
            ...
            $gui->cfields_other_versions[0][$qdx] = $req_mgr->html_table_of_custom_field_values(
                $gui->req_id, $target_version, $argsObj->tproject_id);
    ...
    if ($gui->showReqSpecTitle) {
        $gui->parent_descr = lang_get('req_spec_short') . $gui->pieceSep .
            $gui->req['req_spec_title'];
    }
    ...
    if ($gui->req_cfg->relations->enable) {
        ...
        if ($gui->req_cfg->relations->interproject_linking) {
            $gui->testproject_select = initTestprojectSelect($argsObj->userID,
                $argsObj->tproject_id, $tproject_mgr);
        }
    }
