 ...
 *
 * @param string $format
 *            identifier of document format; value must be in $tlCfg->reports_formats
 * @param integer $doc_kind
 *            Magic number of document kind; see consts.inc.php for list
...
function flushHttpHeader($format, $doc_kind = 0)
{
    ...
    $filename .= $kind_acronym . '-' . date('Y-m-d') . '.' .
    ...
    $contentType .= (is_null($format) || $format == '') ? '' : ("; name='Testlink_" .
        $format . "'");
