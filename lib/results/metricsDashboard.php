...
$labels = init_labels(
    array(
        ...
        'test_plan' => null,
        'progress' => null,
        'href_metrics_dashboard' => null,
        'progress_absolute' => null,
        'no_testplans_available' => null,
        'not_aplicable' => null,
        'platform' => null,
        'th_active_tc' => null,
        'in_percent' => null
...
list ($gui->tplan_metrics, $gui->show_platforms, $platforms) = getMetrics($db,
    $_SESSION['currentUser'], $args, $result_cfg, $labels);

if (! empty($gui->tplan_metrics)) {
    $statusSetForDisplay = $result_cfg['status_label_for_exec_ui'];
    $gui->warning_msg = '';
    $columns = getColumnsDefinition($gui->show_platforms, $statusSetForDisplay,
        $labels, $platforms);
            ...
            foreach ($tplan_metrics['platforms'] as $platform_metric) {
                ...
                foreach ($statusSetForDisplay as $status_verbose => $status_label) {
                    if (isset($platform_metric[$status_verbose])) {
                        ...
                        $rowData[] = getPercentage(
                            $platform_metric[$status_verbose],
                            $platform_metric['active'], $round_precision);
                    } else {
                        $rowData[] = 0;
                        $rowData[] = 0;
                    }
                ...
                $rowData[] = getPercentage($platform_metric['executed'],
                    $platform_metric['active'], $round_precision);
    ...
    $gui->project_metrics = collectTestProjectMetrics($gui->tplan_metrics,
        array(
            'statusSetForDisplay' => $statusSetForDisplay,
...
function getMetrics(&$db, $userObj, $args, $result_cfg, $labels)
{
    ...
    $test_plans = $userObj->getAccessibleTestPlans($db, $tproject_id, null,
        $options);
    ...
    foreach ($test_plans as $key => &$dummy) {
        ...
        if (! is_null($platformSet)) {
            $neurus = $metricsMgr->getExecCountersByPlatformExecStatus($key,
                null,
                array(
                    'getPlatformSet' => true,
            ...
            $mm[$key]['overall'] = $metricsMgr->getExecCountersByExecStatus(
                $key, null, array(
                    'getOnlyActiveTCVersions' => true
                ));
            ...
            foreach ($mm[$key]['overall'] as $status_code => $qty) {
                if ($status_code != 'not_run' && $status_code != 'total' &&
                    $status_code != 'active') {
...
{
    return ($numerator > 0) ? (round(($denominator / $numerator) * 100,
        $round_precision)) : 0;
}
...
function getColumnsDefinition($showPlatforms, $statusLbl, $labels, $platforms)
{
    ...
    foreach ($statusLbl as $lbl) {
        ...
        $colDef[] = array(
            'title' => lang_get($lbl) . " " . $labels['in_percent'],
            'width' => 40,
...
function collectTestProjectMetrics($tplanMetrics, $cfg)
{
    ...
    foreach ($cfg['statusSetForDisplay'] as $status_verbose => $label_key) {
        $mm[$status_verbose]['value'] = getPercentage(
            $tplanMetrics['total'][$status_verbose],
            $tplanMetrics['total']['active'], $cfg['round_precision']);
...
function checkRights(&$db, &$user, $context = null)
{
    ...
    foreach ($checkOrMode as $right) {
        if ($user->hasRightOnProj($db, $right, $context->tproject_id,
