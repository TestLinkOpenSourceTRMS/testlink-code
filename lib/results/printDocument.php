...
list ($args, $tproject_mgr, $decode) = initArgs($db);
...
$subtree = $tree_manager->get_subtree($args->itemID, $my['filters'],
    $my['options']);
...
switch ($doc_info->type) {
    ...
    case DOC_TEST_SPEC:
        ...
        switch ($doc_info->content_range) {
            case 'testsuite':
                ...
                $treeForPlatform[0]['childNodes'] = array(
                    $tInfo
                );
    ...
    case DOC_TEST_PLAN_EXECUTION_ON_BUILD:
        ...
        if ($args->build_id > 0) {
            $xx = $tplan_mgr->get_builds($args->tplan_id, null, null,
                array(
                    'buildID' => $args->build_id
                ));
            $doc_info->build_name = htmlspecialchars(
                $xx[$args->build_id]['name']);
        ...
        $items2use = (object) array(
            'estimatedExecTime' => null,
            'realExecTime' => null
        );
        ...
        switch ($doc_info->content_range) {
            case 'testproject':
                $treeForPlatform = buildContentForTestPlan($db, $subtree, $ctx,
                    $tplan_mgr, $filters, $opx);
                break;

            case 'testsuite':
                $ctx->branchRoot = $args->itemID;
                $opx = array_merge((array) $opx,
                    (array) $my['options']['prepareNode']);
                list ($treeForPlatform, $items2use) = buildContentForTestPlanBranch(
                    $db, $subtree, $ctx, $doc_info, $decode, $tplan_mgr, $opx);
                break;
        }
        ...
        if ($printingOptions['metrics']) {
            ...
            $doc_data->statistics = timeStatistics($items2use, $target, $decode,
                $tplan_mgr);
        }
        break;
}
...
// ----- rendering logic -----
$topText = renderHTMLHeader($doc_info->type . ' ' . $doc_info->title,
    $_SESSION['basehref'], $doc_info->type);
...
if ($treeForPlatform) {
    ...
    switch ($doc_info->type) {
        ...
        case DOC_TEST_PLAN_EXECUTION_ON_BUILD:
            ...
            if ($printingOptions['cfields']) {
                $cfields = $tplan_mgr->html_table_of_custom_field_values(
                    $args->tplan_id, 'design', null, $cfieldFormatting);
                $docText .= '<p>' . $cfields . '</p>';
            }
            break;
    }
    ...
    foreach ($treeForPlatform as $platform_id => $tree2work) {
        ...
        if (isset($tree2work['childNodes']) &&
            ...
            switch ($doc_info->type) {
                case DOC_REQ_SPEC:
                    $docText .= renderReqSpecTreeForPrinting($db, $tree2work,
                        $printingOptions, null, 0, 1, $args->user_id, 0,
                        $args->tproject_id);
                    break;

                case DOC_TEST_SPEC:
                    $docText .= renderSimpleChapter(lang_get('scope'),
                    ...
                    $docText .= renderTestSpecTreeForPrinting($db, $tree2work,
                        $printingOptions, $env, $actionContext, $env->tocPrefix,
                        $indentLevelStart);
                    break;
                ...
                case DOC_TEST_PLAN_EXECUTION_ON_BUILD:
                    ...
                    if ($showPlatforms) {
                        ...
                        $docText .= renderPlatformHeading($tocPrefix,
                            $platforms[$platform_id], $printingOptions);
                    }

                    $actionContext['level'] = 0;
                    $docText .= renderTestPlanForPrinting($db, $tree2work,
...
function initArgs(&$dbHandler)
{
    ...
    if (! is_null($args->apikey)) {
        ...
        $args->tplan_id = isset($_REQUEST['docTestPlanId']) ? intval(
    ...
    if ($args->tproject_id > 0) {
        ...
        $msg = __FILE__ . '::' . __FUNCTION__ .
            " :: Invalid Test Project ID ({$args->tproject_id})";
...
function initEnv(&$dbHandler, &$argsObj, &$tprojectMgr, $userID)
{
    ...
    $doc->additional_info = $argsObj->with_user_assignment ? lang_get(
        'only_test_cases_wta') : '';
    $doc->author = '';
    $doc->title = '';
    ...
    if (is_null($items2use)) {
        ...
        foreach ($items2use as $platID => $itemsForPlat) {
            if (! is_null($itemsForPlat)) {
                $tmp = $tplanMgr->get_estimated_execution_time($tplanID,
                    $itemsForPlat, $platID);
...
function getStatsRealExecTime(&$tplanMgr, &$lastExecBy, $context, $decode)
{
    ...
    if (! is_null($lastExecBy) && ! empty($lastExecBy)) {
        ...
        foreach ($p2loop as $platfID) {
            if (! is_null($lastExecBy[$platfID])) {
                ...
                foreach ($i2loop as $xdx) {
                    ...
                    if ($info['exec_status'] !=
                        $decode['status_descr_code']['not_run']) {
...
/**
 */
function buildContentForTestPlan(&$dbHandler, $itemsTree, $ctx, &$tplanMgr,
    $pnFilters = null, $opt = null)
{
    ...
    foreach ($platformIDSet as $platform_id) {
        ...
        $linkedBy[$platform_id] = $tplanMgr->getLinkedStaticView($tplanID,
            $filters);
        ...
        prepareNode($dbHandler, $tree2work, $dummy4reference, $dummy4reference,
...
/**
 */
function buildContentForTestPlanBranch(&$dbHandler, $itemsTree, $ctx, &$docInfo,
    $decode, &$tplanMgr, $options = null)
{
    $linkedBy = array();
    $branch_tsuites = null;
    $contentByPlatform = array();
    ...
    $docInfo->title = htmlspecialchars(
        isset($tInfo['name']) ? $tInfo['name'] : $docInfo->testplan_name);
    ...
    foreach ($platformIDSet as $platform_id) {
        ...
        $avalon = $tplanMgr->getLTCVNewGeneration($tplanID, $filters,
            $getLTCVOpt);
        if (! is_null($avalon)) {
...
{
    $stats = array();
    $stats['estimated_execution'] = getStatsEstimatedExecTime($tplanMgr,
        $items->estimatedExecTime, $context->tplan_id);

    $stats['real_execution'] = getStatsRealExecTime($tplanMgr,
        $items->realExecTime, $context, $decode);
    return $stats;
}
