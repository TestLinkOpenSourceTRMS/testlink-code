...
foreach ($execSet as $execution) {
    ...
    $bug_urls = buildBugString($db, $execution['exec_id'], $its, $openBugs,
        $resolvedBugs);
    if ($bug_urls) {
        ...
        if (! isset($testcase_bugs[$tc_id])) {
            ...
            $exec_history_link = "<a href=\"javascript:openExecHistoryWindow({$tc_id});\">" .
                "<img title=\"" . $l18n['execution_history'] .
                "\" src=\"{$img['history']}\" /></a> ";
            $edit_link = "<a href=\"javascript:openTCEditWindow({$tc_id});\">" .
                "<img title=\"" . $l18n['design'] .
                "\" src=\"{$img['edit']}\" /></a> ";
 ...
 *
 * @param database $db
 * @param integer $execID
 *            execution id
 * @param array $openBugsArray
 *            array to count open bugs
 * @param array $resolvedBugsArray
 *            array to count resolved bugs
