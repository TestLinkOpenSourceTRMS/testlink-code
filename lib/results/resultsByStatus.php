...
if (! is_null($metrics) && ! empty($metrics)) {
    ...
    foreach ($metrics as $execID => &$exec) {
        ...
        switch ($args->format) {
            case FORMAT_HTML:
                $out[$odx]['testTitle'] = "<!-- " .
                    sprintf("%010d", $exec['external_id']) . " -->";
                $zipper = '';
                if ($args->addOpAccess) {
                    $out[$odx]['testTitle'] .= sprintf($links['full'],
                        $exec['tcase_id'], $exec['tcase_id'],
                        $exec['tcversion_id'], $exec['build_id'],
                        $args->tplan_id, $exec['platform_id'], $exec['tcase_id']);
                    $zipper = '</a>';
                }
                break;
...
displayReport($tplCfg->template_dir . $tplCfg->default_template, $smarty,
    $args->format, $gui->mailCfg);
...
function initializeGui(&$dbh, &$argsObj, &$tplanMgr)
{
    ...
    foreach ($reportCfg as $key => $val) {
        ...
        if ($checkIt = (strpos($key, $needle) !== false)) {
            ...
            if (isset($reportCfg[$key]['misc']) &&
                ...
                ! $reportCfg[$key]['misc']['bugs_not_linked']) {
                $guiObj->bugs_msg = '';
            }
        }
    ...
    $guiObj->buildSet = (array) $tplanMgr->get_builds_for_html_options(
        $argsObj->tplan_id);
    ...
    $guiObj->platformSet = (array) $tplanMgr->getPlatforms($argsObj->tplan_id,
        $getOpt);
 ...
 */
function buildMatrix($dataSet, &$args, $options = [], $platforms = null,
...
function createSpreadsheet($gui, $args, $customFieldColumns = null)
{
    ...
    for ($idx = 0; $idx < $qta_loops; $idx ++) {
        ...
        foreach ($gui->dataSet[$idx] as $ldx => $field) {
            if ($ldx != 'bugString' ||
                ($ldx == 'bugString' && $gui->bugInterfaceOn)) {
...
function getMetrics(&$dbh, &$args, &$gui)
{
    ...
    if ($args->type == $statusCode['not_run']) {
        ...
        $met = $metricsMgr->getNotRunWithTesterAssigned($args->tplan_id, null,
            $opt);
        ...
        $met = $metricsMgr->getExecutionsByStatus($args->tplan_id, $args->type,
            null, $opt);

        $gui->notRunReport = false;
        $gui->info_msg = lang_get(
            'info_' . $resultsCfg['code_status'][$args->type] . '_tc_report');
