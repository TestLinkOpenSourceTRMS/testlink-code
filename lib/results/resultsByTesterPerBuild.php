...
$openBuildsQty = $tplan_mgr->getNumberOfBuilds($args->tplan_id, null,
...
$build_set = $metricsMgr->get_builds($args->tplan_id, testplan::GET_ACTIVE_BUILD,
    $option);
...
foreach ($matrix as $build_id => $build_execution_map) {
    ...
    // build progress
    $build_statistics[$build_id]['progress'] = round(
        $build_statistics[$build_id]['executed'] /
        $build_statistics[$build_id]['total'] * 100, 2);

    // We have to fill this if we want time at BUILD LEVEL
    $build_statistics[$build_id]['total_time'] = minutes2HHMMSS(
...
foreach ($matrix as $build_id => $build_execution_map) {
    ...
    $first_row = $build_set[$build_id]['name'] . " - " .
        $lblx['progress_absolute'] .
        " {$build_statistics[$build_id]['progress']}%" . " - " .
        $lblx['total_time_hhmmss'] .
        " {$build_statistics[$build_id]['total_time']}";
...
$smartTable = new tlExtTable($columns, $rows,
