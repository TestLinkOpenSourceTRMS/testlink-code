...
if (is_null($tsInf)) {
    ...
    $keywordsMetrics = $metricsMgr->getStatusTotalsByKeywordForRender(
        $args->tplan_id, null, array(
            'groupByPlatform' => 1
        ));
            ...
            if (isset($dummy['details'])) {
                foreach ($dummy['details'] as $status_verbose => $value) {
                    $dummy['details'][$status_verbose]['qty'] = lang_get(
                        $tlCfg->results['status_label'][$status_verbose]);
    ...
    if ($gui->do_report['status_ok']) {
        ...
        $gui->displayBuildMetrics = ! is_null(
            $gui->statistics->overallBuildStatus);
    }
    ...
    if ($gui->do_report['status_ok']) {
        ...
        $gui->displayBuildByPlatMetrics = ! is_null(
            $gui->statistics->buildByPlatMetrics);
    }
    ...
    if (! empty($milestonesList)) {
        $gui->statistics->milestones = $metricsMgr->getMilestonesMetrics(
            $args->tplan_id, $milestonesList);
    }
}
...
function initializeGui(&$dbHandler, $argsObj, &$tplanMgr)
{
    ...
    $gui->actionSpreadsheet = $gui->basehref .
        "lib/results/resultsGeneral.php?format=" . FORMAT_XLS .
        "&tplan_id={$gui->tplan_id}&spreadsheet=1";
