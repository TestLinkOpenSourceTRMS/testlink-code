...
function importResults(&$db, &$xml, $context)
{
    ...
    if ($xml->getName() == 'results') {
        ...
        if ($resultData) {
            $resultMap = saveImportedResultData($db, $resultData,
                $executionContext, $context);
        }
    }
    return $resultMap;
...
function saveImportedResultData(&$db, $resultData, $context, $options)
{
    ...
    if (! $checks['status_ok']) {
        $checks['msg'][] = sprintf($l10n['tproject_id_not_found'],
            $context->tprojectID);
    }
    ...
    if ((intval($context->tprojectID) <= 0) && intval($context->tplanID) > 0) {
        $dummy = $tplan_mgr->tree_manager->get_node_hierarchy_info(
            $context->tplanID);
    ...
    for ($idx = 0; $doIt && $idx < $tc_qty; $idx ++) {
        ...
        if ($status_ok) {
            ...
            $info_on_case = $tplan_mgr->getLinkInfo($context->tplanID, $tcase_id,
                $context->platformID);
            if (is_null($info_on_case)) {
                $message = sprintf($l10n['import_results_tc_not_found'],
                    $tcase_identity);
            } elseif (! $result_is_acceptable) {
                $message = sprintf($l10n['import_results_invalid_result'],
                    $tcase_identity, $tcase_exec['result']);
            } else {
                ...
                $execution_ts = ($tcase_exec['timestamp'] != '') ? "'" .
                    $tcase_exec["timestamp"] . "'" : $db->db_now();
                ...
                $addExecDuration = (strlen($tcase_exec['execution_duration']) > 0 &&
                    is_numeric($tcase_exec['execution_duration']));
                ...
                if ($doInsert) {
                    ...
                    if (isset($tcase_exec['bug_id']) &&
                        ! is_null($tcase_exec['bug_id']) &&
                        is_array($tcase_exec['bug_id'])) {
                    ...
                    if (isset($tcase_exec['custom_fields']) &&
                        ! is_null($tcase_exec['custom_fields']) &&
                        is_array($tcase_exec['custom_fields'])) {
                        ...
                        $cfSetByName = $cfieldMgr->get_linked_cfields_at_execution(
                            $context->tprojectID, 1, 'testcase', null, null,
                            null, 'name');
                        ...
                        foreach ($tcase_exec['custom_fields'] as $cf) {
                            ...
                            if (isset($cfSetByName[$cf['name']])) {
                                ...
                                $message = sprintf($l10n['invalid_cf'],
                                    $tcase_identity, $cf['name']);
                            }

                            if (! is_null($ak)) {
                                $cfieldMgr->execution_values_to_db($ak,
                                    $tcversion_id, $execution_id,
                                    $context->tplanID, null, 'plain');
                            }
                        }
                    }
                ...
                $message = sprintf($msgTxt, $tcase_identity, $version,
                    $tester_name, $l10n[$result_code], $execution_ts);
            }
        }
    ...
    &$columnDef)
{
    ...
    if ($checks['status_ok'] && $execValues['tester'] != '') {
        ...
        if (! is_null($userInfo) && isset($userInfo[0]['id'])) {
            ...
            $checks['msg'][] = sprintf(lang_get('invalid_tester'), $identity,
                $execValues['tester']);
        }
    }

    $execValues['bug_id'] = isset($execValues['bug_id']) ? $execValues['bug_id'] : null;
    if ($checks['status_ok'] && ! is_null($execValues['bug_id']) &&
        is_array($execValues['bug_id'])) {
        foreach ($execValues['bug_id'] as $bug_id) {
            if (($field_len = strlen(trim($bug_id))) >
                $columnDef['bug_id']->max_length) {
                $checks['msg'][] = sprintf(lang_get('bug_id_invalid_len'),
                    $field_len, $columnDef['bug_id']->max_length);
                $checks['status_ok'] = false;
                break;
            }
        }
