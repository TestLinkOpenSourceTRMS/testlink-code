 ...
 *
 */
function getStatusForReportsHTMLOptions()
{
...
function initializeGui(&$dbHandler, $args)
{
    ...
    $gui->status_code_label = getStatusForReportsHTMLOptions();
    $gui->report_type = $args->format;

    $reports_cfg = config_get('reportsCfg');
    $ldf = config_get('locales_date_format');
    $date_format = $ldf[((isset($_SESSION['locale'])) ? $_SESSION['locale'] : 'en_GB')];
    $gui->selected_start_date = @strftime($date_format,
        time() - ($reports_cfg->start_date_offset));
