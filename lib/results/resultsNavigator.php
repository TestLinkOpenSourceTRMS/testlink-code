...
if ($gui->do_report['status_ok']) {
    ...
    $gui->menuItems = $reports_mgr->get_list_reports($context, $gui->btsEnabled,
        $args->optReqs, $tlCfg->reports_formats[$args->format]);
}
...
function initArgs()
{
    ...
    if (is_null($args->format)) {
        ...
        $args->format = count($reports_formats) > 0 ? key($reports_formats) : null;
    }
