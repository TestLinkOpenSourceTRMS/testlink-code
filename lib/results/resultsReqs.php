...
if (count($rspecSet)) {
    ...
    foreach ($rspecSet as $rspec_id => $req_spec_info) {
        ...
        foreach ($req_spec_info['requirements'] as $req_id => $req_info) {
            ...
            $eval = evaluateReq($status_code_map,
                $req_cfg->coverageStatusAlgorithm,
...
if (count($rspecSet)) {
    ...
    foreach ($rspecSet as $rspec_id => $req_spec_info) {
        ...
        $req_spec_description = buildReqSpecDescription($eval_status_map,
            $req_spec_info, $req_cfg->external_req_management, $labels,
        ...
        foreach ($req_spec_info['requirements'] as $req_id => $req_info) {
            ...
            $path = implode("/", $path);
            $single_row[] = htmlentities($path, ENT_QUOTES, $charset) .
                $req_spec_description;
            ...
            $single_row[] = $edit_link .
                htmlentities($req_info['req_doc_id'], ENT_QUOTES, $charset) .
                $title_sep .
                htmlentities($req_info['title'], ENT_QUOTES, $charset);

            $single_row[] = "<!-- " . sprintf("%010d", $req_info['version']) .
                " -->" . $req_info['version'];
            ...
            $single_row[] = '<span class="' .
                $eval_status_map[$eval]['css_class'] . '">' .
                $eval_status_map[$eval]['label'] . '</span>';

            $single_row[] = isset($req_type_labels[$req_info['type']]) ? $req_type_labels[$req_info['type']] : sprintf(
                $labels['no_label_for_req_type'], $req_info['type']);
            ...
            $total_count = ($req_cfg->expected_coverage_management &&
                $expected_coverage > 0) ? $expected_coverage : $req_info['tc_counters']['total'];
            ...
            foreach ($status_code_map as $status => $code) {
                ...
                if ($total_count) {
                    ...
                    $percentage_string = commentPercentage($percentage) .
                        " ({$count}/{$total_count})";
            ...
            // complete progress
            $single_row[] = $total_count ? commentPercentage(
                $progress_percentage) : $labels['na'];

            // show all linked tcversions incl exec result
            $linked_tcs_with_status = '';
            if (! empty($req_info['linked_testcases'])) {
                ...
                foreach ($req_info['linked_testcases'] as $ltcase) {
                    ...
                    foreach ($testcases[$tc_id] as $pelem) {
                        ...
                        $pname = ($pelem['platform_id'] > 0 ? ($pelem['platform_name'] .
                            '/') : '');
                        $colored_status = '<span class="' .
                            $eval_status_map[$status]['css_class'] . '">' . '[' .
                            $pname . $status_l10n . ']</span>';

                        $tc_name = $prefix . $ltcase['tc_external_id'] .
                            $title_sep . $ltcase['name'] . " v" .
                            $ltcase['version'];
                        ...
                        if (isset($pelem['exec_status']) &&
                            $pelem['exec_status'] != $status_code_map['not_run']) {
 ...
 */
function buildReqSpecDescription(&$evalcode_status_map, &$spec_info,
    $ext_mgmt_enabled, &$labels, &$spec_types)
{
    $description = "";
    $space = "&nbsp;&nbsp;&nbsp;&nbsp;";
    ...
    $description .= "<br/>" . $space . $labels['type'] . ": " .
        $spec_types[$spec_info['type']];

    if ($ext_mgmt_enabled && $external_count) {
        $description .= "<br/>{$space}{$labels['req_availability']}: " .
            "({$req_count}/{$external_count})";
    } else {
        $description .= "<br/>" . $space . $labels['requirements'] . ": " .
            $req_count;
    }

    foreach ($evalcode_status_map as $status_code => $status_info) {
        $count = isset($spec_info['req_counters'][$status_code]) ? $spec_info['req_counters'][$status_code] : 0;
        if ($count) {
            $description .= "<br/>" . $space . $status_info['long_label'] . ": " .
                $count;
        }
    }
 ...
 */
function evaluateReq(&$status_code, &$algorithm_cfg, &$counters)
{
    // init
    $evaluation = null;
 ...
 * @return string
 */
function commentPercentage($percentage)
{
    $percentage = round($percentage, 2);
    $padded_percentage = sprintf("%010d", $percentage);
    return "<!-- {$padded_percentage} --> {$percentage}% ";
}
 ...
 * @author Andreas Simon
 * @param
 *            resource &$tproject_mgr reference to testproject manager
 * @return array $args array with user input information
 */
function initArgs(&$tproject_mgr, &$tplan_mgr, &$req_cfg)
{
    $args = new stdClass();
    ...
    if (isset($_REQUEST['platform'])) {
        ...
        isset($args->platforms[$_SESSION['platform']])) {
        // ATTENTION:
 ...
 */
function initGui(&$argsObj, &$tplanMgr)
{
    $gui = new stdClass();
...
function setUpReqStatusCfg()
{
    ...
    $evalbl = init_labels(
        array(
            ...
            'uncovered_reqs' => null,
            'passed_nfc' => null,
            'passed_nfc_reqs' => null,
            'failed_nfc' => null,
            'failed_nfc_reqs' => null,
            'blocked_nfc' => null,
            'blocked_nfc_reqs' => null,
            'not_run_nfc' => null,
            'not_run_nfc_reqs' => null,
            'passed' => null,
    ...
    $reqStatusFilter, &$argsObj)
{
    ...
    foreach ($reqSet as $id) {
        ...
        if (in_array($req['status'], $reqStatusFilter, true) ||
            ...
            foreach ($req['linked_testcases'] as $itemID => $dummy) {
                if ($dummy['is_obsolete'] == "1" ||
                    ! isset($itemsInTestPlan[$dummy['id']])) {
    ...
    if (count($tc_ids)) {
        ...
        $filterOnly['build_id'] = ! isset($filters['platform_id']) &&
            isset($filters['build_id']);
        $noFilter = ! isset($filters['platform_id']) &&
            ! isset($filters['build_id']);
        ...
        if ($noFilter || $filterOnly['platform_id']) {
            ...
            $tcaseSet = $tplanMgr->getLTCVNewGeneration($argsObj->tplan_id,
                $filters, $options);
        }
    }
