...
function buildMatrix(&$guiObj, &$argsObj, $forceFormat = null)
{
    ...
    if ($guiObj->matrixCfg->buildColumns['showExecutionResultLatestCreatedBuild']) {
        $buildSet[] = [
            'name' => $lbl['result_on_last_build'] . ' ' . $latestBuild->name
        ];
    }
...
function buildDataSet(&$db, &$args, &$gui, &$exec, $labels, $forceFormat = null)
{
    ...
    $hist_img_tag = '<img title="' . $labels['history'] . '"' . ' src="' .
        $gui->img->history . '" /></a> ';
    $edit_img_tag = '<img title="' . $labels['design'] . '"' . ' src="' .
        $gui->img->edit . '" /></a> ';
            ...
            foreach ($platformSet as $platformID) {
                ...
                foreach ($args->builds->idSet as $buildID) {
                    ...
                    if (($latestExecution[$platformID][$tcaseID]['status'] ==
                        ...
                        (($latestExecution[$platformID][$tcaseID]['build_id'] ==
                        $buildID) &&
                        ($latestExecution[$platformID][$tcaseID]['id'] ==
                        $rf[$buildID]['executions_id']))) {
                        $lexec = $r4build;
                    }
                } // foreach buildIDSet
