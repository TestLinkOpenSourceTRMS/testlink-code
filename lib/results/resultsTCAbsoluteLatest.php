...
function initArgs(&$dbHandler)
{
    ...
    if ($args->tplan_id <= 0) {
        $msg = __FILE__ . '::' . __FUNCTION__ .
            " :: Invalid Test PLAN ID ({$args->tproject_id})";
        throw new Exception($msg);
    }

    if ($args->doAction == 'result' && $args->platform_id <= 0) {
        $msg = __FILE__ . '::' . __FUNCTION__ .
            " :: Invalid PLATFORM ID ({$args->tproject_id})";
        throw new Exception($msg);
    }
...
function buildMatrix(&$guiObj, &$argsObj, $forceFormat = null)
{
    ...
    if ($fo == FORMAT_HTML) {
        ...
        $matrix = new tlHTMLTable($columns, $guiObj->matrix,
            'tl_table_results_tc');
    }
    unset($columns);
...
function initializeGui(&$dbHandler, &$argsObj, $imgSet, &$tplanMgr)
{
    ...
    $guiObj->platforms = $tplanMgr->getPlatforms($argsObj->tplan_id,
        array(
            'outputFormat' => 'map'
        ));
 ...
 * @param int $forceFormat
 */
function buildDataSet(&$db, &$args, &$gui, &$metrics, $labels,
    $forceFormat = null)
{
        ...
        foreach ($tcaseSet as $tcaseID) {
            ...
            foreach ($platformSet as $platformID) {
                ...
                if (! isset($tsuiteCache[$tsuiteID])) {
                    // Get full path
                    $tsuiteCache[$tsuiteID] = implode("/",
                        $treeMgr->get_path($tsuiteID, null, 'name'));
                }
                ...
                if (isset($rf['execution_notes'])) {
                    $nv = is_null($rf['execution_notes']) ? '' : $rf['execution_notes'];
                }
                if ($fo == FORMAT_XLS) {
                    $rows[] = $nv;
                }
 ...
 * @param stdClass $args
 * @param stdClass $gui
 * @param tlTestPlanMetrics $metricsMgr
 */
function doProcess(&$dbH, &$args, &$gui, &$metricsMgr)
{
    ...
    $neverRunOnPP = (array) $metricsMgr->getNeverRunOnSinglePlatform(
        $args->tplan_id, $args->platform_id);

    $execStatus = (array) $metricsMgr->getLatestExecOnSinglePlatformMatrix(
