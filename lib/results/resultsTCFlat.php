...
list ($gui, $labels, $cfg) = initializeGui($db, $args, $smarty->getImages(),
    $tplan_mgr);
$args->cfg = $cfg;
...
if (($gui->activeBuildsQty <= $gui->matrixCfg->buildQtyLimit) ||
    ...
    $execStatus = $metricsMgr->getExecStatusMatrixFlat($args->tplan_id,
        $buildSet, $opt);
    ...
    if ($gui->matrixCfg->buildQtyLimit > 0) {
        $gui->userFeedback = $labels['too_much_data'] . '<br>' .
            sprintf($labels['too_much_builds'], $gui->activeBuildsQty,
                $gui->matrixCfg->buildQtyLimit);
    }
...
function initializeGui(&$dbHandler, &$argsObj, $imgSet, &$tplanMgr)
{
    ...
    foreach ($cfg['results']['code_status'] as $code => $verbose) {
        if (isset($cfg['results']['status_label'][$verbose])) {
            ...
            $guiObj->map_status_css[$code] = $cfg['results']['code_status'][$code] .
                '_text';
        }
    }
...
function createSpreadsheet($gui)
{
    ...
    $styleReportContext = array(
        'font' => array(
            'bold' => true
        )
    );
    $styleDataHeader = array(
        'font' => array(
            'bold' => true
        ),
        'borders' => array(
    ...
    $objPHPExcel->getActiveSheet()
        ->getStyle($cellArea)
        ->applyFromArray($styleReportContext);
    ...
    $objPHPExcel->getActiveSheet()
        ->getStyle($cellArea)
        ->applyFromArray($styleDataHeader);
    ...
    $objPHPExcel->setActiveSheetIndex(0);
    $settings = array();
    $settings['Excel2007'] = array(
        'ext' => '.xlsx',
        'Content-Type' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    );
    $settings['Excel5'] = array(
        'ext' => '.xls',
    ...
    unlink($tmpfname);
    $f2d = 'resultsTCFlat_' . $gui->tproject_name . '_' . $gui->tplan_name .
        $settings[$xlsType]['ext'];
    downloadContentsToFile($content, $f2d,
        array(
            'Content-Type' => $settings[$xlsType]['Content-Type']
        ));
    exit();
}
...
function buildSpreadsheetData(&$db, &$args, &$gui, &$exec, $labels)
{
    ...
    $det = array(
        TESTCASE_EXECUTION_TYPE_MANUAL => $labels['execution_type_manual'],
        TESTCASE_EXECUTION_TYPE_AUTO => $labels['execution_type_auto']
    );
    ...
    for ($ix = 0; $ix < $loop2do; $ix ++) {
        ...
        $rows[$cols['tcase']] = htmlspecialchars(
            "{$eid}:{$metrics[$ix]['name']}", ENT_QUOTES);
