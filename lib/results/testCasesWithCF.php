...
if ($args->doIt) {
    ...
    foreach ($gui->resultSet as $item) {
        ...
        $tcaseName = buildExternalIdString($gui->tcasePrefix,
            $item['tc_external_id']) . ' : ' . $item['tcase_name'];

        $tcLink = "<!-- " . sprintf("%010d", $item['tc_external_id']) . " -->" .
        ...
        foreach ($item['cfields'] as $cf_value) {
            $rowData[] = preg_replace('!\s+!', ' ',
                htmlspecialchars($cf_value, ENT_QUOTES, $charset));
            if ($cf_value) {
                $hasValue = true;
            }
        }
...
function initializeGui(&$dbHandler, &$argsObj)
{
    ...
    $guiObj->tcasePrefix = $tproject_mgr->getTestCasePrefix(
        $argsObj->tproject_id);
    unset($tproject_mgr);
    ...
    foreach ($guiObj->code_status as $code => $verbose) {
        if (isset($resultsCfg['status_label'][$verbose])) {
            $guiObj->status_code_labels[$code] = lang_get(
                $resultsCfg['status_label'][$verbose]);
        }
    }
    return $guiObj;
}
...
function buildResultSet(&$dbHandler, &$guiObj, $tproject_id, $tplan_id)
{
    ...
    $guiObj->cfields = $cfieldMgr->get_linked_cfields_at_execution($tproject_id,
        1, 'testcase', null, null, null, 'name');
    ...
    $cf_map = $cfieldMgr->get_linked_cfields_at_execution($tproject_id, 1,
        'testcase', null, null, $tplan_id, 'exec_id');
    ...
    if (! is_null($cf_map)) {
        foreach ($cf_map as $exec_id => $exec_info) {
            ...
            foreach ($exec_info as $cfield_data) {
                $guiObj->resultSet[$exec_id]['cfields'][$cfield_data['name']] = $cfieldMgr->string_custom_field_value(
                    $cfield_data, null);
            }
        }
    }
