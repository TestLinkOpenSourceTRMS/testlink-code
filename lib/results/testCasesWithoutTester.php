...
if ($tplan_mgr->count_testcases($args->tplan_id) > 0) {
    ...
    if (($gui->row_qty = count($metrics)) > 0) {
        ...
        $gui->pageTitle .= " - " . $gui->labels['match_count'] . ":" .
            $gui->row_qty;
        ...
        foreach ($metrics as &$item) {
            ...
            if ($gui->options->testPriorityEnabled) {
                ...
                $row[] = $tplan_mgr->urgencyImportanceToPriorityLevel(
                    $item['urg_imp']);
            }
        ...
        $gui->tableSet[] = buildTable($data, $args->tproject_id,
            $args->show_platforms, $gui->options->testPriorityEnabled);
    }
}
