...
if ($tplan_mgr->count_testcases($args->tplan_id) > 0) {
    ...
    $gui->cfields = $cfield_mgr->get_linked_cfields_at_testplan_design(
        $args->tproject_id, 1, 'testcase', null, null, null, 'name');
    ...
    $cf_map = $cfield_mgr->get_linked_cfields_at_testplan_design(
        $args->tproject_id, 1, 'testcase', null, null, $args->tplan_id);
...
function buildExtTable($gui, $tcaseMgr, $labels, $edit_icon)
{
    ...
    if (! empty($gui->resultSet)) {
        ...
        foreach ($gui->resultSet as $item) {
            ...
            $name = buildExternalIdString($gui->tcasePrefix,
                $item['tc_external_id']) . $title_sep . $item['tcase_name'];

            // create linked icons
            $edit_link = "<a href=\"javascript:openTCEditWindow({$item['tcase_id']});\">" .
                "<img title=\"{$labels['design']}\" src=\"{$edit_icon}\" /></a> ";

            $rowData[] = "<!-- " . sprintf("%010d", $item['tc_external_id']) .
                " -->" . $edit_link . $name;
            $hasValue = false;
