...
class searchCommands
{
    ...
    private function isReqFeatureEnabled($tproject_id)
    {
        ...
        return isset($info['opt']->requirementsEnabled) ? $info['opt']->requirementsEnabled : 0;
    }
    ...
    {
        return $this->reqSpecMgr->get_all_id_in_testproject($tproject_id);
    }
    ...
    {
        return $this->tprojectMgr->get_all_requirement_ids($tproject_id);
    }
    ...
    private function initArgs()
    {
        ...
        if ($args->tprojectID <= 0) {
            throw new Exception(
                "Error Processing Request - Invalid Test project id " . __FILE__);
        }
        ...
        foreach ($k2w as $key => $value) {
            ...
            if (isset($args->$key) && $args->$key != '') {
                ...
                if ($da != null) {
                    $args->$key = $da['year'] . "-" . $da['month'] . "-" .
                        $da['day'] . $value; // set date in iso format
        ...
        $args->canAccessTestSpec = $args->user->hasRight($this->db,
            'mgt_view_tc', $args->tproject_id);
        $args->canAccessReqSpec = $args->user->hasRight($this->db,
            'mgt_view_req', $args->tproject_id);
    }
    ...
    private function initGui()
    {
        ...
        $this->gui->tcasePrefix = $this->tprojectMgr->getTestCasePrefix(
            $this->args->tprojectID);
        ...
        $this->gui->mainCaption = lang_get('testproject') . " " .
            $this->args->tprojectName;

        $this->gui->search_important_notice = sprintf(
            lang_get('search_important_notice'), $this->args->tprojectName);
    ...
    {
        $this->gui->reqEnabled = $this->isReqFeatureEnabled(
        ...
        if ($this->gui->reqEnabled) {
            $this->gui->design_cf_req = $this->cfieldMgr->get_linked_cfields_at_design(
                $this->args->tproject_id, cfield_mgr::ENABLED, null,
                'requirement');
        }
        ...
        $this->gui->keywords = $this->tprojectMgr->getKeywordSet(
            $this->args->tproject_id);
        $this->gui->filter_by['keyword'] = ! is_null($this->gui->keywords);

        $reqSpecSet = $this->tprojectMgr->genComboReqSpec(
            $this->args->tprojectID);
        $this->gui->filter_by['requirement_doc_id'] = ! is_null($reqSpecSet);
        $reqSpecSet = null;

        $this->gui->status = isset($this->args->status) ? intval(
            $this->args->status) : '';
    ...
    public function searchReqSpec($targetSet, $canUseTarget)
    {
        ...
        $sql = "SELECT RSRV.name, RSRV.scope, LRSR.req_spec_id, RSRV.id," .
            ...
            "AND RSRV.revision = LRSR.revision " . "WHERE LRSR.testproject_id = " .
            $args->tproject_id;
        ...
        if ($canUseTarget) {
            ...
            foreach ($targetSet as $trgt) {
                $target = trim($trgt);
                $filterRS['name'] .= $args->and_or .
                    " RSRV.name $this->likeOp '%{$target}%' ";
            }
            $filterRS['name'] .= ')';
        }
    ...
    public function searchReq($targetSet, $canUseTarget, $req_cf_id)
    {
        ...
        if ($req_cf_id > 0) {
            ...
            switch ($gui->cf_types[$cf_def['type']]) {
                case 'date':
                    $args->custom_field_value = $this->cfieldMgr->cfdate2mktime(
                        $args->custom_field_value);

                    $fi['by_custom_field'] .= " AND CFD.value = {$args->custom_field_value}";
                    break;

                default:
                    $args->custom_field_value = $db->prepare_string(
                        $args->custom_field_value);
        ...
        if ($doSql) {
            ...
            $sql = " /* " . __LINE__ . " */ " .
                ...
                $from['users'] . $from['by_custom_field'] . " WHERE RQ.id IN(" .
                implode(',', $reqSet) . ")";
            ...
            if ($args->reqStatus != '') {
                $doFilter = true;
                $sql .= " AND RQV.status='" .
                    $db->prepare_string($args->reqStatus) . "' ";
            }
            ...
            if ($canUseTarget) {
                ...
                if ($args->rq_title) {
                    ...
                    foreach ($targetSet as $target) {
                        $filterRQ['name'] .= $args->and_or .
                            " NHRQ.name $this->likeOp '%{$target}%' ";
                    }
                    $filterRQ['name'] .= ')';
                }
                ...
                if ($args->rq_doc_id) {
                    ...
                    foreach ($targetSet as $target) {
                        $filterRQ['req_doc_id'] .= $args->and_or .
                            " RQ.req_doc_id $this->likeOp '%{$target}%' ";
                    }
                    $filterRQ['req_doc_id'] .= ')';
                }
            }
    ...
    public function searchTestSuites($targetSet, $canUseTarget)
    {
        ...
        if ($args->ts_title && $canUseTarget) {
            ...
            foreach ($targetSet as $target) {
                $filterSpecial['ts_title'] .= $args->and_or .
                    " NH_TS.name $this->likeOp '%{$target}%' ";
            }
        ...
        if ($args->ts_title || $args->ts_summary) {
            ...
            $sqlFields = " SELECT NH_TS.name, TS.id, TS.details " .
                ...
                $fromTS['by_keyword_id'] . " WHERE TS.id IN (" .
                implode(',', $tsuiteSet) . ")";
    ...
    /**
     */
    public function searchTestCases($tcaseSet, $targetSet, $canUseTarget,
        $tc_cf_id)
    {
        // shortcuts
        $args = &$this->args;
        ...
        if ($tc_cf_id > 0) {
            ...
            switch ($gui->cf_types[$cf_def['type']]) {
                case 'date':
                    $args->custom_field_value = $cfieldMgr->cfdate2mktime(
                        $args->custom_field_value);
        ...
        $sqlPart2 = " FROM {$views['latest_tcase_version_number']} LVN " .
            ...
            " AND TCV.version = LVN.version " . $from['tc_steps'] .
            $from['users'] . $from['by_keyword_id'] . $from['by_custom_field'] .
