...
function initArgs(&$dbHandler)
{
    ...
    $args->refreshTree = getSettingFromFormNameSpace('edit_mode',
...
function systemWideTestCaseSearch(&$dbHandler, &$argsObj, $glue)
{
    ...
    if (strpos($argsObj->targetTestCase, $glue) === false) {
        ...
        $argsObj->targetTestCase = $argsObj->tcasePrefix .
            $argsObj->targetTestCase;
    }
...
function getSettingFromFormNameSpace($mode, $setting)
{
    ...
    return isset($sd[$setting]) ? $sd[$setting] : 0;
}
...
function processTestCase(&$dbHandler, $tplEngine, $args, &$gui, $grants, $cfg)
{
    ...
    if (($args->caller == 'navBar') && ! is_null($args->targetTestCase) &&
        strcmp($args->targetTestCase, $args->tcasePrefix) != 0) {
        ...
        if (! ($get_path_info = ($args->id > 0))) {
            $gui->warning_msg = $args->id == 0 ? lang_get(
                'testcase_does_not_exists') : lang_get('prefix_does_not_exists');
        }
    }
    ...
    if ($args->id > 0) {
        if ($get_path_info || $args->show_path) {
            $gui->path_info = $item_mgr->tree_manager->get_full_path_verbose(
                $args->id);
        }
        $platform_mgr = new tlPlatform($dbHandler, $args->tproject_id);

        $gui->platforms = $platform_mgr->getAllAsMap();
        $gui->direct_link = $item_mgr->buildDirectWebLink($_SESSION['basehref'],
            $args->id);
