...
$args = initArgs($db, $tproject_mgr, $tsuite_mgr, $opt_cfg);
...
if ($init_opt_transfer) {
    $opt_cfg = initializeOptionTransfer($tproject_mgr, $tsuite_mgr, $args,
        $action);
}

// create web editor objects
list ($oWebEditor, $webEditorHtmlNames, $webEditorTemplateKey) = initWebEditors(
    $action, $level, $editorCfg);
if ($get_c_data) {
    $name_ok = 1;
...
if ($doIt) {
    switch ($action) {
            ...
            switch ($level) {
                case 'testsuite':
                    $uploadOp = fileUploadManagement($db, $args->testsuiteID,
                        $args->fileTitle, $tsuite_mgr->getAttachmentTableName());
                    $gui = initializeGui($tsuite_mgr, $args->testsuiteID, $args);
                    $gui->refreshTree = 0;
                    $gui->uploadOp = $uploadOp;
                    $tsuite_mgr->show($smarty, $gui, $template_dir,
                        $args->testsuiteID, null, null);
                    break;

                case 'testproject':
                    $uploadOp = fileUploadManagement($db, $args->tprojectID,
                        $args->fileTitle,
                    ...
                    $tproject_mgr->show($smarty, $gui, $template_dir,
                        $args->tprojectID, null, null);
                    break;
            }
            break;
        ...
        case 'new_testsuite':
            ...
            $gui->hasKeywords = (count($opt_cfg->from->map) > 0) ||
            ...
            $tsuite_mgr->viewer_edit_new($smarty, $template_dir,
                $webEditorHtmlNames, $oWebEditor, $action, $args->containerID,
        ...
        case 'update_testsuite':
            ...
            $gui = initializeGui($tsuite_mgr, $args->testsuiteID, $args);
            $tsuite_mgr->show($smarty, $gui, $template_dir, $args->testsuiteID,
                null, $msg);
            break;
        ...
        case 'add_testsuite':
            ...
            $tsuite_mgr->viewer_edit_new($smarty, $template_dir,
                $webEditorHtmlNames, $oWebEditor, $action, $args->containerID,
                null, $messages, $webEditorTemplateKey, $userInput);
            break;
        ...
        case 'reorder_testproject_testsuites_alpha':
            ...
            $gui = initializeGui($tproject_mgr, $args->tprojectID, $args);
            $gui->refreshTree = true;
            $tproject_mgr->show($smarty, $gui, $template_dir, $args->tprojectID,
                null, null);
            break;
        ...
        default:
            trigger_error("containerEdit.php - No correct GET/POST data",
                E_USER_ERROR);
            break;
    }
}
    ...
    $tsuite_id)
{
    ...
    if (! is_null($testcases)) {
        ...
        $status_warning = array(
            'linked_and_executed' => 1,
            'linked_but_not_executed' => 1,
            'no_links' => 0
        );

        $delete_notice = array(
            'linked_and_executed' => lang_get('delete_notice'),
 ...
 *
 */
function initArgs(&$dbHandler, &$tprojectMgr, &$tsuiteMgr, $optionTransferCfg)
{
    $args = new stdClass();
    $_REQUEST = strings_stripSlashes($_REQUEST);

    $args->kw_link_id = isset($_REQUEST['kw_link_id']) ? intval(
        $_REQUEST['kw_link_id']) : null;
    ...
    $args->testsuiteID = isset($_REQUEST['testsuiteID']) ? intval(
        $_REQUEST['testsuiteID']) : null;
    $args->tsuite_name = isset($_REQUEST['testsuiteName']) ? $_REQUEST['testsuiteName'] : null;

    // Order is critic
    $args->objectID = isset($_REQUEST['objectID']) ? intval(
        $_REQUEST['objectID']) : null;
    $args->containerID = isset($_REQUEST['containerID']) ? intval(
        $_REQUEST['containerID']) : $args->objectID;
    ...
    $args->fileTitle = isset($_REQUEST['fileTitle']) ? trim(
        $_REQUEST['fileTitle']) : '';
    ...
    $args->grants->delete_executed_testcases = $args->user->hasRight($dbHandler,
        'testproject_delete_executed_testcases', $args->tprojectID);

    $args->grants->testcase_mgmt = $args->user->hasRight($dbHandler,
        'mgt_modify_tc', $args->tprojectID);
    ...
    if ($args->treeFormToken > 0) {
        $mode = 'edit_mode';
        $sdata = isset($_SESSION[$mode]) &&
            isset($_SESSION[$mode][$args->treeFormToken]) ? $_SESSION[$mode][$args->treeFormToken] : null;
        $args->testCaseSet = isset($sdata['testcases_to_show']) ? $sdata['testcases_to_show'] : null;
    }

    $dummy = (config_get('testcase_reorder_by')) == 'NAME' ? '_alpha' : '_externalid';
    $lbl2init = array(
        'warning_empty_testsuite_name' => null,
 ...
 *
 */
function deleteTestSuite(&$smartyObj, &$argsObj, &$tsuiteMgr, &$treeMgr,
    &$tcaseMgr, $level)
{
    $feedback_msg = '';
    $system_message = '';
    ...
    if ($argsObj->bSure) {
        ...
        $doRefreshTree = true;
        $feedback_msg = 'ok';
        $smartyObj->assign('user_feedback',
            lang_get('testsuite_successfully_deleted'));
    } else {
        $doRefreshTree = false;
        ...
        if (is_null($testcases) || count($testcases) == 0) {
            ...
            $map_msg = build_del_testsuite_warning_msg($treeMgr, $tcaseMgr,
                $testcases, $argsObj->testsuiteID);
        ...
        if (! $can_delete && ! $argsObj->grants->delete_executed_testcases) {
            $system_message = lang_get(
                'system_blocks_tsuite_delete_due_to_exec_tc');
        }
    ...
    $smartyObj->assign('page_title',
        lang_get('delete') . " " . lang_get('container_title_' . $level));
...
function addTestSuite(&$tsuiteMgr, &$argsObj, $container, &$hash)
{
    ...
    $ret = $tsuiteMgr->create($argsObj->containerID,
        $container['container_name'], $container['details'], $new_order,
    ...
    if ($ret['status_ok']) {
        ...
        writeCustomFieldsToDB($tsuiteMgr->db, $argsObj->tprojectID, $ret['id'],
            $hash);
...
{
    $msg = 'ok';
    $ret = $tsuiteMgr->update($argsObj->testsuiteID,
    ...
    if ($ret['status_ok']) {
        ...
        writeCustomFieldsToDB($tsuiteMgr->db, $argsObj->tprojectID,
            $argsObj->testsuiteID, $hash);
...
function copyTestSuite(&$smartyObj, $template_dir, &$tsuiteMgr, $argsObj)
{
    ...
    $options['action_on_duplicate_name'] = config_get(
    ...
    $op = $tsuiteMgr->copy_to($argsObj->objectID, $argsObj->containerID,
        $argsObj->userID, $options);

    if ($op['status_ok']) {
        $tsuiteMgr->tree_manager->change_child_order($argsObj->containerID,
            $op['id'], $argsObj->target_position, $exclude_node_types);

        // get info to provide feedback
        $dummy = $tsuiteMgr->tree_manager->get_node_hierarchy_info(
            array(
                $argsObj->objectID,
                $argsObj->containerID
            ));
        ...
        $guiObj->user_feedback = sprintf(lang_get($msgk),
            $dummy[$argsObj->objectID]['name'],
...
function moveTestSuite(&$smartyObj, $template_dir, &$tprojectMgr, $argsObj)
{
    ...
    $tprojectMgr->tree_manager->change_parent($argsObj->objectID,
        $argsObj->containerID);
    $tprojectMgr->tree_manager->change_child_order($argsObj->containerID,
        $argsObj->objectID, $argsObj->target_position, $exclude_node_types);

    $guiObj = new stdClass();
    $guiObj->id = $argsObj->tprojectID;
    $guiObj->refreshTree = $argsObj->refreshTree;

    $tprojectMgr->show($smartyObj, $guiObj, $template_dir, $argsObj->tprojectID,
        null, 'ok');
}
 ...
 *
 */
function initializeOptionTransfer(&$tprojectMgr, &$tsuiteMgr, $argsObj,
    $doAction)
{
    ...
    if ($doAction == 'edit_testsuite') {
        $opt_cfg->to->map = $tsuiteMgr->get_keywords_map($argsObj->testsuiteID,
...
{
    $tables = $tprojectMgr->getDBTables(
        array(
            'nodes_hierarchy',
            'node_types',
            'tcversions'
 ...
 *
 */
function copyTestCases(&$smartyObj, $template_dir, &$tsuiteMgr, &$tcaseMgr,
    $argsObj)
{
    $op = array(
        'refreshTree' => false,
    ...
    if (($qty = sizeof($argsObj->tcaseSet)) > 0) {
        ...
        $copyOpt['copy_also'] = array(
            'keyword_assignments' => $argsObj->copyKeywords,
        ...
        $guiObj = new stdClass();
        $guiObj->attachments = getAttachmentInfosFrom($tsuiteMgr,
            $argsObj->objectID);
        $guiObj->id = $argsObj->objectID;
 ...
 *
 */
function moveTestCases(&$smartyObj, $template_dir, &$tsuiteMgr, &$treeMgr,
    $argsObj)
{
    $lbl = $argsObj->l10n;
 ...
 */
function deleteTestCasesViewer(&$dbHandler, &$smartyObj, &$tprojectMgr,
    &$treeMgr, &$tsuiteMgr, &$tcaseMgr, $argsObj, $feedback = null)
{
    $guiObj = new stdClass();
    ...
    if (! is_null($guiObj->testCaseSet) && count($guiObj->testCaseSet) > 0) {
        foreach ($guiObj->testCaseSet as &$child) {
            $external = $tcaseMgr->getExternalID($child['id'], null,
                $tcasePrefix);
            ...
            $child['draw_check'] = $argsObj->grants->delete_executed_testcases ||
                (! $dummy['executed']);
        ...
        foreach ($itemSet as $mainKey) {
            ...
            if (! is_null($guiObj->exec_status_quo[$mainKey])) {
                ...
                foreach ($versionSet as $version_id) {
                    $tplanSet = array_keys(
                        $guiObj->exec_status_quo[$mainKey][$version_id]);
                    foreach ($tplanSet as $tplan_id) {
                        if ($guiObj->display_platform[$mainKey] = ! isset(
                            $guiObj->exec_status_quo[$mainKey][$version_id][$tplan_id][0])) {
                            $stop = true;
                            break;
                        }
                    }
    ...
    if (! is_null($guiObj->testCaseSet) && (sizeof($guiObj->testCaseSet) > 0)) {
        ...
        $guiObj->op_ok = false;
        $guiObj->user_feedback = is_null($guiObj->user_feedback) ? lang_get(
            'no_testcases_available') : $guiObj->user_feedback;
    }
...
{
    $tcaseSet = (array) $tsuiteMgr->get_children_testcases(
        $argsObj->testsuiteID, 'simple');
...
function initializeGui(&$objMgr, $id, $argsObj, $lbl = null)
{
    ...
    $guiObj->fileUploadURL = $_SESSION['basehref'] .
        $objMgr->getFileUploadRelativeURL($id);

    if ($objMgr->my_node_type == $objMgr->node_types_descr_id['testsuite']) {
        $guiObj->direct_link = $objMgr->buildDirectWebLink(
            $_SESSION['basehref'], $guiObj->id, $argsObj->tprojectID);
    }
...
function doBulkSet(&$dbHandler, $argsObj, $tcaseSet, &$tcaseMgr)
{
    if (! empty($tcaseSet)) {
        ...
        if (! is_null($cf_map)) {
            ...
            if (! is_null($cfval)) {
                foreach ($tcaseSet as $tcversion_id => $tcase_id) {
                    $tcaseMgr->cfield_mgr->design_values_to_db($_REQUEST,
                        $tcversion_id, $cfval);
                }
            }
        }
