...
if (is_null($gui->repository_name) &&
    isset($_SESSION['testscript_repositoryName']) && $args->user_action == 'link') {
...
if (! is_null($args->repositoryName) && $args->user_action != 'projectSelected') {
    ...
    $gui->codeTrackerMetaData['repos'] = $cts->getReposForHTMLSelect(
        $gui->project_key);
}

if (! is_null($args->branchName) && $args->user_action != 'projectSelected' &&
    $args->user_action != 'repoSelected') {
    $gui->codeTrackerMetaData['branches'] = $args->branchName;
} else {
    $gui->codeTrackerMetaData['branches'] = $cts->getBranchesForHTMLSelect(
        $gui->project_key, $gui->repository_name);
}

if (! is_null($args->commits) && $args->user_action != 'projectSelected' &&
    $args->user_action != 'repoSelected' &&
    $args->user_action != 'branchSelected') {
    $gui->codeTrackerMetaData['commits'] = $args->commits;
} elseif (($args->user_action == 'branchSelected') ||
    (! is_null($gui->branch_name) &&
    ($args->user_action == 'expand' || $args->user_action == 'collapse'))) {
    $gui->codeTrackerMetaData['commits'] = $cts->getCommitsForHTMLSelect(
        $gui->project_key, $gui->repository_name, $gui->branch_name);
}

if (! is_null($args->files) &&
    ($args->user_action == 'expand' || $args->user_action == 'collapse')) {
    $gui->codeTrackerMetaData['files'] = $args->files;
} elseif ($args->user_action == 'repoSelected' ||
    $args->user_action == 'branchSelected' || $args->user_action == 'expand' ||
    $args->user_action == 'collapse' ||
    ($args->user_action == 'link' && ! is_null($gui->repository_name))) {
    $gui->codeTrackerMetaData['files'] = $cts->getRepoContentForHTMLSelect(
        $gui->project_key, $gui->repository_name, '', $gui->branch_name,
        $gui->commit_id);
}
if ($args->user_action == 'expand') {
    ...
    foreach ($expandArray as $item) {
        $tmpPath .= $item . "/";
        $tmpFileArray[$item][0] = $cts->getRepoContentForHTMLSelect(
            $gui->project_key, $gui->repository_name, $tmpPath,
            $gui->branch_name, $gui->commit_id);
...
if ($args->user_action == 'create') {
    if (! is_null($codeT) && $args->project_key != "" &&
        ...
        $baseURL = $cts->buildViewCodeURL($args->project_key,
        ...
        if ($refPos !== false) {
            ...
            if (strpos($refStr, '?at=refs%2Fheads%2F') !== false) {
                ...
                if (is_null($args->branch_name) &&
                    (isset($args->branchName[$refStr]) ||
                    array_key_exists($refStr, $args->branchName))) {
        ...
        $scriptWritten = writeTestcaseScript($db, $cts, $args->tcversion_id,
            $args->project_key, $args->repository_name, $args->code_path,
            $args->branch_name, $args->commit_id);

        $auditMsg = "audit_testcasescript_added";
        $item_id = $args->tcversion_id;
        $objectType = "testcase_script_links";
        $args->direct_link = $cts->buildViewCodeURL($args->project_key,
            $args->repository_name, $args->code_path, $args->branch_name);
        if ($scriptWritten) {
            ...
            foreach ($linked_cfields as $cfieldID => $cfieldValue) {
                if (is_null($tScriptFieldValue) &&
                    strpos(
                        strtolower(str_replace(" ", "", $cfieldValue['name'])),
                        'testscript') !== false) {
            ...
            if (! is_null($tScriptFieldID)) {
                $cfieldWritten = writeCfieldTestscript($db, $args->user,
                    $tScriptFieldID, $args->tcversion_id, $args->code_path);
            } else {
                $cfieldWritten = true;
            }
            ...
            logAuditEvent(TLS($auditMsg, $args->direct_link), "CREATE", $item_id,
                $objectType);
        } else {
            $gui->msg = sprintf($l18n["error_code_does_not_exist_on_cts"],
                $args->direct_link);
        }
    } else {
...
/**
 */
function writeTestcaseScript(&$dbHandler, &$cts, $tcversion_id, $project_key,
    $repo_name, $code_path, $branch_name, $commit_id)
{
    $result = true;

    $repoCont = $cts->getRepoContent($project_key, $repo_name, $code_path,
...
function writeCfieldTestscript(&$dbHandler, &$user, $field_id, $node_id, $value)
{
    ...
    $sql = " SELECT id, active, is_open, baseline, reviewer_id " .
        " FROM `{$tbl['tcversions']}` " . " WHERE `id` = '{$node_id}'";
...
{
    return $user->hasRight($db, "mgt_modify_tc");
}
