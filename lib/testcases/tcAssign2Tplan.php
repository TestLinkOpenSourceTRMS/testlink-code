...
if (! is_null($tcase_all_info)) {
    foreach ($tcase_all_info as $tcversion_info) {
        if ($tcversion_info['id'] == $args->tcversion_id) {
            ...
            $gui->pageTitle = lang_get('test_case') . ':' .
                $tcversion_info['name'];
            $gui->tcaseIdentity = $tproject_mgr->getTestCasePrefix(
                $args->tproject_id);
            $gui->tcaseIdentity .= $glue . $tcversion_info['tc_external_id'] .
                ':' . $tcversion_info['name'];
            break;
        }
    }
...
if (! is_null(
    ...
    foreach ($tplanSet as $tplan_id => $value) {
        ...
        if ($has_links[$tplan_id]) {
            $linked_platforms = array_flip(
                array_keys($linked_tplans[$tplan_id]));
    ...
    foreach ($gui->tplans as $tplan_id => $platform_info) {
        foreach ($platform_info as $platform_id => $value) {
            $gui->can_do = $gui->can_do ||
                $gui->tplans[$tplan_id][$platform_id]['draw_checkbox'];
        }
    }
}
$smarty = new TLSmarty();
