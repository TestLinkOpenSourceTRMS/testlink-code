...
if ($args->result != '' && $args->tcvx > 0) {
    ...
    // get version number
    $sql = " SELECT TCV.version FROM  {$tables['tcversions']} TCV WHERE TCV.id = " .
        $args->tcvx;
...
if ($doIt) {
    ...
    foreach ($gui->resultSet as $tplan_id => $tcase_set) {
        ...
        $hasExecRight = $_SESSION['currentUser']->hasRight($db,
            'testplan_execute', $args->tproject_id, $tplan_id, true);
        ...
        foreach ($tcase_set as $tcase_platform) {
            foreach ($tcase_platform as $tcase) {
                ...
                if ($args->show_user_column) {
                    if ($tcase['user_id'] > 0 &&
                        isset($args->userSet[$tcase['user_id']])) {
                        $current_row[] = htmlspecialchars(
                            $args->userSet[$tcase['user_id']]['login']);
                    } else {
                        $current_row[] = '';
                    }
                }
                ...
                if ($execCfg->exec_mode->tester == 'assigned_to_me') {
                    $canExec = $canExec &&
                        ($tcase['user_id'] == $_SESSION['userID']);
                }

                if ($canExec) {
                    $ekk = sprintf($exec['common'], $tplan_id,
                        $tcase['platform_id'], $tplan_id, $tcase['build_id'],
                        $tplan_id, $tcversion_id, $tplan_id);

                    $elk = sprintf($exec['passed'], $tplan_id) . $ekk . '&nbsp;' .
                        sprintf($exec['failed'], $tplan_id) . $ekk . '&nbsp;' .
                ...
                $current_row[] = "<!-- " .
                    sprintf("%010d", $tcase['tc_external_id']) . " -->" . $elk .
                    $exec_history_link . $exec_link . $edit_link .
                    htmlspecialchars($tcase['prefix']) . $gui->glueChar .
                    $tcase['tc_external_id'] . " : " .
        ...
        $matrix->title = $gui->l18n['testplan'] . ": " .
            htmlspecialchars($gui->tplanNames[$tplan_id]['name']);
...
function initArgs(&$dbHandler)
{
    ...
    if ($args->tproject_id == 0) {
        throw new Exception(
            __FILE__ . ' Can not work without Test project ID => Aborting');
    }
    ...
    if ($args->user_id != 0) {
        ...
        $args->user_id = isset($_SESSION['userID']) ? intval(
            $_SESSION['userID']) : 0;
        if ($args->user_id == 0) {
            throw new Exception(
                __FILE__ . ' Can not work without User ID => Aborting');
        }
    ...
    if (isset($_REQUEST['show_inactive_and_closed'])) {
        $args->show_inactive_and_closed = (intval(
    ...
    $args->result = isset($_REQUEST['result_' . $args->tpx]) ? $_REQUEST['result_' .
        $args->tpx][0] : '';
...
function initializeGui(&$dbHandler, $argsObj)
{
    ...
    if ($argsObj->show_all_users) {
        ...
        $gui->pageTitle = sprintf($gui->l18n['testcases_assigned_to_user'],
            $gui->tproject_name, $argsObj->user_name);
    }

    $gui->user_id = $argsObj->user_id;
    $gui->tplan_id = $argsObj->tplan_id;

    $gui->directLink = $_SESSION['basehref'] . 'ltx.php?item=xta2m&user_id=' .
        $gui->user_id . '&tplan_id=' . $gui->tplan_id;
