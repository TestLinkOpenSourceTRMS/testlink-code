...
if ($args->compare_selected_versions) {
    ...
    foreach ($attributes as $key => $val) {
        ...
        if ($args->use_daisydiff) {
            ...
            $gui->diff[$key]['diff'] = $diffEngine->inline(
                $gui->diff[$key]['left'], $gui->leftID,
        ...
        $gui->diff[$key]['message'] = sprintf($gui->labels[$msgKey],
            $gui->diff[$key]['heading'], $gui->diff[$key]['count']);
    }
}
$smarty = new TLSmarty();
...
function initializeGUI(&$dbHandler, $argsObj)
{
    ...
    $gui->subtitle = sprintf($gui->labels['diff_subtitle_tc'],
        $argsObj->version_left, $argsObj->version_left, $argsObj->version_right,
        $argsObj->version_right, $gui->tcaseName);
