...
if ($args->do_upload) {
    ...
    // Firefox and Chrome.
    if ($source != 'none' && $source != '' && ! ($doIt = $_FILES['uploadedFile']['size'] <= $gui->importLimitBytes)) {
    ...
    ))
{
    ...
    for ($idx = 0; $idx < $tc_qty; $idx ++) {
        ...
        // Else Import CF data
        $hasCustomFieldsInfo = (isset($tc['customfields']) &&
        ...
        if ($hasRequirements) {
            if ($tprojectHas['reqSpec']) {
                $msg = processRequirements($db, $req_mgr, $name, $ret['id'],
                    $tc['requirements'], $reqSpecSet, $feedbackMsg);
                if (! is_null($msg)) {
                    $resultMap = array_merge($resultMap, $msg);
                }
            } else {
 ...
 */
function processRequirements(&$dbHandler, &$reqMgr, $tcaseName, $tcaseId, $tcReq,
    $reqSpecSet, $messages)
{
    ...
    foreach ($tcReq as $value) {
        ...
        if ($doit) {
            ...
            if (! isset($missingReqSpecMsg[$value['req_spec_title']])) {
                $missingReqSpecMsg[$value['req_spec_title']] = sprintf(
                    $messages['req_spec'], $value['req_spec_title']);
            }
            $resultMsg[] = array(
                $tcaseName,
    ...
    if ($xml->getName() == 'testsuite') {
        ...
        if ($tsuite['name'] != "") {
            ...
            if ($doCF) {
                ...
                if (! is_null($cf)) {
                    processTestSuiteCF($tsuiteMgr, $xml, $cfSpec, $cf, $tsuite,
                        $tproject_id);
                }
            }
 ...
 *
 */
function processTestSuiteCF(&$tsuiteMgr, $xmlObj, &$cfDefinition, &$cfValues,
    $tsuite, $tproject_id)
{
    static $messages;
    static $missingCfMsg;
