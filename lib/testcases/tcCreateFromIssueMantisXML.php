 ...
 * returns:
 */
function importIssueFromXML(&$db, $fileName, $parentID, $tproject_id, $userID,
    $options = null)
{
    $resultMap = null;
    $my = array();
    ...
    if (file_exists($fileName)) {
        ...
        if ($xml !== false) {
            $resultMap = importTestCasesFromIssueSimpleXML($db, $xml, $parentID,
                $tproject_id, $userID, null, $duplicateLogic);
        }
    }
    return $resultMap;
}
...
/**
 */
function importTestCasesFromIssueSimpleXML(&$db, &$simpleXMLObj, $parentID,
...
function getTestCaseSetFromIssueSimpleXMLObj($xmlObj)
{
    ...
    for ($idx = 0; $idx < $loops2do; $idx ++) {
        ...
        if (! is_null($dummy['steps_to_reproduce'])) {
            $isum .= $nl . $l18n['issue_steps_to_reproduce'] . $nl .
                $dummy['steps_to_reproduce'];
        }
        if (! is_null($dummy['additional_information'])) {
            $isum .= $nl . $l18n['issue_additional_information'] . $nl .
