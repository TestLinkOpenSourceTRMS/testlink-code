...
switch ($args->doAction) {
    ...
    case "edit":
    case "doCreate":
        $op = $commandMgr->$pfn($args, $opt_cfg, array_keys($testCaseEditorKeys),
            $_REQUEST);
        $doRender = true;
        break;
...
if ($args->delete_tc_version) {
    ...
    $tsuite_mgr->show($smarty, $gui, $templateCfg->template_dir,
        $args->old_container_id);
    ...
    if ($result['status_ok']) {
        ...
        $user_feedback = sprintf(lang_get('tc_copied'), $tc_info[0]['name'],
            $path);
    }
 ...
 *
 * @param stdClass $cfgObj
 * @param string $otName
 * @param testcase $tcaseMgr
 * @return stdClass
 */
function initArgs(&$cfgObj, $otName, &$tcaseMgr)
{
    ...
    $args->tcase_id = isset($_REQUEST['testcase_id']) ? intval(
        $_REQUEST['testcase_id']) : 0;
    if ($args->tcase_id == 0) {
        $args->tcase_id = isset($_REQUEST['tcase_id']) ? intval(
            $_REQUEST['tcase_id']) : 0;
    }
    if ($args->tcase_id == 0) {
        $args->tcase_id = intval(
            isset($_REQUEST['relation_source_tcase_id']) ? $_REQUEST['relation_source_tcase_id'] : 0);
    }

    $args->tcversion_id = isset($_REQUEST['tcversion_id']) ? intval(
        $_REQUEST['tcversion_id']) : 0;
    ...
    $args->new_container_id = isset($_REQUEST['new_container']) ? intval(
        $_REQUEST['new_container']) : 0;
    $args->old_container_id = isset($_REQUEST['old_container']) ? intval(
        $_REQUEST['old_container']) : 0;
    $args->has_been_executed = isset($_REQUEST['has_been_executed']) ? intval(
        $_REQUEST['has_been_executed']) : 0;
    ...
    $args->estimatedExecDuration = isset(
        $_REQUEST['estimated_execution_duration']) ? $_REQUEST['estimated_execution_duration'] : null;
    ...
    $args->show_mode = (isset($_REQUEST['show_mode']) &&
        $_REQUEST['show_mode'] != '') ? $_REQUEST['show_mode'] : null;

    // Multiple Test Case Steps Feature
    $args->step_number = isset($_REQUEST['step_number']) ? intval(
        $_REQUEST['step_number']) : 0;
    ...
    $args->refreshTree = isset($_SESSION['setting_refresh_tree_on_action']) ? intval(
    ...
    $args->tc_status = isset($_REQUEST['tc_status']) ? intval(
        $_REQUEST['tc_status']) : $args->tcStatusCfg['status_code']['draft'];
    ...
    $args->relation_id = intval(
        isset($_REQUEST['relation_id']) ? $_REQUEST['relation_id'] : 0);

    $args->relation_destination_tcase = isset(
        $_REQUEST['relation_destination_tcase']) ? $_REQUEST['relation_destination_tcase'] : null;
    ...
    $args->dummy = $tcaseMgr->getInternalID($args->relation_destination_tcase,
        $getOpt);
    ...
    $args->tckw_link_id = isset($_GET['tckw_link_id']) ? intval(
        $_GET['tckw_link_id']) : 0;

    $args->tcplat_link_id = isset($_GET['tcplat_link_id']) ? intval(
        $_GET['tcplat_link_id']) : 0;

    $args->tplan_id = isset($_REQUEST['tplan_id']) ? intval(
        $_REQUEST['tplan_id']) : 0;
    $args->platform_id = isset($_REQUEST['platform_id']) ? intval(
    ...
    $args->copyOnlyLatestVersion = isset($_REQUEST['copy_latest_version']) ? 1 : 0;
...
/**
 * initializeOptionTransferCfg
 *
 * @param string $otName
 * @param stdClass $argsObj
 * @param testproject $tprojectMgr
 * @return stdClass
 */
function initializeOptionTransferCfg($otName, &$argsObj, &$tprojectMgr)
{
    ...
    switch ($argsObj->doAction) {
        ...
        case 'doCreate':
            ...
            $otCfg->from->map = $tprojectMgr->get_keywords_map(
...
function createWebEditors($basehref, $editorCfg, $editorSet = null)
{
    ...
    $owe->cfg = [
        'summary' => [
            'height' => $editorsCfg['summary']['height']
        ],
        'preconditions' => [
            'height' => $editorsCfg['preconditions']['height']
        ],
        'steps' => [
            'rows' => null,
...
function initializeGui(&$dbHandler, &$argsObj, $cfgObj, &$tcaseMgr, &$tprojMgr)
{
    ...
    $guiObj->btn_reorder_testcases = lang_get(
    ...
    if ($argsObj->container_id > 0) {
        $pnode_info = $tcaseMgr->tree_manager->get_node_hierarchy_info(
            $argsObj->container_id);
        $node_descr = array_flip(
            $tcaseMgr->tree_manager->get_available_node_types());
        $guiObj->parent_info['name'] = $pnode_info['name'];
        $guiObj->parent_info['description'] = lang_get(
            $node_descr[$pnode_info['node_type_id']]);
    }

    $guiObj->direct_link = $tcaseMgr->buildDirectWebLink($_SESSION['basehref'],
    ...
    $editorKeys)
{
    ...
    foreach ($oWebEditor->cfg as $key => $value) {
        ...
        if ($initWebEditorFromTemplate) {
            $of->Value = getItemTemplateContents('testcase_template',
                $of->InstanceName, '');
    ...
    switch ($argsObj->doAction) {
        ...
        case "addPlatform":
            ...
            $tplDir = (! isset($opObj->template_dir) ||
                is_null($opObj->template_dir)) ? $templateCfg->template_dir : $opObj->template_dir;
 ...
 *
 * @param TLSmarty $tplEng
 * @param stdClass $argsObj
 * @param stdClass $guiObj
 * @param testcase $tcaseMgr
 * @param int $sourceTCVID
 */
function createNewVersion(&$tplEng, &$argsObj, &$guiObj, &$tcaseMgr,
    $sourceTCVID)
{
    $user_feedback = '';
    $msg = lang_get('error_tc_add');

    $op = $tcaseMgr->create_new_version($argsObj->tcase_id, $argsObj->user_id,
        $sourceTCVID);
