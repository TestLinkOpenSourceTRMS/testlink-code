...
switch ($args->level) {
    case "testcase":
        $xmlResponse = remoteExecTestcase($db, $args->testcase_id, $msg);
        break;
    case "testsuite":
    case "testproject":
        // @TODO schlundus, investigate this!
        $tcase_parent_id = $_REQUEST[$args->level . "_id"];
        $xmlResponse = remoteExecTestcaseSet($db, $tcase_parent_id, $msg);
        break;
    default:
...
function remoteExecTestcase(&$db, $tcase_id, $msg)
{
    ...
    $xmlResponse = '<tr><th colspan="2">' . lang_get('result_after_exec') .
        " {$myMessage}</th></tr>";

    if ($myResult != - 1 && $myNotes != - 1) {
        $xmlResponse .= "<tr><td>" . lang_get('tcexec_result') . "</td>" .
            "<td>{$myResult}</td></tr>" . "<tr><td>" . lang_get('tcexec_notes') .
            "</td>" . "<td> {$myNotes}</td></tr>";
    } else {
 ...
 *
 */
function remoteExecTestcaseSet(&$db, $parent_id, $msg)
{
    $cfield_manager = new cfield_mgr($db);
    $tree_manager = new tree($db);
    $xmlResponse = null;
    ...
    foreach ($subtree_list as $_key => $_value) {
        if (is_array($_value) &&
            $_value['node_type_id'] == $node_type['testcase']) {
            $executionResults[$_value['id']] = executeTestCase($_value['id'],
