...
if ($args->doExport || ($args->doExportSkel && ! $gui->oneTestCaseExport)) {
    ...
    if ($pfn) {
        if ($gui->oneTestCaseExport) {
            ...
            $content .= $tsuite_mgr->$pfn($args->container_id,
                $args->tproject_id, $opt);
        }
...
function initArgs(&$dbHandler)
{
    ...
    $args->tproject_id = intval(
        isset($_REQUEST['tproject_id']) ? $_REQUEST['tproject_id'] : 0);
    if ($args->tproject_id > 0) {
        $dummy = $omgr->tree_manager->get_node_hierarchy_info(
            $args->tproject_id);
        if (! is_null($dummy)) {
