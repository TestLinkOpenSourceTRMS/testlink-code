...
if ($args->do_upload) {
    ...
    if ($doIt) {
        ...
        if ($gui->file_check['status_ok'] && $pimport_fn) {
            ...
            $opt['duplicateLogic'] = array(
                'hitCriteria' => $args->hit_criteria,
    ...
    if (file_exists($fileName)) {
        ...
        if ($xml) {
            ...
            if ($xmlKeywords) {
                ...
                $kwMap = is_null($kwMap) ? null : array_change_key_case(
                    array_flip($kwMap), CASE_LOWER);
            }

            if (! $useRecursion && ($xml->getName() == 'testcases')) {
                $resultMap = importTestCasesFromSimpleXML($db, $xml, $parentID,
                    $tproject_id, $userID, $kwMap, $duplicateLogic);
            }

            if ($useRecursion && ($xml->getName() == 'testsuite')) {
                $resultMap = importTestSuitesFromSimpleXML($db, $xml,
                    intval($parentID), intval($tproject_id), $userID, $kwMap,
                    $duplicateLogic, $importIntoProject);
            }
        }
    }
    return $resultMap;
    ...
    ))
{
    ...
    if (is_null($messages)) {
        ...
        $userRights['can_edit_executed'] = $userObj->hasRight($db,
            'testproject_edit_executed_testcases', $tproject_id);
        $userRights['can_link_to_req'] = $userObj->hasRight($db,
            'req_tcase_link_management', $tproject_id);
        $userRights['can_assign_keywords'] = $userObj->hasRight($db,
            'keyword_assignment', $tproject_id);

        $k2l = array(
            'already_exists_updated',
            'original_name',
            'testcase_name_too_long',
            'already_exists_not_updated',
        ...
        $messages['reqspec_warning'] = lang_get(
            'no_reqspec_defined_can_not_import');

        $feedbackMsg['cfield'] = lang_get(
        ...
        $feedbackMsg['attachment'] = lang_get(
            'attachment_skipped_during_import');
    ...
    for ($idx = 0; $idx < $tc_qty; $idx ++) {
        ...
        if ($duplicatedLogic['actionOnHit'] == 'skip' && ! empty($dupInfo)) {
            $resultMap[] = array(
                $name,
                $messages['already_exists_skipped']
            );
            continue;
        }
        ...
        $node_order = isset($tc['node_order']) ? intval($tc['node_order']) : ($idx +
            1);
        $internalid = $tc['internalid'];
        ...
        $attr = null;
        if (isset($tc['estimated_exec_duration']) &&
            ! is_null($tc['estimated_exec_duration'])) {
            $attr['estimatedExecDuration'] = trim(
                $tc['estimated_exec_duration']);
            $attr['estimatedExecDuration'] = $attr['estimatedExecDuration'] == '' ? null : floatval(
                $attr['estimatedExecDuration']);
        }
        ...
        if (! is_null($tc['author_login'])) {
            if (isset($userIDCache[$tc['author_login']])) {
                ...
                if ($userObj->readFromDB($db, tlUser::USER_O_SEARCH_BYLOGIN) ==
                    tl::OK) {
        ...
        if ($name_len > $fieldSizeCfg->testcase_name) {
            ...
            $xx .= sprintf($messages['testcase_name_too_long'], $name_len,
                $fieldSizeCfg->testcase_name) . "\n";
        ...
        if (isset($tc['keywords']) && $tc['keywords']) {
            if (! $userRights['can_assign_keywords']) {
                $resultMap[] = array(
                    $name,
                    $messages['keywords_assignment_skipped_during_import']
                );
            } else {
            ...
            if ($item_id > 0) {
                ...
                if ($owner != $container_id) {
                    ...
                    $stain[$n - 1] = $tcasePrefix .
                        config_get('testcase_cfg')->glue_character . $externalid .
                        ':' . $stain[$n - 1];
                    $stain = implode('/', $stain);
        ...
        $hasRequirements = (isset($tc['requirements']) &&
            ! is_null($tc['requirements']));
        ...
        if ($hasRequirements) {
            if ($tprojectHas['reqSpec']) {
                ...
                if (! $userRights['can_link_to_req']) {
                    $msg[] = array(
                        $name,
                        $messages['req_assignment_skipped_during_import']
                    );
                } else {
                    $msg = processRequirements($db, $req_mgr, $name, $ret,
                        $tc['requirements'], $reqSpecSet, $feedbackMsg, $userID);
                }
                ...
                $msg[] = array(
                    $name,
                    $messages['reqspec_warning']
                );
                $resultMap = array_merge($resultMap, $msg);
            }
        }

        $hasAttachments = (isset($tc['attachments']) &&
            ! is_null($tc['attachments']));
        if ($hasAttachments) {
            ...
            $msg = processAttachments($db, true, $name, $internalid, $fk_id,
                $tc['attachments'], $feedbackMsg);
            if (! is_null($msg)) {
                $resultMap = array_merge($resultMap, $msg);
            }
        }
 ...
 */
function checkXMLTCTsuite($fileName, $recursiveMode)
{
    $xml = @simplexml_load_file_wrapper($fileName);
    $file_check = array(
        'status_ok' => 0,
...
function initArgs()
{
    ...
    $args->container_id = isset($_REQUEST['containerID']) ? intval(
        $_REQUEST['containerID']) : 0;
    $args->bIntoProject = isset($_REQUEST['bIntoProject']) ? intval(
        $_REQUEST['bIntoProject']) : 0;

    $args->containerType = isset($_REQUEST['containerType']) ? intval(
        $_REQUEST['containerType']) : 0;
 ...
 */
function processCustomFields(&$tcaseMgr, $tcaseName, $tcversionId, $cfValues,
    $cfDefinition, $messages)
{
    static $missingCfMsg;
    $cf2insert = null;
    $resultMsg = null;
    ...
    foreach ($cfValues as $value) {
        if (isset($cfDefinition[$value['name']])) {
            $cf2insert[$cfDefinition[$value['name']]['id']] = array(
                'type_id' => $cfDefinition[$value['name']]['type'],
                'cf_value' => $value['value']
            );
        } else {
            if (! isset($missingCfMsg[$value['name']])) {
                $missingCfMsg[$value['name']] = sprintf($messages['cfield'],
                    $value['name'], $messages['tcase']);
            }
            $resultMsg[] = array(
                $tcaseName,
                $missingCfMsg[$value['name']]
            );
        }
    }

    $tcaseMgr->cfield_mgr->design_values_to_db($cf2insert, $tcversionId, null,
        'simple');
    return $resultMsg;
}
    ...
    $tcReq, $reqSpecSet, $messages, $userID)
{
    ...
    foreach ($tcReq as $value) {
        ...
        if ($doit = isset($reqSpecSet[$value['doc_id']]) &&
            ! (isset($cachedReqSpec[$value['req_spec_title']]))) {
            // $cachedReqSpec
        ...
        if ($doit) {
            ...
            if ($useit) {
                ...
                $resultMsg[] = array(
                    $tcaseName,
                    $missingReqMsg[$value['doc_id']]
                );
            }
        } else {
            // We didnt find Req Doc ID in Req Spec Set got from DB
            if (! isset($missingReqInDBMsg[$value['doc_id']])) {
                $missingReqInDBMsg[$value['doc_id']] = sprintf(
                    $messages['reqNotInDB'], $value['doc_id'], '');
            }
            $resultMsg[] = array(
                $tcaseName,
                $missingReqInDBMsg[$value['doc_id']]
            );
        }
    } // foreach
 ...
 * Else return an array of messages.
 */
function processAttachments(&$dbHandler, $isTestCase, $tcaseName, $xmlInternalID,
    $fk_Id, $tcAtt, $messages)
{
    ...
    foreach ($tcAtt as $value) {
        ...
        if ($xmlInternalID == $fk_Id) // internalID matches, seems to be an update
        {
            ...
            if ($addAttachment === false) { // inform user that the attachment has been skipped
                if (! isset($duplicateAttachment[$value['id']])) {
                    $duplicateAttachment[$value['id']] = sprintf(
                        $messages['attachment'], $value['name']);
                }
                $resultMsg[] = array(
                    $tcaseName,
                    $duplicateAttachment[$value['id']]
                );
            }
        } else {
            // Creation
        ...
        if ($addAttachment) {
            ...
            $tableRef = $tables['nodes_hierarchy'];
            if ($isTestCase) {
                $tableRef = $tables['tcversions'];
            }
            $iaOp = $attachRepo->insertAttachment($fk_Id, $tableRef,
                $value['title'], $fileInfo);
        }
    }
...
/**
 */
function importTestCasesFromSimpleXML(&$db, &$simpleXMLObj, $parentID,
    $tproject_id, $userID, $kwMap, $duplicateLogic)
{
    $resultMap = null;
    ...
    for ($idx = 0; $idx < $loops2do; $idx ++) {
        ...
        if ($tc) {
            ...
            if ($cfg->wordwrap->summary > 0) {
                $tc['summary'] = wordwrap($tc['summary'],
                    $cfg->wordwrap->summary);
            }
            if ($cfg->wordwrap->preconditions > 0) {
                $tc['preconditions'] = wordwrap($tc['preconditions'],
                    $cfg->wordwrap->preconditions);
            }
            ...
            if ($cfg->wordwrap->actions > 0 ||
                $cfg->wordwrap->expected_results > 0) {
                foreach ($tc['steps'] as $sdx => $elem) {

                    if ($cfg->wordwrap->actions > 0) {
                        $tc['steps'][$sdx]['actions'] = wordwrap(
                            $tc['steps'][$sdx]['actions'],
                            $cfg->wordwrap->actions);
                    }

                    if ($cfg->wordwrap->expected_results > 0) {
                        $tc['steps'][$sdx]['expected_results'] = wordwrap(
                            $tc['steps'][$sdx]['expected_results'],
                            $cfg->wordwrap->expected_results);
                    }
                }
            }

            $keywords = getKeywordsFromSimpleXMLObj(
                $xmlTCs[$idx]->keywords->keyword);
            if ($keywords) {
                $tc['keywords'] = $keywords;
            }

            $cf = getCustomFieldsFromSimpleXMLObj(
                $xmlTCs[$idx]->custom_fields->custom_field);
            if ($cf) {
                $tc['customfields'] = $cf;
            }

            $requirements = getRequirementsFromSimpleXMLObj(
                $xmlTCs[$idx]->requirements->requirement);
            if ($requirements) {
                $tc['requirements'] = $requirements;
            }

            $attachments = getAttachmentsFromSimpleXMLObj(
                $xmlTCs[$idx]->attachments->attachment);
            if ($attachments) {
...
function getStepsFromSimpleXMLObj($simpleXMLItems)
{
    ...
    $itemStructure['transformations'] = array(
...
function getCustomFieldsFromSimpleXMLObj($simpleXMLItems)
{
    ...
    return getItemsFromSimpleXMLObj($simpleXMLItems, $itemStructure);
}
...
function getKeywordsFromSimpleXMLObj($simpleXMLItems)
{
    ...
    $itemStructure['attributes'] = array(
        'string' => array(
            "name" => 'trim'
        )
    );
 ...
 *
 */
function importTestSuitesFromSimpleXML(&$dbHandler, &$xml, $parentID,
    $tproject_id, $userID, $kwMap, $duplicateLogic, $importIntoProject = 0)
{
    static $tsuiteXML;
    static $tsuiteMgr;
    static $myself;
    ...
    if (is_null($tsuiteXML)) {
        ...
        $doCF = ! is_null(
            ($cfSpec = $tsuiteMgr->get_linked_cfields_at_design(null, null, null,
                $tproject_id, 'name')));
    }
    ...
    if ($xml->getName() == 'testsuite') {
        ...
        if ($tsuite['name'] != "") {
            ...
            if (is_null($info)) {
                $ret = $tsuiteMgr->create($parentID, $tsuite['name'],
            ...
            if ($doCF) {
                $cf = getCustomFieldsFromSimpleXMLObj(
                    $xml->custom_fields->custom_field);
                if (! is_null($cf)) {
                    processTestSuiteCF($tsuiteMgr, $cfSpec, $cf, $tsuite);
                }
            }
            ...
            if ($attachments = getAttachmentsFromSimpleXMLObj(
                $xml->attachments->attachment) && ! is_null($attachments)) {
                ...
                $msg = processAttachments($dbHandler, false, $tsuite['name'],
                    $tsuiteXMLID, $tsuite['id'], $attachments, $feedbackMsg);
                if (! is_null($msg)) {
                    $resultMap = array_merge($resultMap, $msg);
                }
            }
        ...
        for ($idx = 0; $idx < $loop2do; $idx ++) {
            ...
            switch ($target->getName()) {
                case 'testcase':
                    ...
                    $tcData = getTestCaseSetFromSimpleXMLObj(array(
                        $target
                    ));
                    if (trim($tcData[0]['name']) == '') {
                        ...
                        $resultMap = array_merge($resultMap, array(
                            $xx
                        ));
                    } else {
 ...
 *
 */
function processTestSuiteCF(&$tsuiteMgr, &$cfDefinition, &$cfValues, $tsuite)
{
    static $messages;
    static $missingCfMsg;
    ...
    $tsuiteMgr->cfield_mgr->design_values_to_db($cf2insert, $tsuite['id'], null,
        'simple');
    return $resultMsg;
}
...
function getReqSpecSet(&$dbHandler, $tproject_id)
{
    ...
    return $dbHandler->fetchRowsIntoMap($sql, 'req_doc_id');
}
