...
if ($args->tprojectID && $args->doAction == 'doSearch') {
    ...
    if ($args->targetTestCase != "" &&
        strcmp($args->targetTestCase, $gui->tcasePrefix) != 0) {
    ...
    if ($args->steps != "") {
        ...
        $filterSpecial['by_steps'] = $feOp .
            " TCSTEPS.actions like '%{$args->steps}%' ";
    }

    if ($args->expected_results != "") {
        $args->expected_results = $db->prepare_string($args->expected_results);
        $filterSpecial['by_expected_results'] = $feOp .
    ...
    if ($args->custom_field_id > 0) {
        ...
        switch ($gui->cf_types[$cf_def['type']]) {
            case 'date':
                $args->custom_field_value = $tproject_mgr->cfield_mgr->cfdate2mktime(
            ...
            case 'datetime':
                $args->custom_field_value = $tproject_mgr->cfield_mgr->cfdatetime2mktime(
                    $args->custom_field_value);
    ...
    $applyFilters = true;
    if (! is_null($filter) && isset($filter['by_tc_id']) && ! is_null($tcaseID) &&
        ($tcaseID <= 0)) {
        // get the right feedback message
        $applyFilters = false;
        $gui->warning_msg = $tcaseID == 0 ? lang_get('testcase_does_not_exists') : lang_get(
            'prefix_does_not_exists');
    }
    ...
    if (null != $gui->resultSet && count($gui->resultSet) > 0) {
        ...
        foreach ($gui->resultSet as $result) {
            ...
            $history_link = "<a href=\"javascript:openExecHistoryWindow({$result['testcase_id']});\">" .
                "<img title=\"" . lang_get('execution_history') .
                "\" src=\"{$history_icon}\" /></a> ";
            ...
            $tcaseName = htmlentities($gui->tcasePrefix, ENT_QUOTES, $charset) .
                $result['tc_external_id'] . " [v" . $result['version'] . "]" .
                $titleSeperator .
        ...
        $table = new tlExtTable($columns, $matrixData,
            'tl_table_test_case_search');
...
function initArgs(&$tprojectMgr)
{
    ...
    foreach ($k2w as $key => $value) {
        if (isset($args->$key) && $args->$key != '') {
            ...
            if ($da != null) {
                $args->$key = $da['year'] . "-" . $da['month'] . "-" . $da['day'] .
                    $value; // set date in iso format
...
function initializeGui(&$argsObj, &$tprojectMgr)
{
    ...
    $gui->search_important_notice = sprintf(lang_get('search_important_notice'),
...
function initSearch(&$gui, &$argsObj, &$tprojectMgr)
{
    ...
    $gui->tcversion = (is_null($argsObj->version) || $argsObj->version == '') ? '' : intval(
        $argsObj->version);

    $gui->tcasePrefix = $tprojectMgr->getTestCasePrefix($argsObj->tprojectID) .
