...
class testcaseCommands
{
    ...
    /**
     */
    public function __construct(&$db, &$userObj, $tproject_id)
    {
        $this->db = $db;
    ...
    public function initGuiBean(&$argsObj)
    {
        ...
        $obj->codeTrackerEnabled = $this->tprojectMgr->isCodeTrackerEnabled(
            $this->tproject_id);
    ...
    public function initTestCaseBasicInfo(&$argsObj, &$guiObj, $opt = null)
    {
        ...
        if ($my['opt']['accessByStepID']) {
            foreach ($greenCard as $ky) {
                ...
                if ($ky == 0) {
                    $greenCard = $this->tcaseMgr->getIdCardByStepID(
                        $argsObj->step_id);
                    break;
                }
            }
        ...
        $tcaseInfo = $this->tcaseMgr->get_by_id($greenCard['tcase_id'],
            $greenCard['tcversion_id'], null, $gopt);

        $external = $this->tcaseMgr->getExternalID($greenCard['tcase_id'],
        ...
        $guiObj->authorObj = tlUser::getByID($this->db,
            $guiObj->testcase['author_id']);

        $guiObj->updaterObj = null;
        if (! is_null($guiObj->testcase['updater_id'])) {
            $guiObj->updaterObj = tlUser::getByID($this->db,
                $guiObj->testcase['updater_id']);
        }
        ...
        foreach ($cfPlaces as $cfpKey => $cfpFilter) {
            $guiObj->cfieldsDesignTime[$cfpKey] = $this->tcaseMgr->htmlTableOfCFValues(
    ...
    /**
     */
    public function create(&$argsObj, &$otCfg)
    {
        ...
        if ($argsObj->container_id > 0) {
            $pnode_info = $this->tcaseMgr->tree_manager->get_node_hierarchy_info(
                $argsObj->container_id);
            $node_descr = array_flip(
                $this->tcaseMgr->tree_manager->get_available_node_types());
            $guiObj->parent_info['name'] = $pnode_info['name'];
            $guiObj->parent_info['description'] = lang_get(
                $node_descr[$pnode_info['node_type_id']]);
        ...
        $guiObj->main_descr = $guiObj->parent_info['description'] . $sep_1 .
            $guiObj->parent_info['name'] . $sep_2 . lang_get('title_new_tc');

        $otCfg->to->map = array();
        keywords_opt_transf_cfg($otCfg,
            implode(',', array_keys((array) $parentKeywords)));

        $guiObj->tc = array(
            'id' => 0,
            'name' => '',
            'importance' => config_get('testcase_importance_default'),
            'status' => null,
        ...
        foreach ($cfPlaces as $locationKey => $locationFilter) {
            $guiObj->cf[$locationKey] = $this->tcaseMgr->html_table_of_custom_field_inputs(
                null, null, 'design', '', null, null, $argsObj->testproject_id,
                $locationFilter, $_REQUEST);
        }
        ...
        $guiObj->cancelActionJS .= '&edit=testsuite&level=testsuite&containerType=testsuite' .
            "'";
    ...
    /**
     */
    public function doCreate(&$argsObj, &$otCfg, $oWebEditorKeys, $request)
    {
        $guiObj = $this->create($argsObj, $otCfg, $oWebEditorKeys);

        // compute order
        $new_order = config_get('treemenu_default_testcase_order');
        $co = $this->tcaseMgr->tree_manager->getBottomOrder(
            $argsObj->container_id, array(
                'node_type' => 'testcase'
            ));
        if ($co > 0) {
            $new_order = $co + 1;
        ...
        $tcase = $this->tcaseMgr->create($argsObj->container_id, $argsObj->name,
            $argsObj->summary, $argsObj->preconditions, $argsObj->tcaseSteps,
            $argsObj->user_id, $argsObj->assigned_keywords_list, $new_order,
            testcase::AUTOMATIC_ID, $argsObj->exec_type, $argsObj->importance,
            $options);
        ...
        if ($tcase['status_ok']) {
            ...
            if ($argsObj->stay_here) {
                ...
                $guiObj->user_feedback = sprintf(lang_get('tc_created'),
                    $argsObj->name);
                $guiObj->sqlResult = 'ok';
                $guiObj->initWebEditorFromTemplate = true;
                ...
                $this->show($argsObj, $request, array(
                    'status_ok' => 1
                ));
            }
    ...
    {
        $guiObj = $this->initGuiBean($argsObj);
        $otCfg->to->map = $this->tcaseMgr->get_keywords_map($argsObj->tcase_id,
        ...
        $tc_data = $this->tcaseMgr->get_by_id($argsObj->tcase_id,
            $argsObj->tcversion_id, null, $gopt);
        ...
        foreach ($cfPlaces as $locationKey => $locationFilter) {
            ...
            $cf_smarty[$locationKey] = $this->tcaseMgr->html_table_of_custom_field_inputs(
                $argsObj->tcase_id, null, 'design', '', $argsObj->tcversion_id,
                null, null, $locationFilter);
        }
        ...
        $guiObj->cancelActionJS = 'location.href=fRoot+' . "'" .
            ...
            "&tcversion_id=" . $argsObj->tcversion_id . '&edit=testcase&id=' .
    ...
    public function doUpdate(&$argsObj, $request)
    {
        ...
        $ret = $this->tcaseMgr->update($argsObj->tcase_id,
            $argsObj->tcversion_id, $argsObj->name, $argsObj->summary,
            $argsObj->preconditions, $argsObj->tcaseSteps, $argsObj->user_id,
            $argsObj->assigned_keywords_list, testcase::DEFAULT_ORDER,
     ...
     * doAdd2testplan
     */
    public function doAdd2testplan(&$argsObj, $request)
    {
     ...
     * @internal revisions
     */
    public function delete(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->delete_message = '';
        $cfg = config_get('testcase_cfg');

        $guiObj->exec_status_quo = $this->tcaseMgr->getExecStatus(
            $argsObj->tcase_id, null, array(
                'addExecIndicator' => true
            ));
        $guiObj->delete_enabled = 1;
        if ($guiObj->exec_status_quo['executed'] &&
            ! $this->grants->testproject_delete_executed_testcases) {
            $guiObj->delete_message = lang_get(
                'system_blocks_delete_executed_tc_when');
        ...
        if (! is_null($guiObj->exec_status_quo)) {
            ...
            foreach ($versionSet as $version_id) {
                ...
                foreach ($tplanSet as $tplan_id) {
                    if ($guiObj->display_platform = ! isset(
                        $guiObj->exec_status_quo[$version_id][$tplan_id][0])) {
                        $stop = true;
                        break;
                    }
                }
        ...
        $guiObj->main_descr = lang_get('title_del_tc') . TITLE_SEP . $external_id .
            TITLE_SEP . $tcinfo[0]['name'];
    ...
    public function doDelete(&$argsObj, $request)
    {
        ...
        $guiObj->main_descr = lang_get('title_del_tc') . ":" . $external_id .
            TITLE_SEP . htmlspecialchars($tcinfo[0]['name']);

        if ($argsObj->tcversion_id == testcase::ALL_VERSIONS) {
            $guiObj->refreshTree = 1;
            logAuditEvent(TLS("audit_testcase_deleted", $external_id), "DELETE",
                $argsObj->tcase_id, "testcases");
            $guiObj->user_feedback = sprintf(lang_get('tc_deleted'),
                ":" . $external_id . TITLE_SEP . $tcinfo[0]['name']);
        } else {
            $guiObj->main_descr .= " " . lang_get('version') . " " .
                $tcinfo[0]['version'];
            // When deleting JUST one version, there is no need to refresh tree
            $guiObj->refreshTree = 0;
            logAuditEvent(
                TLS("audit_testcase_version_deleted", $tcinfo[0]['version'],
                    $external_id), "DELETE", $argsObj->tcase_id, "testcases");
            $guiObj->user_feedback = sprintf(lang_get('tc_version_deleted'),
                $tcinfo[0]['name'], $tcinfo[0]['version']);
        }
     ...
     * createStep
     */
    public function createStep(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);

        $this->initTestCaseBasicInfo($argsObj, $guiObj);

        $guiObj->main_descr = sprintf(lang_get('create_step'),
            $guiObj->testcase['tc_external_id'] . ':' . $guiObj->testcase['name'],
            $guiObj->testcase['version']);

        $max_step = $this->tcaseMgr->get_latest_step_number(
            $argsObj->tcversion_id);
        $max_step ++;
        ...
        $guiObj->step_set = $this->tcaseMgr->get_step_numbers(
            $argsObj->tcversion_id);
        $guiObj->step_set = is_null($guiObj->step_set) ? '' : implode(",",
            array_keys($guiObj->step_set));
        $guiObj->loadOnCancelURL = sprintf($guiObj->loadOnCancelURL,
            $argsObj->tcase_id, $argsObj->tcversion_id);
     ...
     * doCreateStep
     */
    public function doCreateStep(&$argsObj, $request, $doAndExit = false)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        ...
        $new_step = $this->tcaseMgr->get_latest_step_number(
            $argsObj->tcversion_id);
        $new_step ++;
        ...
        if ($op['status_ok']) {
            ...
            $guiObj->user_feedback = sprintf(lang_get('step_number_x_created'),
                $argsObj->step_number);
            $guiObj->step_exec_type = $guiObj->testcase['execution_type'];
            $guiObj->cleanUpWebEditor = true;
            $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,
                $argsObj->user_id);
            $this->initTestCaseBasicInfo($argsObj, $guiObj);
        }
        ...
        if (! $guiObj->doExit) {
            ...
            // Get all existent steps
            $guiObj->tcaseSteps = $this->tcaseMgr->get_steps(
                $argsObj->tcversion_id);
            $max_step = $this->tcaseMgr->get_latest_step_number(
                $argsObj->tcversion_id);
            $max_step ++;
     ...
     * doCreateStepAndExit
     */
    public function doCreateStepAndExit(&$argsObj, $request)
    {
        ...
        if ($guiObj->doExit) {
            ...
            $opt = array(
                'updateCFOnDB' => ! self::UPDATECFONDB
            );
            $this->show($argsObj, $request, array(
                'status_ok' => true
            ), $opt);
     ...
     * editStep
     */
    public function editStep(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $this->initTestCaseBasicInfo($argsObj, $guiObj);
     ...
     * doUpdateStep
     */
    public function doUpdateStep(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        ...
        $this->tcaseMgr->update_step($argsObj->step_id, $argsObj->step_number,
            $argsObj->steps, $argsObj->expected_results, $argsObj->exec_type);
        ...
        return $this->editStep($argsObj, $request);
    }

    /**
     * doUpdateStepAndExit
     */
    public function doUpdateStepAndExit(&$argsObj, $request)
    {
     ...
     * doReorderSteps
     */
    public function doReorderSteps(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
     ...
     * doDeleteStep
     */
    public function doDeleteStep(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        ...
        $guiObj->refreshTree = 0;
        $step_node = $this->tcaseMgr->tree_manager->get_node_hierarchy_info(
            $argsObj->step_id);

        $tcversion_node = $this->tcaseMgr->tree_manager->get_node_hierarchy_info(
            $step_node['parent_id']);
        $argsObj->tcversion_id = $step_node['parent_id'];
        $argsObj->tcase_id = $tcversion_node['parent_id'];

        $guiObj->user_feedback = '';
        $this->tcaseMgr->delete_step_by_id($argsObj->step_id);
        $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,
            $argsObj->user_id);
     ...
     * doCopyStep
     */
    public function doCopyStep(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
        ...
        $source_info = $this->tcaseMgr->get_steps($argsObj->tcversion_id,
            $argsObj->step_number);
        $source_info = current($source_info);
        $op = $this->tcaseMgr->create_step($argsObj->tcversion_id, $new_step,
            $source_info['actions'], $source_info['expected_results'],
            $source_info['execution_type']);

        if ($op['status_ok']) {
            $guiObj->user_feedback = sprintf(
                lang_get('step_number_x_created_as_copy'), $new_step,
                $argsObj->step_number);
     ...
     * doInsertStep
     */
    public function doInsertStep(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
    ...
    /**
     */
    public function doResequenceSteps(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
    ...
    /**
     */
    public function setImportance(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
        ...
        $this->tcaseMgr->setImportance($argsObj->tcversion_id,
            $argsObj->importance);
        $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,
            $argsObj->user_id);
    ...
    /**
     */
    public function setStatus(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
    ...
    /**
     */
    public function setExecutionType(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
        ...
        $opx = array(
            'updSteps' => $argsObj->applyExecTypeChangeToAllSteps
        );
        $this->tcaseMgr->setExecutionType($argsObj->tcversion_id,
            $argsObj->exec_type, $opx);
    ...
    /**
     */
    public function setEstimatedExecDuration(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
        ...
        $this->tcaseMgr->setEstimatedExecDuration($argsObj->tcversion_id,
            $argsObj->estimatedExecDuration);
        $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,
            $argsObj->user_id);
    ...
    /**
     */
    public function show(&$argsObj, $request, $userFeedback, $opt = null)
    {
        $smartyObj = new TLSmarty();
        ...
        $guiObj->direct_link = $this->tcaseMgr->buildDirectWebLink(
            $_SESSION['basehref'], $argsObj->tcase_id, $argsObj->testproject_id);
        ...
        if ($userFeedback['status_ok']) {
            if ($options['updateTPlanLinkToTCV']) {
                ...
                $guiObj->additionalURLPar = "&updateTCVToThis=" .
                    $identity->version_id . "&followTheWhiteRabbit=1";
            ...
            if ($updateCFOnDB) {
                $cfCtx = array(
                    'tproject_id' => $identity->tproject_id,
                    'enabled' => 1,
                    'node_type' => 'testcase'
                );
                $cf_map = $this->tcaseMgr->cfield_mgr->getLinkedCfieldsAtDesign(
                    $cfCtx);

                $this->tcaseMgr->cfield_mgr->design_values_to_db($request,
                    $identity->version_id, $cf_map);
            }

            $guiObj->attachments[$identity->version_id] = getAttachmentInfosFrom(
                $this->tcaseMgr, $identity->version_id);
        } else {
    ...
    private function buildIdentity($cred)
    {
        ...
        if (property_exists($cred, 'tproject_id')) {
            ...
            throw new Exception(
                __METHOD__ . ' EXCEPTION: test project ID, is mandatory');
        }
    ...
    public function doAddRelation(&$argsObj, &$request)
    {
        ...
        if ($argsObj->destination_tcase_id > 0) {
            ...
            $ret = $this->tcaseMgr->addRelation($source_id, $destination_id,
                $relTypeInfo[0], $argsObj->user_id);
            $guiObj->user_feedback = sprintf(lang_get($ret['msg']),
                $argsObj->relation_destination_tcase);
        } else {
            $guiObj->user_feedback = sprintf(
                lang_get('testcase_doesnot_exists'),
                $argsObj->relation_destination_tcase);
        }
     ...
     * doUpdateStepAndExit
     */
    public function doUpdateStepAndInsert(&$argsObj, $request)
    {
        $this->doUpdateStep($argsObj, $request);
    ...
    /**
     */
    public function removeKeyword(&$argsObj, &$request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        ...
        if ($argsObj->tckw_link_id > 0) {
            $this->tcaseMgr->deleteKeywordsByLink($guiObj->tcase_id,
                $argsObj->tckw_link_id, testcase::AUDIT_ON);
        }
    ...
    public function freeze(&$argsObj, $request)
    {
        $argsObj->isOpen = 0;
        return $this->setIsOpen($argsObj, $request);
    }

    public function unfreeze(&$argsObj, $request)
    {
        $argsObj->isOpen = 1;
        return $this->setIsOpen($argsObj, $request);
    }
    ...
    /**
     */
    public function setIsOpen(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
        ...
        $this->tcaseMgr->setIsOpen(null, $argsObj->tcversion_id,
            $argsObj->isOpen);
        $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,
            $argsObj->user_id);
    ...
    /**
     */
    public function setActiveAttr(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->tcversion_id = $argsObj->tcversion_id;
        ...
        $this->tcaseMgr->updateActiveStatus($argsObj->tcase_id,
        ...
        logAuditEvent(
            TLS($lk, $guiObj->testcase['name'], $guiObj->testcase['version']),
            "{$pre}ACTIVATE", "testcases");

        $this->show($argsObj, $request, [
            'status_ok' => 1
        ], [
            'updateCFOnDB' => false
        ]);
    ...
    /**
     */
    public function addKeyword(&$argsObj, &$request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        ...
        if (null != $argsObj->free_keywords && count($argsObj->free_keywords) > 0) {
            $this->tcaseMgr->addKeywords($guiObj->tcase_id,
                $guiObj->tcversion_id, $argsObj->free_keywords);
            ...
            if (isset($cfx[$info['prefix']]) &&
                ...
                if (method_exists($its, 'addNote')) {
                    $dl = sprintf(lang_get('dlToTCSpecPVCode'), $tcExternalID) .
                        ...
                        $this->tcaseMgr->buildDirectWebLink(
                            $_SESSION['basehref'], $argsObj->tcase_id,
                            $argsObj->testproject_id);

                    // Get keyword for human beins
                    $tbl = tlObject::getDBTables(array(
                        'keywords'
                    ));
     ...
     */
    public function updateTPlanLinkToTCV($argsObj, $request)
    {
        $this->tcaseMgr->updateLatestTPlanLinkToTCV($argsObj->tcversion_id,
            $argsObj->tplan_id);

        $opt = array(
            'updateTPlanLinkToTCV' => true
        );
     ...
     * doStepOperationExit
     */
    public function doStepOperationExit(&$argsObj, $request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        $guiObj->step_exec_type = $argsObj->exec_type;
    ...
    /**
     */
    public function addPlatform(&$argsObj, &$request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
    ...
    /**
     */
    public function removePlatform(&$argsObj, &$request)
    {
        $guiObj = $this->initGuiBean($argsObj);
        $guiObj->user_feedback = '';
        ...
        if ($argsObj->tcplat_link_id > 0) {
            $this->tcaseMgr->deletePlatformsByLink($guiObj->tcase_id,
                $argsObj->tcplat_link_id, testcase::AUDIT_ON);
        }

        // set up for rendering
        $guiObj->template = "archiveData.php?edit=testcase&id={$guiObj->tcase_id}" .
            "&show_mode={$guiObj->show_mode}" . "&caller=removePlatform";
