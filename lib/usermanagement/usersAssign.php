...
if ($args->featureID && $args->doUpdate && $featureMgr &&
    checkRightsForUpdate($db, $args->user, $args->testprojectID,
        $args->featureType, $args->featureID)) {
...
$gui->users = tlUser::getAll($db, "WHERE active=1", null, null,
...
switch ($assignRolesFor) {
    case 'testproject':
        $info = getTestProjectEffectiveRoles($db, $tprojectMgr, $args,
            $gui->users);
    ...
    case 'testplan':
        $info = getTestPlanEffectiveRoles($db, $tplanMgr, $tprojectMgr, $args,
            $gui->users);
        if (is_null($info)) {
...
$gui->grants = getGrantsForUserMgmt($db, $args->user, $target->testprojectID,
    - 1);
$gui->accessTypeImg = '';
...
if (is_null($gui->features) || count($gui->features) == 0) {
    ...
    if (isset($gui->features[$gui->featureID])) {
        ...
        $gui->accessTypeImg = '<img src="' . $imgSet[$accessKey] . '" title="' .
            lang_get('access_' . $accessKey) . '" >';
    }
    $gui->accessTypeImg = '<img src="' . $imgSet[$accessKey] . '" title="' .
        lang_get('access_' . $accessKey) . '" >';
}
...
function initArgs(&$tprojMgr)
{
    ...
    $prjInfo = $tprojMgr->get_by_id($args->testprojectID,
        array(
            'output' => 'name'
        ));
...
function checkRights(&$db, &$user)
{
    ...
    $answers->testplan_user_role_assignment = $user->hasRight($db,
        "testplan_user_role_assignment", $args->testprojectID, - 1);
    if ($answers->testplan_user_role_assignment != "yes") {
        ...
        $answers->testplan_user_role_assignment = $user->hasRight($db,
            "testplan_user_role_assignment", null, $targetTestPlanID);
    }

    $answers->user_role_assignment = $user->hasRight($db, "user_role_assignment",
        null, - 1);
    ...
    if (! $result && ($args->featureType == 'testproject')) {
        ...
        if ($user->hasRight($db, "testproject_user_role_assignment",
            $feature2check, - 1) == "yes") {
            $result = true;
        }
    }
 ...
 * @param testproject $featureType
 * @param int $featureID
 * @return boolean
 */
function checkRightsForUpdate(&$dbHandler, &$user, $testprojectID, $featureType,
    $featureID)
{
    $yes_no = "no";
    switch ($featureType) {
        case 'testproject':
            if ($user->hasRight($dbHandler, "user_role_assignment", $featureID) ==
                "yes" ||
                $user->hasRight($dbHandler, "testproject_user_role_assignment",
                    $featureID, - 1, true) == "yes") {
                $yes_no = "yes";
            }
            break;

        case 'testplan':
            $yes_no = $user->hasRight($dbHandler,
                "testplan_user_role_assignment", $testprojectID, $featureID);
            break;
    }
...
function getTestProjectEffectiveRoles($dbHandler, &$objMgr, &$argsObj, $users)
{
    ...
    foreach ($idSet as $tk) {
        if (! isset($rolesCache[$testprojects[$tk]['effective_role']])) {
            $rolesCache[$testprojects[$tk]['effective_role']] = new tlRole(
                $testprojects[$tk]['effective_role']);
            $rolesCache[$testprojects[$tk]['effective_role']]->readFromDB(
                $dbHandler);
        }
    }

    foreach ($key2loop as $idx) {
        $answer = $rolesCache[$testprojects[$idx]['effective_role']]->hasRight(
    ...
    $effectiveRoles = get_tproject_effective_role($dbHandler,
        array(
            'id' => $argsObj->featureID,
            'is_public' => $featureIsPublic
        ), null, $users);
 ...
 * @return array
 */
function getTestPlanEffectiveRoles(&$dbHandler, &$tplanMgr, $tprojectMgr,
    &$argsObj, &$users)
{
    $features = array();
    $activeTestplans = $tprojectMgr->get_all_testplans($argsObj->testprojectID,
        array(
            'plan_status' => 1
        ));
    ...
    if ($status_ok) {
        $myAccessibleSet = $argsObj->user->getAccessibleTestPlans($dbHandler,
            $argsObj->testprojectID, null, array(
                'output' => 'map'
            ));
        ...
        $key2remove = array_diff($activeKeys, $myKeys);
        ...
        if ($argsObj->user->hasRight($dbHandler, "mgt_users")) {
            ...
            foreach ($key2loop as $idx) {
                if ($argsObj->user->hasRight($dbHandler,
                    "testplan_user_role_assignment", null,
        ...
        $effectiveRoles = get_tplan_effective_role($dbHandler,
            $argsObj->featureID, $tproject_info, null, $users);
        $ret = array(
            $effectiveRoles,
            $features,
            $argsObj->featureID
 ...
 * @return array
 */
function getTestPlanEffectiveRolesNEW(&$dbHandler, &$tplanMgr, $tprojectMgr,
    &$argsObj, &$users)
{
    $features = array();
    $activeTestplans = $tprojectMgr->get_all_testplans($argsObj->testprojectID,
...
function initializeGui(&$dbHandler, $argsObj)
{
    ...
    $gui->optRights = tlRole::getAll($dbHandler, null, null, null,
...
{
    $tg = array(
        'test_project_user_roles_updated' => null,
