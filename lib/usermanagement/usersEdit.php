...
$roles = tlRole::getAll($db, null, null, null,
...
$smarty->assign('external_password_mgmt',
    tlUser::isPasswordMgtExternal($gui->user->authentication));
 ...
 * @return stdClass object with following members
 *         user: tlUser object
 *         status:
...
function doCreate(&$dbHandler, &$argsObj)
{
    ...
    if ($op->status >= tl::OK) {
        ...
        if ($op->status >= tl::OK) {
            tlUser::setExpirationDate($dbHandler, $op->user->dbID,
                $argsObj->expiration_date);

            $statusOk = true;
            $op->template = null;
            logAuditEvent(TLS("audit_user_created", $op->user->login), "CREATE",
                $op->user->dbID, "users");
            $op->user_feedback = sprintf(lang_get('user_created'),
                $op->user->login);
        }
    }
    ...
    $newPasswordSendMethod)
{
    ...
    if ($validator->isValid($smtp_host) || $password_on_screen) {
        $dummy = resetPassword($dbHandler, $argsObj->user_id,
            $newPasswordSendMethod);
        ...
        if ($op->status >= tl::OK) {
            logAuditEvent(TLS("audit_pwd_reset_requested", $userObj->login),
                "PWD_RESET", $argsObj->user_id, "users");
            $op->user_feedback = lang_get('password_reseted');
            if ($password_on_screen) {
                $op->user_feedback = lang_get('password_set') .
                    $dummy['password'];
            }
        } else {
            $op->user_feedback = sprintf(
                lang_get('password_cannot_be_reseted_reason'),
                $op->user_feedback);
        }
    } else {
        $op->status = tl::ERROR;
        $op->user_feedback = lang_get(
            'password_cannot_be_reseted_invalid_smtp_hostname');
    }
    return $op;
}
...
function createNewAPIKey(&$dbHandler, &$argsObj, &$userObj)
{
    ...
    if ($validator->isValid($smtp_host)) {
        ...
        if ($APIKey->addKeyForUser($argsObj->user_id) >= tl::OK) {
            logAuditEvent(TLS("audit_user_apikey_set", $userObj->login),
                "CREATE", $userObj->login, "users");
            ...
            $mail_op = @email_send(config_get('from_email'),
                $userObj->emailAddress, lang_get('mail_apikey_subject'),
                $msgBody);
        }
    } else {
        $op->status = tl::ERROR;
        $op->user_feedback = lang_get(
            'apikey_cannot_be_reseted_invalid_smtp_hostname');
    }
    return $op;
}
 ...
 *            data read from DB
 * @param stdClass $argsObj
 *            data entry from User Interface
 */
...
function initializeGui(&$dbHandler, &$argsObj)
{
    ...
    switch ($argsObj->doAction) {
        case 'edit':
            ...
            if (strlen(trim($argsObj->login)) > 0) {
                $argsObj->user_id = tlUser::doesUserExist($dbHandler,
                    $argsObj->login);
            }
            ...
            if (is_null($argsObj->user_id) || intval($argsObj->user_id) <= 0) {
                ...
                $guiObj->op->user_feedback = sprintf(
                    lang_get('login_does_not_exist'), $argsObj->login);
            } else {
    ...
    $guiObj->grants->mgt_view_events = $userObj->hasRight($dbHandler,
        "mgt_view_events");
    ...
    if (! is_null($guiObj->user)) {
        $guiObj->expDateEnabled = ! isset(
            $noExpirationUsers[$guiObj->user->login]);
    }
