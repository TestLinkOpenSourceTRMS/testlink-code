    ...
    case 'disable':
        ...
        if ($args->user_id != $args->currentUserID) {
            ...
            if ($gui->result >= tl::OK) {
                ...
                if ($gui->result >= tl::OK) {
                    logAuditEvent(TLS("audit_user_disabled", $user->login),
                        "DISABLE", $args->user_id, "users");
                    $gui->user_feedback = sprintf(lang_get('user_disabled'),
                        $user->login);
                }
            }
        }
 ...
 * value: colour
 *
 * If name is not defined on $g_role_colour (this normally happens for user
 * defined roles), will be added with '' as colour (means default colour).
 *
 * @param database $db
...
function buildMatrix(&$guiObj, &$argsObj)
{
    ...
    $actionUrl = '<a href="' . $argsObj->basehref .
        'lib/usermanagement/usersEdit.php?doAction=edit&' . 'loginJustToFixSort=';

    for ($zdx = 0; $zdx < $loop2do; $zdx ++) {
        $guiObj->matrix[$zdx]['handle'] = $actionUrl .
            urlencode($guiObj->matrix[$zdx]['login']) . '&user_id=' .
            $guiObj->matrix[$zdx]['user_id'] . '">' .
            $guiObj->matrix[$zdx]['login'] . "</a>";
    }
