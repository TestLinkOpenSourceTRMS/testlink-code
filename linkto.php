...
if (! isset($_GET['load'])) {
    ...
    if ($op['status_ok']) {
        ...
        if ($op['status_ok'] = ! is_null($tproject_data)) {
            ...
            $op['msg'] = sprintf(lang_get('testproject_not_found'),
                $args->tprojectPrefix);
        }
    }
    ...
    if ($op['status_ok']) {
        ...
        $jump_to = $pfn($db, $args->id, $tproject_data['id'],
            $args->tprojectPrefix, $args->version);
...
function checkTestProject(&$db, &$user, &$args)
{
    ...
    if ($op['status_ok'] = ! is_null($item_info)) {
        ...
        switch ($args->item) {
            case 'testcase':
            case 'testsuite':
                $hasRight = $user->hasRight($db, 'mgt_view_tc',
                    $args->tproject_id);
                break;

            case 'req':
            case 'reqspec':
                $hasRight = $user->hasRight($db, 'mgt_view_req',
                    $args->tproject_id);
                break;
...
function init_args()
{
    ...
    $args->status_ok = ! is_null($args->tprojectPrefix) && ! is_null($args->id) &&
        ! is_null($args->item);
    if (! $args->status_ok) {
        // new try
        // http://<testlink_home>/linkto.php?testcase=KAOS-4
        ...
        $args->status_ok = ! is_null($args->tprojectPrefix) &&
            ! is_null($args->id) && ! is_null($args->item);
    }
    return $args;
}
