        ...
        switch ($args->type) {
            ...
            case 'test_report':
                $param = "&type={$args->type}&level=testproject" .
                    ...
                    "&requirement=y&keyword=y&notes=y&headerNumbering=y&format=" .
                    FORMAT_HTML;
            ...
            case 'testspec':
                $param = "&type={$args->type}&level=testproject&id={$args->tproject_id}" .
                    ...
                    "&requirement=y&keyword=y&headerNumbering=y&format=" .
                    FORMAT_HTML;
            ...
            case 'charts_basic':
                $param = "&tproject_id={$args->tproject_id}&tplan_id={$args->tplan_id}&format=" .
                    FORMAT_HTML;
                $what2launch = $cfg['url'] . "?apikey=$args->apikey{$param}";
                break;
            ...
            case 'report_exec_timeline':
                $param = "&tproject_id={$args->tproject_id}" .
                    "&tplan_id={$args->tplan_id}" . "&format={$args->format}";
                $what2launch = $cfg['url'] . "?apikey=$args->apikey{$param}";
                break;
...
function init_args(&$dbHandler)
{
    ...
    if ($akl == $userAPIkeyLen) {
        ...
        if (setUpEnvForAnonymousAccess($dbHandler, $args->apikey, $kerberos,
            $opt)) {
            $args->light = 'green';
        }
    }
