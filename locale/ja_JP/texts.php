...
<p><i>テスト仕様</i> は、存在する<i>テストスイート</i>そして<i>テストケース</i> の" . "情報を閲覧したり変更したりする場所です。" .
    "異なるバージョンのテストケースを見ることもできます。</p>

<h2>やってみましょう！:</h2>
<ol>
  <li>テストプロジェクトをナビゲーションのツリーから選択します。<i>メモ: " . "テストプロジェクトの変更は右上の" .
    "プルダウンリストを選択することでおこなうことができます。</i></li>
  <li><b>子テストスイート作成</b>ボタンをクリックし、テストスイートを作成します。" .
    "このテストスイートに従ってテストドキュメントが構成されます" . "(例えば、機能/非機能などの種別、コンポーネントやフィーチャーによる分別など)。" .
    "テストスイートには、テストケース、環境設定、関連ドキュメントへの参照、" . "制限事項や、それ以外に必要な情報を記載します。" .
  ...
  <li>作成したテストスイートをナビゲーションから選択し、" . "<b>テストケースを作成</b>ボタンをクリックすることでテストケースを作成します。" .
...
<p><i>一例として、テストケースに重要度「高」、テストスイートに緊急度「低」を設定すると " . "優先度は「中」になります。</i>";
