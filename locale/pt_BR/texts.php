 ...
 *
 * Portuguese (pt_BR) texts for help/instruction pages. Strings for dynamic pages
...
<p>A <i>Especificação de Teste</i> permite aos usuários visualizar e editar todas as <i>Suítes de Teste</i> e <i>Casos de Teste</i> existentes. " . "Os Casos de Teste são versionados e todas as versões anteriores estão disponíveis e podem ser visualizadas e gerenciadas aqui.</p>
...
<ol>
	...
	<li>Atribua suas Especificações de Teste criadas ao <span class=\"help\" onclick=\"javascript:open_help_window('glosary','$locale');\">Plano de Teste</span> quando seus Casos de Teste estiverem prontos.</li>
</ol>
...
<p>Navegue de acordo com palavras-chave e/ou strings procuradas. A busca não é case sensitive. Os resultados incluem apenas Casos de Teste do Projeto de Teste atual.</p>
...
<p>A partir daqui você pode imprimir um único caso de teste, todos os casos de teste dentro de uma suite ou todos os casos de teste de um Projeto de Teste ou Plano de Teste.</p>
<h2>Iniciar:</h2>
<ol>
<li>
<p>Selecione os campos dos casos de teste que você deseja exibir, e então clique em um Caso de Teste, Suíte de Teste, ou Projeto de Teste. Uma página pronta para impressão será exibida.</p>
</li>
<li><p>Use a drop-box \"Mostrar Como\" no painel de navegação para especificar se você quer a informação exibida como HTML, como documento do Open Office Writer ou num documento do Microsoft Word. Veja <span class=\"help\" onclick=\"javascript:open_help_window('printFilter','{$locale}');\">Ajuda</span> para maiores informações.</p>
</li>
<li><p>Use a funcionalidade de impressão do seu browser para imprimir a informação presente.<br />
 <i>Nota: Certifique-se de imprimir somente o frame direito.</i></p></li>
</ol>";
...
<p>Requisitos estão agrupados por <b>documentos de Especificação de Requisitos </b>, os quais estão relacionados ao Projeto de Teste.<br /> O TestLink não suporta (ainda) versões para Especificação de Requisitos e também Requisitos. Logo, a versão do documento deve ser inserida após o <b>Título</b> da Especificação.
...
<p><i>E.g. SRS inclui 200 requisitos, mas somente 50 são adicionados ao Plano de Teste. A cobertura de testes é de 25% (se todos estes requisitos forem testados).</i></p>

<h2><a name='req'>Requisitos</a></h2>

<p>Clique no título da Especificação de Requisitos criada, e se nenhuma existir, clique no nó do projeto para criar uma. Você pode criar, editar, excluir ou importar requisitos para o documento. Cada requisito tem um título, escopo e status.
O status deve ser 'Válido' ou 'Não testável'. Requisitos Não Testáveis não são contabilizados para métricas. Este parâmetro deve ser utilizado para características não implementadas e requisitos modelados incorretamente.</p>
...
<li><p>Utilize a drop-box \"Mostrar Como\" no painel de navegação para especificar se você quer a informação exibida como HTML, como documento do Open Office Writer ou num documento do Microsoft Word. 
...
<p>A página de Atribuição de Palavras-chave é o lugar onde os usuários podem atribuir em lotes Palavras-chave às Suítes de Teste ou Casos de Teste.</p>

<h2>Para Atribuir Palavras-chave:</h2>
<ol>
	<li>Selecione uma Suíte de Teste ou Caso de Teste na árvore à esquerda.</li>
...
<ol>
	...
	<li>Selecionar um Ciclo de Teste no menu drop down e clicar no botão \"Aplicar Filtro\" no painel de navegação.</li>
...
<p><i>Nota: O TestLink deve ser configurado para colaborar com seu Bugtracker se você quiser criar ou rastrear problemas reportados diretamente da GUI.</i></p>";
...
$TLS_htmltext['showMetrics'] 		= "<p>Os relatórios estão relacionados a um Plano de Teste " .
		"(definido no topo do navegador). Este Plano de Teste pode diferir do Plano de Teste corrente para execução. Você também pode selecionar formatos dos relatórios:</p>
<ul>
...
<p>O botão imprimir ativa a impressão para um único relatório (sem navegação).</p><p>Existem vários relatórios separados a sua escolha. Seus propósitos e funções estão descritas abaixo.</p>
...
O 'último status' é determinado pelo ciclo mais recente onde os Casos de Teste foram executados. Por exemplo, se um Caso de Teste foi executado em diversos Ciclos de Teste, somente o último resultado é considerado.</p>

<p> O 'último resultado' é um conceito utilizado em vários relatórios, e é determinado como a seguir:</p>
<ul>
<li>A ordem em cada Ciclo de Teste é adicionada ao Plano de Teste determina qual é o Ciclo de Teste mais recente. Os resultados do Ciclo de Teste mais recente prevalecerão sobre os Ciclos de Teste mais antigos. Por exemplo, se você marcar um teste com status 'Falhou' no Ciclo de Teste 1 e no Ciclo de Teste 2 como 'Passou', o último resultado será 'Passou'.</li>
<li>Se um Caso de Teste é executado diversas vezes em um mesmo Ciclo de Teste, o resultado mais recente prevalecerá. Por exemplo, se o Ciclo de Teste 3 é liberado para a equipe de testes e o Testador 1 marcar 'Passou' as 14h, e o Testador 2 marcar 'Falhou' as 15h, isto aparecerá como 'Falhou'.</li>
<li>Casos de Teste marcados como 'Não Executado' no último Ciclo de Teste não serão considerados. Por exemplo, se você marcar um Caso de Teste como 'Passou' no Ciclo de Teste 1 e não executar no Ciclo de Teste 2, o último resultado será considerado como 'Passou'.</li>
</ul>
<p>As seguintes tabelas são exibidas:</p>
<ul>
	<li><b>Resultados por Suíte de Teste de Nível Top</b>
	Lista os resultados Top de cada Suíte de Teste. Total de Casos de Teste, Passou, Falhou, Bloqueado, Não Executado e o percentual completo são listados. Um Caso de Teste 'completo' é aquele que foi marcado como Passou, Falhou ou Bloqueado.
	Resultados por Suítes de Teste de Nível Top incluem todas as Suítes de nível inferior.</li>
	<li><b>Resultados por Palavra-chave</b>
	Lista todas as palavras-chave que estão atribuídas aos Casos de Teste no Plano de Teste corrente, e os resultados associados e eles.</li>
	<li><b>Resultados por Testador</b>
	Lista cada Testador que tem Casos de Teste associados a ele no Plano de Teste corrente. Os Casos de Teste que não estão atribuídos são computados abaixo com a descrição 'desatribuir'.</li>
</ul>

<h3>Status Geral dos Ciclos de Teste</h3>
<p>Lista os resultados de execução para cada Ciclo de Teste. Para cada Ciclo de Teste, o total de Casos de Teste, total com Passou, % Passou, total Falha, % Falha, Bloqueado, % Bloqueado, Não Executado e % Não Executado.  Se um Caso de Teste foi executado duas vezes no mesmo Ciclo de Teste, a execução mais recente será considerada.</p>
...
<li>por um conjunto de particionamento dos totais (Soma / Passou / Falhou / Bloqueado / Não Executado) e todas execuções realizadas na Suíte de Teste. Se um Caso de Teste foi executado mais de uma vez em múltiplos Ciclos de Teste, todas as execuções que foram gravadas serão exibidas nos Ciclos de Teste selecionados. No entanto, o resumo para esta Suíte de Teste somente incluirá o último resultado para o Ciclo de Teste selecionado.</li>
</ol>

<h3>Relatórios de Casos de Teste Bloqueados, com Falha e Não Executados</h3>
<p>Estes relatórios exibem todos os Casos de Teste Bloqueados, com Falha e Não Executados. A lógica do último resultado dos testes (que está descrita nas Métricas Gerais do Plano de Teste) é novamente empregada para determinar se um Caso de Teste deve ser considerado Bloquedo, com Falha ou Não Executado. Casos de Teste Bloqueado e com Falha exibirão os bugs associados se o usuário estiver utilizando um sistema de Bugtracking.</p>

<h3>Relatório de Testes</h3>
<p>Exibe o status de cada Caso de Teste em todos os Ciclos de Teste. O resultado da execução mais recente será utilizado se um Caso de Teste for executado múltiplas vezes em um mesmo Ciclo de Teste. É recomendado exportar este relatório para o formato em Excel para um fácil manuseio se um grande conjunto de dados estiver em utilização.</p>


<h3>Gráficos - Métricas Gerais do Plano de Teste</h3>
<p>A lógica do último resultado é utilizada para todos os gráficos que você verá. Os gráficos são animados para ajudar o usuário à visualizar as métricas do Plano de Teste atual. Os quatro gráficos fornecidos são:</p>
...
<p>As seções e barras dos gráficos são coloridos de modo que o usuário possa identificar o número aproximado de Casos de Teste com status Passou, Falhou, Bloqueado e Não Executado.</p>
...
<p>Este relatório exibe cada Caso de Teste com todos os bugs abertos para ele em todo o projeto. Este relatório está disponível somente se um Bugtracking estiver conectado.</p>";
...
<p>Esta página permite atualizar Casos de Teste para uma versão mais nova (diferente) da Especificação de Casos de Teste quando alterada. Isto frequentemente acontece quando uma funcionalidade é alterada durante os testes." .
...
<p>O TestLink permite definir a urgência das Suítes de Teste para afetar a prioridade dos Casos de Teste. A priorização dos testes depende da importância do Caso de Teste e da urgência definida no Plano de Teste. O Líder de Teste deve especificar um conjunto de Casos de Teste que devem ser testados primeiro. Isso ajuda a assegurar que os testes cobrirão os requisitos mais importantes também sob a pressão do tempo.</p>

<h2>Iniciar:</h2>
<ol>
	<li>Escolha uma Suíte de Teste para definiar a urgência do produto/componente no navegador ao lado esquerdo da janela.</li>
...
<p><i>Por exemplo, um Caso de Teste com prioridade Alta em uma Suíte de Teste com urgência Baixa será de prioridade Média.</i>";
