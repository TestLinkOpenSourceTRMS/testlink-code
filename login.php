...
switch ($args->action) {
    ...
    case 'oauth':
        ...
        if ($user_token->status['status'] == tl::OK) {
            doSessionStart(true);
            $op = doAuthorize($db, $user_token->options->user, 'oauth',
                $user_token->options);
            $doAuthPostProcess = true;
        } else {
...
function initGui(&$db, $args)
{
    ...
    if ((strcmp($secCfg, 'ALWAYS') == 0) ||
        ...
        ! isset($_SESSION['getSecurityNotesDone']))) {
    ...
    $gui->login_disabled = (('LDAP' == $gui->authCfg['method']) &&
        ! checkForLDAPExtension()) ? 1 : 0;
 ...
 *
 * @param
 *            database &$dbHandler DataBase Handler
 * @param
 *            stdClass &$guiObj some gui elements that will be used to give feedback
 */
...
function renderLoginScreen($guiObj)
{
    ...
    $g_tlLogger->deleteEventsFor(null,
        strtotime("-{$logPeriodToDelete} days UTC"));
...
function authorizePostProcessing($argsObj, $op)
{
    ...
    if ($op['status'] == tl::OK) {
        ...
        logAuditEvent(
            TLS("audit_login_succeeded", $argsObj->login,
        ...
        if ($argsObj->action == 'ajaxlogin') {
            ...
            if (! empty($argsObj->destination)) {
                ...
                $where = strpos($argsObj->destination,
                    $baseFolder . 'linkto.php');
                ...
                $afterLinkTo = $baseFolder . 'linkto.php?' .
                    cleanInput($dummy[1]);
                ...
                $ad .= ($argsObj->preqURI ? "&reqURI=" .
                    urlencode($argsObj->preqURI) : "");
