...
function checkTestPlan(&$db, &$user, &$args)
{
    ...
    $item_info = $tplan_mgr->get_by_id($args->tplan_id, array(
        'output' => 'minimun'
    ));
...
function check_exec(&$dbHandler, &$argsObj)
{
    if ($argsObj->feature_id > 0) {
        ...
        $sql = "SELECT testplan_id,platform_id,tcversion_id " . "FROM {$tb} WHERE id=" . $argsObj->feature_id;
        ...
        $argsObj->status_ok = ($argsObj->tplan_id > 0) && ($argsObj->tcversion_id > 0);
    }
}
...
function launch_inner_exec(&$dbHandler, &$tplMgr)
{
    ...
    if ($op['status_ok']) {
        ...
        $info = $tplan_mgr->get_by_id($args->tplan_id, array(
            'output' => 'minimun'
        ));
...
function launch_inner_xta2m(&$dbHandler, &$tplMgr)
{
    ...
    $jt = $_SESSION['basehref'] . '/lib/testcases/' . 'tcAssignedToUser.php?user_id=' . $args->target_user_id . $k2c = array(
        'tplan_id',
        'build_id'
    );
    foreach ($k2c as $tg) {
...
function launch_outer_xta2m(&$tplMgr, $argsObj)
{
    ...
    $gui->title = lang_get('main_page_title');
    $gui->mainframe = 'ltx.php?item=xta2m&load=1' . '&user_id=' . $argsObj->target_user_id . '&tplan_id=' . $argsObj->tplan_id;
