...
function init_args(&$dbHandler)
{
    ...
    $args->tplan_id = intval(
        isset($_GET['tplan_id']) ? $_GET['tplan_id'] : null);
    $args->tcversion_id = intval(
        isset($_GET['tcversion_id']) ? $_GET['tcversion_id'] : null);
    $args->platform_id = intval(
        isset($_GET['platform_id']) ? $_GET['platform_id'] : null);
    $args->build_id = intval(
        isset($_GET['build_id']) ? $_GET['build_id'] : null);
...
function build_link_exec(&$argsObj)
{
    ...
    if ($argsObj->feature_id > 0) {
        ...
        $lk .= "&tplan_id=" . $argsObj->tplan_id . "&platform_id=" .
            $argsObj->platform_id . "&tcversion_id=" . $argsObj->tcversion_id;
    }
...
function process_exec(&$dbHandler, $context)
{
    ...
    $ret['url'] = "lib/execute/execSetResults.php?level=testcase" .
        "&version_id=" . $context['tcversion_id'] . "&id=" . $info['parent_id'] .
        "&setting_testplan=" . $context['setting_testplan'] . "&setting_build=" .
        $context['setting_build'] . "&setting_platform=" .
        $context['setting_platform'];
...
{
    $argsObj->status_ok = ($argsObj->target_user_id > 0 && $argsObj->tplan_id > 0);
...
function launch_inner_exec(&$dbHandler, &$tplMgr)
{
    ...
    if ($op['status_ok']) {
        $treeframe = $itemCode[$args->item] . '?loadExecDashboard=0' .
            '&setting_testplan=' . $args->tplan_id . '&setting_build=' .
            $args->build_id . '&setting_platform=' . $args->platform_id;
...
function launch_outer_xta2m(&$tplMgr, $argsObj)
{
    ...
    $gui->title = lang_get('main_page_title');
    $gui->mainframe = 'ltx.php?item=xta2m&load=1' . '&user_id=' .
        $argsObj->target_user_id . '&tplan_id=' . $argsObj->tplan_id;
