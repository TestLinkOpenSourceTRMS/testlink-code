...
require_once("../functions/header.php");
  ...
  doHeader();
	...
	echo "<table width='100%' class=userinfotable><tr><td bgcolor='#99CCFF' class='subTitle'>Status By Keyword</td></tr></table>";

	echo "<table class=userinfotable width='100%'>";

	echo "<tr><td width='14%' class=tctablehdrCenter>Keyword</td><td width='14%' class=tctablehdrCenter>Total</td><td width='14%' class=tctablehdrCenter>Pass</td><td width='14%' class=tctablehdrCenter>Fail</td><td width='14%' class=tctablehdrCenter>Blocked</td><td width='14%' class=tctablehdrCenter>Not Run</td><td width='14%' class=tctablehdrCenter>% Complete</td></tr>";
		...
		for ($i = 0; $i < count($result4); $i++)
		{
			...
			if($result4[$i] != "")
			{
				...
				$sql = "select tcid,status from results,project,component,category,testcase where project.id = '" . $_SESSION['project'] . "' and project.id = component.projid and component.id = category.compid and category.id = testcase.catid and testcase.id = results.tcid and keywords like '%" . $result4[$i] . "%' order by build";
