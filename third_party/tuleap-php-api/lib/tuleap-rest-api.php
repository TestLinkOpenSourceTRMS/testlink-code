...
class tuleap
{
  ...
  function getIssue($issueID)
  {
    try
    {
	  $item = $this->_get("/artifacts/$issueID");
   ...
   *
   */
  public function getUserId($name, $cfg)
  {
    $item = $this->_get("/users?query=".urlencode($name));
    return $item[0]->id;
  }

  public function getUserDisplayName($name, $cfg)
  {
    $item = $this->_get("/users?query=".urlencode($name));
  ...
  public function addIssue($title, $text, $cfg)
  {
  $cfield = 'custom_field_6_'.$cfg->cf.'_'.$_REQUEST['id'];
  $userid = $this->getUserId($_REQUEST[$cfield], $cfg);
  $username = $this->getUserDisplayName($_SESSION['currentUser']->firstName, $cfg);
	$values = array(array('field_id'=>$cfg->fields->email, 'bind_value_ids'=> array($username)),
						  array('field_id'=>$cfg->fields->summary, 'value'=>$title),
						  array('field_id'=>$cfg->fields->description, 'value'=>$text),
              array('field_id'=>$cfg->fields->assign, 'bind_value_ids'=> array($userid)));

  $body = array('tracker'=>array('id'=>$cfg->tracker),
	'values'=>$values);
  ...
  {
	    $url = "/artifacts/".$issueID;
    $op = $this->_request_json('POST',$url,$noteText);
    return $op;
  }
  /**
   *
  ...
  **/
  protected function _request($method, $cmd, $body = NULL, $ignoreStatusCode = 0,$reporter = null)
  {
    ...
    $header = [ "X-Auth-AccessKey: {$this->apiKey}",
